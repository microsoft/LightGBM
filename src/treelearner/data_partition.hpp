...
#pragma omp parallel for schedule(static)
      ...
      std::memcpy(indices_.data(), used_data_indices_,
                  used_data_count_ * sizeof(data_size_t));
    }
  }
  ...
  void ResetByLeafPred(const std::vector<int>& leaf_pred, int num_leaves) {
    ...
    for (data_size_t i = 0; i < static_cast<data_size_t>(leaf_pred.size());
         ++i) {
    ...
    for (int i = 0; i < num_leaves_; ++i) {
      ...
      std::copy(indices_per_leaf[i].begin(), indices_per_leaf[i].end(),
                indices_.begin() + leaf_begin_[i]);
...
#pragma omp parallel for schedule(static)
      ...
      if (i < nblock) {
        if (left_cnts_buf_[i] > 0) {
          std::memcpy(left_start + left_write_pos_buf_[i],
                      temp_left_indices_.data() + offsets_buf_[i],
        ...
        if (right_cnts_buf_[j] > 0) {
          std::memcpy(right_start + right_write_pos_buf_[j],
                      temp_right_indices_.data() + offsets_buf_[j],
                      right_cnts_buf_[j] * sizeof(data_size_t));
        }
      }
    }
   ...
   */
  void SetUsedDataIndices(const data_size_t* used_data_indices,
 ...
 private:
  ...
  /*! \brief Store all data's indices, order by
   * leaf[data_in_leaf0,..,data_leaf1,..] */
  ...
  int num_threads_;
  /*! \brief Buffer for multi-threading data partition, used to store offset for
   * different threads */
  std::vector<data_size_t> offsets_buf_;
  /*! \brief Buffer for multi-threading data partition, used to store left count
   * after split for different threads */
  std::vector<data_size_t> left_cnts_buf_;
  /*! \brief Buffer for multi-threading data partition, used to store right
   * count after split for different threads */
  std::vector<data_size_t> right_cnts_buf_;
  /*! \brief Buffer for multi-threading data partition, used to store write
   * position of left leaf for different threads */
  std::vector<data_size_t> left_write_pos_buf_;
  /*! \brief Buffer for multi-threading data partition, used to store write
   * position of right leaf for different threads */

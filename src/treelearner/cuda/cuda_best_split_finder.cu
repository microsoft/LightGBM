...
__device__ void FindBestSplitsForLeafKernelInner(const hist_t* feature_hist_ptr,
  ...
  const bool reverse, const bool skip_default_bin, const bool na_as_missing, const uint8_t assume_out_default_left,
  // output parameters
...
__global__ void FindBestSplitsForOneLeafKernel(
  ...
  const double* leaf_gain,
  const double* sum_gradients_in_leaf,
  const double* sum_hessians_in_leaf,
  ...
  uint8_t* out_default_left = cuda_best_split_default_left + task_index;
  const double parent_gain = *leaf_gain;
  const double sum_gradients = *sum_gradients_in_leaf;
  const double sum_hessians = (*sum_hessians_in_leaf) + 2 * kEpsilon;
  const double num_data = *num_data_in_leaf;
  const unsigned int output_offset = is_larger ? (task_index + gridDim.x) : task_index;
  ...
  const hist_t* hist_ptr = *leaf_hist + feature_hist_offsets[inner_feature_index] * 2;
  FindBestSplitsForLeafKernelInner(hist_ptr,
    ...
    num_data, reverse, skip_default_bin, na_as_missing, assume_out_default_left, out_threshold, out_gain, out_default_left,
...
void CUDABestSplitFinder::LaunchFindBestSplitsForLeafKernel(
  ...
  FindBestSplitsForOneLeafKernel<<<num_tasks_, MAX_NUM_BIN_IN_FEATURE, 0, cuda_streams_[0]>>>(
    // input feature information
  ...
  if (larger_leaf_index >= 0) {
    FindBestSplitsForOneLeafKernel<<<num_tasks_, MAX_NUM_BIN_IN_FEATURE, 0, cuda_streams_[1]>>>(
      // input feature information
      cuda_feature_hist_offsets_,
      cuda_feature_mfb_offsets_,
      cuda_feature_default_bins_,
...
__device__ void ReduceBestSplit(uint8_t* found, double* gain, uint32_t* shared_read_index,
  uint32_t num_features_aligned, uint32_t thread_offset) {
  const uint32_t threadIdx_x = threadIdx.x - thread_offset;
...
__global__ void SyncBestSplitForLeafKernel(const int* smaller_leaf_index, const int* larger_leaf_index,
  ...
  const int num_features_ref,
  const int num_features_aligned) {

  const uint32_t threadIdx_x = threadIdx.x;
  const uint32_t blockIdx_x = blockIdx.x;

  __shared__ uint8_t best_found[NUM_FEATURES_PER_SYNC_BLOCK];
  __shared__ double best_gain[NUM_FEATURES_PER_SYNC_BLOCK];
  __shared__ uint32_t shared_read_index[NUM_FEATURES_PER_SYNC_BLOCK];

  const bool is_smaller = (blockIdx_x == 0);
  const int feature_index = static_cast<int>(threadIdx_x);
  const uint32_t read_index = is_smaller ? threadIdx_x : threadIdx_x + num_features_ref; 
  if (feature_index < num_features_ref) {
    best_found[feature_index] = cuda_best_split_found[read_index];
    best_gain[feature_index] = cuda_best_split_gain[read_index];
    shared_read_index[feature_index] = read_index;
  } else {
    best_found[feature_index] = 0;
  }

  __syncthreads();
  ReduceBestSplit(best_found, best_gain, shared_read_index,
    num_features_aligned, 0);
  ...
  if (threadIdx.x == 0) {
    ...
    if (best_found[0]) {
      cuda_leaf_best_split_gain[leaf_index_ref] = best_gain[0];
      cuda_leaf_best_split_feature[leaf_index_ref] = is_smaller ? static_cast<int>(best_read_index) :
        static_cast<int>(best_read_index) - num_features_ref;
      cuda_leaf_best_split_default_left[leaf_index_ref] = cuda_best_split_default_left[best_read_index];
      cuda_leaf_best_split_threshold[leaf_index_ref] = cuda_best_split_threshold[best_read_index];
      cuda_leaf_best_split_left_sum_gradient[leaf_index_ref] = cuda_best_split_left_sum_gradient[best_read_index];
      cuda_leaf_best_split_left_sum_hessian[leaf_index_ref] = cuda_best_split_left_sum_hessian[best_read_index];
      cuda_leaf_best_split_left_count[leaf_index_ref] = cuda_best_split_left_count[best_read_index];
      cuda_leaf_best_split_left_gain[leaf_index_ref] = cuda_best_split_left_gain[best_read_index]; 
      cuda_leaf_best_split_left_output[leaf_index_ref] = cuda_best_split_left_output[best_read_index];
      cuda_leaf_best_split_right_sum_gradient[leaf_index_ref] = cuda_best_split_right_sum_gradient[best_read_index];
      cuda_leaf_best_split_right_sum_hessian[leaf_index_ref] = cuda_best_split_right_sum_hessian[best_read_index];
      cuda_leaf_best_split_right_count[leaf_index_ref] = cuda_best_split_right_count[best_read_index];
      cuda_leaf_best_split_right_gain[leaf_index_ref] = cuda_best_split_right_gain[best_read_index]; 
      cuda_leaf_best_split_right_output[leaf_index_ref] = cuda_best_split_right_output[best_read_index];
    } else {
      cuda_leaf_best_split_gain[leaf_index_ref] = kMinScore;
    }
  }
}
...
void CUDABestSplitFinder::LaunchSyncBestSplitForLeafKernel(const int* smaller_leaf_index, const int* larger_leaf_index) {
  ...
  int num_features_aligned = 1;
  num_features -= 1;
  while (num_features > 0) {
    num_features_aligned <<= 1;
    num_features >>= 1;
  }
  SyncBestSplitForLeafKernel<<<2, NUM_FEATURES_PER_SYNC_BLOCK>>>(smaller_leaf_index, larger_leaf_index,
...
__global__ void FindBestFromAllSplitsKernel(const int* cuda_cur_num_leaves,
  ...
  __syncthreads();
  ReduceBestGainForLeaves(thread_best_gain, thread_best_leaf, cuda_cur_num_leaves_ref);
  if (threadIdx_x == 0) {
    *out_best_leaf = thread_best_leaf[0];
    cuda_best_split_info_buffer[8] = thread_best_leaf[0];
  }
}

__global__ void PrepareLeafBestSplitInfo(const int* cuda_smaller_leaf_index, const int* cuda_larger_leaf_index,
  ...
  const int smaller_leaf_index_ref = *cuda_smaller_leaf_index;
  const int larger_leaf_index_ref = *cuda_larger_leaf_index;
  const unsigned int threadIdx_x = threadIdx.x;
  if (threadIdx_x == 0) {
    cuda_best_split_info_buffer[0] = smaller_leaf_index_ref;
  } else if (threadIdx_x == 1) {
    cuda_best_split_info_buffer[1] = cuda_leaf_best_split_feature[smaller_leaf_index_ref];
  } else if (threadIdx_x == 2) {
    cuda_best_split_info_buffer[2] = cuda_leaf_best_split_threshold[smaller_leaf_index_ref];
  } else if (threadIdx_x == 3) {
    cuda_best_split_info_buffer[3] = cuda_leaf_best_split_default_left[smaller_leaf_index_ref];
  } else if (threadIdx_x == 4) {
    cuda_best_split_info_buffer[4] = larger_leaf_index_ref;
  } else if (threadIdx_x == 5) {
    cuda_best_split_info_buffer[5] = cuda_leaf_best_split_feature[larger_leaf_index_ref];
  } else if (threadIdx_x == 6) {
    cuda_best_split_info_buffer[6] = cuda_leaf_best_split_threshold[larger_leaf_index_ref];
  } else if (threadIdx_x == 7) {
    cuda_best_split_info_buffer[7] = cuda_leaf_best_split_default_left[larger_leaf_index_ref];
  } 
}

void CUDABestSplitFinder::LaunchFindBestFromAllSplitsKernel(const int* cuda_cur_num_leaves,
  const int* cuda_smaller_leaf_index, const int* cuda_larger_leaf_index, std::vector<int>* leaf_best_split_feature,
  ...
  PrepareLeafBestSplitInfo<<<1, 8, 0, cuda_streams_[0]>>>(cuda_smaller_leaf_index, cuda_larger_leaf_index,
  ...
  std::vector<int> cpu_leaf_best_split_info_buffer(9);
  SynchronizeCUDADevice();
  CopyFromCUDADeviceToHost<int>(cpu_leaf_best_split_info_buffer.data(), cuda_best_split_info_buffer_, 9);
  const int smaller_leaf_index = cpu_leaf_best_split_info_buffer[0];
  const int larger_leaf_index = cpu_leaf_best_split_info_buffer[4];
  (*leaf_best_split_feature)[smaller_leaf_index] = cpu_leaf_best_split_info_buffer[1];
  (*leaf_best_split_threshold)[smaller_leaf_index] = static_cast<uint32_t>(cpu_leaf_best_split_info_buffer[2]);
  (*leaf_best_split_default_left)[smaller_leaf_index] = static_cast<uint8_t>(cpu_leaf_best_split_info_buffer[3]);
  if (larger_leaf_index >= 0) {
    (*leaf_best_split_feature)[larger_leaf_index] = cpu_leaf_best_split_info_buffer[5];
    (*leaf_best_split_threshold)[larger_leaf_index] = static_cast<uint32_t>(cpu_leaf_best_split_info_buffer[6]);
    (*leaf_best_split_default_left)[larger_leaf_index] = static_cast<uint8_t>(cpu_leaf_best_split_info_buffer[7]);
  }
  *best_leaf_index = cpu_leaf_best_split_info_buffer[8];
}

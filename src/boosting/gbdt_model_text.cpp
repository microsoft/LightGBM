...
std::string GBDT::DumpModel(int start_iteration, int num_iteration,
  ...
  std::vector<double> feature_importances =
      FeatureImportance(num_iteration, feature_importance_type);
...
std::string GBDT::ModelToIfElse(int num_iteration) const {
  ...
  str_buf << "void GBDT::PredictRaw(const double* features, double *output, const "
  ...
  str_buf << "void GBDT::PredictRawByMap(const std::unordered_map<int, double>& features, double* "
             "output, const PredictionEarlyStopInstance* early_stop) const {"
          << '\n';
  ...
  // PredictByMap
  str_buf << "void GBDT::PredictByMap(const std::unordered_map<int, double>& features, double* "
             "output, const PredictionEarlyStopInstance* early_stop) const {"
          << '\n';
  ...
  str_buf << "void GBDT::PredictLeafIndexByMap(const std::unordered_map<int, double>& features, "
             "double* output) const {"
          << '\n';
  str_buf << "\t" << "int total_tree = num_iteration_for_pred_ * num_tree_per_iteration_;" << '\n';
  str_buf << "\t" << "for (int i = 0; i < total_tree; ++i) {" << '\n';
...
bool GBDT::SaveModelToIfElse(int num_iteration, const char* filename) const {
  ...
  if (ifs.good()) {
    std::string origin((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
...
std::string GBDT::SaveModelToString(int start_iteration, int num_iteration,
                                    int feature_importance_type) const {
  std::stringstream ss;
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  std::stable_sort(
      ...
      [](const std::pair<size_t, std::string>& lhs, const std::pair<size_t, std::string>& rhs) {
...
bool GBDT::SaveModelToFile(int start_iteration, int num_iteration, int feature_importance_type,
  ...
  std::string str_to_write =
      SaveModelToString(start_iteration, num_iteration, feature_importance_type);
...
bool GBDT::LoadModelFromString(const char* buffer, size_t len) {
      ...
      if (!Common::StartsWith(cur_line, "Tree=")) {
        ...
        if (strs.size() == 1) {
          ...
          if (strs[0] == "feature_names") {
            ...
            Log::Fatal("Wrong line at model file: %s",
                       cur_line.substr(0, std::min<size_t>(128, cur_line.size())).c_str());
          }
        }
      } else {
        break;
  ...
  if (key_vals.count("monotone_constraints")) {
    monotone_constraints_ =
        CommonC::StringToArray<int8_t>(key_vals["monotone_constraints"].c_str(), ' ');
  ...
  if (!key_vals.count("tree_sizes")) {
    ...
    std::vector<size_t> tree_sizes =
        CommonC::StringToArray<size_t>(key_vals["tree_sizes"].c_str(), ' ');

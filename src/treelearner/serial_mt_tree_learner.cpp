...
#include "cost_effective_mt_gradient_boosting.hpp"

namespace LightGBM {

SerialMTTreeLearner::SerialMTTreeLearner(const Config* config)
    : config_(config), col_sampler_(config) {
}

SerialMTTreeLearner::~SerialMTTreeLearner() {
}

void SerialMTTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  if (CostEfficientMTGradientBoosting::IsEnable(config_)) {
    cegb_.reset(new CostEfficientMTGradientBoosting(this));
    cegb_->Init();
  }
}

void SerialMTTreeLearner::GetShareStates(const Dataset* dataset,
...
void SerialMTTreeLearner::ResetTrainingDataInner(const Dataset* train_data,
                                               bool is_constant_hessian,
...
void SerialMTTreeLearner::ResetConfig(const Config* config) {
...
Tree* SerialMTTreeLearner::Train(const score_t* gradients, const score_t *hessians, bool /*is_first_tree*/) {
  Common::FunctionTimer fun_timer("SerialMTTreeLearner::Train", global_timer);
...
void SerialMTTreeLearner::Train_serial2(Tree* tree, const score_t* gradients, const score_t* hessians, bool /*is_first_tree*/){
...
Tree* SerialMTTreeLearner::FitByExistingTree(const Tree* old_tree, const score_t* gradients, const score_t *hessians) const {
...
Tree* SerialMTTreeLearner::FitByExistingTree(const Tree* old_tree, const std::vector<int>& leaf_pred,
...
void SerialMTTreeLearner::BeforeTrain() {
  Common::FunctionTimer fun_timer("SerialMTTreeLearner::BeforeTrain", global_timer);
...
bool SerialMTTreeLearner::BeforeFindBestSplit(const Tree* tree, int left_leaf, int right_leaf) {
  Common::FunctionTimer fun_timer("SerialMTTreeLearner::BeforeFindBestSplit", global_timer);
...
void SerialMTTreeLearner::FindBestSplits(const Tree* tree) {
  FindBestSplits(tree, nullptr);
}

void SerialMTTreeLearner::FindBestSplits(const Tree* tree, const std::set<int>* force_features) {
  ...
  if (LGBM_config_::current_learner == use_cpu_learner) {
    SerialMTTreeLearner::ConstructHistograms(is_feature_used, use_subtract);
  } else {
...
void SerialMTTreeLearner::ConstructHistograms(
    const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  Common::FunctionTimer fun_timer("SerialMTTreeLearner::ConstructHistograms",
                                  global_timer);
...
void SerialMTTreeLearner::FindBestSplitsFromHistograms(
    const std::vector<int8_t>& is_feature_used, bool use_subtract, const Tree* tree) {
  Common::FunctionTimer fun_timer(
      "SerialMTTreeLearner::FindBestSplitsFromHistograms", global_timer);
...
int32_t SerialMTTreeLearner::ForceSplits(Tree* tree, int* left_leaf,
...
std::set<int> SerialMTTreeLearner::FindAllForceFeatures(Json force_split_leaf_setting) {
...
void SerialMTTreeLearner::SplitInner(Tree* tree, int best_leaf, int* left_leaf,
                                   int* right_leaf, bool update_cnt) {
  Common::FunctionTimer fun_timer("SerialMTTreeLearner::SplitInner", global_timer);
...
void SerialMTTreeLearner::RenewTreeOutput(Tree* tree, const ObjectiveFunction* obj, std::function<double(const label_t*, int)> residual_getter,
...
void SerialMTTreeLearner::ComputeBestSplitForFeature(
...
double SerialMTTreeLearner::GetParentOutput(const Tree* tree, const LeafSplits* leaf_splits) const {
...
void SerialMTTreeLearner::RecomputeBestSplitForLeaf(Tree* tree, int leaf, SplitInfo* split) {

...
void VotingParallelTreeLearner<TREELEARNER_T>::Init(const Dataset* train_data,
  ...
  size_t buffer_size =
      2 * top_k_ * std::max(max_bin * kHistEntrySize, sizeof(LightSplitInfo) * num_machines_);
  ...
  for (int j = 0; j < train_data->num_features(); ++j) {
    smaller_leaf_histogram_array_global_[j].Init(
        smaller_leaf_histogram_data_.data() + offsets[j] * 2, &feature_metas_[j]);
    larger_leaf_histogram_array_global_[j].Init(
        larger_leaf_histogram_data_.data() + offsets[j] * 2, &feature_metas_[j]);
  }
}
...
template <typename TREELEARNER_T>
bool VotingParallelTreeLearner<TREELEARNER_T>::BeforeFindBestSplit(const Tree* tree, int left_leaf,
                                                                   int right_leaf) {
...
void VotingParallelTreeLearner<TREELEARNER_T>::GlobalVoting(
    int leaf_idx, const std::vector<LightSplitInfo>& splits, std::vector<int>* out) {
  out->clear();
  if (leaf_idx < 0) {
    return;
  }
  ...
  std::vector<LightSplitInfo> feature_best_split(this->train_data_->num_total_features(),
                                                 LightSplitInfo());
...
void VotingParallelTreeLearner<TREELEARNER_T>::CopyLocalHistogram(
    const std::vector<int>& smaller_top_features, const std::vector<int>& larger_top_features) {
  for (int i = 0; i < this->num_features_; ++i) {
    ...
    while (cur_used_features < cur_total_feature) {
      ...
      if (smaller_idx < smaller_top_features.size()) {
        int inner_feature_index =
            this->train_data_->InnerFeatureIndex(smaller_top_features[smaller_idx]);
        ++cur_used_features;
        ...
        std::memcpy(input_buffer_.data() + reduce_scatter_size_,
                    ...
                    this->smaller_leaf_histogram_array_[inner_feature_index].SizeOfHistogram());
      ...
      if (larger_idx < larger_top_features.size()) {
        int inner_feature_index =
            this->train_data_->InnerFeatureIndex(larger_top_features[larger_idx]);
        ++cur_used_features;
        // mark local aggregated feature
        if (i == rank_) {
        ...
        std::memcpy(input_buffer_.data() + reduce_scatter_size_,
                    ...
                    this->larger_leaf_histogram_array_[inner_feature_index].SizeOfHistogram());
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  if (this->larger_leaf_splits_ != nullptr) {
    ...
    if (larger_leaf_index >= 0) {
      const data_size_t local_data_on_larger_leaf =
          this->data_partition_->leaf_count(larger_leaf_index);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_; ++feature_index) {
    ...
    this->train_data_->FixHistogram(feature_index, this->smaller_leaf_splits_->sum_gradients(),
                                    this->smaller_leaf_splits_->sum_hessians(),
                                    this->smaller_leaf_histogram_array_[feature_index].RawData());

    this->ComputeBestSplitForFeature(
        this->smaller_leaf_histogram_array_, feature_index, real_feature_index, true,
        this->smaller_leaf_splits_->num_data_in_leaf(), this->smaller_leaf_splits_.get(),
        &smaller_bestsplit_per_features[feature_index], smaller_leaf_parent_output);
    // only has root leaf
    ...
    if (use_subtract) {
      ...
      this->train_data_->FixHistogram(feature_index, this->larger_leaf_splits_->sum_gradients(),
                                      this->larger_leaf_splits_->sum_hessians(),
                                      this->larger_leaf_histogram_array_[feature_index].RawData());
    }
    this->ComputeBestSplitForFeature(
        this->larger_leaf_histogram_array_, feature_index, real_feature_index, true,
        this->larger_leaf_splits_->num_data_in_leaf(), this->larger_leaf_splits_.get(),
        &larger_bestsplit_per_features[feature_index], larger_leaf_parent_output);
    OMP_LOOP_EX_END();
  }
  OMP_THROW_EX();
  ...
  for (int i = 0; i < num_machines_; ++i) {
    for (int j = 0; j < top_k_; ++j) {
      ...
      std::memcpy(&smaller_top_k_splits_global.back(), output_buffer_.data() + offset,
                  sizeof(LightSplitInfo));
      offset += sizeof(LightSplitInfo);
      larger_top_k_splits_global.push_back(LightSplitInfo());
      std::memcpy(&larger_top_k_splits_global.back(), output_buffer_.data() + offset,
                  sizeof(LightSplitInfo));
      offset += sizeof(LightSplitInfo);
    }
  }
  ...
  GlobalVoting(this->smaller_leaf_splits_->leaf_index(), smaller_top_k_splits_global,
               &smaller_top_features);
  GlobalVoting(this->larger_leaf_splits_->leaf_index(), larger_top_k_splits_global,
               &larger_top_features);
...
void VotingParallelTreeLearner<TREELEARNER_T>::FindBestSplitsFromHistograms(
  ...
  double smaller_leaf_parent_output =
      this->GetParentOutput(tree, this->smaller_leaf_splits_global_.get());
  double larger_leaf_parent_output =
      this->GetParentOutput(tree, this->larger_leaf_splits_global_.get());
...
#pragma omp parallel for schedule(static) num_threads(this->share_state_->num_threads)
  for (int feature_index = 0; feature_index < this->num_features_; ++feature_index) {
    ...
    if (smaller_is_feature_aggregated_[feature_index]) {
      ...
      this->ComputeBestSplitForFeature(
          smaller_leaf_histogram_array_global_.get(), feature_index, real_feature_index,
    ...
    if (larger_is_feature_aggregated_[feature_index]) {
      ...
      this->ComputeBestSplitForFeature(
          larger_leaf_histogram_array_global_.get(), feature_index, real_feature_index,
...
template <typename TREELEARNER_T>
void VotingParallelTreeLearner<TREELEARNER_T>::Split(Tree* tree, int best_Leaf, int* left_leaf,
                                                     int* right_leaf) {
  ...
  if (best_split_info.left_count < best_split_info.right_count) {
    smaller_leaf_splits_global_->Init(
        *left_leaf, this->data_partition_.get(), best_split_info.left_sum_gradient,
        best_split_info.left_sum_hessian, best_split_info.left_output);
    larger_leaf_splits_global_->Init(
        *right_leaf, this->data_partition_.get(), best_split_info.right_sum_gradient,

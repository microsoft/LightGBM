 ...
 public:
  ...
  class Loader {
   public:
    ...
    // virtual BulkNeuralInput* FromBulkBond(const UnionBondInput& p_data) const
    // = 0;
  };

  // Create a neural input factory that knows how to create a variety of
  // NeuralInput classes.
  NeuralInputFactory();
  ...
  void AddTransform(const char *p_transform, LoadFunction p_loader,
                    bool p_replace = false);

  // Add registration for a particular transform, throwing exceptions if
  // registration fails, or if p_loader is NULL.
  void AddTransform(const char *p_transform, Loader::Ptr p_loader,
                    bool p_replace = false);
  ...
  NeuralInput *Load(const char *p_transform, IConfiguration &p_config,
  ...
  NeuralInput *Load(IConfiguration &p_config, int p_ID,
  ...
  template <class InputType,
            InputType *(*fun)(IConfiguration &p_config, const char *p_section,
                              IFeatureMap &p_featureMap)>
  static NeuralInput *LoadAdapt(IConfiguration &p_config, const char *p_section,
                                IFeatureMap &p_featureMap) {
...
class BulkNeuralInputFactory : public boost::noncopyable {
 public:
  ...
  // Create an instance of BulkNeuralInput from a list of NeuralInput objects
  // and their offset in the output array. Throws an exception if at least one
  // of the NeuralInput objects does not support bulk optimization.
  virtual std::unique_ptr<BulkNeuralInput> ConvertToBulkInput(
      const std::vector<InputAndIndex> &p_inputs,
      IFeatureMap &p_featureMap) const = 0;
};

...
namespace {
const FreeForm2::TypeImpl &DerefType(
    const FreeForm2::TypeImpl &p_arrayType,
  ...
  if (p_arrayType.Primitive() != FreeForm2::Type::Array) {
    ...
    err << "The array operand in an array dereference expression "
        << "is not an array (instead, it is a " << p_arrayType << ")";
    throw FreeForm2::ParseError(err.str(), p_sourceLocation);
  }

  const FreeForm2::ArrayType &arrayType =
      static_cast<const FreeForm2::ArrayType &>(p_arrayType);
  return arrayType.GetDerefType();
}
}  // namespace

FreeForm2::ArrayDereferenceExpression::ArrayDereferenceExpression(
...
const FreeForm2::TypeImpl &FreeForm2::ArrayDereferenceExpression::GetType()
    const {
  if (!m_index.GetType().IsIntegerType()) {
    std::ostringstream err;
    err << "The index operand in an array dereference expression "
        << "is not an integer type (instead, it is a " << m_index.GetType()
        << ")";
...
size_t FreeForm2::ArrayDereferenceExpression::GetNumChildren() const {
  return 2;
}

void FreeForm2::ArrayDereferenceExpression::Accept(Visitor &p_visitor) const {
...
void FreeForm2::ArrayDereferenceExpression::AcceptReference(
...
const FreeForm2::Expression &FreeForm2::ArrayDereferenceExpression::GetArray()
    const {
  return m_array;
}

const FreeForm2::Expression &FreeForm2::ArrayDereferenceExpression::GetIndex()
    const {
  return m_index;
}

size_t FreeForm2::ArrayDereferenceExpression::GetVersion() const {
  return m_version;
}

FreeForm2::VariableID FreeForm2::ArrayDereferenceExpression::GetBaseArrayId()
    const {

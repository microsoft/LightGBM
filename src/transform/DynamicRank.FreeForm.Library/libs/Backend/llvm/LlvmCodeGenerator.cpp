...
namespace
{
LlvmCodeGenerator::CompiledValue&
CompileFloatEquality(CompilationState& m_state,
                     LlvmCodeGenerator::CompiledValue& p_left,
                     LlvmCodeGenerator::CompiledValue& p_right)
{
    ...
    CHECK_LLVM_RET(type);
    FF2_ASSERT(p_right.getType() && type->getTypeID() == p_right.getType()->getTypeID());

    LlvmCodeGenerator::CompiledValue* small = llvm::ConstantFP::get(type, 10E-9);
    CHECK_LLVM_RET(small);
    LlvmCodeGenerator::CompiledValue* negSmall = llvm::ConstantFP::get(type, -10E-9);
    ...
    GenerateConditional cond(m_state,
                             *rightIsSmall,
    ...
    LlvmCodeGenerator::CompiledValue* diff = builder.CreateFSub(&p_left, &p_right);
    CHECK_LLVM_RET(diff);
    LlvmCodeGenerator::CompiledValue* normDiff = builder.CreateFDiv(diff, &p_right);
                               ...
                               llvm::Type* p_floatType)
{
    ...
    llvm::Function* rand = p_state.GetRuntimeLibrary().FindFunction(CStackSizedString("rand"));
    CHECK_LLVM_RET(rand);
    ...
    if (p_floatType
        && p_floatType->getPrimitiveSizeInBits() < rand->getReturnType()->getPrimitiveSizeInBits())
    {
...
LlvmCodeGenerator::CompiledValue*
FreeForm2::LlvmCodeGenerator::GetResult()
{
    return m_stack.top();
}

llvm::Function* 
FreeForm2::LlvmCodeGenerator::GetFuction() const
{
    return m_function;
}

void
FreeForm2::LlvmCodeGenerator::Visit(const SelectNthExpression& p_expr)
{
    // Ensure that the number of selection elements can be represented in an
    // Int type.
    const llvm::APInt maxInt = llvm::APInt::getSignedMaxValue(m_state.GetIntBits());
    ...
    index = &ValueConversion::Do(*index, 
                                 p_expr.GetIndex().GetType(), 
    ...
    for (llvm::APInt i(m_state.GetIntBits(), 1); i.slt(numChildren); ++i)
    {
        ...
        CompiledValue* childValue = llvm::ConstantInt::get(&m_state.GetIntType(), child);
    ...
    CompiledValue* bounds = llvm::ConstantInt::get(&m_state.GetIntType(), numChildren);
    ...
    // (out-of-bounds).
    CompiledValue* boundsSelect 
        = m_state.GetBuilder().CreateSelect(inBounds, 
                                            select, 
                                            &m_state.CreateZeroValue(p_expr.GetType()));
...
void
FreeForm2::LlvmCodeGenerator::Visit(const SelectRangeExpression& p_expr)
{
    llvm::IRBuilder<>& builder = m_state.GetBuilder();
    ...
    CompiledValue* bounds = builder.CreateExtractValue(array, ArrayCodeGen::boundsPosition);
    CHECK_LLVM_RET(bounds);
    ...
    CompiledValue* guardCondition = builder.CreateAnd(validateStart, validateCount);
    CHECK_LLVM_RET(guardCondition);

    GenerateConditional startGuard(m_state, *guardCondition, "SelectRange start guard");

    CompiledValue* srcCount = builder.CreateExtractValue(array, ArrayCodeGen::countPosition);
    ...
    CompiledValue* oldPtr = builder.CreateExtractValue(array, ArrayCodeGen::pointerPosition);
    ...
    CompiledValue* correctCount = builder.CreateSelect(checkCount, maxCount, count);
    CHECK_LLVM_RET(correctCount);

    // Get the new bounds bit vector.
    CompiledValue& subArrayBounds = ArrayCodeGen::ShiftBounds(m_state, *bounds, 1);

    CompiledValue& newBounds = ArrayCodeGen::UnshiftBound(m_state, subArrayBounds, *correctCount);

    // Get the new array count.
    CompiledValue* newCount = builder.CreateMul(subArrayCount, correctCount);
    CHECK_LLVM_RET(newCount);

    CompiledValue& newArray = ArrayCodeGen::CreateArray(m_state, destType, newBounds, *newCount, *newPtr);
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ArrayLengthExpression& p_expr)
{
    CompiledValue* array = m_stack.top();
    m_stack.pop();
    
    CompiledValue* bounds 
        = m_state.GetBuilder().CreateExtractValue(array, ArrayCodeGen::boundsPosition);
    CHECK_LLVM_RET(bounds);
    ...
    // bits.
    FF2_ASSERT(retType.getPrimitiveSizeInBits() < length.getType()->getPrimitiveSizeInBits());
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ArrayDereferenceExpression& p_expr)
{
    ...
    // Length of the array we're dereferencing is in the least significant bits.
    LlvmCodeGenerator::CompiledValue& arrayLength = ArrayCodeGen::MaskBounds(m_state, *bounds);
    ...
    GenerateConditional inBoundsCase(m_state, *inBounds, "array dereference guard");

    LlvmCodeGenerator::CompiledValue* val = NULL;
    LlvmCodeGenerator::CompiledValue* pointer = m_state.GetBuilder().CreateExtractValue(array, ArrayCodeGen::pointerPosition);
    CHECK_LLVM_RET(pointer);
    if (p_expr.GetType().Primitive() == Type::Array)
    {
        const ArrayType& arrayType = static_cast<const ArrayType&>(p_expr.GetType());

        LlvmCodeGenerator::CompiledValue& resultBounds = ArrayCodeGen::ShiftBounds(m_state, *bounds, 1);
        ...
        CompiledValue* arrayCount
            = m_state.GetBuilder().CreateExtractValue(array, ArrayCodeGen::countPosition);
        CHECK_LLVM_RET(arrayCount);

        CompiledValue* subArrayCount = m_state.GetBuilder().CreateUDiv(arrayCount, &arrayLength);
        CHECK_LLVM_RET(subArrayCount);

        CompiledValue* indexSelect = m_state.GetBuilder().CreateMul(subArrayCount, index);
        ...
        val = &ArrayCodeGen::CreateArray(m_state, arrayType, resultBounds, *subArrayCount, *elementPtr);
    }
    else
    {
...
void
FreeForm2::LlvmCodeGenerator::VisitReference(const ArrayDereferenceExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Allocate(const Allocation& p_allocation)
{
    switch (p_allocation.GetAllocationType())
    {
        case Allocation::ArrayLiteral:
        {
            ...
            const ArrayType& arrayInfo = static_cast<const ArrayType&>(p_allocation.GetType());

            PreAllocatedArray preAllocatedArray;

            // Calculate array bounds.
            std::pair<ArrayCodeGen::ArrayBoundsType, ArrayCodeGen::ArrayCountType> encoded 
                = ArrayCodeGen::EncodeDimensions(arrayInfo);
            preAllocatedArray.m_bounds 
                = llvm::ConstantInt::get(m_state.GetContext(), 
            ...
            if (arrayInfo.GetMaxElements() > 0)
            {
                ...
                LlvmCodeGenerator::CompiledValue* arraySize 
                    = llvm::ConstantInt::get(m_state.GetContext(), 
                                             llvm::APInt(sizeof(encoded.second) * 8, 
                                                         arrayInfo.GetMaxElements()));
                CHECK_LLVM_RET(arraySize);
                preAllocatedArray.m_array = m_state.GetBuilder().CreateAlloca(&elementType, arraySize);
            }
            else
            {
            ...
            m_allocatedArrays.insert(std::make_pair(p_allocation.GetAllocationId(), preAllocatedArray));
            
            break;
        }

        // Declarations not implemented in FF2.
        case Allocation::Declaration: __attribute__((__fallthrough__));
        
        // Literal streams not implemented in FF2
        case Allocation::LiteralStream: __attribute__((__fallthrough__));
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ArrayLiteralExpression& p_expr)
{
    ...
    {
        LlvmCodeGenerator::CompiledValue* element = m_state.GetBuilder().CreateConstInBoundsGEP1_32(preAllocatedArray.m_array, i);
        CHECK_LLVM_RET(element);
        m_state.GetBuilder().CreateStore(m_stack.top(), element);
        m_stack.pop();
    }

    m_stack.push(&ArrayCodeGen::CreateArray(m_state,
...
bool
FreeForm2::LlvmCodeGenerator::AlternativeVisit(const ConditionalExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ConditionalExpression& p_expr)
{
...
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToFloatExpression& p_expr)
{
    ...
    m_stack.push(&ValueConversion::Do(*child, p_expr.GetChildType(), p_expr.GetType(), m_state));
}


void 
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToIntExpression& p_expr)
{
    CompiledValue* child = m_stack.top();
    m_stack.pop();
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToUInt64Expression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToInt32Expression& p_expr)
{
    CompiledValue* child = m_stack.top();
    m_stack.pop();
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToUInt32Expression& p_expr)
{
    CompiledValue* child = m_stack.top();
    m_stack.pop();
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToBoolExpression& p_expr)
{
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const ConvertToImperativeExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const DeclarationExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const DirectPublishExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ExternExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralIntExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralUInt64Expression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralInt32Expression& p_expr)
{
    const llvm::Type& returnType = m_state.GetType(p_expr.GetType());
    FF2_ASSERT(returnType.isIntegerTy());
    ...
    const llvm::APInt value(returnType.getPrimitiveSizeInBits(), static_cast<UInt64>(intValue), true);
    CompiledValue* ret = llvm::ConstantInt::get(m_state.GetContext(), value);
    CHECK_LLVM_RET(ret);
    m_stack.push(ret);
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralUInt32Expression& p_expr)
{
    const llvm::Type& returnType = m_state.GetType(p_expr.GetType());
    FF2_ASSERT(returnType.isIntegerTy());
    const llvm::APInt value(returnType.getPrimitiveSizeInBits(), 
                            p_expr.GetConstantValue().m_int, 
                            false);
    CompiledValue* ret = llvm::ConstantInt::get(m_state.GetContext(), value);
    CHECK_LLVM_RET(ret);
    m_stack.push(ret);
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralFloatExpression& p_expr)
{
    CompiledValue* ret 
        = llvm::ConstantFP::get(m_state.GetContext(), 
                                llvm::APFloat(p_expr.GetConstantValue().m_float));
    CHECK_LLVM_RET(ret);
    m_stack.push(ret);
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralBoolExpression& p_expr)
{
    CompiledValue* ret 
        = llvm::ConstantInt::get(m_state.GetContext(), 
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const LiteralVoidExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const LiteralStreamExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const LiteralWordExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LiteralInstanceHeaderExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
bool
FreeForm2::LlvmCodeGenerator::AlternativeVisit(const LetExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const LetExpression&)
{
...
bool
FreeForm2::LlvmCodeGenerator::AlternativeVisit(const BlockExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const BlockExpression&)
{
    // This should have been managed by AlternativeVisit.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const MutationExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const MatchExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const MatchOperatorExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const MatchGuardExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const MatchBindExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const MemberAccessExpression&)
{
    // Not yet supported.
    Unreachable(__FILE__, __LINE__);
}
...
//void
//FreeForm2::LlvmCodeGenerator::Visit(const NeuralInputResultExpression& p_expr)
//{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const PhiNodeExpression& p_expr)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const PublishExpression& p_expr)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const FeatureRefExpression& p_expr)
{   
    FF2_ASSERT(m_state.GetFeatureArgument() != NULL);

    // Call below encodes size of m_index, so we check it's what we expect.
    llvm::Value* address 
        = m_state.GetBuilder().CreateConstInBoundsGEP1_32(m_state.GetFeatureArgument(),
                                                          p_expr.m_index, 
    ...
    // full integer.
    BOOST_STATIC_ASSERT(sizeof(Result::IntType) > sizeof(Expression::FeatureType));
...
void
FreeForm2::LlvmCodeGenerator::Visit(const FunctionExpression&)
{
    // Not supported in FF2.
    Unreachable(__FILE__, __LINE__);
}
...
bool
FreeForm2::LlvmCodeGenerator::AlternativeVisit(const RangeReduceExpression& p_expr)
{
    llvm::IRBuilder<>& builder = m_state.GetBuilder();
    ...
    CompiledValue* step = llvm::ConstantInt::get(m_state.GetContext(),
                                                 llvm::APInt(m_state.GetIntBits(), 1));
    CHECK_LLVM_RET(step);
    ...
    llvm::BasicBlock* condBlock 
        = llvm::BasicBlock::Create(m_state.GetContext(), llvm::Twine("range-reduce-condition"), currentFun);
    ...
    llvm::BasicBlock* loopBlock 
        = llvm::BasicBlock::Create(m_state.GetContext(), llvm::Twine("range-reduce-loop"), currentFun);
    CHECK_LLVM_RET(loopBlock);
    llvm::BasicBlock* afterLoopBlock 
        = llvm::BasicBlock::Create(m_state.GetContext(), llvm::Twine("range-reduce-after"), currentFun);
    ...
    llvm::PHINode* loopVar = builder.CreatePHI(index.getType(), 2, llvm::Twine("range-reduce-loop-var"));
    ...
    llvm::PHINode* accVar = builder.CreatePHI(initial.getType(), 2, llvm::Twine("range-reduce-acc-var"));
    CHECK_LLVM_RET(accVar);
    accVar->addIncoming(&initial, originalBlock);
    m_state.SetVariableValue(p_expr.GetReduceId(), *accVar);

    // Visit the initial condition (that the low limit is less than the high limit).  
    // This guards against iterating through the loop once under these conditions.
    ...
    llvm::Function* add = llvm::Intrinsic::getDeclaration(&m_state.GetModule(),
    ...
    llvm::PHINode* end = builder.CreatePHI(initial.getType(), 2, llvm::Twine("range-reduce-skip-acc"));
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ForEachLoopExpression&)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ComplexRangeLoopExpression&)
{
    // TODO: Implement (TFS #461742)
    Unreachable(__FILE__, __LINE__);
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const RangeReduceExpression& p_expr)
{
    // Handled by AlternativeVisit.
    Unreachable(__FILE__, __LINE__);
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const UnaryOperatorExpression& p_expr)
{
    switch (p_expr.m_op)
    {
...
void
FreeForm2::LlvmCodeGenerator::VisitUnaryMinus(const UnaryOperatorExpression& p_expr)
{
    LlvmCodeGenerator::CompiledValue* child = m_stack.top();
    m_stack.pop();
...
LlvmCodeGenerator::CompiledValue& 
CompileLogCall(CompilationState& p_state,
               llvm::ArrayRef<llvm::Type*> p_type,
               LlvmCodeGenerator::CompiledValue& p_child) 
{
    llvm::Function* fun = llvm::Intrinsic::getDeclaration(&p_state.GetModule(), 
                                                          llvm::Intrinsic::log,
                                                          p_type);
    CHECK_LLVM_RET(fun);
    LlvmCodeGenerator::CompiledValue* log = p_state.GetBuilder().CreateCall(fun, &p_child);
    CHECK_LLVM_RET(log);
    return *log;
}
...
void
FreeForm2::LlvmCodeGenerator::VisitUnaryLog(const UnaryOperatorExpression& p_expr, 
                                            bool p_addOne)
{
    ...
    arg = &ValueConversion::Do(*arg, p_expr.m_child.GetType(), p_expr.GetType(), m_state);
    ...
    // negative infinity.
    LlvmCodeGenerator::CompiledValue* zero 
...
void
FreeForm2::LlvmCodeGenerator::VisitUnaryNot(const UnaryOperatorExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::VisitUnaryAbs(const UnaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType().IsFloatingPointType() || p_expr.GetType().IsIntegerType());
    ...
    if (p_expr.GetType().IsIntegerType())
    {
        ...
        cond = m_state.GetBuilder().CreateICmpSGE(child, zero, llvm::Twine("abs test"));
    ...
    {
        // Generate the condition.
        cond = m_state.GetBuilder().CreateFCmpOGE(child, zero, llvm::Twine("abs test"));
...
void 
FreeForm2::LlvmCodeGenerator::VisitUnaryRound(const UnaryOperatorExpression& p_expr)
{
    ...
    CHECK_LLVM_RET(zero);
    CompiledValue* cmp = builder.CreateFCmpOGE(child, zero, llvm::Twine("round comparison"));
    CHECK_LLVM_RET(cmp);
...
void FreeForm2::LlvmCodeGenerator::VisitUnaryTrunc(const UnaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType().IsIntegerType());
    if (p_expr.m_child.GetType().IsIntegerType())
    {
    ...
    CompiledValue* ret 
        = &ValueConversion::Do(*child, p_expr.m_child.GetType(), p_expr.GetType(), m_state);
...
void
FreeForm2::LlvmCodeGenerator::Visit(const BinaryOperatorExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::VisitPlus(const BinaryOperatorExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::VisitMinus(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
...
void
FreeForm2::LlvmCodeGenerator::VisitMultiply(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
...
void
FreeForm2::LlvmCodeGenerator::VisitAnd(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
...
void
FreeForm2::LlvmCodeGenerator::VisitOr(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
...
void
FreeForm2::LlvmCodeGenerator::VisitLog(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType().IsFloatingPointType());
    ...
    {
        left = &ValueConversion::Do(*left, p_expr.GetChildType(), p_expr.GetType(), m_state);
        right = &ValueConversion::Do(*right, p_expr.GetChildType(), p_expr.GetType(), m_state);
    }
    ...
    CompiledValue* cmp = m_state.GetBuilder().CreateAnd(cmpLeft, cmpRight, llvm::Twine("log guard"));
    ...
    CHECK_LLVM_RET(negInfinity);
    CompiledValue* select = m_state.GetBuilder().CreateSelect(cmp, log, negInfinity);
    CHECK_LLVM_RET(select);
    m_stack.push(select);
}
...
typedef LlvmCodeGenerator::CompiledValue& (*OperatorFun)(CompilationState& m_state, 
                                                         LlvmCodeGenerator::CompiledValue& p_left, 
                                                         LlvmCodeGenerator::CompiledValue& p_right);

    
static LlvmCodeGenerator::CompiledValue& 
GenerateSDivInstruction(CompilationState& m_state, 
                       LlvmCodeGenerator::CompiledValue& p_left, 
                       LlvmCodeGenerator::CompiledValue& p_right)
{
...
static LlvmCodeGenerator::CompiledValue& 
GenerateUDivInstruction(CompilationState& m_state, 
                       LlvmCodeGenerator::CompiledValue& p_left, 
                       LlvmCodeGenerator::CompiledValue& p_right)
{
    LlvmCodeGenerator::CompiledValue* div 
...
static LlvmCodeGenerator::CompiledValue& 
GenerateSModInstruction(CompilationState& m_state, 
                       LlvmCodeGenerator::CompiledValue& p_left, 
                       LlvmCodeGenerator::CompiledValue& p_right)
{
...
static LlvmCodeGenerator::CompiledValue& 
GenerateUModInstruction(CompilationState& m_state, 
                       LlvmCodeGenerator::CompiledValue& p_left, 
                       LlvmCodeGenerator::CompiledValue& p_right)
{
    LlvmCodeGenerator::CompiledValue* mod 
...
LlvmCodeGenerator::CompiledValue& 
CompileGuardedDivMod(CompilationState& m_state,
                     LlvmCodeGenerator::CompiledValue& p_left, 
                     LlvmCodeGenerator::CompiledValue& p_right,
                     OperatorFun p_operator, 
                     LlvmCodeGenerator::CompiledValue& underflowResult) 
{
    ...
    FF2_ASSERT(p_left.getType() && p_right.getType()
               ...
               && p_right.getType()->isIntegerTy(p_left.getType()->getPrimitiveSizeInBits()));
    ...
    LlvmCodeGenerator::CompiledValue* zeroCond 
        = m_state.GetBuilder().CreateICmpNE(zero, 
                                            &p_right, 
    ...
    LlvmCodeGenerator::CompiledValue* minInt 
        = llvm::ConstantInt::get(p_left.getType(), 
...
void
FreeForm2::LlvmCodeGenerator::VisitDivides(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
...
void
FreeForm2::LlvmCodeGenerator::VisitMod(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
    ...
    if (p_expr.GetType().IsIntegerType())
    {
        ...
        if (p_expr.GetType().IsSigned())
        {
            ret = &CompileGuardedDivMod(m_state, *left, *right, GenerateSModInstruction, zero);
        }
        else
        {
            ret = &CompileGuardedDivMod(m_state, *left, *right, GenerateUModInstruction, zero);
        }
    }
    else if (p_expr.GetType().IsFloatingPointType())
    {
...
void
FreeForm2::LlvmCodeGenerator::VisitPow(const BinaryOperatorExpression& p_expr)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
    FF2_ASSERT(p_expr.GetType().IsIntegerType()
    ...
    if (p_expr.GetChildType().IsIntegerType())
    {
        ...
        left = &ValueConversion::Do(*left, p_expr.GetChildType(), floatType, m_state);
        right = &ValueConversion::Do(*right, p_expr.GetChildType(), floatType, m_state);
    }
    else
    {
        FF2_ASSERT(left->getType()
                   && right->getType()
    ...
    llvm::Function* fun 
        = llvm::Intrinsic::getDeclaration(&m_state.GetModule(), 
                                          llvm::Intrinsic::pow,
                                          floatType);
    CHECK_LLVM_RET(fun);
    ...
    if (p_expr.GetType().IsIntegerType())
    {
        ret = &ValueConversion::Do(*ret, TypeImpl::GetFloatInstance(true), p_expr.GetType(), m_state);
    }
...
void
FreeForm2::LlvmCodeGenerator::VisitMaxMin(const BinaryOperatorExpression& p_expr, 
                                          bool p_minimum)
{
    FF2_ASSERT(p_expr.GetType() == p_expr.GetChildType());
    ...
    CHECK_LLVM_RET(cmp);
    select = m_state.GetBuilder().CreateSelect(cmp,
                                               p_minimum ? right : left, 
...
void
FreeForm2::LlvmCodeGenerator::VisitEqual(const BinaryOperatorExpression& p_expr, 
                                         bool p_inequality)
{
    CompiledValue* left = m_stack.top();
    m_stack.pop();
...
void
FreeForm2::LlvmCodeGenerator::VisitCompare(const BinaryOperatorExpression& p_expr, 
                                           bool p_less, 
                                           bool p_equal)
{ 
    CompiledValue* left = m_stack.top();
...
llvm::Function* 
FreeForm2::LlvmCodeGenerator::CreateFeatureFunction(const TypeImpl& p_returnType)
{
    ...
    else if (m_destinationFunctionType == CompilerFactory::DocumentSetEvaluation)
    {
        // Create feature array input type.
        llvm::PointerType* featurePointerType
            = llvm::PointerType::get(llvm::PointerType::get(&m_state.GetFeatureType(), 0), 0);
        CHECK_LLVM_RET(featurePointerType);
        arguments.push_back(featurePointerType);

        // Create document index input type.
        llvm::IntegerType* docIndexType
            =&m_state.GetInt32Type();
        CHECK_LLVM_RET(docIndexType);
        arguments.push_back(docIndexType);

        // Create document count input type.
        llvm::IntegerType* docCountType
            = &m_state.GetInt32Type();
    ...
    llvm::Function* fun = llvm::Function::Create(funType, 
                                                 llvm::Function::ExternalLinkage, 
                                                 llvm::Twine("<freeformmain>"), 
                                                 &m_state.GetModule());
    CHECK_LLVM_RET(fun);
    ...
    else if (m_destinationFunctionType == CompilerFactory::DocumentSetEvaluation)
    {
        ...
        llvm::Value* featureArrayPointer
            = m_state.GetBuilder().CreateInBoundsGEP(&m_state.GetFeatureArrayPointer(),
                                                     &m_state.GetAggregatedDocumentIndex(),
                                                    llvm::Twine("init feature array"));
        CHECK_LLVM_RET(featureArrayPointer);

        llvm::Value* featureArray = m_state.GetBuilder().CreateLoad(featureArrayPointer);
...
void 
FreeForm2::LlvmCodeGenerator::CreateAllocations()
{
...
FreeForm2::LlvmCodeGenerator::CreateReturn(const TypeImpl& p_type)
{
    ...
    if (p_type.Primitive() == Type::Array)
    {
        ...
        ret = m_state.GetBuilder().CreateRet(&ArrayCodeGen::IssueReturn(m_state, 
                                                                        value, 
                                                                        arrayType,
                                                                        arraySpace));
    }
...
bool 
FreeForm2::LlvmCodeGenerator::AlternativeVisit(const FeatureSpecExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const FeatureSpecExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::Visit(const FeatureGroupSpecExpression&)
{
    // Not supported in FF2.
    Unreachable(__FILE__, __LINE__);
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const ReturnExpression&)
{
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const StreamDataExpression&)
{
    // Not supported by FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const UpdateStreamDataExpression&)
{
    // Not supported by FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const VariableRefExpression& p_expr)
{
...
void
FreeForm2::LlvmCodeGenerator::VisitReference(const VariableRefExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::VisitReference(const ThisExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::VisitReference(const UnresolvedAccessExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
//        = dynamic_cast<const FreeForm2::NeuralInputResultExpression*>(&block->GetChild(0));
...
//        = dynamic_cast<const FreeForm2::FeatureSpecExpression*>(&input->m_child);
//
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const ImportFeatureExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const StateExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void 
FreeForm2::LlvmCodeGenerator::Visit(const StateMachineExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ExecuteMachineExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ExecuteStreamRewritingStateMachineGroupExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ExecuteMachineGroupExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const YieldExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const RandFloatExpression& p_expr)
{
    CompiledValue& randValue = CreateRandomFloat(m_state, &m_state.GetType(p_expr.GetType()));
    m_stack.push(&randValue);
}


void
FreeForm2::LlvmCodeGenerator::Visit(const RandIntExpression& p_expr)
{
    llvm::IRBuilder<>& builder = m_state.GetBuilder();
    ...
    CompiledValue* intValInRange = builder.CreateFPToSI(valInRange, &m_state.GetType(p_expr.GetType()));
...
void
FreeForm2::LlvmCodeGenerator::Visit(const ThisExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const UnresolvedAccessExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const TypeInitializerExpression&)
{
    // Not supported in FF2.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const AggregateContextExpression&)
{
    // TODO: Implement me.
    FF2_UNREACHABLE();
}
...
void
FreeForm2::LlvmCodeGenerator::Visit(const DebugExpression&)
{
...
llvm::Function&
FreeForm2::LlvmCodeGenerator::Compile(

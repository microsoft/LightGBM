...
-  For running on AMD, get AMD APP SDK (downloads `for Linux`_ and `for Windows`_) (you may want to replace the ``OpenCL.dll`` from GPU driver package with the one from the SDK, if the one shipped with the driver lacks some functions).

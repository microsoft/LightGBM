...
__device__ __forceinline__ T ShuffleReduceSum(T value, T* shared_mem_buffer, const size_t len) {
  const uint32_t warpLane = threadIdx.x % warpSize;
  const uint32_t warpID = threadIdx.x / warpSize;
  const data_size_t warp_len =
      min(static_cast<data_size_t>(warpSize),
          static_cast<data_size_t>(len) - static_cast<data_size_t>(warpID * warpSize));
...
__device__ __forceinline__ void BitonicArgSort_1024(const VAL_T* scores, INDEX_T* indices,
  ...
  for (INDEX_T outer_depth = depth - 1; outer_depth >= 1; --outer_depth) {
    ...
    const bool ascending =
        ASCENDING ? (outer_segment_index % 2 == 0) : (outer_segment_index % 2 > 0);
...
__device__ __forceinline__ void BitonicArgSort_2048(const VAL_T* scores, INDEX_T* indices) {
  for (INDEX_T base = 0; base < 2048; base += 1024) {
    for (INDEX_T outer_depth = 10; outer_depth >= 1; --outer_depth) {
      ...
      const INDEX_T outer_segment_index = threadIdx.x / outer_segment_length;
      const bool ascending = ((base == 0) ^ ASCENDING) ? (outer_segment_index % 2 > 0)
                                                       : (outer_segment_index % 2 == 0);
...
__device__ void BitonicArgSortDevice(const VAL_T* values, INDEX_T* indices, const int len) {
  ...
  for (int block_index = 0; block_index < num_blocks; ++block_index) {
    const int this_index =
        block_index * static_cast<int>(BLOCK_DIM) + static_cast<int>(threadIdx.x);
    if (this_index < len) {
    ...
    for (int depth = max_depth - 1; depth > max_depth - static_cast<int>(MAX_DEPTH); --depth) {
      ...
      const bool ascending = ASCENDING ? (segment_index % 2 == 0) : (segment_index % 2 == 1);
      {
        ...
        const int offset = (segment_index == num_total_segment - 1 && ascending == ASCENDING)
                               ? (num_total_segment * segment_length - len)
                               : 0;
        if (half_segment_index % 2 == 0) {
  ...
  for (int depth = max_depth - static_cast<int>(MAX_DEPTH); depth >= 1; --depth) {
    ...
    for (int inner_depth = depth + 1; inner_depth <= max_depth - static_cast<int>(MAX_DEPTH);
         ++inner_depth) {
      const int half_segment_length = (1 << (max_depth - inner_depth - 1));
      for (int block_index = 0; block_index < num_blocks; ++block_index) {
        const int this_index =
    ...
    for (int block_index = 0; block_index < num_blocks; ++block_index) {
      ...
      __syncthreads();
      for (int inner_depth = max_depth - static_cast<int>(MAX_DEPTH) + 1; inner_depth < max_depth;
           ++inner_depth) {
        const int half_segment_length = (1 << (max_depth - inner_depth - 1));
        const int half_segment_index = this_index / half_segment_length;
        if (half_segment_index % 2 == 0) {
...
void BitonicArgSortItemsGlobal(const double* scores, const int num_queries,
                               const data_size_t* cuda_query_boundaries, data_size_t* out_indices);
...
__device__ void ShuffleSortedPrefixSumDevice(const VAL_T* in_values, const INDEX_T* sorted_indices,
                                             REDUCE_VAL_T* out_values, const INDEX_T num_data) {
  __shared__ REDUCE_VAL_T shared_buffer[32];
  const INDEX_T num_data_per_thread =
      (num_data + static_cast<INDEX_T>(blockDim.x) - 1) / static_cast<INDEX_T>(blockDim.x);
...
template <typename VAL_T, typename INDEX_T, typename WEIGHT_T, typename WEIGHT_REDUCE_T,
          bool ASCENDING, bool USE_WEIGHT>
__global__ void PercentileGlobalKernel(const VAL_T* values, const WEIGHT_T* weights,
                                       const INDEX_T* sorted_indices,
                                       const WEIGHT_REDUCE_T* weights_prefix_sum,
                                       const double alpha, const INDEX_T len, VAL_T* out_value) {
  if (!USE_WEIGHT) {
    ...
    const VAL_T v2 = values[sorted_indices[pos]];
    *out_value =
        static_cast<VAL_T>(v1 - (v1 - v2) * (threshold - weights_prefix_sum[pos - 1]) /
...
template <typename VAL_T, typename INDEX_T, typename WEIGHT_T, typename WEIGHT_REDUCE_T,
          bool ASCENDING, bool USE_WEIGHT>
void PercentileGlobal(const VAL_T* values, const WEIGHT_T* weights, INDEX_T* indices,
                      WEIGHT_REDUCE_T* weights_prefix_sum,
                      WEIGHT_REDUCE_T* weights_prefix_sum_buffer, const double alpha,
                      const INDEX_T len, VAL_T* cuda_out_value) {
  if (len <= 1) {
  ...
  if (USE_WEIGHT) {
    GlobalInclusiveArgPrefixSum<WEIGHT_T, WEIGHT_REDUCE_T, INDEX_T>(
        indices, weights, weights_prefix_sum, weights_prefix_sum_buffer, static_cast<size_t>(len));
  }
  SynchronizeCUDADevice(__FILE__, __LINE__);
  PercentileGlobalKernel<VAL_T, INDEX_T, WEIGHT_T, WEIGHT_REDUCE_T, ASCENDING, USE_WEIGHT>
      <<<1, GLOBAL_PREFIX_SUM_BLOCK_SIZE>>>(values, weights, indices, weights_prefix_sum, alpha,
                                            len, cuda_out_value);
  SynchronizeCUDADevice(__FILE__, __LINE__);
}

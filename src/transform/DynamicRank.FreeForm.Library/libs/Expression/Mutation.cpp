...
FreeForm2::MutationExpression::MutationExpression(const Annotations& p_annotations,
                                                  const Expression& p_lvalue, 
...
void 
FreeForm2::MutationExpression::Accept(Visitor& p_visitor) const
{
...
FreeForm2::MutationExpression::GetType() const
{
...
size_t FreeForm2::MutationExpression::GetNumChildren() const
{
    return 2;
}
...
FreeForm2::MutationExpression::GetLeftValue() const
{
    return m_lvalue;
}
...
const FreeForm2::Expression& 
FreeForm2::MutationExpression::GetRightValue() const
{
    return m_rvalue;
}
...
{
    memcpy(m_initializers, p_initializers, sizeof(Initializer) * m_numInitializers);
    ValidateMembers();
}
...
FreeForm2::TypeInitializerExpression::Alloc(const Annotations& p_annotations,
                                            ...
                                            size_t p_numInitializers)
{
    const size_t memSize = sizeof(TypeInitializerExpression) 
...
void
FreeForm2::TypeInitializerExpression::ValidateMembers() const
{
    ...
    else
    {
        ...
        const StateMachineType& type = static_cast<const StateMachineType&>(m_type);
        for (const StructType::Member* iter = type.BeginMembers(); iter != type.EndMembers(); ++iter)
        {
    ...
    for (const Initializer* iter = BeginInitializers(); iter != EndInitializers(); ++iter)
    {
        ...
        {
            std::ostringstream err;
            err << "Mismatched types in initializer (" << memberType << " and " << initType << ")";
            throw ParseError(err.str(), GetSourceLocation());
        }
...
void
FreeForm2::TypeInitializerExpression::Accept(Visitor& p_visitor) const
{
...
const FreeForm2::TypeImpl&
FreeForm2::TypeInitializerExpression::GetType() const
{
    return m_type;
}
...
size_t
FreeForm2::TypeInitializerExpression::GetNumChildren() const
{

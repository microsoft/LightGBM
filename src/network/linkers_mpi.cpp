...
Linkers::~Linkers() {
  // Don't call MPI_Finalize() here: If the destructor was called because only
  // this node had an exception, calling MPI_Finalize() will cause all nodes to
  // hang. Instead we will handle finalize/abort for MPI in main().
}
...
void Linkers::MpiAbortIfIsParallel() {
  try {
    ...
    std::cerr << "Exception was raised before aborting MPI. Aborting process..."
              << std::endl
              << std::flush;
    abort();
  }

 ...
 public:
  ...
  void ResetGoss() {
    ...
    if (config_->top_rate + config_->other_rate <= 0.5) {
      auto bag_data_cnt = static_cast<data_size_t>(
          (config_->top_rate + config_->other_rate) * num_data_);
  ...
  data_size_t BaggingHelper(Random* cur_rand, data_size_t start,
    ...
    ArrayArgs<score_t>::ArgMaxAtK(
        &tmp_gradients, 0, static_cast<int>(tmp_gradients.size()), top_k - 1);
      ...
      if (grad >= threshold) {
        ...
        if (cur_rand->NextFloat() < prob) {
          ...
          for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_;
               ++cur_tree_id) {
            size_t idx =
                static_cast<size_t>(cur_tree_id) * num_data_ + start + i;
...
#pragma omp parallel for schedule(static, 1)
    for (int i = 0; i < num_threads_; ++i) {
      ...
      data_size_t cur_left_count = BaggingHelper(
          &cur_rand, cur_start, cur_cnt, tmp_indices_.data() + cur_start,
          tmp_indice_right_.data() + cur_start);
    ...
    for (int i = 0; i < num_threads_; ++i) {
      ...
      if (right_cnts_buf_[i] > 0) {
        std::memcpy(
            ...
            tmp_indice_right_.data() + offsets_buf_[i],

...
//   DEBUG_PRINT(dbgs() << "JIT: Lazy stub emitted at [" << Stub << "] for function '"
//         << F->getName() << "'\n");
...
bool JITEmitter::finishFunction(MachineFunction &F) {
  ...
  if (!Relocations.empty()) {
    ...
    for (unsigned i = 0, e = Relocations.size(); i != e; ++i) {
      ...
      if (!MR.letTargetResolve()) {
        if (MR.isExternalSymbol()) {
          ResultPtr = TheJIT->getPointerToNamedFunction(MR.getExternalSymbol(),
                                                        false);
        //   DEBUG_PRINT(dbgs() << "JIT: Map \'" << MR.getExternalSymbol() << "\' to ["
        //                << ResultPtr << "]\n");
      ...
      if (MR.isGOTRelative() && MemMgr->isManagingGOT()) {
        ...
        //   DEBUG_PRINT(dbgs() << "JIT: GOT was out of date for " << ResultPtr
  ...
  if (MemMgr->isManagingGOT()) {
    ...
    //   DEBUG_PRINT(dbgs() << "JIT: GOT was out of date for " << (void*)BufferBegin
    //                << " pointing at " << ((void**)MemMgr->getGOTBase())[idx]
    //                << "\n");
...
//   DEBUG_PRINT(dbgs() << "JIT: Ran out of space for native code.  Reattempting.\n");
...
//   DEBUG_PRINT(dbgs() << "JIT: Emitted constant pool at [" << ConstantPoolBase
...
llvm::GetJitMachineRelocations(const JIT* p_jit)
{
    const JITEmitter* emitter = static_cast<const JITEmitter*>(p_jit->getCodeEmitter());

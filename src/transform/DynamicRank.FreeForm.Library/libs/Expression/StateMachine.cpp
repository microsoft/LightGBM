...
void
FreeForm2::StateExpression::Accept(Visitor& p_visitor) const
{
...
{
    const size_t allocSize = sizeof(StateMachineExpression) 
        + (p_numStates > 0 ? (p_numStates - 1) * sizeof(const StateExpression*) : 0);
...
void
FreeForm2::StateMachineExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();
...
{
    size_t bytes = sizeof(ExecuteMachineExpression) 
        + (std::max<size_t>(p_numYieldActions, 1) - 1) 
        * sizeof(std::pair<std::string, const Expression*>);
...
void 
FreeForm2::ExecuteMachineExpression::DeleteAlloc(ExecuteMachineExpression* p_allocated)
{
...
void
FreeForm2::ExecuteMachineExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();
...
void 
FreeForm2::ExecuteMachineGroupExpression::DeleteAlloc(ExecuteMachineGroupExpression* p_allocated)
{
    // Manually call dtor for expression.
...
void
FreeForm2::ExecuteMachineGroupExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();
...
void 
FreeForm2::ExecuteStreamRewritingStateMachineGroupExpression::DeleteAlloc(ExecuteStreamRewritingStateMachineGroupExpression* p_allocated)
{
    // Manually call dtor for expression.
...
void
FreeForm2::ExecuteStreamRewritingStateMachineGroupExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();
...
bool
FreeForm2::ExecuteStreamRewritingStateMachineGroupExpression::IsNearChunk() const
{
...
void
FreeForm2::YieldExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();

...
namespace
{
    ...
    void UnifyLoopCounters(const Iter p_iter, 
                           unsigned int p_numCounters,
                           FreeForm2::TypeManager& p_typeManager, 
                           FreeForm2::SimpleExpressionOwner& p_owner)
    {
        using namespace FreeForm2;
        BOOST_CONCEPT_ASSERT((boost_concepts::WritableIterator<Iter, const Expression*>));
        const TypeImpl* unifiedType = &TypeImpl::GetUnknownType();

        Iter iter = p_iter;
        for (size_t i = 0; i < p_numCounters; ++i, ++iter)
        {
            unifiedType = &TypeUtil::Unify(*unifiedType, (*iter)->GetType(), p_typeManager, false, true);
        }
...
bool
FreeForm2::OperandPromotionVisitor::AlternativeVisit(const BinaryOperatorExpression& p_expr)
{
...
void
FreeForm2::OperandPromotionVisitor::Visit(const BinaryOperatorExpression& p_expr)
{
...
void
FreeForm2::OperandPromotionVisitor::Visit(const FunctionCallExpression& p_expr)
{
    ...
    for (size_t i = 0; i < p_expr.GetNumParameters(); ++i, ++iter)
    {
        FF2_ASSERT(iter != GetStack().rend());
        const Expression& expression = **iter;
        const TypeImpl& parameterType = *parameterTypes[p_expr.GetNumParameters() - i - 1];
...
void
FreeForm2::OperandPromotionVisitor::Visit(const ConditionalExpression& p_expr)
{
...
void
FreeForm2::OperandPromotionVisitor::Visit(const RangeReduceExpression& p_expr)
{
...
void
FreeForm2::OperandPromotionVisitor::Visit(const ForEachLoopExpression& p_expr)
{
    FF2_ASSERT(GetStack().size() >= 4);
...
void
FreeForm2::OperandPromotionVisitor::Visit(const ComplexRangeLoopExpression& p_expr)
{

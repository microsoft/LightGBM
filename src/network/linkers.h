...
class Linkers {
 public:
  ...
  /*!
   * \brief Abort the MPI session if it was initialized (called in case there
   * was a error that needs abrupt ending)
   */
...
inline void Linkers::SendRecv(int send_rank, char* send_data, int64_t send_len,
                              int recv_rank, char* recv_data,
                              int64_t recv_len) {
  auto start_time = std::chrono::high_resolution_clock::now();
  std::thread send_worker([this, send_rank, send_data, send_len]() {
...
inline void Linkers::SendRecv(int send_rank, char* send_data, int send_len,
  ...
  if (send_len < SocketConfig::kSocketBufferSize) {
    ...
    // if buffer is not enough, use another thread to send, since send will be
    // blocking
    std::thread send_worker([this, send_rank, send_data, send_len]() {
      Send(send_rank, send_data, send_len);
    });
...
inline void Linkers::Recv(int rank, char* data, int len) const {
  ...
  int read_cnt = 0;
  while (read_cnt < len) {
    MPI_SAFE_CALL(MPI_Recv(data + read_cnt, len - read_cnt, MPI_BYTE, rank,
...
inline void Linkers::Send(int rank, char* data, int len) const {
  ...
  MPI_SAFE_CALL(
      MPI_Isend(data, len, MPI_BYTE, rank, 0, MPI_COMM_WORLD, &send_request));
...
inline void Linkers::SendRecv(int send_rank, char* send_data, int send_len,
  ...
  MPI_SAFE_CALL(MPI_Isend(send_data, send_len, MPI_BYTE, send_rank, 0,
                          MPI_COMM_WORLD, &send_request));
  // then receive, blocking
  MPI_Status status;
  int read_cnt = 0;
  while (read_cnt < recv_len) {
    MPI_SAFE_CALL(MPI_Recv(recv_data + read_cnt, recv_len - read_cnt, MPI_BYTE,

...
if [[ $TASK == "check-links" ]]; then
    ...
    lychee --exclude="${lychee_exclude_github[@]}" "${lychee_args[@]}"
    lychee --exclude="${lychee_exclude_common[@]}" --include="^https://github\.com" --include_fragments=false "${lychee_args[@]}" || exit 1
fi

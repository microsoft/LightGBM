...
class BaggingSampleStrategy : public SampleStrategy {
 public:
  BaggingSampleStrategy(const Config* config, const Dataset* train_data,
                        const ObjectiveFunction* objective_function, int num_tree_per_iteration)
  ...
  void Bagging(int iter, TreeLearner* tree_learner, score_t* /*gradients*/,
               score_t* /*hessians*/) override {
    ...
    if ((bag_data_cnt_ < num_data_ && iter % config_->bagging_freq == 0) || need_re_bagging_) {
      ...
      if (!config_->bagging_by_query) {
        auto left_cnt = bagging_runner_.Run<true>(
            num_data_,
            [=](int, data_size_t cur_start, data_size_t cur_cnt, data_size_t* left, data_size_t*) {
              data_size_t cur_left_count = 0;
              if (balanced_bagging_) {
                cur_left_count = BalancedBaggingHelper(cur_start, cur_cnt, left);
              } else {
...
#pragma omp parallel for schedule(static) num_threads(num_threads_)
        for (data_size_t i = 0; i < num_sampled_queries_; ++i) {
          OMP_LOOP_EX_BEGIN();
          sampled_query_boundaries_[i + 1] = query_boundaries_[bag_query_indices_[i] + 1] -
                                             query_boundaries_[bag_query_indices_[i]];
          OMP_LOOP_EX_END();
        }
        OMP_THROW_EX();

        const int num_blocks = Threading::For<data_size_t>(
            0, num_sampled_queries_ + 1, 128,
            [this](int thread_index, data_size_t start_index, data_size_t end_index) {
              for (data_size_t i = start_index + 1; i < end_index; ++i) {
                sampled_query_boundaries_[i] += sampled_query_boundaries_[i - 1];
              }
              sampled_query_boundaries_thread_buffer_[thread_index] =
                  sampled_query_boundaries_[end_index - 1];
            });

        for (int thread_index = 1; thread_index < num_blocks; ++thread_index) {
          sampled_query_boundaries_thread_buffer_[thread_index] +=
              sampled_query_boundaries_thread_buffer_[thread_index - 1];
        }
        ...
        Threading::For<data_size_t>(
            0, num_sampled_queries_, 1,
            [this](int /*thread_index*/, data_size_t start_index, data_size_t end_index) {
              for (data_size_t sampled_query_id = start_index; sampled_query_id < end_index;
                   ++sampled_query_id) {
                ...
                const data_size_t sampled_query_start =
                    sampled_query_boundaries_[sampled_query_id];
      ...
      if (!is_use_subset_) {
        ...
        if (config_->device_type == std::string("cuda")) {
          CopyFromHostToCUDADevice<data_size_t>(
              cuda_bag_data_indices_.RawData(), bag_data_indices_.data(),
  ...
  void ResetSampleConfig(const Config* config, bool is_change_dataset) override {
    ...
    bool balance_bagging_cond =
        (config->pos_bagging_fraction < 1.0 || config->neg_bagging_fraction < 1.0) &&
        (num_pos_data > 0);
    if ((config->bagging_fraction < 1.0 || balance_bagging_cond) && config->bagging_freq > 0) {
      need_re_bagging_ = false;
      if (!is_change_dataset && config_ != nullptr &&
      ...
      if (balance_bagging_cond) {
        ...
        bag_data_cnt_ =
            static_cast<data_size_t>(num_pos_data * config_->pos_bagging_fraction) +
  ...
  bool IsHessianChange() const override { return false; }

  data_size_t num_sampled_queries() const override { return num_sampled_queries_; }

  const data_size_t* sampled_query_indices() const override { return bag_query_indices_.data(); }

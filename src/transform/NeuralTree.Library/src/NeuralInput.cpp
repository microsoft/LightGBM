...
void
NeuralInput::FillBondData(NeuralInputBondData& p_data) const
{
...
double 
NeuralInput::EvaluateInput(UInt32 /*input*/) const
{
...
double
NeuralInput::Evaluate(UInt32** p_featureVectorArray,
...
UInt32 
NeuralInput::GetAssociatedFeature() const
{
    return static_cast<UInt32>(-1);
}

double
NeuralInput::GetSlope() const
{
	return 1.0;
}

double
NeuralInput::GetIntercept() const
{
	return 0;
}

void 
NeuralInput::LoadSegments(DynamicRank::Config& p_config,
                          const char* szSection)
{
    char szSegments[1024];
    if (!p_config.GetStringParameter(
            szSection,
            "Segments",
            szSegments,
            sizeof(szSegments)))
    ...
    for (std::vector<std::string>::iterator it = segmentNames.begin(); it != segmentNames.end(); ++it)
    {
        if (!it->empty())
        {
...
void
NeuralInput::SetSegments(const std::vector<std::string>& p_segments)
{
    if (p_segments.empty())
    {
         m_segments.reset();
...
bool 
NeuralInput::Save(FILE* fpOutput,
                  size_t nInputId,
...
bool
NeuralInput::EqualInternal(const NeuralInput* p_input) const
{
    if (!p_input)
    {
        return false;
    ...
    if (m_segments.get() != nullptr && p_input->m_segments.get() != nullptr && *m_segments != *p_input->m_segments)
    {
        return false;
    }
    return true;
...
bool 
NeuralInput::Train(double /*dblLearningRate*/, 
                   double /*outputHigh*/, 
                   double /*outputLow*/, 
                   double /*dblOutputDelta*/, 
                   UInt32 /*inputHigh*/[], 
                   UInt32 /*inputLow*/[])
{
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInput;
Sizeof_NeuralInput<sizeof(NeuralInput)> sizeof_NeuralInput;
#endif

// Use static assert to force the user to consider the external size and serialization if a new member is added.
// For each new member please add it to the template serialize and generate all the bin files for all the nets in searchgold.
// If the new member has an external size, then the size has to be reported within this function.
#ifdef DEBUG
...
size_t
NeuralInput::GetExternalSize() const
{
    size_t s = 0;
    if (m_segments)
    {
...
bool 
NeuralInputUnary::ReadAssociatedFeature(DynamicRank::Config& p_config,
                                        const char* szSection,
                                        ...
                                        UInt32* piFeature)
{
    *piFeature = static_cast<UInt32>(-1);
    ...
    if (p_config.GetStringParameter(szSection, "Name", szName, sizeof(szName)))
    {
        ...
        {
            Log::Warning("DR:ReadAssociatedFeature: Could not find index for feature name: %s in section: %s", szName, szSection);
            return false; // input error, invalid feature name
        }
        return true;
    }
    Log::Warning("DR:ReadAssociatedFeature: Could not find 'Name' of the feature for section: %s", szSection);
    return false; // feature is not specified
}


UInt32 
NeuralInputUnary::GetAssociatedFeature() const
{
    return m_iFeature;
}
...
void 
NeuralInputUnary::GetAllAssociatedFeatures(std::vector<UInt32>& associatedFeaturesList) const
{
...
NeuralInputUnary::NeuralInputUnary(int p_feature) : NeuralInput(), m_iFeature(p_feature)
{
}
...
NeuralInputUnary::NeuralInputUnary(const NeuralInputUnaryBondData& p_data)
    : NeuralInput(p_data),
      m_iFeature(p_data.m_iFeature)
{
}


void
NeuralInputUnary::FillBondData(NeuralInputUnaryBondData& p_data) const
{
    // Fill base class.
...
bool 
NeuralInputUnary::Save(FILE* fpOutput,
                       size_t nInputId,
                       const IFeatureMap& p_featureMap) const
{
...
void 
NeuralInputUnary::CopyFrom(const NeuralInputUnary& p_neuralInputUnary)
{
...
bool 
NeuralInputUnary::Equal(const NeuralInput* p_input) const
{
    ...
    const NeuralInputUnary* other = dynamic_cast<const NeuralInputUnary*>(p_input);
    if (!other)
    {
        return false;
    }
...
double 
NeuralInputUnary::Evaluate(UInt32 input[]) const
{
...
UInt32 
NeuralInputUnary::GetFeature() const
{
    return m_iFeature; 
}
...
size_t
NeuralInputUnary::GetSize() const
{
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputUnary;
Sizeof_NeuralInputUnary<sizeof(NeuralInputUnary) - sizeof(NeuralInput)> sizeof_NeuralInputUnary;
#endif
...
size_t
NeuralInputUnary::GetExternalSize() const
{
...
NeuralInputLinear*
NeuralInputLinear::Load(DynamicRank::Config& p_config,
                        const char* szSection,
                        IFeatureMap& p_featureMap)
{
    UInt32 iFeature;

    if (!NeuralInputUnary::ReadAssociatedFeature(p_config,szSection, p_featureMap, &iFeature))
    {
        return NULL;
    }
    ...
    {
        Log::Warning("NeuralInputLinear::Load: Slope or Intercept not provided (double value) for section: %s", szSection);
        return NULL;
    }

    NeuralInputLinear* pNeuralInputLinear = new NeuralInputLinear(iFeature, slope, intercept);
    return pNeuralInputLinear;
}


bool 
NeuralInputLinear::Equal(const NeuralInput* p_input) const
{
    ...
    const NeuralInputLinear* other = dynamic_cast<const NeuralInputLinear*>(p_input);
    if (!other)
    {
        return false;
    }    
...
NeuralInputLinear::NeuralInputLinear() : NeuralInputUnary(0), m_slope(0.0), m_intercept(0.0)
{
}
...
void 
NeuralInputLinear::FillBond(UnionBondInput& p_data) const
{
...
void
NeuralInputLinear::FillBondData(NeuralInputLinearBondData& p_data) const
{
    // Fill base class.
...
double 
NeuralInputLinear::GetMax() const
{
...
double 
NeuralInputLinear::GetMin() const
{
...
double
NeuralInputLinear::GetSlope() const
{
    return m_slope;
}
...
double
NeuralInputLinear::GetIntercept() const
{
    return m_intercept;
}
...
double 
NeuralInputLinear::EvaluateInput(UInt32 val) const
{
...
bool 
NeuralInputLinear::Save(FILE* fpOutput,
                        size_t nInputId,
                        const IFeatureMap& p_featureMap) const
{
...
size_t
NeuralInputLinear::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputLinear;
Sizeof_NeuralInputLinear<sizeof(NeuralInputLinear) - sizeof(NeuralInputUnary)> sizeof_NeuralInputLinear;
#endif
...
size_t
NeuralInputLinear::GetExternalSize() const
{
    return NeuralInputUnary::GetExternalSize();
}


NeuralInputLogLinear*
NeuralInputLogLinear::Load(DynamicRank::Config& p_config,
                           const char* szSection,
                           IFeatureMap& p_featureMap)
{
    UInt32 iFeature;
    ...
    NeuralInputLogLinear *pNeuralInputLogLinear = new NeuralInputLogLinear(iFeature, slope, intercept);
    return pNeuralInputLogLinear;
}

	
NeuralInputLogLinear::NeuralInputLogLinear(int id,
                                           double slope,
                                           double intercept)
...
NeuralInputLogLinear::NeuralInputLogLinear(const NeuralInputLogLinearBondData& p_data)
...
void 
NeuralInputLogLinear::FillBond(UnionBondInput& p_data) const
{
...
void
NeuralInputLogLinear::FillBondData(NeuralInputLogLinearBondData& p_data) const
{
    // Fill base class.
...
bool 
NeuralInputLogLinear::Equal(const NeuralInput* p_input) const
{
...
double 
NeuralInputLogLinear::EvaluateInput(UInt32 val) const
{
...
bool 
NeuralInputLogLinear::Save(FILE *fpOutput,
                           size_t nInputId,
                           const IFeatureMap& p_featureMap) const
{
    // The base class can write out the generic portion
...
size_t
NeuralInputLogLinear::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputLogLinear;
Sizeof_NeuralInputLogLinear<sizeof(NeuralInputLogLinear) - sizeof(NeuralInputLinear)> sizeof_NeuralInputLogLinear;
#endif
...
// If the new member has an external size, then the size has to be reported within this function.
#ifdef DEBUG
BOOST_STATIC_ASSERT(sizeof(NeuralInputLogLinear) - sizeof(NeuralInputLinear) == 0);   
#else
BOOST_STATIC_ASSERT(sizeof(NeuralInputLogLinear) - sizeof(NeuralInputLinear) == 0);  
#endif
...
size_t
NeuralInputLogLinear::GetExternalSize() const
{
    return NeuralInputLinear::GetExternalSize();
}
   

NeuralInputBucket::NeuralInputBucket(int p_id, 
                                     double p_min, 
                                     bool p_mininclusive, 
                                     double p_max, 
                                     bool p_maxinclusive) 
...
void 
NeuralInputBucket::FillBond(UnionBondInput& p_data) const
{
...
void
NeuralInputBucket::FillBondData(NeuralInputBucketBondData& p_data) const
{
    // Fill base class.
    NeuralInputUnary::FillBondData(p_data);
...
bool 
NeuralInputBucket::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInputUnary::Equal(p_input))
    {
        return false;
    ...
    const NeuralInputBucket* other = dynamic_cast<const NeuralInputBucket*>(p_input);
    if (!other)
    {
        return false;
    }     
...
bool
NeuralInputBucket::GetMinInclusive() const
{
    return m_fMinInclusive;
}


bool
NeuralInputBucket::GetMaxInclusive() const
{
    return m_fMaxInclusive;
}


UInt32
NeuralInputBucket::GetMinValue() const
{
    return m_nMinValue;
}
...
UInt32
NeuralInputBucket::GetMaxValue() const
{
    return m_nMaxValue;
}
...
double 
NeuralInputBucket::GetMax() const
{
...
double 
NeuralInputBucket::GetMin() const
{
...
bool 
NeuralInputBucket::Save(FILE* fpOutput,
                        size_t nInputId,
                        const IFeatureMap& p_featureMap) const
{
    // The base class can write out the generic portion
...
NeuralInputBucket*
NeuralInputBucket::Load(DynamicRank::Config& p_config,
                        const char* szSection,
                        IFeatureMap& p_featureMap)
{
    UInt32 iFeature;

    if (!NeuralInputUnary::ReadAssociatedFeature(
            p_config,szSection, p_featureMap, &iFeature))
    {
        return NULL;
    }
...
size_t
NeuralInputBucket::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputBucket;
Sizeof_NeuralInputBucket<sizeof(NeuralInputBucket) - sizeof(NeuralInputUnary)> sizeof_NeuralInputBucket;
#endif

size_t
NeuralInputBucket::GetExternalSize() const
{
    return NeuralInputUnary::GetExternalSize();
}
...
NeuralInputRational*
NeuralInputRational::Load(DynamicRank::Config& p_config,
                          const char* szSection,
                          IFeatureMap& p_featureMap)
{
    UInt32 iFeature;
    ...
    double dblDampingFactor = p_config.GetDoubleParameter(szSection, "DampingFactor", 0.0);
    if (dblDampingFactor <= 0.0)
    {
        return NULL;
    }

    NeuralInputRational* pNeuralInputRational = new NeuralInputRational(iFeature, dblDampingFactor);
    return pNeuralInputRational;
}
										 
										 
NeuralInputRational::NeuralInputRational(int p_id, 
                                         double p_dblDampingFactor) 
    : NeuralInputUnary(p_id),
...
NeuralInputRational::NeuralInputRational(const NeuralInputRationalBondData& p_data)
    : NeuralInputUnary(p_data),
      m_dblDampingFactor(p_data.m_dblDampingFactor)
{
}


void 
NeuralInputRational::FillBond(UnionBondInput& p_data) const
{  
...
void 
NeuralInputRational::FillBondData(NeuralInputRationalBondData& p_data) const
{
    // Fill base class.
    NeuralInputUnary::FillBondData(p_data);
...
bool 
NeuralInputRational::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInputUnary::Equal(p_input))
    {
        return false;
    ...
    const NeuralInputRational* other = dynamic_cast<const NeuralInputRational*>(p_input);
    if (!other)
    {
        return false;
    }    
...
double 
NeuralInputRational::EvaluateInput(UInt32 input) const
{
...
double
NeuralInputRational::GetDampingFactor() const
{
...
double 
NeuralInputRational::GetMin() const
{
    return 0.0;
}
...
double 
NeuralInputRational::GetMax() const
{
    return 1.0;
}
...
bool 
NeuralInputRational::Save(FILE* fpOutput,
                          size_t nInputId,
                          const IFeatureMap& p_featureMap) const
{
    // The base class can write out the generic portion
...
size_t
NeuralInputRational::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputRational;
Sizeof_NeuralInputRational<sizeof(NeuralInputRational) - sizeof(NeuralInputUnary)> sizeof_NeuralInputRational;
#endif
...
// If the new member has an external size, then the size has to be reported within this function.
#ifdef DEBUG
BOOST_STATIC_ASSERT(sizeof(NeuralInputRational) - sizeof(NeuralInputUnary) == 8);   
#else
BOOST_STATIC_ASSERT(sizeof(NeuralInputRational) - sizeof(NeuralInputUnary) == 8);  
#endif
...
size_t
NeuralInputRational::GetExternalSize() const
{
    return NeuralInputUnary::GetExternalSize();
}
...
void 
NeuralInputCached::FillBond(UnionBondInput& p_data) const
{  
...
bool 
NeuralInputCached::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInputUnary::Equal(p_input))
    {
        return false;
    ...
    const NeuralInputCached* other = dynamic_cast<const NeuralInputCached*>(p_input);
    if (!other)
    {
        return false;
    }    
...
double 
NeuralInputCached::EvaluateInput(UInt32 val) const
{
    if (val < m_cacheSize)
    {
...
double 
NeuralInputCached::GetMax() const
{
    return m_input->GetMax();
}


double 
NeuralInputCached::GetMin() const
{
    return m_input->GetMin();
}


bool 
NeuralInputCached::Save(FILE* fpOutput,
                        size_t nInputId,
                        const IFeatureMap& p_featureMap) const
{  
...
bool 
NeuralInputCached::Train(double dblLearningRate, double outputHigh, 
                         ...
                         UInt32 inputHigh[], UInt32 inputLow[])
{
    ...
    ret = m_input->Train(dblLearningRate, outputHigh, outputLow, 
                         dblOutputDelta, inputHigh, inputLow);
    if (ret)
        m_cacheSize = 0;
    return ret;
}
...
NeuralInputUnary*
NeuralInputCached::Load(size_t nCacheSize, 
                        NeuralInputUnary *pChild)
{
#ifdef INPUT_CACHE
...
const NeuralInputUnary*
NeuralInputCached::GetBaseInput() const
{
    return m_input.get();
}
...
size_t
NeuralInputCached::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputCached;
Sizeof_NeuralInputCached<sizeof(NeuralInputCached) - sizeof(NeuralInputUnary)> sizeof_NeuralInputCached;
#endif
...
size_t
NeuralInputCached::GetExternalSize() const
{
...
void 
NeuralInputTanh::FillBond(UnionBondInput& p_data) const
{
...
void
NeuralInputTanh::FillBondData(NeuralInputTanhBondData& p_data) const
{
    // Fill base class.
    NeuralInput::FillBondData(p_data);
...
bool 
NeuralInputTanh::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInput::EqualInternal(p_input))
    {
        return false;
    ...
    if (m_cInputs != other->m_cInputs
        || m_locked != other->m_locked
        || m_threshold != other->m_threshold
        || memcmp(m_rgId, other->m_rgId, c_maxInputs * sizeof(m_rgId[0]))
        || memcmp(m_rgWeights, other->m_rgWeights, c_maxInputs * sizeof(m_rgWeights[0])))
    {
        return false;
    }
...
NeuralInputTanh*
NeuralInputTanh::Load(DynamicRank::Config& p_config,
                      const char* szSection,
                      IFeatureMap& p_featureMap)
{
    char buff[256];
    ...
    result->m_threshold = p_config.GetDoubleParameter(szSection, "Threshold", 0.0);
    ...
    while (inputs < result->c_maxInputs)
    {
        ...
        {
            if (_snprintf_s(buff, sizeof(buff), _TRUNCATE, "Name:%d", inputs + 1) == -1)
            {
                return NULL;
            }
        }
        ...
        {
            if (_snprintf_s(buff, sizeof(buff), _TRUNCATE, "Weight:%d", inputs + 1) == -1)
            {
                return NULL;
            }
        }

        result->m_rgWeights[inputs] = p_config.GetDoubleParameter(szSection, buff, 0.01);
...
void
NeuralInputTanh::GetAllAssociatedFeatures(std::vector<UInt32>& associatedFeaturesList) const
{
...
double 
NeuralInputTanh::Evaluate(UInt32 input[]) const
{
...
bool 
NeuralInputTanh::Save(FILE *fpOutput,
                      size_t nInputId, 
                      const IFeatureMap& p_featureMap) const
{
    // All we can do here is write out the section name and the
...
bool
NeuralInputTanh::Train(double dblLearningRate, double outputHigher, 
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputTanh;
Sizeof_NeuralInputTanh<sizeof(NeuralInputTanh) - sizeof(NeuralInput)> sizeof_NeuralInputTanh;
#endif
...
size_t
NeuralInputTanh::GetExternalSize() const
{
    return NeuralInput::GetExternalSize();
}
...
double 
NeuralInputTanhUnary::EvaluateInput(UInt32 input) const
{
...
bool 
NeuralInputTanhUnary::Save(FILE *fpOutput, size_t nInputId, 
                           const IFeatureMap& p_featureMap) const
{
    NeuralInputUnary::Save(fpOutput, nInputId, p_featureMap);
...
NeuralInputTanhUnary::NeuralInputTanhUnary(UInt32 iFeature, double dblWeights,
    ...
    :m_dblThreshold(dblThreshold),m_dblWeights(dblWeights),
     m_fLocked(fLocked),NeuralInputUnary(iFeature)	 
{
}


NeuralInputTanhUnary::NeuralInputTanhUnary(const NeuralInputTanhUnaryBondData& p_data)
    : NeuralInputUnary(p_data),
...
void 
NeuralInputTanhUnary::FillBond(UnionBondInput& p_data) const
{
    p_data.m_inputType = "tanh";
    // Fill bond struct of the class.
...
void
NeuralInputTanhUnary::FillBondData(NeuralInputTanhUnaryBondData& p_data) const
{
    // Fill base class.
    NeuralInputUnary::FillBondData(p_data);
...
bool 
NeuralInputTanhUnary::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInputUnary::Equal(p_input))
    {
        return false;
    ...
    const NeuralInputTanhUnary* other = dynamic_cast<const NeuralInputTanhUnary*>(p_input);
    if (!other)
    {
        return false;
    }    
    if (m_fLocked != other->m_fLocked
        || m_dblWeights != other->m_dblWeights
...
NeuralInputTanhUnary*
NeuralInputTanhUnary::Load(DynamicRank::Config& p_config,
                           const char* szSection,
                           IFeatureMap& p_featureMap)
{
    UInt32 iFeature;
    ...
    if (p_config.GetStringParameter(szSection, "Name:2", szSecondFeatureName, sizeof(szSecondFeatureName)))
    {
        return NULL;
    }

    double dblThreshold = p_config.GetDoubleParameter(szSection, "Threshold", 0.0);
    ...
    NeuralInputTanhUnary* pNeuralInputTanhUnary = new NeuralInputTanhUnary(iFeature,dblWeight,dblThreshold,fLocked);
    return pNeuralInputTanhUnary;
}


double
NeuralInputTanhUnary::GetWeight() const
{
    return m_dblWeights;
}
...
double
NeuralInputTanhUnary::GetThreshold() const
{
...
double 
NeuralInputTanhUnary::GetMin() const
{
    return -1.0;
}
...
double 
NeuralInputTanhUnary::GetMax() const
{
    return 1.0;
}
...
bool 
NeuralInputTanhUnary::Train(double dblLearningRate, double outputHigher, 
                            double outputLower, double dblOutputDelta, 
                            UInt32 inputHigh[], 
                            UInt32 inputLow[])
{
...
size_t
NeuralInputTanhUnary::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputTanhUnary;
Sizeof_NeuralInputTanhUnary<sizeof(NeuralInputTanhUnary) - sizeof(NeuralInputUnary)> sizeof_NeuralInputTanhUnary;
#endif

size_t
NeuralInputTanhUnary::GetExternalSize() const
{
    return NeuralInputUnary::GetExternalSize();
}
...
double 
NeuralInputUseAsFloat::EvaluateInput(UInt32 input) const
{
...
bool 
NeuralInputUseAsFloat::Save(FILE *fpOutput, size_t nInputId, 
                           const IFeatureMap& p_featureMap) const
{
    NeuralInputUnary::Save(fpOutput, nInputId, p_featureMap);
...
NeuralInputUseAsFloat::NeuralInputUseAsFloat(const NeuralInputUseAsFloatBondData& p_data)
    : NeuralInputUnary(p_data)
{
}
...
void 
NeuralInputUseAsFloat::FillBond(UnionBondInput& p_data) const
{
...
void
NeuralInputUseAsFloat::FillBondData(NeuralInputUseAsFloatBondData& p_data) const
{
    // Fill base class.
    NeuralInputUnary::FillBondData(p_data);
}
...
void 
NeuralInputUseAsFloat::CopyFrom(const NeuralInputUseAsFloat& p_neuralInputUseAsFloat)
{
...
bool 
NeuralInputUseAsFloat::Equal(const NeuralInput* p_input) const
{
    if (!NeuralInputUnary::Equal(p_input))
    {
        return false;
...
NeuralInputUseAsFloat::Load(DynamicRank::Config& p_config,
                            const char* szSection,
                            IFeatureMap& p_featureMap)
{
    UInt32 iFeature;
    ...
    NeuralInputUseAsFloat* pNeuralInputUseAsFloat = new NeuralInputUseAsFloat(iFeature);
    return pNeuralInputUseAsFloat;
}


double 
NeuralInputUseAsFloat::GetMin() const
{
    return 0.0;
}
...
double 
NeuralInputUseAsFloat::GetMax() const
{
    return 1.0;
}
...
size_t
NeuralInputUseAsFloat::GetSize() const
{
    return sizeof(*this) + GetExternalSize();
}
...
// Do a 'set COMPUTE_BOOST_STATIC_ASSERT=1' command, then build in order to get error output that contains the struct size.
#if defined(COMPUTE_BOOST_STATIC_ASSERT)
template<int s> struct Sizeof_NeuralInputUseAsFloat;
Sizeof_NeuralInputUseAsFloat<sizeof(NeuralInputUseAsFloat) - sizeof(NeuralInputUnary)> sizeof_NeuralInputUseAsFloat;
#endif
...
// If the new member has an external size, then the size has to be reported within this function.
#ifdef DEBUG
BOOST_STATIC_ASSERT(sizeof(NeuralInputUseAsFloat) - sizeof(NeuralInputUnary) == 0);   
#else
BOOST_STATIC_ASSERT(sizeof(NeuralInputUseAsFloat) - sizeof(NeuralInputUnary) == 0);  
#endif
...
size_t
NeuralInputUseAsFloat::GetExternalSize() const
{
    return NeuralInputUnary::GetExternalSize();
}


BulkOptimizationUnsupported::BulkOptimizationUnsupported(const std::string& p_message)

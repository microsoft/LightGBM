...
std::string GBDT::DumpModel(int start_iteration, int num_iteration) const {
  ...
  if (num_iteration > 0) {
    ...
    num_used_model =
        std::min(end_iteration * num_tree_per_iteration_, num_used_model);
  }
  ...
  for (size_t i = 0; i < feature_importances.size(); ++i) {
    size_t feature_importances_int =
        static_cast<size_t>(feature_importances[i]);
  ...
  str_buf << '\n'
          ...
          << "{";
  if (!pairs.empty()) {
    str_buf << "\"" << pairs[0].second
            << "\":" << std::to_string(pairs[0].first);
    for (size_t i = 1; i < pairs.size(); ++i) {
      str_buf << ",";
      str_buf << "\"" << pairs[i].second
...
std::string GBDT::ModelToIfElse(int num_iteration) const {
  ...
  if (num_iteration > 0) {
    num_used_model =
        std::min(num_iteration * num_tree_per_iteration_, num_used_model);
  }
  ...
  pred_str_buf << "\t\t\t"
               << "output[k] += (*PredictTreePtr[i * num_tree_per_iteration_ + "
                  "k])(features);"
               << '\n';
  pred_str_buf << "\t\t"
               << "}" << '\n';
  ...
  str_buf << "void GBDT::PredictRaw(const double* features, double *output, "
  ...
  // PredictRawByMap
  str_buf << "double (*PredictTreeByMapPtr[])(const std::unordered_map<int, "
             "double>&) = { ";
  for (int i = 0; i < num_used_model; ++i) {
    if (i > 0) {
      str_buf << " , ";
  ...
  pred_str_buf_map << "\t\t"
                   ...
                   << '\n';
  pred_str_buf_map << "\t\t\t"
                   << "output[k] += (*PredictTreeByMapPtr[i * "
                      "num_tree_per_iteration_ + k])(features);"
                   << '\n';
  pred_str_buf_map << "\t\t"
                   << "}" << '\n';
  pred_str_buf_map << "\t\t"
  ...
  str_buf << "void GBDT::PredictRawByMap(const std::unordered_map<int, "
             "double>& features, double* output, const "
             "PredictionEarlyStopInstance* early_stop) const {"
          << '\n';
  ...
  // Predict
  str_buf << "void GBDT::Predict(const double* features, double *output, const "
             "PredictionEarlyStopInstance* early_stop) const {"
          << '\n';
  str_buf << "\t"
  ...
  // PredictByMap
  str_buf << "void GBDT::PredictByMap(const std::unordered_map<int, double>& "
             "features, double* output, const PredictionEarlyStopInstance* "
             "early_stop) const {"
          << '\n';
  str_buf << "\t"
  ...
  str_buf << "void GBDT::PredictLeafIndex(const double* features, double "
             "*output) const {"
          << '\n';
  str_buf
      << "\t"
  ...
  str_buf << "double (*PredictTreeLeafByMapPtr[])(const "
  ...
  str_buf << "void GBDT::PredictLeafIndexByMap(const std::unordered_map<int, "
             "double>& features, double* output) const {"
          << '\n';
  str_buf
      << "\t"
...
std::string GBDT::SaveModelToString(int start_iteration,
...
bool GBDT::LoadModelFromString(const char* buffer, size_t len) {
      ...
      if (!Common::StartsWith(cur_line, "Tree=")) {
        ...
        if (strs.size() == 1) {
          ...
          if (strs[0] == "feature_names") {
            ...
            key_vals[strs[0]] =
                cur_line.substr(std::strlen("monotone_constraints="));
          } else {
            // Use first 128 chars to avoid exceed the message buffer.
            Log::Fatal(
                "Wrong line at model file: %s",
                cur_line.substr(0, std::min<size_t>(128, cur_line.size()))
                    .c_str());
          }
        }
      } else {
  ...
  if (key_vals.count("num_tree_per_iteration")) {
    Common::Atoi(key_vals["num_tree_per_iteration"].c_str(),
  ...
  if (key_vals.count("monotone_constraints")) {
    monotone_constraints_ = Common::StringToArray<int8_t>(
        key_vals["monotone_constraints"].c_str(), ' ');
    if (monotone_constraints_.size() !=
        static_cast<size_t>(max_feature_idx_ + 1)) {
  ...
  if (key_vals.count("objective")) {
    ...
    loaded_objective_.reset(
        ObjectiveFunction::CreateObjectiveFunction(ParseObjectiveAlias(str)));
  ...
  if (!key_vals.count("tree_sizes")) {
    ...
    std::vector<size_t> tree_sizes =
        Common::StringToArray<size_t>(key_vals["tree_sizes"].c_str(), ' ');
...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < num_trees; ++i) {
      ...
      if (Common::StartsWith(cur_line, "Tree=")) {
        ...
        Log::Fatal("Model format error, expect a tree here. met %s",
                   cur_line.c_str());
      }
      OMP_LOOP_EX_END();
    }
...
std::vector<double> GBDT::FeatureImportance(int num_iteration,
  ...
  if (importance_type == 0) {
    for (int iter = 0; iter < num_used_model; ++iter) {
      for (int split_idx = 0; split_idx < models_[iter]->num_leaves() - 1;
           ++split_idx) {

...
#define WeightedPercentileFun(T, data_reader, weight_reader, cnt_data, alpha)                  \
  {                                                                                            \
    ...
    std::stable_sort(sorted_idx.begin(), sorted_idx.end(), [&](data_size_t a, data_size_t b) { \
    ...
    size_t pos = std::upper_bound(weighted_cdf.begin(), weighted_cdf.end(), threshold) -       \
                 weighted_cdf.begin();                                                         \
    ...
    if (weighted_cdf[pos + 1] - weighted_cdf[pos] >= 1.0f) {                                   \
      return static_cast<T>((threshold - weighted_cdf[pos]) /                                  \
                                (weighted_cdf[pos + 1] - weighted_cdf[pos]) * (v2 - v1) +      \
                            v1);                                                               \
    } else {                                                                                   \
...
class RegressionL2loss : public ObjectiveFunction {
 public:
  explicit RegressionL2loss(const Config& config) : deterministic_(config.deterministic) {
    sqrt_ = config.reg_sqrt;
  }

  explicit RegressionL2loss(const std::vector<std::string>& strs) : deterministic_(false) {
    sqrt_ = false;
    for (auto str : strs) {
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] =
            static_cast<score_t>(static_cast<score_t>((score[i] - label_[i])) * weights_[i]);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        if (std::abs(diff) <= alpha_) {
          ...
          gradients[i] = static_cast<score_t>(Common::Sign(diff) *
                                              static_cast<score_t>(weights_[i]) * alpha_);
        }
        hessians[i] = static_cast<score_t>(weights_[i]);
      }
    }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        hessians[i] =
            static_cast<score_t>(-label_[i] * (1 - rho_) * exp_1_score + (2 - rho_) * exp_2_score);
      }
    } else {
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        double exp_2_score = std::exp((2 - rho_) * score[i]);
        gradients[i] =
            static_cast<score_t>((-label_[i] * exp_1_score + exp_2_score) * weights_[i]);
        hessians[i] = static_cast<score_t>(
            (-label_[i] * (1 - rho_) * exp_1_score + (2 - rho_) * exp_2_score) * weights_[i]);
      }
    }
  }

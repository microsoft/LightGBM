 ...
 *        Different machine will find best split on different features, then
 * sync global best split It is recommonded used when #data is small or #feature
 * is large
 */
 ...
 *        Workers use local data to construct histograms locally, then sync up
 * global histograms. It is recommonded used when #data is large or #feature is
 * small
 */
template <typename TREELEARNER_T>
class DataParallelTreeLearner : public TREELEARNER_T {
 ...
 private:
  ...
  /*! \brief Buffer for network receive */
  std::vector<char> output_buffer_;
  /*! \brief different machines will aggregate histograms for different
     features, use this to mark local aggregate features*/
 ...
 * Here using voting to reduce features, and only aggregate histograms for
 * selected features. When #data is large and #feature is large, you can use
 * this to have better speed-up
 */
template <typename TREELEARNER_T>
class VotingParallelTreeLearner : public TREELEARNER_T {
 ...
 protected:
  void BeforeTrain() override;
  bool BeforeFindBestSplit(const Tree* tree, int left_leaf,
                           int right_leaf) override;
  void FindBestSplits() override;
  void FindBestSplitsFromHistograms(const std::vector<int8_t>& is_feature_used,
                                    bool use_subtract) override;

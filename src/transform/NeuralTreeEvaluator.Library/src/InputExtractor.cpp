    ...
    std::vector<boost::shared_ptr<const DynamicRank::NeuralInput>>& inputs)
{
    ...
    for (std::vector<boost::shared_ptr<const NeuralInput>>::const_iterator it = inputs.cbegin(); it!=inputs.cend(); ++it)
    {
...
InputExtractor* InputExtractor::CreateFromConfig(DynamicRank::Config* config)
{
    ...
    {
        boost::shared_ptr<NeuralInput> input(inputFactory.Load(*config, (int)inputId, *featureMap));
        if (input == NULL)
        {
...
InputExtractor* InputExtractor::CreateFromInputStr(const string& str)
{
    ...
    if (config == NULL)
    {
		Log::Warning("Unable to read input string as valid transform, in bad format or does not exist");
		return NULL;
    }
...
UInt32 InputExtractor::GetInputCount() const
{
...
const std::string InputExtractor::GetInputName(UInt32 index) const
{
    ...
    // Linear/tanh/bucket/other transforms are unary, and all refer to their feature by the "Name" key.
    const DynamicRank::NeuralInputCached* cachedInput = dynamic_cast<const DynamicRank::NeuralInputCached*>(m_inputs[index].get());
    if (cachedInput != NULL && dynamic_cast<const NeuralInputUnary*>(cachedInput->GetBaseInput()) != NULL)
    {

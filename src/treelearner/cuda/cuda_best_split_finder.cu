...
__device__ void ReduceBestGainWarp(double gain, bool found, uint32_t thread_index,
                                   double* out_gain, bool* out_found, uint32_t* out_thread_index) {
...
__device__ uint32_t ReduceBestGain(double gain, bool found, uint32_t thread_index,
                                   double* shared_gain_buffer, bool* shared_found_buffer,
                                   uint32_t* shared_thread_index_buffer) {
  const uint32_t warpID = threadIdx.x / warpSize;
  const uint32_t warpLane = threadIdx.x % warpSize;
  const uint32_t num_warp = blockDim.x / warpSize;
  ReduceBestGainWarp(gain, found, thread_index, shared_gain_buffer + warpID,
                     shared_found_buffer + warpID, shared_thread_index_buffer + warpID);
  __syncthreads();
  if (warpID == 0) {
  ...
  for (unsigned int s = 1; s < cuda_cur_num_leaves; s *= 2) {
    if (tid % (2 * s) == 0 && (tid + s) < cuda_cur_num_leaves) {
      ...
      if ((leaves[tid] == -1 && leaves[tid_s] != -1) ||
          (leaves[tid] != -1 && leaves[tid_s] != -1 && gain[tid_s] > gain[tid])) {
        gain[tid] = gain[tid_s];
...
__device__ void ReduceBestGainForLeavesWarp(double gain, int leaf_index, double* out_gain,
                                            int* out_leaf_index) {
  const uint32_t mask = 0xffffffff;
  const uint32_t warpLane = threadIdx.x % warpSize;
  for (uint32_t offset = warpSize / 2; offset > 0; offset >>= 1) {
    ...
    const double other_gain = __shfl_down_sync(mask, gain, offset);
    if ((leaf_index != -1 && other_leaf_index != -1 && other_gain > gain) ||
        (leaf_index == -1 && other_leaf_index != -1)) {
      gain = other_gain;
...
__device__ int ReduceBestGainForLeaves(double gain, int leaf_index, double* shared_gain_buffer,
                                       int* shared_leaf_index_buffer) {
  const uint32_t warpID = threadIdx.x / warpSize;
  const uint32_t warpLane = threadIdx.x % warpSize;
  const uint32_t num_warp = blockDim.x / warpSize;
  ReduceBestGainForLeavesWarp(gain, leaf_index, shared_gain_buffer + warpID,
                              shared_leaf_index_buffer + warpID);
  __syncthreads();
  if (warpID == 0) {
    gain = warpLane < num_warp ? shared_gain_buffer[warpLane] : kMinScore;
...
__device__ void FindBestSplitsForLeafKernelInner(
    ...
    const SplitFindTask* task, CUDARandom* cuda_random,
    // input config parameter values
    const double lambda_l1, const double lambda_l2, const double path_smooth,
    ...
    const double parent_gain, const double sum_gradients, const double sum_hessians,
    const data_size_t num_data, const double parent_output,
    // output parameters
  ...
  const bool skip_sum = REVERSE
                            ? (task->skip_default_bin && (task->num_bin - 1 - threadIdx_x) ==
                                                             static_cast<int>(task->default_bin))
                            : (task->skip_default_bin && (threadIdx_x + task->mfb_offset) ==
                                                             static_cast<int>(task->default_bin));
  ...
  if (!REVERSE && task->na_as_missing && task->mfb_offset == 1) {
    const hist_t sum_gradients_non_default =
        ShuffleReduceSum<hist_t>(local_grad_hist, shared_double_buffer, blockDim.x);
    __syncthreads();
    const hist_t sum_hessians_non_default =
        ShuffleReduceSum<hist_t>(local_hess_hist, shared_double_buffer, blockDim.x);
    if (threadIdx_x == 0) {
  ...
  if (REVERSE) {
    if (threadIdx_x >= static_cast<unsigned int>(task->na_as_missing) &&
        threadIdx_x <= task->num_bin - 2 && !skip_sum) {
      const double sum_right_gradient = local_grad_hist;
      const double sum_right_hessian = local_hess_hist;
      const data_size_t right_count =
          static_cast<data_size_t>(__double2int_rn(sum_right_hessian * cnt_factor));
      ...
      if (sum_left_hessian >= min_sum_hessian_in_leaf && left_count >= min_data_in_leaf &&
        ...
        double current_gain = CUDALeafSplits::GetSplitGains<USE_L1, USE_SMOOTHING>(
            sum_left_gradient, sum_left_hessian, sum_right_gradient, sum_right_hessian, lambda_l1,
    ...
    const uint32_t end = (task->na_as_missing && task->mfb_offset == 1)
                             ? static_cast<uint32_t>(task->num_bin - 2)
    ...
    if (threadIdx_x <= end && !skip_sum) {
      ...
      const data_size_t left_count =
          static_cast<data_size_t>(__double2int_rn(sum_left_hessian * cnt_factor));
  ...
  const uint32_t result =
      ReduceBestGain(local_gain, threshold_found, threadIdx_x, shared_double_buffer,
                     shared_bool_buffer, shared_int_buffer);
  if (threadIdx_x == 0) {
  ...
  if (threshold_found && threadIdx_x == best_thread_index) {
    ...
    if (REVERSE) {
      ...
      const double left_output =
          CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
              sum_left_gradient, sum_left_hessian, lambda_l1, lambda_l2, path_smooth, left_count,
              parent_output);
      const double right_output =
          CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
              sum_right_gradient, sum_right_hessian, lambda_l1, lambda_l2, path_smooth,
      ...
      cuda_best_split_info->left_gain = CUDALeafSplits::GetLeafGainGivenOutput<USE_L1>(
          sum_left_gradient, sum_left_hessian, lambda_l1, lambda_l2, left_output);
      cuda_best_split_info->right_value = right_output;
      cuda_best_split_info->right_gain = CUDALeafSplits::GetLeafGainGivenOutput<USE_L1>(
          sum_right_gradient, sum_right_hessian, lambda_l1, lambda_l2, right_output);
    } else {
      const double sum_left_gradient = local_grad_hist;
...
template <bool USE_RAND, bool USE_L1, bool USE_SMOOTHING, bool REVERSE, typename BIN_HIST_TYPE,
          typename ACC_HIST_TYPE, bool USE_16BIT_BIN_HIST, bool USE_16BIT_ACC_HIST>
__device__ void FindBestSplitsDiscretizedForLeafKernelInner(
  ...
  const double sum_hessians =
      static_cast<double>(sum_gradients_hessians & 0x00000000ffffffff) * hess_scale;
  const double cnt_factor = num_data / sum_hessians;
  const double min_gain_shift = parent_gain + min_gain_to_split;
    ...
    if (threadIdx_x < feature_num_bin_minus_offset && !skip_sum) {
      ...
      if (USE_16BIT_BIN_HIST && !USE_16BIT_ACC_HIST) {
        ...
        local_grad_hess_hist =
            (static_cast<int64_t>(static_cast<int16_t>(local_grad_hess_hist_int32 >> 16)) << 32) |
            (static_cast<int64_t>(local_grad_hess_hist_int32 & 0x0000ffff));
      } else {
  ...
  __syncthreads();
  local_gain = kMinScore;
  local_grad_hess_hist = ShufflePrefixSum<ACC_HIST_TYPE>(
      local_grad_hess_hist, reinterpret_cast<ACC_HIST_TYPE*>(shared_int_buffer));
  ...
  if (REVERSE) {
    if (threadIdx_x >= static_cast<unsigned int>(task->na_as_missing) &&
        threadIdx_x <= task->num_bin - 2 && !skip_sum) {
      sum_right_gradient_hessian =
          USE_16BIT_ACC_HIST
              ? (static_cast<int64_t>(static_cast<int16_t>(local_grad_hess_hist >> 16)) << 32) |
                    static_cast<int64_t>(local_grad_hess_hist & 0x0000ffff)
              : local_grad_hess_hist;
      sum_right_gradient = static_cast<double>(static_cast<int32_t>(
                               (sum_right_gradient_hessian & 0xffffffff00000000) >> 32)) *
                           grad_scale;
      sum_right_hessian = static_cast<double>(static_cast<int32_t>(sum_right_gradient_hessian &
                                                                   0x00000000ffffffff)) *
                          hess_scale;
      ...
      sum_left_gradient = static_cast<double>(static_cast<int32_t>(
                              (sum_left_gradient_hessian & 0xffffffff00000000) >> 32)) *
                          grad_scale;
      sum_left_hessian = static_cast<double>(static_cast<int32_t>(sum_left_gradient_hessian &
                                                                  0x00000000ffffffff)) *
                         hess_scale;
      ...
      if (sum_left_hessian >= min_sum_hessian_in_leaf && left_count >= min_data_in_leaf &&
        ...
        double current_gain = CUDALeafSplits::GetSplitGains<USE_L1, USE_SMOOTHING>(
            ...
            sum_right_hessian + kEpsilon, lambda_l1, lambda_l2, path_smooth, left_count,
            right_count, parent_output);
        // gain with split is worse than without split
        if (current_gain > min_gain_shift) {
          local_gain = current_gain - min_gain_shift;
  ...
  if (threshold_found && threadIdx_x == best_thread_index) {
    ...
    const double left_output = CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
        sum_left_gradient, sum_left_hessian, lambda_l1, lambda_l2, path_smooth, left_count,
        parent_output);
    const double right_output = CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
        sum_right_gradient, sum_right_hessian, lambda_l1, lambda_l2, path_smooth, right_count,
        parent_output);
    cuda_best_split_info->left_sum_gradients = sum_left_gradient;
    cuda_best_split_info->left_sum_hessians = sum_left_hessian;
...
__device__ void FindBestSplitsForLeafKernelCategoricalInner(
    ...
    const double lambda_l1, const double lambda_l2, const double path_smooth,
    const data_size_t min_data_in_leaf, const double min_sum_hessian_in_leaf,
    const double min_gain_to_split, const double cat_smooth, const double cat_l2,
    const int max_cat_threshold, const int min_data_per_group,
    // input parent node information
    const double parent_gain, const double sum_gradients, const double sum_hessians,
    const data_size_t num_data, const double parent_output,
    // output parameters
  ...
  if (task->is_one_hot) {
    ...
    if (threadIdx_x >= bin_start && threadIdx_x < bin_end) {
      ...
      data_size_t cnt = static_cast<data_size_t>(__double2int_rn(hess * cnt_factor));
      if (cnt >= min_data_in_leaf && hess >= min_sum_hessian_in_leaf) {
        ...
        if (other_count >= min_data_in_leaf) {
          ...
          if (sum_other_hessian >= min_sum_hessian_in_leaf &&
              (!USE_RAND || static_cast<int>(threadIdx_x) == rand_threshold)) {
            const double sum_other_gradient = sum_gradients - grad;
            double current_gain = CUDALeafSplits::GetSplitGains<USE_L1, USE_SMOOTHING>(
                sum_other_gradient, sum_other_hessian, grad, hess + kEpsilon, lambda_l1, l2,
                path_smooth, other_count, cnt, parent_output);
            if (current_gain > min_gain_shift) {
    ...
    __syncthreads();
    const uint32_t result =
        ReduceBestGain(local_gain, threshold_found, threadIdx_x, shared_gain_buffer,
                       shared_found_buffer, shared_thread_index_buffer);
    if (threadIdx_x == 0) {
      best_thread_index = result;
    }
    __syncthreads();
    if (threshold_found && threadIdx_x == best_thread_index) {
      ...
      *(cuda_best_split_info->cat_threshold) =
          static_cast<uint32_t>(threadIdx_x + task->mfb_offset);
      ...
      const double left_output =
          CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
              sum_left_gradient, sum_left_hessian, lambda_l1, l2, path_smooth, left_count,
              parent_output);
      const double right_output =
          CUDALeafSplits::CalculateSplittedLeafOutput<USE_L1, USE_SMOOTHING>(
              sum_right_gradient, sum_right_hessian, lambda_l1, l2, path_smooth, right_count,
              parent_output);
      cuda_best_split_info->left_sum_gradients = sum_left_gradient;
      cuda_best_split_info->left_sum_hessians = sum_left_hessian;
      cuda_best_split_info->left_count = left_count;
      ...
      cuda_best_split_info->left_value = left_output;
      cuda_best_split_info->left_gain = CUDALeafSplits::GetLeafGainGivenOutput<USE_L1>(
          sum_left_gradient, sum_left_hessian, lambda_l1, l2, left_output);
      cuda_best_split_info->right_value = right_output;
      cuda_best_split_info->right_gain = CUDALeafSplits::GetLeafGainGivenOutput<USE_L1>(
          sum_right_gradient, sum_right_hessian, lambda_l1, l2, right_output);
    }
  } else {
    ...
    __syncthreads();
    const int local_used_bin =
        ShuffleReduceSum<uint16_t>(is_valid_bin, shared_mem_buffer_uint16, blockDim.x);
    if (threadIdx_x == 0) {
    ...
    if (threshold_found && threadIdx_x == best_thread_index) {
      ...
      if (best_dir == 1) {
        ...
        for (int i = 0; i < threadIdx_x + 1; ++i) {
          (cuda_best_split_info->cat_threshold)[i] =
              shared_index_buffer[used_bin - 1 - i] + task->mfb_offset;
        }
      }
      cuda_best_split_info->default_left = false;
...
__global__ void FindBestSplitsForLeafKernel(
    ...
    // input task information
    const int num_tasks, const SplitFindTask* tasks, CUDARandom* cuda_randoms,
    // input leaf information
    const CUDALeafSplitsStruct* smaller_leaf_splits,
    const CUDALeafSplitsStruct* larger_leaf_splits,
    // input config parameter values
    const data_size_t min_data_in_leaf, const double min_sum_hessian_in_leaf,
    const double min_gain_to_split, const double lambda_l1, const double lambda_l2,
    const double path_smooth, const double cat_smooth, const double cat_l2,
    const int max_cat_threshold, const int min_data_per_group,
    // output
    CUDASplitInfo* cuda_best_split_info) {
  ...
  const double sum_gradients =
      IS_LARGER ? larger_leaf_splits->sum_of_gradients : smaller_leaf_splits->sum_of_gradients;
  const double sum_hessians =
      (IS_LARGER ? larger_leaf_splits->sum_of_hessians : smaller_leaf_splits->sum_of_hessians) +
      2 * kEpsilon;
  const data_size_t num_data =
      IS_LARGER ? larger_leaf_splits->num_data_in_leaf : smaller_leaf_splits->num_data_in_leaf;
  const double parent_output =
      IS_LARGER ? larger_leaf_splits->leaf_value : smaller_leaf_splits->leaf_value;
  ...
  CUDARandom* cuda_random =
      USE_RAND ? (IS_LARGER ? cuda_randoms + task_index * 2 + 1 : cuda_randoms + task_index * 2)
               : nullptr;
  if (is_feature_used_bytree[inner_feature_index]) {
    const hist_t* hist_ptr =
    ...
    if (task->is_categorical) {
      FindBestSplitsForLeafKernelCategoricalInner<USE_RAND, USE_L1, USE_SMOOTHING>(
          ...
          task, cuda_random,
          // input config parameter values
          lambda_l1, lambda_l2, path_smooth, min_data_in_leaf, min_sum_hessian_in_leaf,
          min_gain_to_split, cat_smooth, cat_l2, max_cat_threshold, min_data_per_group,
          // input parent node information
...
__global__ void FindBestSplitsDiscretizedForLeafKernel(
    ...
    const double min_gain_to_split, const double lambda_l1, const double lambda_l2,
    const double path_smooth, const double cat_smooth, const double cat_l2,
    const int max_cat_threshold, const int min_data_per_group, const int max_cat_to_onehot,
    // gradient scale
  ...
  const int inner_feature_index = task->inner_feature_index;
  const double parent_gain = IS_LARGER ? larger_leaf_splits->gain : smaller_leaf_splits->gain;
  const int64_t sum_gradients_hessians = IS_LARGER
                                             ? larger_leaf_splits->sum_of_gradients_hessians
                                             : smaller_leaf_splits->sum_of_gradients_hessians;
  const data_size_t num_data =
      IS_LARGER ? larger_leaf_splits->num_data_in_leaf : smaller_leaf_splits->num_data_in_leaf;
  const double parent_output =
      IS_LARGER ? larger_leaf_splits->leaf_value : smaller_leaf_splits->leaf_value;
  ...
  CUDARandom* cuda_random =
      USE_RAND ? (IS_LARGER ? cuda_randoms + task_index * 2 + 1 : cuda_randoms + task_index * 2)
               : nullptr;
  const bool use_16bit_bin = IS_LARGER ? (larger_leaf_num_bits_in_histogram_bin <= 16)
                                       : (smaller_leaf_num_bits_in_histogram_bin <= 16);
  if (is_feature_used_bytree[inner_feature_index]) {
    if (task->is_categorical) {
      ...
      if (!task->reverse) {
        if (use_16bit_bin) {
          const int32_t* hist_ptr =
              reinterpret_cast<const int32_t*>(IS_LARGER ? larger_leaf_splits->hist_in_leaf
                                                         : smaller_leaf_splits->hist_in_leaf) +
              task->hist_offset;
          FindBestSplitsDiscretizedForLeafKernelInner<USE_RAND, USE_L1, USE_SMOOTHING, false,
                                                      int32_t, int32_t, true, true>(
              // input feature information
              hist_ptr,
              // input task information
              task, cuda_random,
...
__device__ void FindBestSplitsForLeafKernelInner_GlobalMemory(
    ...
    CUDASplitInfo* cuda_best_split_info,
    // buffer
    hist_t* hist_grad_buffer_ptr, hist_t* hist_hess_buffer_ptr) {
  const double cnt_factor = num_data / sum_hessians;
  const double min_gain_shift = parent_gain + min_gain_to_split;
  ...
  if (!REVERSE) {
    if (task->na_as_missing && task->mfb_offset == 1) {
      ...
      for (unsigned int bin = bin_start; bin < static_cast<uint32_t>(task->num_bin);
           bin += blockDim.x) {
      ...
      const hist_t sum_gradients_non_default =
          ShuffleReduceSum<double>(thread_sum_gradients, shared_double_buffer, blockDim.x);
      __syncthreads();
      const hist_t sum_hessians_non_default =
          ShuffleReduceSum<double>(thread_sum_hessians, shared_double_buffer, blockDim.x);
      if (threadIdx_x == 0) {
      ...
      for (unsigned int bin = threadIdx_x; bin < feature_num_bin_minus_offset; bin += blockDim.x) {
        const bool skip_sum = (task->skip_default_bin &&
                               (bin + task->mfb_offset) == static_cast<int>(task->default_bin));
        if (!skip_sum) {
      ...
      if (!skip_sum) {
        ...
        if (sum_left_hessian >= min_sum_hessian_in_leaf && left_count >= min_data_in_leaf &&
          ...
          double current_gain = CUDALeafSplits::GetSplitGains<USE_L1, USE_SMOOTHING>(
              ...
              lambda_l1, lambda_l2, path_smooth, left_count, right_count, parent_output);
          // gain with split is worse than without split
          if (current_gain > min_gain_shift) {
            local_gain = current_gain - min_gain_shift;
  ...
  if (threshold_found && threadIdx_x == best_thread_index) {
    ...
    if (REVERSE) {
      ...
      const unsigned int best_bin =
          (task->na_as_missing && task->mfb_offset == 1)
              ? threshold_value
              : static_cast<uint32_t>(threshold_value - task->mfb_offset);
...
__device__ void FindBestSplitsForLeafKernelCategoricalInner_GlobalMemory(
    ...
    const data_size_t num_data, const double parent_output,
    // buffer
    hist_t* hist_grad_buffer_ptr, hist_t* hist_hess_buffer_ptr, hist_t* hist_stat_buffer_ptr,
  ...
  if (task->is_one_hot) {
    ...
    // left to right
    for (int bin = static_cast<int>(threadIdx_x); bin < used_bin && bin < max_num_cat;
         bin += static_cast<int>(blockDim.x)) {
    ...
    if (threshold_found && threadIdx_x == best_thread_index) {
      ...
      if (best_dir == 1) {
        ...
        for (int i = 0; i < best_threshold + 1; ++i) {
          (cuda_best_split_info->cat_threshold)[i] =
              hist_index_buffer_ptr[used_bin - 1 - i] + task->mfb_offset;
        }
      }
      cuda_best_split_info->default_left = false;
      const hist_t sum_left_gradient = best_sum_left_gradient;
...
__global__ void FindBestSplitsForLeafKernel_GlobalMemory(
    ...
    CUDASplitInfo* cuda_best_split_info,
    // buffer
    hist_t* feature_hist_grad_buffer, hist_t* feature_hist_hess_buffer,
    hist_t* feature_hist_stat_buffer, data_size_t* feature_hist_index_buffer) {
  const unsigned int task_index = blockIdx.x;
  const SplitFindTask* task = tasks + task_index;
  const double parent_gain = IS_LARGER ? larger_leaf_splits->gain : smaller_leaf_splits->gain;
  const double sum_gradients =
  ...
  if (is_feature_used_bytree[task->inner_feature_index]) {
    ...
    if (task->is_categorical) {
      FindBestSplitsForLeafKernelCategoricalInner_GlobalMemory<USE_RAND, USE_L1, USE_SMOOTHING>(
          ...
          parent_gain, sum_gradients, sum_hessians, num_data, parent_output,
          // buffer
          hist_grad_buffer_ptr, hist_hess_buffer_ptr, hist_stat_buffer_ptr, hist_index_buffer_ptr,
          // output parameters
          out);
    } else {
      if (!task->reverse) {
        FindBestSplitsForLeafKernelInner_GlobalMemory<USE_RAND, USE_L1, USE_SMOOTHING, false>(
            ...
            out,
            // buffer
            hist_grad_buffer_ptr, hist_hess_buffer_ptr);
      } else {
...
#define LaunchFindBestSplitsForLeafKernel_ARGS                                    \
  smaller_leaf_splits, larger_leaf_splits, smaller_leaf_index, larger_leaf_index, \
      is_smaller_leaf_valid, is_larger_leaf_valid

#define FindBestSplitsForLeafKernel_ARGS                                                      \
  num_tasks_, cuda_split_find_tasks_.RawData(), cuda_randoms_.RawData(), smaller_leaf_splits, \
      larger_leaf_splits, min_data_in_leaf_, min_sum_hessian_in_leaf_, min_gain_to_split_,    \
      lambda_l1_, lambda_l2_, path_smooth_, cat_smooth_, cat_l2_, max_cat_threshold_,         \
      min_data_per_group_, cuda_best_split_info_

#define GlobalMemory_Buffer_ARGS                                                                  \
  cuda_feature_hist_grad_buffer_, cuda_feature_hist_hess_buffer_, cuda_feature_hist_stat_buffer_, \
...
void CUDABestSplitFinder::LaunchFindBestSplitsForLeafKernelInner0(
    LaunchFindBestSplitsForLeafKernel_PARAMS) {
  if (lambda_l1_ <= 0.0f) {
    LaunchFindBestSplitsForLeafKernelInner1<USE_RAND, false>(
        LaunchFindBestSplitsForLeafKernel_ARGS);
  } else {
    LaunchFindBestSplitsForLeafKernelInner1<USE_RAND, true>(
        LaunchFindBestSplitsForLeafKernel_ARGS);
  }
}

template <bool USE_RAND, bool USE_L1>
void CUDABestSplitFinder::LaunchFindBestSplitsForLeafKernelInner1(
    LaunchFindBestSplitsForLeafKernel_PARAMS) {
  if (!use_smoothing_) {
    LaunchFindBestSplitsForLeafKernelInner2<USE_RAND, USE_L1, false>(
        LaunchFindBestSplitsForLeafKernel_ARGS);
  } else {
    LaunchFindBestSplitsForLeafKernelInner2<USE_RAND, USE_L1, true>(
        LaunchFindBestSplitsForLeafKernel_ARGS);
  }
}

template <bool USE_RAND, bool USE_L1, bool USE_SMOOTHING>
void CUDABestSplitFinder::LaunchFindBestSplitsForLeafKernelInner2(
    LaunchFindBestSplitsForLeafKernel_PARAMS) {
...
#define LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS                         \
  smaller_leaf_splits, larger_leaf_splits, smaller_leaf_index, larger_leaf_index, \
      is_smaller_leaf_valid, is_larger_leaf_valid, grad_scale, hess_scale,        \
      smaller_num_bits_in_histogram_bins, larger_num_bits_in_histogram_bins

#define FindBestSplitsDiscretizedForLeafKernel_ARGS                                             \
  cuda_is_feature_used_bytree_, num_tasks_, cuda_split_find_tasks_.RawData(),                   \
      cuda_randoms_.RawData(), smaller_leaf_splits, larger_leaf_splits,                         \
      smaller_num_bits_in_histogram_bins, larger_num_bits_in_histogram_bins, min_data_in_leaf_, \
      min_sum_hessian_in_leaf_, min_gain_to_split_, lambda_l1_, lambda_l2_, path_smooth_,       \
      cat_smooth_, cat_l2_, max_cat_threshold_, min_data_per_group_, max_cat_to_onehot_,        \
      grad_scale, hess_scale, cuda_best_split_info_

void CUDABestSplitFinder::LaunchFindBestSplitsDiscretizedForLeafKernel(
    LaunchFindBestSplitsDiscretizedForLeafKernel_PARAMS) {
  if (!is_smaller_leaf_valid && !is_larger_leaf_valid) {
    return;
  }
  if (!extra_trees_) {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner0<false>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  } else {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner0<true>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  }
}

template <bool USE_RAND>
void CUDABestSplitFinder::LaunchFindBestSplitsDiscretizedForLeafKernelInner0(
    LaunchFindBestSplitsDiscretizedForLeafKernel_PARAMS) {
  if (lambda_l1_ <= 0.0f) {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner1<USE_RAND, false>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  } else {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner1<USE_RAND, true>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  }
}

template <bool USE_RAND, bool USE_L1>
void CUDABestSplitFinder::LaunchFindBestSplitsDiscretizedForLeafKernelInner1(
    LaunchFindBestSplitsDiscretizedForLeafKernel_PARAMS) {
  if (!use_smoothing_) {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner2<USE_RAND, USE_L1, false>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  } else {
    LaunchFindBestSplitsDiscretizedForLeafKernelInner2<USE_RAND, USE_L1, true>(
        LaunchFindBestSplitsDiscretizedForLeafKernel_ARGS);
  }
}

template <bool USE_RAND, bool USE_L1, bool USE_SMOOTHING>
void CUDABestSplitFinder::LaunchFindBestSplitsDiscretizedForLeafKernelInner2(
    LaunchFindBestSplitsDiscretizedForLeafKernel_PARAMS) {
  if (!use_global_memory_) {
    if (is_smaller_leaf_valid) {
...
__global__ void SyncBestSplitForLeafKernel(const int smaller_leaf_index,
                                           const int larger_leaf_index,
                                           CUDASplitInfo* cuda_leaf_best_split_info,
                                           // input parameters
                                           const SplitFindTask* tasks,
                                           const CUDASplitInfo* cuda_best_split_info,
                                           const int num_tasks, const int num_tasks_aligned,
                                           const int num_blocks_per_leaf, const bool larger_only,
  ...
  const bool is_smaller =
      (blockIdx_x < static_cast<unsigned int>(num_blocks_per_leaf) && !larger_only);
  const uint32_t leaf_block_index =
      (is_smaller || larger_only) ? blockIdx_x
                                  : (blockIdx_x - static_cast<unsigned int>(num_blocks_per_leaf));
  const int task_index = static_cast<int>(leaf_block_index * blockDim.x + threadIdx_x);
  const uint32_t read_index = is_smaller ? static_cast<uint32_t>(task_index)
                                         : static_cast<uint32_t>(task_index + num_tasks);
  ...
  __syncthreads();
  const uint32_t best_read_index =
      ReduceBestGain(best_gain, best_found, shared_read_index, shared_gain_buffer,
                     shared_found_buffer, shared_thread_index_buffer);
  if (threadIdx.x == 0) {
    const int leaf_index_ref = is_smaller ? smaller_leaf_index : larger_leaf_index;
    const unsigned buffer_write_pos =
        static_cast<unsigned int>(leaf_index_ref) + leaf_block_index * num_leaves;
    ...
    if (best_split_info->is_valid) {
      *cuda_split_info = *best_split_info;
      cuda_split_info->inner_feature_index =
          is_smaller ? tasks[best_read_index].inner_feature_index
...
__global__ void SyncBestSplitForLeafKernelAllBlocks(const int smaller_leaf_index,
                                                    const int larger_leaf_index,
  ...
  if (!larger_only) {
    if (blockIdx.x == 0) {
      ...
      for (unsigned int block_index = 1; block_index < num_blocks_per_leaf; ++block_index) {
        const unsigned int leaf_read_pos = static_cast<unsigned int>(smaller_leaf_index) +
  ...
  if (larger_leaf_index >= 0) {
    if (blockIdx.x == 1 || larger_only) {
      ...
      for (unsigned int block_index = 1; block_index < num_blocks_per_leaf; ++block_index) {
        const unsigned int leaf_read_pos = static_cast<unsigned int>(larger_leaf_index) +
                                           block_index * static_cast<unsigned int>(num_leaves);
        const CUDASplitInfo* other_split_info = cuda_leaf_best_split_info + leaf_read_pos;
...
__global__ void SetInvalidLeafSplitInfoKernel(CUDASplitInfo* cuda_leaf_best_split_info,
...
void CUDABestSplitFinder::LaunchSyncBestSplitForLeafKernel(const int host_smaller_leaf_index,
                                                           const int host_larger_leaf_index,
                                                           const bool is_smaller_leaf_valid,
                                                           const bool is_larger_leaf_valid) {
  if (!is_smaller_leaf_valid || !is_larger_leaf_valid) {
    SetInvalidLeafSplitInfoKernel<<<1, 1>>>(cuda_leaf_best_split_info_, is_smaller_leaf_valid,
                                            is_larger_leaf_valid, host_smaller_leaf_index,
  ...
  const int num_blocks_per_leaf =
      (num_tasks_ + NUM_TASKS_PER_SYNC_BLOCK - 1) / NUM_TASKS_PER_SYNC_BLOCK;
  if (host_larger_leaf_index >= 0 && is_smaller_leaf_valid && is_larger_leaf_valid) {
    SyncBestSplitForLeafKernel<<<num_blocks_per_leaf, NUM_TASKS_PER_SYNC_BLOCK, 0,
                                 cuda_streams_[0]>>>(
        host_smaller_leaf_index, host_larger_leaf_index, cuda_leaf_best_split_info_,
        cuda_split_find_tasks_.RawData(), cuda_best_split_info_, num_tasks_, num_tasks_aligned,
        num_blocks_per_leaf, false, num_leaves_);
    if (num_blocks_per_leaf > 1) {
      SyncBestSplitForLeafKernelAllBlocks<<<1, 1, 0, cuda_streams_[0]>>>(
          host_smaller_leaf_index, host_larger_leaf_index, num_blocks_per_leaf, num_leaves_,
          cuda_leaf_best_split_info_, false);
    }
    SynchronizeCUDADevice(__FILE__, __LINE__);
    SyncBestSplitForLeafKernel<<<num_blocks_per_leaf, NUM_TASKS_PER_SYNC_BLOCK, 0,
                                 cuda_streams_[1]>>>(
        host_smaller_leaf_index, host_larger_leaf_index, cuda_leaf_best_split_info_,
        cuda_split_find_tasks_.RawData(), cuda_best_split_info_, num_tasks_, num_tasks_aligned,
        num_blocks_per_leaf, true, num_leaves_);
    if (num_blocks_per_leaf > 1) {
      SyncBestSplitForLeafKernelAllBlocks<<<1, 1, 0, cuda_streams_[1]>>>(
          host_smaller_leaf_index, host_larger_leaf_index, num_blocks_per_leaf, num_leaves_,
          cuda_leaf_best_split_info_, true);
    }
  } else {
    ...
    SyncBestSplitForLeafKernel<<<num_blocks_per_leaf, NUM_TASKS_PER_SYNC_BLOCK>>>(
        host_smaller_leaf_index, host_larger_leaf_index, cuda_leaf_best_split_info_,
        cuda_split_find_tasks_.RawData(), cuda_best_split_info_, num_tasks_, num_tasks_aligned,
        num_blocks_per_leaf, larger_only, num_leaves_);
    if (num_blocks_per_leaf > 1) {
      SynchronizeCUDADevice(__FILE__, __LINE__);
      SyncBestSplitForLeafKernelAllBlocks<<<1, 1>>>(
          host_smaller_leaf_index, host_larger_leaf_index, num_blocks_per_leaf, num_leaves_,
          cuda_leaf_best_split_info_, larger_only);
    }
  }
}
...
__global__ void FindBestFromAllSplitsKernel(const int cur_num_leaves,
  ...
  const int threadIdx_x = static_cast<int>(threadIdx.x);
  for (int leaf_index = threadIdx_x; leaf_index < cur_num_leaves;
       leaf_index += static_cast<int>(blockDim.x)) {
  ...
  const int best_leaf_index = ReduceBestGainForLeaves(
      thread_best_gain, thread_best_leaf_index, gain_shared_buffer, leaf_index_shared_buffer);
  if (threadIdx_x == 0) {
    ...
    if (best_leaf_index != -1) {
      ...
      cuda_best_split_info_buffer[7] =
          cuda_leaf_best_split_info[best_leaf_index].num_cat_threshold;
    }
  }
}
...
__global__ void PrepareLeafBestSplitInfo(const int smaller_leaf_index, const int larger_leaf_index,
  ...
  if (threadIdx_x == 0) {
    cuda_best_split_info_buffer[0] =
        cuda_leaf_best_split_info[smaller_leaf_index].inner_feature_index;
  ...
  if (larger_leaf_index >= 0) {
    if (threadIdx_x == 3) {
      cuda_best_split_info_buffer[3] =
          cuda_leaf_best_split_info[larger_leaf_index].inner_feature_index;
...
void CUDABestSplitFinder::LaunchFindBestFromAllSplitsKernel(
  ...
  FindBestFromAllSplitsKernel<<<1, NUM_THREADS_FIND_BEST_LEAF, 0, cuda_streams_[1]>>>(
      cur_num_leaves, cuda_leaf_best_split_info_, cuda_best_split_info_buffer_);
  ...
  SynchronizeCUDADevice(__FILE__, __LINE__);
  CopyFromCUDADeviceToHost<int>(host_leaf_best_split_info_buffer.data(),
                                cuda_best_split_info_buffer_, 8, __FILE__, __LINE__);
  ...
  *smaller_leaf_best_split_default_left =
      static_cast<uint8_t>(host_leaf_best_split_info_buffer[2]);
  if (larger_leaf_index >= 0) {
    ...
    *larger_leaf_best_split_default_left =
        static_cast<uint8_t>(host_leaf_best_split_info_buffer[5]);
  }
  *best_leaf_index = host_leaf_best_split_info_buffer[6];
  *num_cat_threshold = host_leaf_best_split_info_buffer[7];
}

__global__ void AllocateCatVectorsKernel(CUDASplitInfo* cuda_split_infos, size_t len,
  ...
  if (i < len) {
    if (has_categorical_feature) {
      ...
      cuda_split_infos[i].cat_threshold_real =
          cat_threshold_real_vec + i * max_num_categories_in_split;
...
void CUDABestSplitFinder::LaunchAllocateCatVectorsKernel(CUDASplitInfo* cuda_split_infos,
                                                         uint32_t* cat_threshold_vec,
                                                         int* cat_threshold_real_vec, size_t len) {
  const int num_blocks = (static_cast<int>(len) + NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER - 1) /
                         NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER;
  AllocateCatVectorsKernel<<<num_blocks, NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER>>>(
      cuda_split_infos, len, max_num_categories_in_split_, has_categorical_feature_,
      cat_threshold_vec, cat_threshold_real_vec);
}

__global__ void InitCUDARandomKernel(const int seed, const int num_tasks,
...
void CUDABestSplitFinder::LaunchInitCUDARandomKernel() {
  const int num_blocks =
      (static_cast<int>(cuda_randoms_.Size()) + NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER - 1) /
      NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER;
  InitCUDARandomKernel<<<num_blocks, NUM_THREADS_PER_BLOCK_BEST_SPLIT_FINDER>>>(
      extra_seed_, static_cast<int>(cuda_randoms_.Size()), cuda_randoms_.RawData());
}

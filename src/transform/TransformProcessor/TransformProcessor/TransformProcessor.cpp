...
bool TransformProcessor::CloseToZero(double val) {
  // Set an EPS to deal with the situation of "-0.00000" and make the result consistent with the previous version.
  if ((val < EPS && val > -EPS) || (val < 0.001 && val > 0.0))
    return true;
  return false;
}

vector<FeatureEvaluatorExtendedInfo> GetEvaluators(IniFileParserInterface* input_ini_interface, std::map<string, int> column_index_mapping) {
  ...
  for (int i = 0; i < input_ini_interface->GetInputCount(); i++) {
    ...
    if (evaluator->IsRawFeatureEvaluator()) {
      ...
      string key = input_ini_interface->GetInputName(i).substr(transform_str.length());
      if (column_index_mapping.find(key) != column_index_mapping.end()) {
        num = column_index_mapping.find(key)->second;
      }
      else {
        Log::Warning("Feature cannot be found!\n\tFeature: %s\n\tInput expression: %s", key.c_str(), input_ini_interface->GetInputName(i).c_str());
      }
      ...
      string key = input_ini_interface->GetInputName(i).substr(transform_str.length());
      IniFileParserInterface* from_freeform2 = IniFileParserInterface::Createfromfreeform2(key);
      FeatureEvaluator* evaluator = new FeatureEvaluator(from_freeform2, 0);
      FeatureMap* featuremap = new FeatureMap(from_freeform2);
      vector<int> intList;
      for (int i = 0; i < evaluator->GetRequiredRawFeatureIndices().size(); i++) {
        int requiredRawFeatureIndex = evaluator->GetRequiredRawFeatureIndices()[i];
        int num = -1;
        string rawFeatureName = featuremap->GetRawFeatureName(requiredRawFeatureIndex);
        if (column_index_mapping.find(rawFeatureName) != column_index_mapping.end()) {
          num = column_index_mapping.find(rawFeatureName)->second;
        }
        else {
          Log::Warning("Feature cannot be found!\n\tFeature: %s\n\tInput expression: %s", rawFeatureName.c_str(), key.c_str());
        }
...
TransformProcessor::TransformProcessor(const std::string& transform_str, const std::string& header_str, int label_id) {
  auto t0 = std::chrono::high_resolution_clock::now();
  this->_label_id = label_id; 
  if (label_id < 0) {
    Log::Warning("Label id %d, found no label.", label_id);
  }
  this->_from_input_str = IniFileParserInterface::CreateFromInputStr(transform_str);
  ...
  while ((pos = header_str.find(delimiter)) != string::npos) {
    ...
    this->_column_index_mapping.insert(std::pair<string, int>(header_str.substr(0, pos), index));
    index++;
    header_str.erase(0, pos + delimiter.length());
  }
  this->_column_index_mapping.insert(std::pair<string, int>(header_str, index));
  this->_feature_evaluator_list = GetEvaluators(this->_from_input_str, this->_column_index_mapping);
  // time
  auto t1 = std::chrono::high_resolution_clock::now();
  Log::Info("Initialize transform time: %.2f seconds", std::chrono::duration<double, std::milli>(t1-t0)*1e-3);
}

void TransformProcessor::Parse(const char* str, vector<string>* out_feature_strs, double* out_label, string delimiter) {
...
void TransformProcessor::Apply(vector<string>* input_row, vector<pair<int, double>>* out_features) {
  int num1 = 0;
  out_features->clear();
  for (vector<FeatureEvaluatorExtendedInfo>::iterator feature_evaluator = this->_feature_evaluator_list.begin();
      feature_evaluator != this->_feature_evaluator_list.end(); feature_evaluator++) {
    double result = 0;
    if (feature_evaluator->israw_) {
      if (feature_evaluator->index_ > -1 && feature_evaluator->index_ < input_row->size()) {
        result = ConvertStrToDouble((*input_row)[feature_evaluator->index_]);
      }
    } 
    else {
      size_t inputsize = feature_evaluator->required_feature_mapped_indexes_.size();
      uint32_t* input = new uint32_t[inputsize];
      int index = 0;
      string str2 = "";
      for (int i = 0; i < feature_evaluator->required_feature_mapped_indexes_.size(); i++) {
        input[index] = 0u;
        int feature_mapped_index = feature_evaluator->required_feature_mapped_indexes_[i];
        if (feature_mapped_index != -1 && feature_mapped_index < input_row->size()) {
...
string TransformedData::ToString(int precision) {
  ...
  for(auto f: this->Features()) {
    std::stringstream ss;
    ss << f.first << ":" << setiosflags(ios::fixed) << std::setprecision(precision) << f.second << " ";
    output_str += ss.str();
  }
  return output_str;
}
...
// Default: precision=5
vector<string> TransformedData::ToString(vector<TransformedData> dataset, int precision) {
  vector<string> output_vector;
  for(auto d: dataset)
    output_vector.push_back(d.ToString(precision));

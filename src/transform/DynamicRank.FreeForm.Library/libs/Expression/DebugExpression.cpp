...
DebugExpression::DebugExpression(const Annotations& p_annotations,
    ...
    : Expression(p_annotations),
      m_child(p_child),
      m_childText(p_childText)
{
}


void 
DebugExpression::Accept(Visitor& p_visitor) const
{
...
DebugExpression::GetType() const
{
    ...
    if (!checkType->IsLeafType())
    {
        ...
        err << "Cannot debug the expression " << m_childText
            << " of type " << *checkType
            << ". Only arrays and primitive types are supported.";
...
size_t 
DebugExpression::GetNumChildren() const
{
    return 1;
}
...
const Expression&
DebugExpression::GetChild() const
{
    return m_child;
}
...
const std::string&
DebugExpression::GetChildText() const
{
    return m_childText;
}

...
FreeForm2::TypeCheckingVisitor::TypeCheckingVisitor()
    : m_lastExpressionReturns(true), m_hasSideEffects(false) {}

void FreeForm2::TypeCheckingVisitor::AssertSideEffects(
    const FreeForm2::SourceLocation &p_sourceLocation) const {
  if (!m_hasSideEffects) {
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  if (m_lastExpressionReturns) {
    if (m_functions.size() > 0 && m_functions.top().m_allPathsReturn) {
      ...
      throw ParseError(err.str(),
                       p_expr.GetChild(numChildren).GetSourceLocation());
    }
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  const bool conditionalReturnsValue =
  ...
  const bool ifExprReturns =
      m_functions.size() > 0 && m_functions.top().m_allPathsReturn;
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  FF2_ASSERT(m_variableTypes.find(p_expr.GetReduceId()) ==
                 m_variableTypes.end() &&
  ...
  m_variableTypes.insert(std::make_pair(stepKey, &p_expr.GetLow().GetType()));
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  FF2_ASSERT(m_variableTypes.find(p_expr.GetIteratorId()) ==
             m_variableTypes.end());
  const auto insertKey = p_expr.GetIteratorId();
  m_variableTypes.insert(std::make_pair(insertKey, &p_expr.GetIteratorType()));
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  // Save the state of the name->type mapping to restore it after the function
  // body is evaluated.
  ...
  BOOST_FOREACH (auto &param, p_expr.GetParameters()) {
    FF2_ASSERT(m_variableTypes.find(param.m_parameter->GetId()) ==
               m_variableTypes.end());
    m_variableTypes.insert(std::make_pair(param.m_parameter->GetId(),
                                          &param.m_parameter->GetType()));
  }
  ...
  for (unsigned int i = 0; i < functionType.GetParameterCount(); ++i) {
    ...
    if (!functionType.BeginParameters()[i]->IsConst()) {
      ...
      const VariableRefExpression *refExpression =
          dynamic_cast<const VariableRefExpression *>(
              p_expr.GetParameters()[i]);
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  for (unsigned int i = 0; i < p_expr.GetNumChildren() - 1; i++) {
    FF2_ASSERT(m_variableTypes.find(p_expr.GetBound()[i].first) ==
               m_variableTypes.end());
    m_variableTypes.insert(std::make_pair(
        p_expr.GetBound()[i].first, &p_expr.GetBound()[i].second->GetType()));
  }
...
bool FreeForm2::TypeCheckingVisitor::AlternativeVisit(
  ...
  for (unsigned int i = 0; i < p_expr.GetNumMachineInstances(); i++) {
    ...
    AssertSideEffects(p_expr.GetMachineInstances()[i]
                          .m_machineDeclaration->GetSourceLocation());
  }

  FF2_ASSERT(m_variableTypes.find(p_expr.GetMachineIndexID()) ==
             m_variableTypes.end());
  const auto insertKey = p_expr.GetMachineIndexID();
  m_variableTypes.insert(
      std::make_pair(insertKey, &FreeForm2::TypeImpl::GetUInt32Instance(true)));
...
void FreeForm2::TypeCheckingVisitor::Visit(const MutationExpression &p_expr) {
  m_hasSideEffects = true;
  UniformExpressionVisitor::Visit(p_expr);
}

void FreeForm2::TypeCheckingVisitor::Visit(
    const DeclarationExpression &p_expr) {
  FF2_ASSERT(m_variableTypes.find(p_expr.GetId()) == m_variableTypes.end());
  m_variableTypes.insert(
      std::make_pair(p_expr.GetId(), &p_expr.GetDeclaredType()));
...
void FreeForm2::TypeCheckingVisitor::Visit(
  ...
  FeatureSpecExpression::PublishFeatureMap::const_iterator featureNameToType =
      m_publishFeatureMap->find(
          FeatureSpecExpression::FeatureName(p_expr.GetFeatureName()));
  ...
  const ArrayType &type =
      static_cast<const ArrayType &>(featureNameToType->second);

  if (!TypeUtil::IsAssignable(type.GetChildType(),
                              p_expr.GetValue().GetType())) {
    std::ostringstream err;
...
void FreeForm2::TypeCheckingVisitor::Visit(const PublishExpression &p_expr) {
  m_hasSideEffects = true;
  ...
  if (!TypeUtil::IsAssignable(featureNameToType->second,
                              p_expr.GetValue().GetType())) {
    std::ostringstream err;
    err << "Invalid publish type: " << p_expr.GetValue().GetType()
...
void FreeForm2::TypeCheckingVisitor::Visit(const ReturnExpression &p_expr) {
  if (m_functions.size() == 0) {
    std::ostringstream err;
  ...
  if (!TypeUtil::IsAssignable(*m_functions.top().m_returnType,
                              p_expr.GetValue().GetType())) {
    std::ostringstream err;
...
void FreeForm2::TypeCheckingVisitor::Visit(
  ...
  if (find == m_variableTypes.end()) {
    std::ostringstream err;
    err << "Variable referenced before its declaration (ID " << p_expr.GetId()
        << ")";
    throw ParseError(err.str(), p_expr.GetSourceLocation());
  }
...
void FreeForm2::TypeCheckingVisitor::Visit(const DebugExpression &p_expr) {
  m_hasSideEffects = true;
  UniformExpressionVisitor::Visit(p_expr);
}

void FreeForm2::TypeCheckingVisitor::VisitReference(
    const VariableRefExpression &p_expr) {
  Visit(p_expr);
}

void FreeForm2::TypeCheckingVisitor::Visit(const Expression &p_expr) {

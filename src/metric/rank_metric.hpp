      ...
      for (data_size_t i = 0; i < num_queries_; ++i) {
        ...
        if (inverse_max_dcgs_[i][0] <= 0.0f) {
          ...
          DCGCalculator::CalDCG(eval_at_, label_ + query_boundaries_[i],
                                ...
                                query_boundaries_[i + 1] - query_boundaries_[i], false, &tmp_dcg);
          // calculate NDCG
          for (size_t j = 0; j < eval_at_.size(); ++j) {
...
class WNDCGMetric:public Metric {
 public:
  explicit WNDCGMetric(const Config& config) {
    // get eval position
    eval_at_ = config.eval_at;
    auto label_gain = config.label_gain;
    DCGCalculator::DefaultEvalAt(&eval_at_);
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    for (auto k : eval_at_) {
      name_.emplace_back(std::string("wndcg@") + std::to_string(k));
    }
    num_data_ = num_data;
    // get label
    label_ = metadata.label();
    ...
    query_boundaries_ = metadata.query_boundaries();
    if (query_boundaries_ == nullptr) {
      Log::Fatal("The WNDCG metric requires query information");
    }
    // get query weights
    query_weights_ = metadata.query_weights();
    if (query_weights_ == nullptr) {

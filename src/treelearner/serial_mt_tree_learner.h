 ...
 */
#ifndef LIGHTGBM_TREELEARNER_SERIAL_MT_TREE_LEARNER_H_
#define LIGHTGBM_TREELEARNER_SERIAL_MT_TREE_LEARNER_H_
...
class CostEfficientMTGradientBoosting;

/*!
* \brief Used for learning a tree by single machine
*/
class SerialMTTreeLearner: public TreeLearner {
 public:
  friend CostEfficientMTGradientBoosting;
  explicit SerialMTTreeLearner(const Config* config);
  ...
  void Train_serial2(Tree* tree, const score_t* gradients, const score_t* hessians, bool is_first_tree) override;
  ...
  void SetBaggingData2(const Dataset* subset, const data_size_t* used_indices, data_size_t num_data) override {
...
#pragma omp parallel for schedule(static, 1)
  ...
  */
  ...
  std::unique_ptr<CostEfficientMTGradientBoosting> cegb_;
};

inline data_size_t SerialMTTreeLearner::GetGlobalDataCountInLeaf(int leaf_idx) const {
  if (leaf_idx >= 0) {
...
#endif   // LightGBM_TREELEARNER_SERIAL_MT_TREE_LEARNER_H_

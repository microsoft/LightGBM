...
public:
  ...
  */
  Feature(int feature_idx, BinMapper* bin_mapper,
    ...
    bin_data_.reset(Bin::CreateBin(num_data, bin_mapper_->num_bin(),
      bin_mapper_->sparse_rate(), is_enable_sparse, &is_sparse_, bin_mapper_->GetDefaultBin(), bin_mapper_->GetMaxHeavyBin(), bin_mapper_->bin_type()));
  }
  /*!
  ...
  */
  Feature(const void* memory, data_size_t num_all_data,
    ...
    if (is_sparse_) {
      bin_data_.reset(Bin::CreateSparseBin(num_data, bin_mapper_->num_bin(), bin_mapper_->GetDefaultBin(), bin_mapper_->GetMaxHeavyBin(), bin_mapper_->bin_type()));
    } else {

...
if [[ $OS_NAME == "macos" ]] && [[ $COMPILER == "clang" ]] && [[ $(sw_vers -productVersion | cut -d'.' -f2) -ge "14" ]]; then
    export CMAKE_OPTS="-DOpenMP_C_FLAGS=\"-Xpreprocessor -fopenmp -I$(brew --prefix libomp)/include\" -DOpenMP_C_LIB_NAMES=omp -DOpenMP_CXX_FLAGS=\"-Xpreprocessor -fopenmp -I$(brew --prefix libomp)/include\" -DOpenMP_CXX_LIB_NAMES=omp -DOpenMP_omp_LIBRARY=$(brew --prefix libomp)/lib/libomp.dylib"
else
    export CMAKE_OPTS=""
fi

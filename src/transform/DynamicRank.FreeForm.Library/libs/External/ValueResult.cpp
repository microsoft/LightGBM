...
namespace
{
    ...
    private:
        ...
        Clone() const 
        {
            return std::auto_ptr<ResultIteratorImpl>(new ValueResultIteratorImpl(m_pos, m_idx));
        }
...
FreeForm2::ValueResult::ValueResult(const ArrayType& p_arrayType,
...
FreeForm2::ValueResult::Parse(SIZED_STRING p_result, TypeManager& p_typeManager)
{
...
FreeForm2::ValueResult::GetType() const
{
    return m_type;
}


FreeForm2::Result::IntType 
FreeForm2::ValueResult::GetInt() const
{
...
FreeForm2::Result::UInt64Type 
FreeForm2::ValueResult::GetUInt64() const
{
...
int
FreeForm2::ValueResult::GetInt32() const
{
...
unsigned int
FreeForm2::ValueResult::GetUInt32() const
{
...
FreeForm2::Result::FloatType 
FreeForm2::ValueResult::GetFloat() const
{
...
bool 
FreeForm2::ValueResult::GetBool() const
{
...
FreeForm2::ResultIterator
FreeForm2::ValueResult::BeginArray() const
{
    FF2_ASSERT(GetType().Primitive() == Type::Array);
    return ResultIterator(std::auto_ptr<ResultIteratorImpl>(
        new ValueResultIteratorImpl(m_val.m_array.m_elements,
                                    0)));
}


FreeForm2::ResultIterator
FreeForm2::ValueResult::EndArray() const
{
    FF2_ASSERT(GetType().Primitive() == Type::Array);
    return ResultIterator(std::auto_ptr<ResultIteratorImpl>(
        new ValueResultIteratorImpl(m_val.m_array.m_elements + m_val.m_array.m_numElements,
                                    m_val.m_array.m_numElements)));
}


boost::shared_ptr<ValueResult>
FreeForm2::ValueResult::Parse(Tokenizer& p_tok, SIZED_STRING p_original, TypeManager& p_typeManager)
{
    ...
    switch (token)
    {
        case TOKEN_INT:
        {
            ...
            result.reset(new ValueResult(boost::lexical_cast<Result::IntType>(value)));
        ...
        {
            std::string value(p_tok.GetValue().pcData, p_tok.GetValue().cbData);
            result.reset(new ValueResult(boost::lexical_cast<Result::FloatType>(value)));
        ...
        case TOKEN_ATOM:
        {
            ...
            if (value == "infinity")
            {
                result.reset(new ValueResult(std::numeric_limits<Result::FloatType>::infinity()));
            }
            else if (value == "-")
            {
        ...
        case TOKEN_OPEN_ARRAY:
        {
            ...
            if (p_tok.GetToken() != TOKEN_CLOSE_ARRAY)
            {
                ...
                {
                    const ArrayType& arrayType = static_cast<const ArrayType&>(curr.GetType().GetImplementation());
            ...
            const ArrayType& type = p_typeManager.GetArrayType(TypeImpl::GetCommonType(basePrimitive, true),
                                                               true,
                                                               subDimensions + 1, 
                                                               sumElements);

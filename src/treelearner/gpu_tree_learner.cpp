...
GPUTreeLearner::GPUTreeLearner(const Config* config) : SerialTreeLearner(config) {
  use_bagging_ = false;
...
union Float_t {
  int64_t i;
  double f;
  static int64_t ulp_diff(Float_t a, Float_t b) { return abs(a.i - b.i); }
};
...
int GPUTreeLearner::GetNumWorkgroupsPerFeature(data_size_t leaf_num_data) {
  ...
  printf("Computing histogram for %d examples and (%d * %d) feature groups\n", leaf_num_data,
         dword_features_, num_dense_feature4_);
  printf(
      "We can have at most %d workgroups per feature4 for efficiency reasons.\n"
      "Best workgroup size per feature for full utilization is %d\n",
      static_cast<int>(ceil(t)), (1 << exp_workgroups_per_feature));
#endif
  exp_workgroups_per_feature = std::min(
      exp_workgroups_per_feature, static_cast<int>(ceil(log(static_cast<double>(t)) / log(2.0))));
  if (exp_workgroups_per_feature < 0) exp_workgroups_per_feature = 0;
...
void GPUTreeLearner::GPUHistogram(data_size_t leaf_num_data, bool use_all_features) {
  ...
  if (num_workgroups > preallocd_max_num_wg_) {
    ...
    Log::Info("Increasing preallocd_max_num_wg_ to %d for launching more workgroups",
  ...
  // there will be 2^exp_workgroups_per_feature = num_workgroups / num_dense_feature4 sub-histogram
  // per feature4 and we will launch num_feature workgroups for this kernel will launch threads for
  // all features the queue should be asynchronous, and we will can WaitAndGetHistograms() before
  // we start processing dense feature groups
  if (leaf_num_data == num_data_) {
    kernel_wait_obj_ = boost::compute::wait_list(queue_.enqueue_1d_range_kernel(
        histogram_fulldata_kernels_[exp_workgroups_per_feature], 0, num_workgroups * 256, 256));
  } else {
    if (use_all_features) {
      kernel_wait_obj_ = boost::compute::wait_list(queue_.enqueue_1d_range_kernel(
          histogram_allfeats_kernels_[exp_workgroups_per_feature], 0, num_workgroups * 256, 256));
    } else {
      kernel_wait_obj_ = boost::compute::wait_list(queue_.enqueue_1d_range_kernel(
          histogram_kernels_[exp_workgroups_per_feature], 0, num_workgroups * 256, 256));
    }
  }
  // copy the results asynchronously. Size depends on if double precision is used
  size_t output_size =
      num_dense_feature4_ * dword_features_ * device_bin_size_ * hist_bin_entry_sz_;
  boost::compute::event histogram_wait_event;
  host_histogram_outputs_ = reinterpret_cast<void*>(queue_.enqueue_map_buffer_async(
      device_histogram_outputs_, boost::compute::command_queue::map_read, 0, output_size,
      histogram_wait_event, kernel_wait_obj_));
...
void GPUTreeLearner::AllocateGPUMemory() {
  ...
  if (!num_dense_feature_groups_) {
    Log::Warning(
        "GPU acceleration is disabled because no non-trivial dense features can be found");
    return;
  }
  // allocate memory for all features (FIXME: 4 GB barrier on some devices, need to split to
  // multiple buffers)
  device_features_.reset();
  device_features_ = std::unique_ptr<boost::compute::vector<Feature4>>(
  ...
  // make ordered_gradients and Hessians larger (including extra room for prefetching), and pin
  // them
  ...
  pinned_gradients_ = boost::compute::buffer(
      ctx_, allocated_num_data_ * sizeof(score_t),
      boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr,
      ordered_gradients_.data());
  ptr_pinned_gradients_ = queue_.enqueue_map_buffer(
      pinned_gradients_, boost::compute::command_queue::map_write_invalidate_region, 0,
      allocated_num_data_ * sizeof(score_t));
  pinned_hessians_ = boost::compute::buffer();  // deallocate
  pinned_hessians_ = boost::compute::buffer(
      ctx_, allocated_num_data_ * sizeof(score_t),
      boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr,
      ordered_hessians_.data());
  ptr_pinned_hessians_ = queue_.enqueue_map_buffer(
      pinned_hessians_, boost::compute::command_queue::map_write_invalidate_region, 0,
      allocated_num_data_ * sizeof(score_t));
  // allocate space for gradients and Hessians on device
  // we will copy gradients and Hessians in after ordered_gradients_ and ordered_hessians_ are
  // constructed
  ...
  device_feature_masks_ =
      boost::compute::buffer(ctx_, num_dense_feature4_ * dword_features_,
                             boost::compute::memory_object::read_only, nullptr);
  pinned_feature_masks_ = boost::compute::buffer(
      ctx_, num_dense_feature4_ * dword_features_,
      boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr,
      feature_masks_.data());
  ptr_pinned_feature_masks_ = queue_.enqueue_map_buffer(
      pinned_feature_masks_, boost::compute::command_queue::map_write_invalidate_region, 0,
      num_dense_feature4_ * dword_features_);
  ...
  device_data_indices_ = std::unique_ptr<boost::compute::vector<data_size_t>>(
      new boost::compute::vector<data_size_t>(allocated_num_data_, ctx_));
  ...
  if (!device_subhistograms_) {
    // only initialize once here, as this will not need to change when ResetTrainingData() is
    // called
    device_subhistograms_ =
        std::unique_ptr<boost::compute::vector<char>>(new boost::compute::vector<char>(
            preallocd_max_num_wg_ * dword_features_ * device_bin_size_ * hist_bin_entry_sz_,
            ctx_));
  }
  // create atomic counters for inter-group coordination
  sync_counters_.reset();
  sync_counters_ = std::unique_ptr<boost::compute::vector<int>>(
      new boost::compute::vector<int>(num_dense_feature4_, ctx_));
  ...
  device_histogram_outputs_ = boost::compute::buffer(
      ctx_, num_dense_feature4_ * dword_features_ * device_bin_size_ * hist_bin_entry_sz_,
      boost::compute::memory_object::write_only | boost::compute::memory_object::alloc_host_ptr,
      nullptr);
  // find the dense feature-groups and group then into Feature4 data structure (several
  // feature-groups packed into 4 bytes)
...
// device_bin_mults_.push_back(1);
  ...
  int nthreads = std::min(OMP_NUM_THREADS(),
                          static_cast<int>(dense_feature_group_map_.size()) / dword_features_);
  ...
  for (int i = 0; i < nthreads; ++i) {
    host4_vecs[i] = reinterpret_cast<Feature4*>(
        boost::alignment::aligned_alloc(4096, num_data_ * sizeof(Feature4)));
    host4_bufs[i] = boost::compute::buffer(
        ctx_, num_data_ * sizeof(Feature4),
        boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr,
        host4_vecs[i]);
    host4_ptrs[i] = reinterpret_cast<Feature4*>(queue_.enqueue_map_buffer(
        host4_bufs[i], boost::compute::command_queue::map_write_invalidate_region, 0,
        num_data_ * sizeof(Feature4)));
  }
    ...
    if (dword_features_ == 8) {
      ...
      for (int s_idx = 0; s_idx < 8; ++s_idx) {
        ...
        if (dynamic_cast<DenseBinIterator<uint8_t, true>*>(bin_iters[s_idx]) == 0) {
          Log::Fatal(
              "GPU tree learner assumes that all bins are Dense4bitsBin when num_bin <= 16, but "
              "feature %d is not",
              dense_ind[s_idx]);
        }
      }
      // this guarantees that the RawGet() function is inlined, rather than using virtual function
      // dispatching
      ...
      for (int j = 0; j < num_data_; ++j) {
        host4[j].s[0] =
            (uint8_t)((iters[0].RawGet(j) * dev_bin_mult[0] + ((j + 0) & (dev_bin_mult[0] - 1))) |
                      ((iters[1].RawGet(j) * dev_bin_mult[1] + ((j + 1) & (dev_bin_mult[1] - 1)))
                       << 4));
        host4[j].s[1] =
            (uint8_t)((iters[2].RawGet(j) * dev_bin_mult[2] + ((j + 2) & (dev_bin_mult[2] - 1))) |
                      ((iters[3].RawGet(j) * dev_bin_mult[3] + ((j + 3) & (dev_bin_mult[3] - 1)))
                       << 4));
        host4[j].s[2] =
            (uint8_t)((iters[4].RawGet(j) * dev_bin_mult[4] + ((j + 4) & (dev_bin_mult[4] - 1))) |
                      ((iters[5].RawGet(j) * dev_bin_mult[5] + ((j + 5) & (dev_bin_mult[5] - 1)))
                       << 4));
        host4[j].s[3] =
            (uint8_t)((iters[6].RawGet(j) * dev_bin_mult[6] + ((j + 6) & (dev_bin_mult[6] - 1))) |
                      ((iters[7].RawGet(j) * dev_bin_mult[7] + ((j + 7) & (dev_bin_mult[7] - 1)))
                       << 4));
      }
      ...
      for (int s_idx = 0; s_idx < 4; ++s_idx) {
        ...
        // this guarantees that the RawGet() function is inlined, rather than using virtual
        // function dispatching
        if (dynamic_cast<DenseBinIterator<uint8_t, false>*>(bin_iter) != 0) {
          // Dense bin
          DenseBinIterator<uint8_t, false> iter =
              *static_cast<DenseBinIterator<uint8_t, false>*>(bin_iter);
          for (int j = 0; j < num_data_; ++j) {
            host4[j].s[s_idx] = (uint8_t)(iter.RawGet(j) * dev_bin_mult[s_idx] +
                                          ((j + s_idx) & (dev_bin_mult[s_idx] - 1)));
          }
        } else if (dynamic_cast<DenseBinIterator<uint8_t, true>*>(bin_iter) != 0) {
          // Dense 4-bit bin
          DenseBinIterator<uint8_t, true> iter =
              *static_cast<DenseBinIterator<uint8_t, true>*>(bin_iter);
          for (int j = 0; j < num_data_; ++j) {
            host4[j].s[s_idx] = (uint8_t)(iter.RawGet(j) * dev_bin_mult[s_idx] +
                                          ((j + s_idx) & (dev_bin_mult[s_idx] - 1)));
          }
...
#pragma omp critical
    queue_.enqueue_write_buffer(device_features_->get_buffer(),
                                (uint64_t)i * num_data_ * sizeof(Feature4),
                                num_data_ * sizeof(Feature4), host4);
#if GPU_DEBUG >= 1
    printf("first example of feature-group tuple is: %d %d %d %d\n", host4[0].s[0], host4[0].s[1],
           host4[0].s[2], host4[0].s[3]);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
          for (int j = 0; j < num_data_; ++j) {
            host4[j].s[i >> 1] |=
                (uint8_t)((iter.RawGet(j) *
                               device_bin_mults_[copied_feature4 * dword_features_ + i] +
                           ((j + i) &
                            (device_bin_mults_[copied_feature4 * dword_features_ + i] - 1)))
                          << ((i & 1) << 2));
          }
        } else {
          Log::Fatal(
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    ...
    queue_.enqueue_write_buffer(device_features_->get_buffer(),
                                (num_dense_feature4_ - 1) * (uint64_t)num_data_ * sizeof(Feature4),
                                num_data_ * sizeof(Feature4), host4);
#if GPU_DEBUG >= 1
  ...
  Log::Info(
      ...
      dense_feature_group_map_.size(),
      ((dense_feature_group_map_.size() + (dword_features_ - 1)) / dword_features_) * num_data_ *
          sizeof(Feature4) / (1024.0 * 1024.0),
...
std::string GPUTreeLearner::GetBuildLog(const std::string& opts) {
  boost::compute::program program =
      boost::compute::program::create_with_source(kernel_source_, ctx_);
  try {
    program.build(opts);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(guided)
  for (int i = 0; i <= kMaxLogWorkgroupsPerFeature; ++i) {
    ...
    opts << " -D POWER_FEATURE_WORKGROUPS=" << i << " -D USE_CONSTANT_BUF=" << use_constants
         << " -D USE_DP_FLOAT=" << int(config_->gpu_use_dp)
...
#pragma omp critical
    ...
    // kernel with all data indices (for root node, and assumes that root node always uses all
    // features)
  ...
  for (int i = 0; i <= kMaxLogWorkgroupsPerFeature; ++i) {
    ...
    if (share_state_->is_constant_hessian) {
      ...
      histogram_kernels_[i].set_args(*device_features_, device_feature_masks_, num_data_,
                                     ...
                                     *device_subhistograms_, *sync_counters_,
                                     device_histogram_outputs_);
      histogram_allfeats_kernels_[i].set_args(*device_features_, device_feature_masks_, num_data_,
                                              *device_data_indices_, num_data_, device_gradients_,
                                              0.0f, *device_subhistograms_, *sync_counters_,
                                              device_histogram_outputs_);
      ...
      histogram_kernels_[i].set_args(*device_features_, device_feature_masks_, num_data_,
                                     *device_data_indices_, num_data_, device_gradients_,
                                     device_hessians_, *device_subhistograms_, *sync_counters_,
                                     device_histogram_outputs_);
      histogram_allfeats_kernels_[i].set_args(*device_features_, device_feature_masks_, num_data_,
                                              *device_data_indices_, num_data_, device_gradients_,
                                              device_hessians_, *device_subhistograms_,
                                              *sync_counters_, device_histogram_outputs_);
      histogram_fulldata_kernels_[i].set_args(*device_features_, device_feature_masks_, num_data_,
                                              *device_data_indices_, num_data_, device_gradients_,
                                              device_hessians_, *device_subhistograms_,
                                              *sync_counters_, device_histogram_outputs_);
...
void GPUTreeLearner::InitGPU(int platform_id, int device_id) {
  ...
  if (platform_id >= 0 && device_id >= 0) {
    ...
    if (static_cast<int>(platforms.size()) > platform_id) {
      const std::vector<boost::compute::device> platform_devices =
          platforms[platform_id].devices();
  ...
  // ignore the feature groups that contain categorical features when producing warnings about
  // max_bin. these groups may contain larger number of bins due to categorical features, but not
  // due to the setting of max_bin.
...
void GPUTreeLearner::ResetTrainingDataInner(const Dataset* train_data, bool is_constant_hessian,
...
void GPUTreeLearner::BeforeTrain() {
  ...
  if (!use_bagging_ && num_dense_feature_groups_) {
    if (!share_state_->is_constant_hessian) {
      hessians_future_ = queue_.enqueue_write_buffer_async(device_hessians_, 0,
                                                           num_data_ * sizeof(score_t), hessians_);
    } else {
    ...
    gradients_future_ = queue_.enqueue_write_buffer_async(device_gradients_, 0,
                                                          num_data_ * sizeof(score_t), gradients_);
  }
  ...
  if (data_partition_->leaf_count(0) != num_data_ && num_dense_feature_groups_) {
    // On GPU, we start copying indices, gradients and Hessians now, instead at
    // ConstructHistogram() copy used gradients and Hessians to ordered buffer
    ...
    indices_future_ =
        boost::compute::copy_async(indices, indices + cnt, device_data_indices_->begin(), queue_);
    if (!share_state_->is_constant_hessian) {
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      ...
      hessians_future_ = queue_.enqueue_write_buffer_async(
          device_hessians_, 0, cnt * sizeof(score_t), ordered_hessians_.data());
    } else {
      // setup hessian parameters only
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    ...
    gradients_future_ = queue_.enqueue_write_buffer_async(
        device_gradients_, 0, cnt * sizeof(score_t), ordered_gradients_.data());
  }
}
...
#endif
    indices_future_ = boost::compute::copy_async(indices + begin, indices + end,
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      ...
      hessians_future_ = queue_.enqueue_write_buffer_async(
          device_hessians_, 0, (end - begin) * sizeof(score_t), ptr_pinned_hessians_);
    }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    ...
    gradients_future_ = queue_.enqueue_write_buffer_async(
        device_gradients_, 0, (end - begin) * sizeof(score_t), ptr_pinned_gradients_);
...
bool GPUTreeLearner::ConstructGPUHistogramsAsync(const std::vector<int8_t>& is_feature_used,
  ...
  if (data_indices != nullptr && num_data != num_data_) {
    indices_future_ = boost::compute::copy_async(data_indices, data_indices + num_data,
                                                 device_data_indices_->begin(), queue_);
  }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      ...
      gradients_future_ = queue_.enqueue_write_buffer_async(
          device_gradients_, 0, num_data * sizeof(score_t), ptr_pinned_gradients_);
    } else {
      gradients_future_ = queue_.enqueue_write_buffer_async(device_gradients_, 0,
                                                            num_data * sizeof(score_t), gradients);
    }
  }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      ...
      hessians_future_ = queue_.enqueue_write_buffer_async(
          device_hessians_, 0, num_data * sizeof(score_t), ptr_pinned_hessians_);
    } else {
      hessians_future_ = queue_.enqueue_write_buffer_async(device_hessians_, 0,
                                                           num_data * sizeof(score_t), hessians);
    }
  }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static, 1024) \
    reduction(+ : used_dense_feature_groups) if (num_dense_feature_groups_ >= 2048)
  for (int i = 0; i < num_dense_feature_groups_; ++i) {
  ...
  printf("\n");
  printf("%d feature groups, %d used, %d\n", num_dense_feature_groups_, used_dense_feature_groups,
         use_all_features);
#endif
  ...
  if (!use_all_features) {
    queue_.enqueue_write_buffer(device_feature_masks_, 0, num_dense_feature4_ * dword_features_,
...
void GPUTreeLearner::ConstructHistograms(const std::vector<int8_t>& is_feature_used,
                                         bool use_subtract) {
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  // ConstructGPUHistogramsAsync will return true if there are available feature groups dispatched
  // to GPU
  bool is_gpu_used = ConstructGPUHistogramsAsync(is_feature_used, nullptr,
                                                 smaller_leaf_splits_->num_data_in_leaf(), nullptr,
                                                 nullptr, nullptr, nullptr);
  // then construct sparse features on CPU
  train_data_->ConstructHistograms<false, 0>(
      is_sparse_feature_used, smaller_leaf_splits_->data_indices(),
...
// #define GPU_DEBUG_COMPARE
  ...
  for (int i = 0; i < num_dense_feature_groups_; ++i) {
    ...
    hist_t* ptr_smaller_leaf_hist_data = smaller_leaf_histogram_array_[0].RawData() - kHistOffset;
    hist_t* current_histogram =
        ptr_smaller_leaf_hist_data + train_data_->GroupBinBoundary(dense_feature_group_index) * 2;
    ...
    if (num_data != num_data_) {
      train_data_->FeatureGroupBin(dense_feature_group_index)
          ->ConstructHistogram(smaller_leaf_splits_->data_indices(), 0, num_data,
                               ordered_gradients_.data(), ordered_hessians_.data(),
                               current_histogram);
    } else {
      train_data_->FeatureGroupBin(dense_feature_group_index)
          ->ConstructHistogram(0, num_data, gradients_, hessians_, current_histogram);
    }
  ...
  if (larger_leaf_histogram_array_ != nullptr && !use_subtract) {
    ...
    is_gpu_used =
        ConstructGPUHistogramsAsync(is_feature_used, larger_leaf_splits_->data_indices(),
                                    larger_leaf_splits_->num_data_in_leaf(), gradients_, hessians_,
                                    ordered_gradients_.data(), ordered_hessians_.data());
    // then construct sparse features on CPU
    train_data_->ConstructHistograms<false, 0>(
        is_sparse_feature_used, larger_leaf_splits_->data_indices(),
        larger_leaf_splits_->num_data_in_leaf(), gradients_, hessians_, ordered_gradients_.data(),
        ordered_hessians_.data(), share_state_.get(), ptr_larger_leaf_hist_data);
    // wait for GPU to finish, only if GPU is actually used
    if (is_gpu_used) {
      if (config_->gpu_use_dp) {
        // use double precision
...
void GPUTreeLearner::FindBestSplits(const Tree* tree) {
  ...
  for (int feature_index = 0; feature_index < num_features_; ++feature_index) {
    ...
    PrintHistograms(smaller_leaf_histogram_array_[feature_index].RawData() - kHistOffset,
                    bin_size);
...
void GPUTreeLearner::Split(Tree* tree, int best_Leaf, int* left_leaf, int* right_leaf) {
  ...
  printf("Splitting leaf %d with feature %d thresh %d gain %f stat %f %f %f %f\n", best_Leaf,
         ...
         best_split_info.left_sum_gradient, best_split_info.right_sum_gradient,
         best_split_info.left_sum_hessian, best_split_info.right_sum_hessian);
#endif

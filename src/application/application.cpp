...
void Application::LoadData() {
  ...
  if (boosting_->NumberOfTotalModel() > 0 &&
    ...
    predictor.reset(
        new Predictor(boosting_.get(), -1, true, false, false, false, -1, -1));
    predict_fun = predictor->GetPredictFunction();
  }

  // sync up random seed for data partition
  if (config_.is_parallel_find_bin) {
    config_.data_random_seed =
        Network::GlobalSyncUpByMin(config_.data_random_seed);
  }

  Log::Debug("Loading train file...");
  DatasetLoader dataset_loader(config_, predict_fun, config_.num_class,
                               config_.data.c_str());
  // load Training data
  if (config_.is_parallel_find_bin) {
    // load data for parallel training
    train_data_.reset(dataset_loader.LoadFromFile(
        config_.data.c_str(), config_.initscore_filename.c_str(),
        Network::rank(), Network::num_machines()));
  } else {
    // load data for single machine
    train_data_.reset(dataset_loader.LoadFromFile(
        config_.data.c_str(), config_.initscore_filename.c_str(), 0, 1));
  }
    ...
    for (size_t i = 0; i < config_.valid.size(); ++i) {
      ...
      auto new_dataset = std::unique_ptr<Dataset>(
          dataset_loader.LoadFromFileAlignWithOtherDataset(
              ...
              config_.valid_data_initscores[i].c_str(), train_data_.get()));
...
void Application::InitTrain() {
  ...
  for (size_t i = 0; i < valid_datas_.size(); ++i) {
    ...
    Log::Debug("Number of data points in validation set #%zu: %zu", i + 1,
...
void Application::Predict() {
  if (config_.task == TaskType::KRefitTree) {
    ...
    predictor.Predict(config_.data.c_str(), config_.output_result.c_str(),
                      config_.header, config_.predict_disable_shape_check);
...
#pragma omp parallel for schedule(static)
    ...
    DatasetLoader dataset_loader(config_, nullptr, config_.num_class,
                                 config_.data.c_str());
    train_data_.reset(dataset_loader.LoadFromFile(
        config_.data.c_str(), config_.initscore_filename.c_str(), 0, 1));
    train_metric_.clear();
    ...
    // create predictor
    Predictor predictor(boosting_.get(), config_.num_iteration_predict,

...
class Metric {
 public:
  ...
  virtual std::vector<double> Eval(
      const double* score, const ObjectiveFunction* objective) const = 0;
   ...
   */
  LIGHTGBM_EXPORT static Metric* CreateMetric(const std::string& type,
                                              const Config& config);
};
...
class DCGCalculator {
 public:
   ...
   */
  static void CalDCG(const std::vector<data_size_t>& ks, const label_t* label,
                     const double* score, data_size_t num_data,
   ...
   */
  static double CalMaxDCGAtK(data_size_t k, const label_t* label,

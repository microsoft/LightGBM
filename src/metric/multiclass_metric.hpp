...
class MultiSoftmaxLoglossMetric : public MulticlassMetric<MultiSoftmaxLoglossMetric> {
 public:
  ...
  inline static const std::string Name(const Config&) { return "multi_logloss"; }
};
    ...
    for (int i = 0; i < num_class_; ++i) {
      ...
      for (int j = i + 1; j < num_class_; ++j) {
        ...
        class_i_j_indices.assign(sorted_data_idx_.begin() + i_start,
                                 sorted_data_idx_.begin() + i_start + class_sizes_[i]);
        class_i_j_indices.insert(class_i_j_indices.end(), sorted_data_idx_.begin() + j_start,
        ...
        if (weights_ == nullptr) {
          for (size_t k = 0; k < dist.size(); ++k) {
            ...
            if (label_[a] == i) {
              if (std::fabs(curr_dist - last_j_dist) < kEpsilon) {
                S[i][j] += num_j - 0.5 * num_current_j;  // members of class j with same distance
                                                         // as a contribute 0.5
              } else {
                S[i][j] += num_j;
              }
          ...
          for (size_t k = 0; k < dist.size(); ++k) {
            ...
            if (label_[a] == i) {
              if (std::fabs(curr_dist - last_j_dist) < kEpsilon) {
                S[i][j] +=
                    curr_weight * (num_j - 0.5 * num_current_j);  // members of class j with same

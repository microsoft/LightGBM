...
namespace
{
    const FreeForm2::Expression& GetInitializer(const FreeForm2::StateMachineExpression& p_machine, 
                                                const FreeForm2::CompoundType::Member& p_memberInfo)
    {
        const FreeForm2::TypeInitializerExpression& expr = p_machine.GetInitializer();
        for (const FreeForm2::TypeInitializerExpression::Initializer* iter = expr.BeginInitializers(); 
             iter != expr.EndInitializers(); 
             ++iter)
        {
...
FreeForm2::MemberAccessExpression::MemberAccessExpression(const Annotations& p_annotations,
                                                          const Expression& p_struct,
                                                          const CompoundType::Member& p_memberInfo,
                                                          size_t p_version)
    : Expression(Annotations(p_annotations.m_sourceLocation, ValueBounds(*p_memberInfo.m_type))),
      m_struct(p_struct), 
      m_memberInfo(p_memberInfo),
      m_version(p_version)
{
    FF2_ASSERT(CompoundType::IsCompoundType(m_struct.GetType()));
    const CompoundType& compoundType = static_cast<const CompoundType&>(m_struct.GetType());
    FF2_ASSERT(compoundType.FindMember(p_memberInfo.m_name) != NULL);
}


const FreeForm2::TypeImpl&
FreeForm2::MemberAccessExpression::GetType() const
{
...
size_t
FreeForm2::MemberAccessExpression::GetNumChildren() const
{
    return 1;
}
...
void
FreeForm2::MemberAccessExpression::Accept(Visitor& p_visitor) const
{
...
void
FreeForm2::MemberAccessExpression::AcceptReference(Visitor& p_visitor) const
{
    size_t stackSize = p_visitor.StackSize();
...
bool
FreeForm2::MemberAccessExpression::IsConstant() const
{
    if (m_struct.GetType().Primitive() == Type::StateMachine)
    {
        const StateMachineType& machine = static_cast<const StateMachineType&>(m_struct.GetType());
...
FreeForm2::MemberAccessExpression::GetConstantValue() const
{
...
FreeForm2::MemberAccessExpression::GetStruct() const
{
    return m_struct;
}
...
size_t
FreeForm2::MemberAccessExpression::GetVersion() const
{
    return m_version;
}


FreeForm2::UnresolvedAccessExpression::UnresolvedAccessExpression(const Annotations& p_annotations,
...
const FreeForm2::TypeImpl&
FreeForm2::UnresolvedAccessExpression::GetType() const
{
...
size_t
FreeForm2::UnresolvedAccessExpression::GetNumChildren() const
{
    return 1;
}
...
void
FreeForm2::UnresolvedAccessExpression::Accept(Visitor& p_visitor) const
{
    size_t stackSize = p_visitor.StackSize();
...
void
FreeForm2::UnresolvedAccessExpression::AcceptReference(Visitor& p_visitor) const
{
    size_t stackSize = p_visitor.StackSize();
...
const FreeForm2::Expression&
FreeForm2::UnresolvedAccessExpression::GetObject() const
{
    return m_object;
}
...
const std::string&
FreeForm2::UnresolvedAccessExpression::GetMemberName() const
{
    return m_memberName;
}

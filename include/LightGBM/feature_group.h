      ...
      if (bin_mappers_[sub_feature]->bin_type() == BinType::NumericalBin) {
        ...
        if (USE_INDICES) {
          return bin_data_->Split(min_bin, max_bin, default_bin, most_freq_bin,
                                  ...
                                  data_indices, start, cnt, lte_indices,
                                  gt_indices);
        } else {
          return bin_data_->Split(min_bin, max_bin, default_bin, most_freq_bin,
                                  missing_type, default_left, *threshold, start,
        ...
        if (USE_INDICES) {
          return bin_data_->SplitCategorical(
              ...
              data_indices, start, cnt, lte_indices, gt_indices);
        } else {
          return bin_data_->SplitCategorical(min_bin, max_bin, most_freq_bin,
                                             threshold, num_threshold, start,
                                             cnt, lte_indices, gt_indices);
        }
      }
    } else {
      ...
      if (bin_mappers_[sub_feature]->bin_type() == BinType::NumericalBin) {
        ...
        if (USE_INDICES) {
          return multi_bin_data_[sub_feature]->Split(
              ...
              default_left, *threshold, data_indices, start, cnt, lte_indices,
              gt_indices);
        } else {
          return multi_bin_data_[sub_feature]->Split(
              min_bin, max_bin, default_bin, most_freq_bin, missing_type,
              default_left, *threshold, start, cnt, lte_indices, gt_indices);
        }
      } else {
        if (USE_INDICES) {
          ...
          return multi_bin_data_[sub_feature]->SplitCategorical(
              min_bin, max_bin, most_freq_bin, threshold, num_threshold, start,
              cnt, lte_indices, gt_indices);
        }
      }
    }

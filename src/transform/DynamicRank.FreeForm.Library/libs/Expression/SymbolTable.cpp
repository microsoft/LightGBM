...
namespace
{
    static const FreeForm2::LiteralBoolExpression c_falseExpression(FreeForm2::Annotations(), false);
    static const FreeForm2::LiteralBoolExpression c_trueExpression(FreeForm2::Annotations(), true);
}


FreeForm2::SymbolTable::SymbolTable(SimpleExpressionOwner& p_owner, 
                                    DynamicRank::IFeatureMap* p_featureMap)
    : m_owner(p_owner), m_featureMap(p_featureMap), m_localStackStart(0), m_allowFeatures(true)
{
    // Push true and false symbols onto stack.
    static const char* c_false = "false";
    Bind(FreeForm2::SymbolTable::Symbol(CStackSizedString(c_false)), &c_falseExpression); 

    static const char* c_true = "true";
    Bind(FreeForm2::SymbolTable::Symbol(CStackSizedString(c_true)), &c_trueExpression); 
...
void 
FreeForm2::SymbolTable::Bind(const Symbol& p_symbol, const Expression* p_expr)
{
    ...
    else
    {
        ...
        err << "Failed to bind name '" << p_symbol
            << "'.  Bound names can only contain alphanumeric characters, hyphens, and underscores.";
...
FreeForm2::SymbolTable::FindLocal(const Symbol& p_symbol) const
{
...
bool 
FreeForm2::SymbolTable::FindFeatureIndex(SIZED_STRING p_str, UInt32& p_index) const
{
...
const FreeForm2::Expression& 
FreeForm2::SymbolTable::Lookup(const Symbol& p_symbol) const
{
    ...
    if (FindFeatureIndex(p_symbol.GetSymbolName(), index))
    {
        ...
        boost::shared_ptr<Expression> exp(new FeatureRefExpression(Annotations(), index));
...
bool 
FreeForm2::SymbolTable::IsBound(const Symbol& p_symbol) const
{
    UInt32 index;
    return (FindLocal(p_symbol) != NULL || FindFeatureIndex(p_symbol.GetSymbolName(), index));
}


size_t 
FreeForm2::SymbolTable::GetNumLocal() const
{
...
void 
FreeForm2::SymbolTable::SetAllowFeatures(bool p_allowFeatures)
{
...
bool 
FreeForm2::SymbolTable::GetAllowFeatures() const
{
...
bool 
FreeForm2::SymbolTable::Symbol::operator==(const Symbol& p_other) const
{
    return m_isParameterized == p_other.m_isParameterized
        && m_hash == p_other.m_hash
		&& m_paramHash == p_other.m_paramHash
        ...
        && (m_isParameterized ? m_param.cbData == p_other.m_param.cbData
                             && memcmp(m_param.pcData, p_other.m_param.pcData, m_param.cbData) == 0
                             : true);
}


SIZED_STRING
FreeForm2::SymbolTable::Symbol::GetSymbolName() const
{
    return m_str;
}
...
bool
FreeForm2::SymbolTable::Symbol::IsParameterized() const
{
...
std::string
FreeForm2::SymbolTable::Symbol::ToString() const
{
...
std::ostream&
FreeForm2::operator<<(std::ostream& p_out, const FreeForm2::SymbolTable::Symbol& p_symbol)
{

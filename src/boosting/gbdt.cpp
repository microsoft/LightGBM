...
void GBDT::Init(const Config* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  use_goss_as_strategy = config_->data_sample_strategy == std::string("goss");
  use_goss_as_boosting = config_->boosting == std::string("goss");
  if (use_goss_as_boosting && use_goss_as_strategy) {
    Log::Warning("You have set both data_sample_strategy and boosting to GOSS, LightGBM will only do once.");
  }
...
data_size_t GBDT::GOSSHelper(data_size_t start, data_size_t cnt, data_size_t* buffer) {
  if (cnt <= 0) {
    return 0;
  }
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
  ...
  if (use_goss_as_boosting) {
    Bagging(iter_);  // will be overrided by GOSS::Bagging() 
  } else if (use_goss_as_strategy) {
    Log::Info("Using GOSS");
    GOSS(iter_);
  } else {
    GBDT::Bagging(iter_); 

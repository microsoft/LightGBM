...
FreeForm2::Token 
FreeForm2::Tokenizer::GetToken() const
{
    return m_current;
}
...
FreeForm2::Token 
FreeForm2::Tokenizer::AdvanceInput()
{
    ...
    do
    {
        ...
        if (m_input.cbData > 0 && m_input.pbData[0] == '#')
        {
            ...
            while (m_input.cbData > 0 
                && m_input.pcData[0] != '\r' && m_input.pcData[0] != '\n')
            {
                AdvanceChar();
            }
        }
    ...
    else if (isdigit(m_input.pbData[0]) || (m_input.pbData[0] == '-' 
        && m_input.cbData > 1 && isdigit(m_input.pbData[1])))
    {
        ...
        if (m_input.cbData > 0 && (m_input.pcData[0] == 'e' || m_input.pcData[0] == 'E'))
        {
            tok = TOKEN_FLOAT;
            AdvanceChar();

            if (m_input.cbData > 0 && (m_input.pcData[0] == '-' || m_input.pcData[0] == '+'))
            {
    ...
    else if (ispunct(m_input.pbData[0]) && m_input.pcData[0] != '#' && m_input.pcData[0] != '@')
    {
        // Parse atom.
        m_value.pbData = m_input.pbData;

        for (AdvanceChar();
             m_input.cbData > 0 && ispunct(m_input.pbData[0]) 
...
SIZED_STRING 
FreeForm2::Tokenizer::GetValue() const
{
    return m_value;
}
...
unsigned int
FreeForm2::Tokenizer::GetPosition() const
{
...
void 
FreeForm2::Tokenizer::AdvanceChar()
{
    m_input.pbData++;
    m_input.cbData--;
}
...
const char* 
FreeForm2::Tokenizer::TokenName(Token p_token)
{
    switch (p_token)
    {
        ...
        case TOKEN_OPEN_ARRAY: return "open array";
        case TOKEN_CLOSE_ARRAY: return "close array";
...
void
FreeForm2::Tokenizer::StartMacro(SIZED_STRING p_macroName)
{
    if (m_macroState.IsInPlayback())
    {
        std::ostringstream err;
...
void
FreeForm2::Tokenizer::EndMacro()
{
    m_macroState.EndMacro();
}


bool
FreeForm2::Tokenizer::DeleteMacro(SIZED_STRING p_name)
{
...
bool
FreeForm2::Tokenizer::IsExpandingMacro() const
{
...
bool
FreeForm2::Tokenizer::IsRecordingMacro() const
{
    return m_macroState.IsRecording();
}


FreeForm2::Tokenizer::MacroState::MacroState()
...
void
FreeForm2::Tokenizer::MacroState::StartMacro(SIZED_STRING p_macroName)
{
...
void
FreeForm2::Tokenizer::MacroState::RecordToken(Token p_token, SIZED_STRING p_value)
{
...
void
FreeForm2::Tokenizer::MacroState::EndMacro()
{
    FF2_ASSERT(IsRecording() && "Macro recording not started");
    FF2_ASSERT(!m_recordingStream->empty() && "Macro token stream cannot be empty");
...
bool
FreeForm2::Tokenizer::MacroState::IsRecording() const
{
...
bool
FreeForm2::Tokenizer::MacroState::IsInPlayback() const
{
...
bool
FreeForm2::Tokenizer::MacroState::PlayMacro(SIZED_STRING p_name)
{
    ...
    if (find != m_macros.end())
    {
        ...
        for (auto iter = m_playbackStack.cbegin(); iter != m_playbackStack.cend(); ++iter)
        {
            if (&stream == iter->first)
            {
                std::ostringstream err;
                err << "Macro definition is malformed: recursive macros are not allowed " 
...
void
FreeForm2::Tokenizer::MacroState::Advance()
{
...
bool
FreeForm2::Tokenizer::MacroState::DeleteMacro(SIZED_STRING p_name)
{
    const auto find = m_macros.find(p_name);
...
bool
FreeForm2::Tokenizer::MacroState::SizedStringLess::operator()(
    SIZED_STRING p_left,
    SIZED_STRING p_right) const
{
    if (p_left.cbData < p_right.cbData)
    {
        return std::char_traits<char>::compare(p_left.pcData, p_right.pcData, p_left.cbData) <= 0;
    }
    else
    {
        return std::char_traits<char>::compare(p_left.pcData, p_right.pcData, p_right.cbData) < 0;
    }
}

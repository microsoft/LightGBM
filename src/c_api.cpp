...
#define UNIQUE_LOCK(mtx) std::unique_lock<yamc::alternate::shared_mutex> lock(mtx);

#define SHARED_LOCK(mtx) yamc::shared_lock<yamc::alternate::shared_mutex> lock(&mtx);
...
class SingleRowPredictorInner {
 public:
  ...
  SingleRowPredictorInner(int predict_type, Boosting* boosting, const Config& config,
                          int start_iter, int num_iter) {
    ...
    iter_ = num_iter;
    predictor_.reset(new Predictor(boosting, start_iter, iter_, is_raw_score, is_predict_leaf,
                                   predict_contrib, early_stop_, early_stop_freq_,
                                   early_stop_margin_));
    num_pred_in_one_row =
        boosting->NumPredictOneRow(start_iter, iter_, is_predict_leaf, predict_contrib);
  ...
  bool IsPredictorEqual(const Config& config, int iter, Boosting* boosting) {
    return early_stop_ == config.pred_early_stop &&
           early_stop_freq_ == config.pred_early_stop_freq &&
           early_stop_margin_ == config.pred_early_stop_margin && iter_ == iter &&
           num_total_model_ == boosting->NumberOfTotalModel();
  }
...
struct SingleRowPredictor {
 public:
  SingleRowPredictor(yamc::alternate::shared_mutex* booster_mutex, const char* parameters,
                     const int data_type, const int32_t num_cols, int predict_type,
                     Boosting* boosting, int start_iter, int num_iter)
      : config(Config::Str2Map(parameters)),
        data_type(data_type),
        num_cols(num_cols),
        single_row_predictor_inner(predict_type, boosting, config, start_iter, num_iter),
        booster_mutex(booster_mutex) {
    if (!config.predict_disable_shape_check && num_cols != boosting->MaxFeatureIdx() + 1) {
      Log::Fatal(
          ...
          "You can set ``predict_disable_shape_check=true`` to discard this error, but please be "
          "aware what you are doing.",
          num_cols, boosting->MaxFeatureIdx() + 1);
    }
  }
 ...
 private:
  ...
  // Prevent the booster from being modified while we have a predictor relying on it during
  // prediction
 ...
 public:
  ...
  void ResetTrainingData(const Dataset* train_data) {
    if (train_data != train_data_) {
      ...
      CreateObjectiveAndMetrics();
      // reset the boosting
      boosting_->ResetTrainingData(train_data_, objective_fun_.get(),
                                   Common::ConstPtrInVectorWrapper<Metric>(train_metric_));
    }
  }

  static void CheckDatasetResetConfig(
      const Config& old_config, const std::unordered_map<std::string, std::string>& new_param) {
    Config new_config;
    ...
    if (new_param.count("bin_construct_sample_cnt") &&
        new_config.bin_construct_sample_cnt != old_config.bin_construct_sample_cnt) {
      Log::Fatal(
  ...
  void SetSingleRowPredictorInner(int start_iteration, int num_iteration, int predict_type,
                                  const Config& config) {
    UNIQUE_LOCK(mutex_)
    if (single_row_predictor_[predict_type].get() == nullptr ||
        !single_row_predictor_[predict_type]->IsPredictorEqual(config, num_iteration,
                                                               boosting_.get())) {
  ...
  std::unique_ptr<SingleRowPredictor> InitSingleRowPredictor(int predict_type, int start_iteration,
                                                             int num_iteration, int data_type,
                                                             int32_t num_cols,
                                                             const char* parameters) {
    // Workaround https://github.com/microsoft/LightGBM/issues/6142 by locking here
    // This is only a workaround because if predictors are initialized differently it may still
    // behave incorrectly, and because multiple racing Predictor initializations through
    // LGBM_BoosterPredictForMat suffers from that same issue of Predictor init writing things in
    // the booster. Once #6142 is fixed (predictor doesn't write in the Booster as should have been
    // the case since 1c35c3b9ede9adab8ccc5fd7b4b2b6af188a79f0), this line can be removed.
    UNIQUE_LOCK(mutex_)

    return std::unique_ptr<SingleRowPredictor>(
        new SingleRowPredictor(&mutex_, parameters, data_type, num_cols, predict_type,
                               boosting_.get(), start_iteration, num_iteration));
  }

  void PredictSingleRow(
      int predict_type, int ncol,
      std::function<std::vector<std::pair<int, double>>(int row_idx)> get_row_fun,
      const Config& config, double* out_result, int64_t* out_len) const {
  ...
  std::shared_ptr<Predictor> CreatePredictor(int start_iteration, int num_iteration,
                                             int predict_type, int ncol,
    ...
    return std::make_shared<Predictor>(boosting_.get(), start_iteration, num_iteration,
                                       is_raw_score, is_predict_leaf, predict_contrib,
  ...
  void Predict(int start_iteration, int num_iteration, int predict_type, int nrow, int ncol,
    ...
    int64_t num_pred_in_one_row = boosting_->NumPredictOneRow(start_iteration, num_iteration,
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  void PredictSparse(
      int start_iteration, int num_iteration, int predict_type, int64_t nrow, int ncol,
      std::function<std::vector<std::pair<int, double>>(int64_t row_idx)> get_row_fun,
      const Config& config, int64_t* out_elements_size,
      std::vector<std::vector<std::unordered_map<int, double>>>* agg_ptr, int32_t** out_indices,
      void** out_data, int data_type, bool* is_data_float32_ptr, int num_matrices) const {
    auto predictor = CreatePredictor(start_iteration, num_iteration, predict_type, ncol, config);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  void PredictSparseCSR(
      int start_iteration, int num_iteration, int predict_type, int64_t nrow, int ncol,
      std::function<std::vector<std::pair<int, double>>(int64_t row_idx)> get_row_fun,
      const Config& config, int64_t* out_len, void** out_indptr, int indptr_type,
      int32_t** out_indices, void** out_data, int data_type) const {
    SHARED_LOCK(mutex_);
    // Get the number of trees per iteration (for multiclass scenario we output multiple sparse
    // matrices)
    ...
    int64_t elements_size = 0;
    PredictSparse(start_iteration, num_iteration, predict_type, nrow, ncol, get_row_fun, config,
                  &elements_size, &agg, out_indices, out_data, data_type, &is_data_float32,
                  num_matrices);
    ...
    for (int m = 0; m < num_matrices; ++m) {
      for (int64_t i = 0; i < static_cast<int64_t>(agg.size()); ++i) {
        ...
        if (i == 0) {
          ...
          row_matrix_offsets[row_vector_cnt] = static_cast<int64_t>(
              row_sizes[row_vector_cnt - 1] + row_matrix_offsets[row_vector_cnt - 1]);
        }
        row_vector_cnt++;
      }
      if (m == 0) {
      ...
      if (m + 1 < num_matrices) {
        matrix_offsets[m + 1] =
            static_cast<int64_t>(matrix_offsets[m] + row_matrix_offsets[row_vector_cnt - 1] +
                                 row_sizes[row_vector_cnt - 1]);
      }
    }
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (int64_t i = 0; i < static_cast<int64_t>(agg.size()); ++i) {
        ...
        if (is_indptr_int32) {
          (reinterpret_cast<int32_t*>(*out_indptr))[indptr_loop_index] =
              static_cast<int32_t>(indptr_value);
        } else {
  ...
  void PredictSparseCSC(
      int start_iteration, int num_iteration, int predict_type, int64_t nrow, int ncol,
      std::function<std::vector<std::pair<int, double>>(int64_t row_idx)> get_row_fun,
      const Config& config, int64_t* out_len, void** out_col_ptr, int col_ptr_type,
      int32_t** out_indices, void** out_data, int data_type) const {
    SHARED_LOCK(mutex_);
    // Get the number of trees per iteration (for multiclass scenario we output multiple sparse
    // matrices)
    ...
    for (int m = 0; m < num_matrices; ++m) {
      ...
      if (is_col_ptr_int32) {
        (reinterpret_cast<int32_t*>(*out_col_ptr))[col_ptr_index] =
            static_cast<int32_t>(col_ptr_value);
      } else {
      ...
      for (int64_t i = 1; i < static_cast<int64_t>(column_sizes[m].size()); ++i) {
        ...
        if (is_col_ptr_int32) {
          (reinterpret_cast<int32_t*>(*out_col_ptr))[col_ptr_index] =
              static_cast<int32_t>(column_start_indices[m][i]);
        } else {
      ...
      if (is_col_ptr_int32) {
        (reinterpret_cast<int32_t*>(*out_col_ptr))[col_ptr_index] =
            static_cast<int32_t>(last_column_start_index + last_column_size);
      } else {
        (reinterpret_cast<int64_t*>(*out_col_ptr))[col_ptr_index] =
            last_column_start_index + last_column_size;
      }
      if (m + 1 < num_matrices) {
        matrix_start_indices[m + 1] =
            matrix_start_indices[m] + last_column_start_index + last_column_size;
      }
      col_ptr_index++;
    }
    // Note: we parallelize across matrices instead of rows because of the
    // column_counts[m][col_idx] increment inside the loop
    OMP_INIT_EX();
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    for (int m = 0; m < num_matrices; ++m) {
      OMP_LOOP_EX_BEGIN();
      for (int64_t i = 0; i < static_cast<int64_t>(agg.size()); ++i) {
        ...
        for (auto it = row_vector[m].begin(); it != row_vector[m].end(); ++it) {
          ...
          int64_t element_index = column_start_indices[m][col_idx] + matrix_start_indices[m] +
  ...
  void Predict(int start_iteration, int num_iteration, int predict_type, const char* data_filename,
               int data_has_header, const Config& config, const char* result_filename) const {
    SHARED_LOCK(mutex_)
    bool is_predict_leaf = false;
    bool is_raw_score = false;
    bool predict_contrib = false;
    ...
    Predictor predictor(boosting_.get(), start_iteration, num_iteration, is_raw_score,
                        is_predict_leaf, predict_contrib, config.pred_early_stop,
                        config.pred_early_stop_freq, config.pred_early_stop_margin);
    bool bool_data_has_header = data_has_header > 0 ? true : false;
    predictor.Predict(data_filename, result_filename, bool_data_has_header,
                      config.predict_disable_shape_check, config.precise_float_parser);
  }
  ...
  std::string SaveModelToString(int start_iteration, int num_iteration,
    ...
    return boosting_->SaveModelToString(start_iteration, num_iteration, feature_importance_type);
  }

  std::string DumpModel(int start_iteration, int num_iteration,
                        int feature_importance_type) const {
    return boosting_->DumpModel(start_iteration, num_iteration, feature_importance_type);
  }
  ...
  int GetEvalNames(char** out_strs, const int len, const size_t buffer_len,
                   size_t* out_buffer_len) const {
    SHARED_LOCK(mutex_)
    *out_buffer_len = 0;
    int idx = 0;
    for (const auto& metric : train_metric_) {
  ...
  int GetFeatureNames(char** out_strs, const int len, const size_t buffer_len,
                      size_t* out_buffer_len) const {
    SHARED_LOCK(mutex_)
    *out_buffer_len = 0;
    int idx = 0;
...
std::function<std::vector<double>(int row_idx)> RowFunctionFromDenseMatric(
    const void* data, int num_row, int num_col, int data_type, int is_row_major);

std::function<std::vector<std::pair<int, double>>(int row_idx)> RowPairFunctionFromDenseMatric(
    const void* data, int num_row, int num_col, int data_type, int is_row_major);

std::function<std::vector<std::pair<int, double>>(int row_idx)> RowPairFunctionFromDenseRows(
    const void** data, int num_col, int data_type);

template <typename T>
std::function<std::vector<std::pair<int, double>>(T idx)> RowFunctionFromCSR(
    const void* indptr, int indptr_type, const int32_t* indices, const void* data, int data_type,
    int64_t nindptr, int64_t nelem);

// Row iterator of on column for CSC matrix
class CSC_RowIterator {
 public:
  CSC_RowIterator(const void* col_ptr, int col_ptr_type, const int32_t* indices, const void* data,
                  int data_type, int64_t ncol_ptr, int64_t nelem, int col_idx);
  ~CSC_RowIterator() {}
...
int LGBM_DumpParamAliases(int64_t buffer_len, int64_t* out_len, char* out_str) {
  API_BEGIN();
...
static inline int SampleCount(int32_t total_nrow, const Config& config) {
  return static_cast<int>(
      total_nrow < config.bin_construct_sample_cnt ? total_nrow : config.bin_construct_sample_cnt);
}
...
int LGBM_GetSampleCount(int32_t num_total_row, const char* parameters, int* out) {
  API_BEGIN();
  if (out == nullptr) {
...
int LGBM_SampleIndices(int32_t num_total_row, const char* parameters, void* out,
                       int32_t* out_len) {
...
int LGBM_DatasetCreateFromSampledColumn(double** sample_data, int** sample_indices, int32_t ncol,
  ...
  *out = loader.ConstructFromSampleData(sample_data, sample_indices, ncol, num_per_col,
                                        num_sample_row, static_cast<data_size_t>(num_local_row),
                                        num_dist_row);
  API_END();
}

int LGBM_DatasetCreateByReference(const DatasetHandle reference, int64_t num_total_row,
                                  DatasetHandle* out) {
  API_BEGIN();
...
int LGBM_DatasetCreateFromSerializedReference(const void* ref_buffer, int32_t ref_buffer_size,
                                              int64_t num_row, int32_t num_classes,
                                              const char* parameters, DatasetHandle* out) {
  API_BEGIN();
  auto param = Config::Str2Map(parameters);
  Config config;
  ...
  *out = loader.LoadFromSerializedReference(static_cast<const char*>(ref_buffer),
                                            ...
                                            static_cast<data_size_t>(num_row), num_classes);
  API_END();
}

int LGBM_DatasetInitStreaming(DatasetHandle dataset, int32_t has_weights, int32_t has_init_scores,
                              int32_t has_queries, int32_t nclasses, int32_t nthreads,
...
int LGBM_DatasetPushRows(DatasetHandle dataset, const void* data, int data_type, int32_t nrow,
...
int LGBM_DatasetPushRowsWithMetadata(DatasetHandle dataset, const void* data, int data_type,
                                     int32_t nrow, int32_t ncol, int32_t start_row,
                                     const float* labels, const float* weights,
                                     const double* init_scores, const int32_t* queries,
                                     int32_t tid) {
  API_BEGIN();
  ...
  const int max_omp_threads =
      p_dataset->omp_max_threads() > 0 ? p_dataset->omp_max_threads() : OMP_NUM_THREADS();
...
int LGBM_DatasetPushRowsByCSR(DatasetHandle dataset, const void* indptr, int indptr_type,
                              const int32_t* indices, const void* data, int data_type,
                              int64_t nindptr, int64_t nelem, int64_t, int64_t start_row) {
  API_BEGIN();
  auto p_dataset = reinterpret_cast<Dataset*>(dataset);
  auto get_row_fun =
      RowFunctionFromCSR<int>(indptr, indptr_type, indices, data, data_type, nindptr, nelem);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  OMP_THROW_EX();
  if (!p_dataset->wait_for_manual_finish() &&
      (start_row + nrow == static_cast<int64_t>(p_dataset->num_data()))) {
    p_dataset->FinishLoad();
  }
  API_END();
}

int LGBM_DatasetPushRowsByCSRWithMetadata(DatasetHandle dataset, const void* indptr,
                                          int indptr_type, const int32_t* indices,
                                          const void* data, int data_type, int64_t nindptr,
                                          int64_t nelem, int64_t start_row, const float* labels,
                                          const float* weights, const double* init_scores,
                                          const int32_t* queries, int32_t tid) {
  API_BEGIN();
#ifdef LABEL_T_USE_DOUBLE
  Log::Fatal("Don't support LABEL_T_USE_DOUBLE");
#endif
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  p_dataset->InsertMetadataAt(static_cast<int32_t>(start_row), nrow, labels, weights, init_scores,
                              queries);
...
int LGBM_DatasetCreateFromMat(const void* data, int data_type, int32_t nrow, int32_t ncol,
                              ...
                              const DatasetHandle reference, DatasetHandle* out) {
  return LGBM_DatasetCreateFromMats(1, &data, data_type, &nrow, ncol, &is_row_major, parameters,
                                    reference, out);
}

int LGBM_DatasetCreateFromMats(int32_t nmat, const void** data, int data_type, int32_t* nrow,
  ...
  for (int j = 0; j < nmat; ++j) {
    get_row_fun.push_back(
        RowFunctionFromDenseMatric(data[j], nrow[j], ncol, data_type, is_row_major[j]));
  }
  ...
  if (reference == nullptr) {
    ...
    DatasetLoader loader(config, nullptr, 1, nullptr);
    ret.reset(loader.ConstructFromSampleData(
        Vector2Ptr<double>(&sample_values).data(), Vector2Ptr<int>(&sample_idx).data(), ncol,
        VectorSize<double>(sample_values).data(), sample_cnt, total_nrow, total_nrow));
  } else {
...
int LGBM_DatasetCreateFromCSR(const void* indptr, int indptr_type, const int32_t* indices,
  ...
  if (reference == nullptr) {
    ...
    DatasetLoader loader(config, nullptr, 1, nullptr);
    ret.reset(loader.ConstructFromSampleData(
        Vector2Ptr<double>(&sample_values).data(), Vector2Ptr<int>(&sample_idx).data(),
        static_cast<int>(num_col), VectorSize<double>(sample_values).data(), sample_cnt, nrow,
        nrow));
  } else {
...
int LGBM_DatasetCreateFromCSRFunc(void* get_row_funptr, int num_rows, int64_t num_col,
  ...
  auto get_row_fun =
      *static_cast<std::function<void(int idx, std::vector<std::pair<int, double>>&)>*>(
          get_row_funptr);
  auto param = Config::Str2Map(parameters);
  Config config;
  config.Set(param);
...
int LGBM_DatasetCreateFromCSC(const void* col_ptr, int col_ptr_type, const int32_t* indices,
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    ...
    ret.reset(loader.ConstructFromSampleData(
        Vector2Ptr<double>(&sample_values).data(), Vector2Ptr<int>(&sample_idx).data(),
        static_cast<int>(sample_values.size()), VectorSize<double>(sample_values).data(),
        sample_cnt, nrow, nrow));
  } else {
    ret.reset(new Dataset(nrow));
    ret->CreateValid(reinterpret_cast<const Dataset*>(reference));
  }
...
int LGBM_DatasetCreateFromArrow(int64_t n_chunks, const ArrowArray* chunks,
                                const ArrowSchema* schema, const char* parameters,
                                const DatasetHandle reference, DatasetHandle* out) {
  API_BEGIN();
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
    ...
    ret.reset(loader.ConstructFromSampleData(
        Vector2Ptr<double>(&sample_values).data(), Vector2Ptr<int>(&sample_idx).data(),
        table.get_num_columns(), VectorSize<double>(sample_values).data(), sample_count,
        table.get_num_rows(), table.get_num_rows()));
  } else {
...
int LGBM_DatasetGetSubset(const DatasetHandle handle, const int32_t* used_row_indices,
  ...
  CheckElementsIntervalClosed(used_row_indices, lower, upper, num_used_row_indices,
...
int LGBM_DatasetSetFeatureNames(DatasetHandle handle, const char** feature_names,
...
int LGBM_DatasetGetFeatureNames(DatasetHandle handle, const int len, int* num_feature_names,
  ...
  for (int i = 0; i < *num_feature_names; ++i) {
    if (i < len) {
      std::memcpy(feature_names[i], inside_feature_name[i].c_str(),
                  std::min(inside_feature_name[i].size() + 1, buffer_len));
...
int LGBM_DatasetSerializeReferenceToBinary(DatasetHandle handle, ByteBufferHandle* out,
                                           int32_t* out_len) {
  API_BEGIN();
  auto dataset = reinterpret_cast<Dataset*>(handle);
...
int LGBM_DatasetSetField(DatasetHandle handle, const char* field_name, const void* field_data,
  ...
  if (type == C_API_DTYPE_FLOAT32) {
    is_success = dataset->SetFloatField(field_name, reinterpret_cast<const float*>(field_data),
                                        static_cast<int32_t>(num_element));
  } else if (type == C_API_DTYPE_INT32) {
    is_success = dataset->SetIntField(field_name, reinterpret_cast<const int*>(field_data),
                                      static_cast<int32_t>(num_element));
  } else if (type == C_API_DTYPE_FLOAT64) {
    is_success = dataset->SetDoubleField(field_name, reinterpret_cast<const double*>(field_data),
                                         static_cast<int32_t>(num_element));
  }
  if (!is_success) {
...
int LGBM_DatasetSetFieldFromArrow(DatasetHandle handle, const char* field_name, int64_t n_chunks,
                                  const ArrowArray* chunks, const ArrowSchema* schema) {
  API_BEGIN();
  auto dataset = reinterpret_cast<Dataset*>(handle);
...
int LGBM_DatasetGetField(DatasetHandle handle, const char* field_name, int* out_len,
  ...
  if (dataset->GetFloatField(field_name, out_len, reinterpret_cast<const float**>(out_ptr))) {
    ...
    *out_type = C_API_DTYPE_INT32;
    is_success = true;
  } else if (dataset->GetDoubleField(field_name, out_len,
...
int LGBM_DatasetGetFeatureNumBin(DatasetHandle handle, int feature, int* out) {
  API_BEGIN();
  auto dataset = reinterpret_cast<Dataset*>(handle);
  int num_features = dataset->num_total_features();
  if (feature < 0 || feature >= num_features) {
    Log::Fatal(
        "Tried to retrieve number of bins for feature index %d, "
        "but the valid feature indices are [0, %d].",
...
int LGBM_BoosterCreateFromModelfile(const char* filename, int* out_num_iterations,
                                    BoosterHandle* out) {
  API_BEGIN();
...
int LGBM_BoosterLoadModelFromString(const char* model_str, int* out_num_iterations,
                                    BoosterHandle* out) {
  API_BEGIN();
...
int LGBM_BoosterResetTrainingData(BoosterHandle handle, const DatasetHandle train_data) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
...
int LGBM_BoosterRefit(BoosterHandle handle, const int32_t* leaf_preds, int32_t nrow,
                      int32_t ncol) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
...
int LGBM_BoosterUpdateOneIterCustom(BoosterHandle handle, const float* grad, const float* hess,
                                    int* is_finished) {
  API_BEGIN();
...
int LGBM_BoosterGetNumPredict(BoosterHandle handle, int data_idx, int64_t* out_len) {
  API_BEGIN();
...
int LGBM_BoosterPredictForFile(BoosterHandle handle, const char* data_filename,
                               int data_has_header, int predict_type, int start_iteration,
  ...
  OMP_SET_NUM_THREADS(config.num_threads);
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  ref_booster->Predict(start_iteration, num_iteration, predict_type, data_filename,
                       data_has_header, config, result_filename);
  API_END();
}

int LGBM_BoosterCalcNumPredict(BoosterHandle handle, int num_row, int predict_type,
                               int start_iteration, int num_iteration, int64_t* out_len) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  *out_len = static_cast<int64_t>(num_row) *
             ref_booster->GetBoosting()->NumPredictOneRow(start_iteration, num_iteration,
                                                          predict_type == C_API_PREDICT_LEAF_INDEX,
                                                          predict_type == C_API_PREDICT_CONTRIB);
  API_END();
}

// Naming: In future versions of LightGBM, public API named around `FastConfig` should be made
// named around `SingleRowPredictor`, because it is specific to single row prediction, and doesn't
// actually hold only config. For now this is kept as `FastConfig` for backwards compatibility. At
// the same time, one should consider removing the old non-fast single row public API that stores
// its Predictor in the Booster, because that will enable removing these Predictors from the
// Booster, and associated initialization code.
...
int LGBM_BoosterPredictForCSR(BoosterHandle handle, const void* indptr, int indptr_type,
  ...
  ref_booster->Predict(start_iteration, num_iteration, predict_type, nrow,
                       static_cast<int>(num_col), get_row_fun, config, out_result, out_len);
  API_END();
}

int LGBM_BoosterPredictSparseOutput(BoosterHandle handle, const void* indptr, int indptr_type,
                                    const int32_t* indices, const void* data, int data_type,
  ...
  if (matrix_type == C_API_MATRIX_TYPE_CSR) {
    ...
    auto get_row_fun =
        RowFunctionFromCSR<int64_t>(indptr, indptr_type, indices, data, data_type, nindptr, nelem);
    int64_t nrow = nindptr - 1;
    ref_booster->PredictSparseCSR(start_iteration, num_iteration, predict_type, nrow,
                                  static_cast<int>(num_col_or_row), get_row_fun, config, out_len,
                                  out_indptr, indptr_type, out_indices, out_data, data_type);
  } else if (matrix_type == C_API_MATRIX_TYPE_CSC) {
    int num_threads = OMP_NUM_THREADS();
    int ncol = static_cast<int>(nindptr - 1);
    std::vector<std::vector<CSC_RowIterator>> iterators(num_threads,
                                                        std::vector<CSC_RowIterator>());
    for (int i = 0; i < num_threads; ++i) {
      for (int j = 0; j < ncol; ++j) {
        iterators[i].emplace_back(indptr, indptr_type, indices, data, data_type, nindptr, nelem,
                                  j);
      }
    }
    std::function<std::vector<std::pair<int, double>>(int64_t row_idx)> get_row_fun =
    ...
    ref_booster->PredictSparseCSC(start_iteration, num_iteration, predict_type, num_col_or_row,
                                  ncol, get_row_fun, config, out_len, out_indptr, indptr_type,
                                  out_indices, out_data, data_type);
  } else {
...
int LGBM_BoosterFreePredictSparse(void* indptr, int32_t* indices, void* data, int indptr_type,
                                  int data_type) {
  API_BEGIN();
  if (indptr_type == C_API_DTYPE_INT32) {
...
int LGBM_BoosterPredictForCSRSingleRow(BoosterHandle handle, const void* indptr, int indptr_type,
  ...
  ref_booster->PredictSingleRow(predict_type, static_cast<int32_t>(num_col), get_row_fun, config,
                                out_result, out_len);
  API_END();
}

int LGBM_BoosterPredictForCSRSingleRowFastInit(BoosterHandle handle, const int predict_type,
                                               const int start_iteration, const int num_iteration,
                                               const int data_type, const int64_t num_col,
  ...
  std::unique_ptr<SingleRowPredictor> single_row_predictor =
      ref_booster->InitSingleRowPredictor(start_iteration, num_iteration, predict_type, data_type,
                                          static_cast<int32_t>(num_col), parameter);
...
int LGBM_BoosterPredictForCSRSingleRowFast(FastConfigHandle fastConfig_handle, const void* indptr,
                                           const int indptr_type, const int32_t* indices,
                                           const void* data, const int64_t nindptr,
                                           const int64_t nelem, int64_t* out_len,
                                           double* out_result) {
  API_BEGIN();
  SingleRowPredictor* single_row_predictor =
      reinterpret_cast<SingleRowPredictor*>(fastConfig_handle);
  auto get_row_fun = RowFunctionFromCSR<int>(indptr, indptr_type, indices, data,
                                             single_row_predictor->data_type, nindptr, nelem);
  single_row_predictor->Predict(get_row_fun, out_result, out_len);
  API_END();
}

int LGBM_BoosterPredictForCSC(BoosterHandle handle, const void* col_ptr, int col_ptr_type,
                              const int32_t* indices, const void* data, int data_type,
  ...
  for (int i = 0; i < num_threads; ++i) {
    for (int j = 0; j < ncol; ++j) {
      iterators[i].emplace_back(col_ptr, col_ptr_type, indices, data, data_type, ncol_ptr, nelem,
                                j);
    }
  }
  std::function<std::vector<std::pair<int, double>>(int row_idx)> get_row_fun = [&iterators,
                                                                                 ncol](int i) {
    std::vector<std::pair<int, double>> one_row;
    one_row.reserve(ncol);
    const int tid = omp_get_thread_num();
  ...
  ref_booster->Predict(start_iteration, num_iteration, predict_type, static_cast<int>(num_row),
                       ncol, get_row_fun, config, out_result, out_len);
  API_END();
}

int LGBM_BoosterValidateFeatureNames(BoosterHandle handle, const char** data_names,
  ...
  if (booster_num_features != data_num_features) {
    Log::Fatal("Model was trained on %d features, but got %d input features to predict.",
               booster_num_features, data_num_features);
  }
  std::vector<std::vector<char>> tmp_names(booster_num_features,
                                           std::vector<char>(out_buffer_len));
  std::vector<char*> booster_names = Vector2Ptr(&tmp_names);
  LGBM_BoosterGetFeatureNames(handle, data_num_features, &booster_num_features, out_buffer_len,
                              &out_buffer_len, booster_names.data());
  for (int i = 0; i < booster_num_features; ++i) {
    if (strcmp(data_names[i], booster_names[i]) != 0) {
      Log::Fatal("Expected '%s' at position %d but found '%s'", booster_names[i], i,
                 data_names[i]);
    }
  }
  API_END();
...
int LGBM_BoosterPredictForMat(BoosterHandle handle, const void* data, int data_type, int32_t nrow,
...
int LGBM_BoosterPredictForMatSingleRow(BoosterHandle handle, const void* data, int data_type,
                                       int32_t ncol, int is_row_major, int predict_type,
...
int LGBM_BoosterPredictForMatSingleRowFastInit(BoosterHandle handle, const int predict_type,
                                               const int start_iteration, const int num_iteration,
                                               const int data_type, const int32_t ncol,
                                               const char* parameter,
                                               FastConfigHandle* out_fastConfig) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);

  std::unique_ptr<SingleRowPredictor> single_row_predictor = ref_booster->InitSingleRowPredictor(
      predict_type, start_iteration, num_iteration, data_type, ncol, parameter);
...
int LGBM_BoosterPredictForMatSingleRowFast(FastConfigHandle fastConfig_handle, const void* data,
                                           int64_t* out_len, double* out_result) {
  API_BEGIN();
  SingleRowPredictor* single_row_predictor =
      reinterpret_cast<SingleRowPredictor*>(fastConfig_handle);
  // Single row in row-major format:
  auto get_row_fun = RowPairFunctionFromDenseMatric(data, 1, single_row_predictor->num_cols,
                                                    single_row_predictor->data_type, 1);
  single_row_predictor->Predict(get_row_fun, out_result, out_len);
  API_END();
}

int LGBM_BoosterPredictForMats(BoosterHandle handle, const void** data, int data_type,
                               int32_t nrow, int32_t ncol, int predict_type, int start_iteration,
...
int LGBM_BoosterPredictForArrow(BoosterHandle handle, int64_t n_chunks, const ArrowArray* chunks,
                                const ArrowSchema* schema, int predict_type, int start_iteration,
                                int num_iteration, const char* parameter, int64_t* out_len,
                                double* out_result) {
  API_BEGIN();
  ...
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  ref_booster->Predict(
      start_iteration, num_iteration, predict_type, static_cast<int>(table.get_num_rows()),
      static_cast<int>(table.get_num_columns()), row_fn, config, out_result, out_len);
  API_END();
}

int LGBM_BoosterSaveModel(BoosterHandle handle, int start_iteration, int num_iteration,
                          int feature_importance_type, const char* filename) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  ref_booster->SaveModelToFile(start_iteration, num_iteration, feature_importance_type, filename);
  API_END();
}

int LGBM_BoosterSaveModelToString(BoosterHandle handle, int start_iteration, int num_iteration,
                                  int feature_importance_type, int64_t buffer_len,
                                  int64_t* out_len, char* out_str) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  std::string model =
      ref_booster->SaveModelToString(start_iteration, num_iteration, feature_importance_type);
...
int LGBM_BoosterDumpModel(BoosterHandle handle, int start_iteration, int num_iteration,
                          int feature_importance_type, int64_t buffer_len, int64_t* out_len,
                          char* out_str) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  std::string model =
      ref_booster->DumpModel(start_iteration, num_iteration, feature_importance_type);
  *out_len = static_cast<int64_t>(model.size()) + 1;
  if (*out_len <= buffer_len) {
    std::memcpy(out_str, model.c_str(), *out_len);
  }
...
int LGBM_BoosterGetLeafValue(BoosterHandle handle, int tree_idx, int leaf_idx, double* out_val) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
...
int LGBM_BoosterSetLeafValue(BoosterHandle handle, int tree_idx, int leaf_idx, double val) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
...
int LGBM_NetworkInit(const char* machines, int local_listen_port, int listen_time_out,
                     int num_machines) {
  API_BEGIN();
  Config config;
...
int LGBM_NetworkInitWithFunctions(int num_machines, int rank, void* reduce_scatter_ext_fun,
                                  void* allgather_ext_fun) {
  API_BEGIN();
  if (num_machines > 1) {
    Network::Init(num_machines, rank, (ReduceScatterFunction)reduce_scatter_ext_fun,
...
template <typename T>
std::function<std::vector<double>(int row_idx)> RowFunctionFromDenseMatric_helper(
    const void* data, int num_row, int num_col, int is_row_major) {
...
std::function<std::vector<std::pair<int, double>>(int row_idx)> RowPairFunctionFromDenseMatric(
    const void* data, int num_row, int num_col, int data_type, int is_row_major) {
  auto inner_function =
      RowFunctionFromDenseMatric(data, num_row, num_col, data_type, is_row_major);
...
std::function<std::vector<std::pair<int, double>>(int row_idx)> RowPairFunctionFromDenseRows(
  ...
  return [=](int row_idx) {
    auto inner_function =
        RowFunctionFromDenseMatric(data[row_idx], 1, num_col, data_type, /* is_row_major */ true);
...
std::function<std::vector<std::pair<int, double>>(T idx)> RowFunctionFromCSR_helper(
    const void* indptr, const int32_t* indices, const void* data) {
...
std::function<std::pair<int, double>(int idx)> IterateFunctionFromCSC_helper(
    const void* col_ptr, const int32_t* indices, const void* data, int col_idx) {
  const T1* data_ptr = reinterpret_cast<const T1*>(data);
...
std::function<std::pair<int, double>(int idx)> IterateFunctionFromCSC(
    const void* col_ptr, int col_ptr_type, const int32_t* indices, const void* data, int data_type,
    int64_t ncol_ptr, int64_t, int col_idx) {
...
CSC_RowIterator::CSC_RowIterator(const void* col_ptr, int col_ptr_type, const int32_t* indices,
                                 const void* data, int data_type, int64_t ncol_ptr, int64_t nelem,
                                 int col_idx) {
  iter_fun_ = IterateFunctionFromCSC(col_ptr, col_ptr_type, indices, data, data_type, ncol_ptr,
                                     nelem, col_idx);
}

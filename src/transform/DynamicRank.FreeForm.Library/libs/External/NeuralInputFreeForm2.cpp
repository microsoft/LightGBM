...
namespace
{
    ...
    bool ReadAssociatedFeature(DynamicRank::Config& p_config,
                               ...
                               UInt32 *p_feature)
    {
        *p_feature = static_cast<UInt32>(-1);
        ...
        if (p_config.GetStringParameter(p_section, "Name", szName, sizeof(szName)))
        {
            ...
            {
                Log::Warning("DR:ReadAssociatedFeature Could not find index for feature name: %s in section: %s", szName, p_section);
                return false; // input error, invalid feature name
            }
            return true;
         }

        Log::Warning("DR:ReadAssociatedFeature Could not find Name of the feature for section: %s", p_section);
...
FreeForm2::NeuralInputFreeForm2::NeuralInputFreeForm2(const std::string& p_input,
    ...
    : m_input(p_input),
      ...
      m_map(&p_map),
      m_fun(NULL),
      m_program(Program::Parse<Program::sexpression>(CStackSizedString(p_input.c_str(), p_input.size()),
                                                     p_map, 
                                                     true,
                                                     nullptr,
                                                     NULL))
...
void
FreeForm2::NeuralInputFreeForm2::Init()
{
...
double 
FreeForm2::NeuralInputFreeForm2::Evaluate(UInt32 p_input[]) const 
{
  try
    {
...
bool 
FreeForm2::NeuralInputFreeForm2::Train(double p_learningRate, 
                                       double p_outputHigh, 
                                       double p_outputLow, 
...
void 
FreeForm2::NeuralInputFreeForm2::GetAllAssociatedFeatures(
    std::vector<UInt32>& p_associatedFeaturesList) const
{
    std::copy(m_features.begin(), m_features.end(), std::back_inserter(p_associatedFeaturesList));
}


void
FreeForm2::NeuralInputFreeForm2::Compile(Compiler* p_compiler)
{
    // Only compile once.
    if (m_program == NULL)
    {
    ...
    try
    {
        if (p_compiler != NULL)
        {
            std::unique_ptr<CompilerResults> results = p_compiler->Compile(*m_program, false);
            const ExecutableCompilerResults* exec 
                = boost::polymorphic_downcast<ExecutableCompilerResults*>(results.get());
            m_exec = exec->GetExecutable();
        }
        else
        {
            std::unique_ptr<Compiler> compiler(CompilerFactory::CreateExecutableCompiler(
                Compiler::c_defaultOptimizationLevel,
                CompilerFactory::SingleDocumentEvaluation));
            std::unique_ptr<CompilerResults> results = compiler->Compile(*m_program, false);
            const ExecutableCompilerResults* exec 
                = boost::polymorphic_downcast<ExecutableCompilerResults*>(results.get());
            m_exec = exec->GetExecutable();
        }
    ...
    {
        Log::Fatal("NeuralInputFreeForm2::Compile,Failed to compile %s: %s", m_transform, p_except.what());
        Log::Fatal("NeuralInputFreeForm2::Compile,Failed to compile %s: %s", m_transform, GetStringRepresentation().c_str());
        throw;
    }
}


const FreeForm2::Program&
FreeForm2::NeuralInputFreeForm2::GetProgram() const
{
...
double 
FreeForm2::NeuralInputFreeForm2::GetMin() const
{
    return -DBL_MAX;
}
...
double 
FreeForm2::NeuralInputFreeForm2::GetMax() const
{
    return DBL_MAX;
}
...
size_t
FreeForm2::NeuralInputFreeForm2::GetSize() const
{
...
size_t
FreeForm2::NeuralInputFreeForm2::GetExternalSize() const
{
...
bool 
FreeForm2::NeuralInputFreeForm2::Save(FILE *p_out, size_t p_input, const DynamicRank::IFeatureMap& p_map) const
{
    // Write header.
    bool success = DynamicRank::NeuralInput::Save(p_out, p_input, p_map);
    success = success 
        && fprintf(p_out, "Transform=%s\n", (m_transform != NULL ? m_transform : "<NULL>"));
    typedef boost::split_iterator<std::string::const_iterator> SplitIter;

    unsigned int numLine = 1;
    for (SplitIter iter = boost::make_split_iterator(m_input, 
...
std::string
FreeForm2::NeuralInputFreeForm2::LoadProgram(DynamicRank::Config& p_config,
                                             const char* p_section,
                                             ...
                                             const char* p_transform)
{
    ...
    if (numLine == 2 && !found)
    {
        Log::Warning("NeuralInputFreeForm2::Load NeuralInputFreeForm2::Load %s", p_section);
        return "";
    }
    ...
    for (unsigned int i = 0; i < 10; i++)
    {
        ...
        if (found)
        {
            Log::Warning("NeuralInputFreeForm2::Found ignored parameter %s in section %s: did you skip line number %s?", lineStr.c_str(), p_section, to_string(numLine + i).c_str());
            return "";
        }
    }
...
FreeForm2::NeuralInputFreeForm2*
FreeForm2::NeuralInputFreeForm2::Load(DynamicRank::Config& p_config,
                                      const char* p_section,
                                      DynamicRank::IFeatureMap* p_featureMap,
                                      const char* p_transform)
{
    std::string programStr = LoadProgram(p_config, p_section, p_featureMap, p_transform);
    if (programStr.empty())
    {
        return NULL;
    }
    ...
    {
        Log::Warning("NeuralInputFreeForm2::Load Failed to load freeform2: %s (program is '%s')", p_except.what(), programStr.c_str());
        return NULL;
    }
}
...
std::string
FreeForm2::NeuralInputFreeForm2::GetStringRepresentation() const
{
    return m_input;
}
...
bool 
FreeForm2::NeuralInputFreeForm2::IsFreeForm2() const
{
...
bool
FreeForm2::NeuralInputFreeForm2::Equal(const DynamicRank::NeuralInput* p_other) const
{
...
void 
FreeForm2::NeuralInputFreeForm2::FillBond(DynamicRank::UnionBondInput& p_data) const
{
    // Selector. 
...
FreeForm2::NeuralInputFreeForm2::NeuralInputFreeForm2(const DynamicRank::UnionBondInput& p_data)
{
    if("freeform2" != p_data.m_inputType)
    {
        Log::Fatal("Input type '%s' is not supported. Accepted type is freeform2.", p_data.m_inputType.c_str());
    }
    const DynamicRank::NeuralInputFreeForm2BondData& data = p_data.m_freeform2.value();
...
void
FreeForm2::NeuralInputCompiler::Compile(const std::vector<NeuralInputFreeForm2*>& p_inputs,

...
class TextReader {
 public:
  ...
  TextReader(const char* filename, bool is_skip_first_line,
             size_t progress_interval_bytes = SIZE_MAX)
      : filename_(filename),
        is_skip_first_line_(is_skip_first_line),
    ...
    if (is_skip_first_line_) {
      ...
      Log::Debug("Skipped header \"%s\" in file %s", first_line_.c_str(),
                 filename_);
    }
  }
  /*!
   ...
   */
  inline std::string first_line() { return first_line_; }
  /*!
  ...
  INDEX_T ReadAllAndProcess(
      const std::function<void(INDEX_T, const char*, size_t)>& process_fun) {
    last_line_ = "";
    INDEX_T total_cnt = 0;
    size_t bytes_read = 0;
    PipelineReader::Read(
        ...
        [&](const char* buffer_process, size_t read_cnt) {
          ...
          bytes_read += read_cnt;
          if (prev_bytes_read / read_progress_interval_bytes_ <
              bytes_read / read_progress_interval_bytes_) {
            Log::Debug("Read %.1f GBs from %s.", 1.0 * bytes_read / kGbs,
                       filename_);
          }
    ...
    if (last_line_.size() > 0) {
      Log::Info(
          "Warning: last line of %s has no end of line, still using this line",
          filename_);
      process_fun(total_cnt, last_line_.c_str(), last_line_.size());
      ++total_cnt;
      last_line_ = "";
  ...
  INDEX_T ReadAllLines() {
    return ReadAllAndProcess([=](INDEX_T, const char* buffer, size_t size) {
  ...
  std::vector<char> ReadContent(size_t* out_len) {
    ...
    *out_len = 0;
    ...
    do {
      ...
      ret.insert(ret.end(), buffer_read.begin(),
                 buffer_read.begin() + read_cnt);
      *out_len += read_cnt;
    } while (read_cnt > 0);
    return ret;
  }

  INDEX_T SampleFromFile(Random* random, INDEX_T sample_cnt,
        ...
        [&](INDEX_T line_idx, const char* buffer, size_t size) {
          if (cur_sample_cnt < sample_cnt) {
            ...
            const size_t idx = static_cast<size_t>(
                random->NextInt(0, static_cast<int>(line_idx + 1)));
  ...
  /*!
  * \brief Read part of text data from file in memory,
  use filter_fun to filter data
  ...
  */
  ...
  INDEX_T SampleAndFilterFromFile(
      const std::function<bool(INDEX_T)>& filter_fun,
      std::vector<INDEX_T>* out_used_data_indices, Random* random,
      INDEX_T sample_cnt, std::vector<std::string>* out_sampled_data) {
    INDEX_T cur_sample_cnt = 0;
    out_used_data_indices->clear();
    INDEX_T total_cnt = ReadAllAndProcess(
        [&](INDEX_T line_idx, const char* buffer, size_t size) {
          ...
          if (is_used) {
            if (cur_sample_cnt < sample_cnt) {
              ...
              const size_t idx = static_cast<size_t>(random->NextInt(
                  0, static_cast<int>(out_used_data_indices->size())));
              if (idx < static_cast<size_t>(sample_cnt)) {
                out_sampled_data->operator[](idx) = std::string(buffer, size);
              }
            }
  ...
  INDEX_T ReadAllAndProcessParallelWithFilter(
      const std::function<void(INDEX_T, const std::vector<std::string>&)>&
          process_fun,
  ...
  INDEX_T ReadAllAndProcessParallel(
      const std::function<void(INDEX_T, const std::vector<std::string>&)>&
          process_fun) {
    return ReadAllAndProcessParallelWithFilter(
        process_fun, [](INDEX_T, INDEX_T) { return true; });
  }

  INDEX_T ReadPartAndProcessParallel(
      const std::vector<INDEX_T>& used_data_indices,
      const std::function<void(INDEX_T, const std::vector<std::string>&)>&
          process_fun) {
    return ReadAllAndProcessParallelWithFilter(
        process_fun, [&used_data_indices](INDEX_T used_cnt, INDEX_T total_cnt) {
          if (static_cast<size_t>(used_cnt) < used_data_indices.size() &&
              total_cnt == used_data_indices[used_cnt]) {
            return true;
          } else {
            return false;
          }

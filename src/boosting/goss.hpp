 ...
 public:
  ...
  void Bagging(int iter, TreeLearner* tree_learner, score_t* gradients,
    ...
    if (!is_use_subset_) {
      ...
      if (config_->device_type == std::string("cuda")) {
        CopyFromHostToCUDADevice<data_size_t>(cuda_bag_data_indices_.RawData(),
                                              bag_data_indices_.data(),
  ...
  void ResetSampleConfig(const Config* config, bool /*is_change_dataset*/) override {
    ...
    if (config_->top_rate + config_->other_rate <= 0.5) {
      auto bag_data_cnt =
          static_cast<data_size_t>((config_->top_rate + config_->other_rate) * num_data_);
  ...
  bool IsHessianChange() const override { return true; }

 private:
  data_size_t Helper(data_size_t start, data_size_t cnt, data_size_t* buffer, score_t* gradients,
                     score_t* hessians) {
    if (cnt <= 0) {
      return 0;
    }
    ...
    ArrayArgs<score_t>::ArgMaxAtK(&tmp_gradients, 0, static_cast<int>(tmp_gradients.size()),
                                  top_k - 1);

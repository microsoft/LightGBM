...
// Default value is true, which restricts llvm code gen to be single-threaded
// globally, due to LLVM's restrictions that may or may not exists in current
// version.
// TODO: we're setting this flag to false experimentally in restricted scenario
// (e.g. URPService TC2) to allow multi-threaded llvm code gen, so that we can
// alleviate some long-standing pain caused by extremely time-consuming
// compilation process of very large freeform files. After double confirming
// that the version of LLVM integrated in current code base is fully
...
//   by m_memoryManager and m_relocations contain relocation table which should
//   be serialized with the code.
// * As a result of deserialization.  In this case binary code is allocaed by
// m_codeHeap
//   and relocation table precedes it in the memory.  Re-serilaization just
//   writes an extent of memory specified by m_funcInfo.
class LlvmExecutableImpl : public ExecutableImpl {
 public:
  ...
  LlvmExecutableImpl(
      LlvmCompilerImpl &p_compiler, const ProgramImpl &p_program,
      bool p_dumpExecutable,
      CompilerFactory::DestinationFunctionType p_destinationFunctionType);

  // Take and wrap deserialized executable code.
  // p_binary and p_binarySize defines a serialized representation of function
  // preceded by relocation table. LlvmExecutableImpl(unsigned char* p_binary,
  //                    size_t p_binarySize,
  //                    const TypeImpl& p_type,
  //                    boost::shared_ptr<CodeHeap> p_codeHeap,
  //                    CompilerFactory::DestinationFunctionType
  //                    p_destinationFunctionType);
  ...
  virtual Executable::DirectEvalFun EvaluationFunction() const override;
  ...
  // static LlvmExecutableImpl* DeserializeBinary(unsigned char* p_binary, const
  // boost::shared_ptr<CodeHeap>& p_codeHeap, size_t p_codeSize);

 private:
  // Function to support aggregated freefom.
  template <typename ReturnType>
  ReturnType EvaluateInternal(
      ReturnType (*p_fun)(const Executable::FeatureType *const *p_features,
                          UInt32 p_currentDocument, UInt32 p_documentCount,
                          Int64 *p_cache),
      const Executable::FeatureType *const *p_features,
      UInt32 p_currentDocument, UInt32 p_documentCount, Int64 *p_cache,

...
def train(params, train_set, num_boost_round=100,
          ...
          keep_training_booster=False, callbacks=None, min_acceptable_gain=0):
    ...
    min_acceptable_gain: float >= 0 or None, optional (default=None)
        If specified, early stopping will trigger when the loss hasn't improved
        by more than min_acceptable_gain in the specified number of early_stopping_rounds.
        Useful when running grid search.
    ...
    if early_stopping_rounds is not None:
        if min_acceptable_gain is None or min_acceptable_gain < 0:
            callbacks.add(callback.early_stopping(early_stopping_rounds, min_acceptable_gain=0, verbose=bool(verbose_eval)))
        else:
            callbacks.add(callback.early_stopping(early_stopping_rounds, min_acceptable_gain=min_acceptable_gain, verbose=bool(verbose_eval)))

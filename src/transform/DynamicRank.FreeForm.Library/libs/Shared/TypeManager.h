...
namespace FreeForm2
{
    ...
    public:
        ...
        // Gets the static instance of the TypeManager. This function is not thread-safe.
        ...
        static std::auto_ptr<TypeManager> CreateTypeManager(const TypeManager& p_parent);

        // Create a type manager with the given parent.
        static std::auto_ptr<TypeManager> CreateTypeManager(const ExternalDataManager& p_parent);
        ...
        virtual
        const ArrayType&
        GetArrayType(const ArrayType& p_type) = 0;
        ...
        virtual
        const StructType&
        GetStructType(const StructType& p_type) = 0;
        ...
        virtual
        const ObjectType&
        GetObjectType(const ObjectType& p_type) = 0;
        ...
        virtual
        const StateMachineType&
        GetStateMachineType(const StateMachineType& p_type) = 0;
        ...
        virtual
        const FunctionType&
        GetFunctionType(const FunctionType& p_type) = 0;
    ...
    protected:
        ...
        CreateArrayType(const TypeImpl& p_child, 
                        bool p_isConst,
                        unsigned int p_dimensions, 
        ...
        boost::shared_ptr<const ArrayType>
        CopyArrayType(const ArrayType& p_arrayType);

        // Create a struct type.
        boost::shared_ptr<const StructType>
        CreateStructType(const std::string& p_name,
                         const std::string& p_externName,
                         const std::vector<StructType::MemberInfo>& p_members,
                         bool p_isConst);
        ...
        boost::shared_ptr<const StructType>
        CopyStructType(const StructType& p_type);

        // Create an object type.
        boost::shared_ptr<const ObjectType>
        CreateObjectType(const std::string& p_name,
                         const std::string& p_externName,
                         const std::vector<ObjectType::ObjectMember>& p_members,
                         bool p_isConst);
        ...
        boost::shared_ptr<const ObjectType>
        CopyObjectType(const ObjectType& p_type);

        // Create a new state machine type.
        boost::shared_ptr<const StateMachineType>
        CreateStateMachineType(const std::string& p_name,
                               const CompoundType::Member* p_members,
                               size_t p_numMembers,
        ...
        // Create a deep copy of a state machine type.
        boost::shared_ptr<const StateMachineType>
        CopyStateMachineType(const StateMachineType& p_type);
        
        // Create a function type.
        boost::shared_ptr<const FunctionType>
        CreateFunctionType(const TypeImpl& p_returnType,
                           const TypeImpl* const* p_params,
                           size_t p_numParams);

        // Create a deep copy of a state machine type.
        boost::shared_ptr<const FunctionType>
        CopyFunctionType(const FunctionType& p_type);

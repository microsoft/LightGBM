...
def _pandas_to_numpy(
    data: pd_DataFrame, target_dtype: "np.typing.DTypeLike"
) -> np.ndarray:
    _check_for_bad_pandas_dtypes(data.dtypes)
    try:
...
class Dataset:
    ...
    def __init_from_pyarrow_table(
        ...
        if not PYARROW_INSTALLED:
            raise LightGBMError(
                "Cannot init dataframe from Arrow without `pyarrow` installed."
            )

        # Check that the input is valid: we only handle numbers (for now)
        if not all(
            arrow_is_integer(t) or arrow_is_floating(t) for t in table.schema.types
        ):
            raise ValueError(
        ...
        if _is_pyarrow_array(data) or _is_pyarrow_table(data):
            ...
            if _is_pyarrow_table(data):
                if field_name != "init_score":
                    raise ValueError(
                        "pyarrow table provided for field other than init_score"
                    )

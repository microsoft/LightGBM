...
class StringArray {
 public:
  StringArray(size_t num_elements, size_t string_size)
      : _string_size(string_size), _array(num_elements + 1, nullptr) {
    _allocate_strings(num_elements, string_size);
  }

  ~StringArray() { _release_strings(); }
   ...
   */
  char **data() noexcept { return _array.data(); }
   ...
   */
  size_t get_num_elements() noexcept { return _array.size() - 1; }
 ...
 private:
   ...
   */
  bool _in_bounds(size_t index) noexcept { return index < get_num_elements(); }

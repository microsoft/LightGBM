...
boost::shared_ptr<FreeForm2::BlockExpression> FreeForm2::BlockExpression::Alloc(
    const Annotations &p_annotations, const Expression **p_children,
    unsigned int p_numChildren, unsigned int p_numBound) {
  FF2_ASSERT(p_numChildren > 0);

  size_t bytes =
      sizeof(BlockExpression) + (p_numChildren - 1) * sizeof(Expression *);

  // Allocate a shared_ptr that deletes an BlockExpression
  // allocated in a char[].
  boost::shared_ptr<BlockExpression> exp;
  exp.reset(new (new char[bytes]) BlockExpression(p_annotations, p_children,
                                                  p_numChildren, p_numBound),
            DeleteAlloc);
  return exp;
}

const FreeForm2::TypeImpl &FreeForm2::BlockExpression::GetType() const {
  return *m_returnType;
}

size_t FreeForm2::BlockExpression::GetNumChildren() const {
  return m_numChildren;
}

unsigned int FreeForm2::BlockExpression::GetNumBound() const {
  return m_numBound;
}

void FreeForm2::BlockExpression::Accept(Visitor &p_visitor) const {
...
const FreeForm2::Expression &FreeForm2::BlockExpression::GetChild(
    unsigned int p_index) const {
...
void FreeForm2::BlockExpression::DeleteAlloc(BlockExpression *p_allocated) {

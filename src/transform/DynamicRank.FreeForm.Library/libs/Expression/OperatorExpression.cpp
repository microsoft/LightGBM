...
FreeForm2::UnaryOperatorExpression::UnaryOperatorExpression(
...
void FreeForm2::UnaryOperatorExpression::Accept(Visitor &p_visitor) const {
...
const FreeForm2::TypeImpl &FreeForm2::UnaryOperatorExpression::GetType() const {
  return m_type;
}

const FreeForm2::TypeImpl &FreeForm2::UnaryOperatorExpression::InferType()
    const {
  const TypeImpl &operandType =
      UnaryOperator::GetBestOperandType(m_op, m_child.GetType());
...
size_t FreeForm2::UnaryOperatorExpression::GetNumChildren() const { return 1; }

FreeForm2::BinaryOperatorExpression::BinaryOperatorExpression(
...
FreeForm2::BinaryOperatorExpression::BinaryOperatorExpression(
    ...
    const Expression &p_rightChild, const BinaryOperator::Operation p_binaryOp,
...
void FreeForm2::BinaryOperatorExpression::Accept(Visitor &p_visitor) const {
  size_t stackSize = p_visitor.StackSize();
...
const FreeForm2::TypeImpl &FreeForm2::BinaryOperatorExpression::InferChildType(
    TypeManager &p_typeManager) const {
  const TypeImpl *unifiedType(&TypeImpl::GetUnknownType());

  for (size_t i = 0; i < m_numChildren; i++) {
    const TypeImpl &type = TypeUtil::Unify(
        m_children[i]->GetType(), *unifiedType, p_typeManager, false, true);

    if (!type.IsValid()) {
      std::ostringstream err;
      err << "Arguments to binary operators (except index) "
          << "are expected to be of a unifiable type.  The first " << i
          << " elements are of type '" << *unifiedType << "', element " << i + 1
          << " is of type '" << m_children[i]->GetType() << "'";
      throw ParseError(err.str(), GetSourceLocation());
    }
    unifiedType = &type;
  }
  ...
  if (!unifiedType->IsLeafType() && unifiedType->Primitive() != Type::Unknown) {
    std::ostringstream err;
    err << "Expected fixed-size type; got type: " << unifiedType;
    throw ParseError(err.str(), GetSourceLocation());
  }

  const TypeImpl &childType =
      BinaryOperator::GetBestOperandType(m_binaryOp, *unifiedType);

  if (!childType.IsValid()) {
    std::ostringstream err;
    err << "Invalid operand type " << *unifiedType << " supplied to operator";
    throw ParseError(err.str(), GetSourceLocation());
  }
...
const FreeForm2::TypeImpl &FreeForm2::BinaryOperatorExpression::GetType()
    const {
  return *m_resultType;
}

const FreeForm2::TypeImpl &FreeForm2::BinaryOperatorExpression::GetChildType()
    const {
  return *m_childType;
}

size_t FreeForm2::BinaryOperatorExpression::GetNumChildren() const {
...
FreeForm2::BinaryOperatorExpression::Alloc(
    const Annotations &p_annotations,
    const std::vector<const Expression *> &p_children,
    const BinaryOperator::Operation p_binaryOp, TypeManager &p_typeManager) {
  ...
  // allocated in a char[].
  boost::shared_ptr<BinaryOperatorExpression> exp(
      new (new char[bytes]) BinaryOperatorExpression(p_annotations, p_children,
                                                     p_binaryOp, p_typeManager),
      DeleteAlloc);
  return exp;
}
...
FreeForm2::BinaryOperatorExpression::Alloc(
  ...
  // allocated in a char[].
  boost::shared_ptr<BinaryOperatorExpression> exp(
      new (new char[bytes]) BinaryOperatorExpression(
          p_annotations, p_leftChild, p_rightChild, p_binaryOp, p_typeManager),
      DeleteAlloc);
  return exp;
}

void FreeForm2::BinaryOperatorExpression::DeleteAlloc(
    BinaryOperatorExpression *p_allocated) {

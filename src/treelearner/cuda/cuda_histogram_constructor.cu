...
__global__ void CUDAConstructHistogramKernel(const int* leaf_index,
  ...
  const data_size_t* leaf_num_data_offsets, const uint8_t* data, const uint32_t* feature_group_offsets) {
  const unsigned int threadIdx_x = threadIdx.x;
  const int num_feature_groups_ref = *num_feature_groups;
  const int leaf_index_ref = *leaf_index;
  const data_size_t num_data_in_smaller_leaf_ref = leaf_num_data_offsets[leaf_index_ref + 1] - leaf_num_data_offsets[leaf_index_ref];
...
void CUDAHistogramConstructor::LaunchConstructHistogramKernel(
  ...
  CUDAConstructHistogramKernel<<<grid_dim, block_dim>>>(cuda_smaller_leaf_index, cuda_gradients_, cuda_hessians_,
    cuda_data_indices_in_smaller_leaf, cuda_hist_, cuda_num_feature_groups_, cuda_leaf_num_data_offsets, cuda_data_,

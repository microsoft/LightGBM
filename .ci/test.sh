...
if [[ $TASK == "lint" ]]; then
    ...
    pwsh -command "Invoke-ScriptAnalyzer -Path "${BUILD_DIRECTORY}"/.ci -Recurse -EnableExit -Severity Warning,Error -IncludeDefaultRules -IncludeRule PSAvoidExclaimOperator" || exit 1
    # ,AvoidSemicolonsAsLineTerminators,AvoidUsingDoubleQuotesForConstantString,PlaceCloseBrace,PlaceOpenBrace,UseConsistentIndentation,UseConsistentWhitespace,UseCorrectCasing

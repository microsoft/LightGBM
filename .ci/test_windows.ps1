...
if ($env:TASK -eq "regular") {
  ...
  conda install -y -n $env:CONDA_ENV notebook
  jupyter nbconvert --ExecutePreprocessor.timeout=60 --to notebook --execute --inplace *.ipynb ; Check-Output $LastExitCode  # run all notebooks
}

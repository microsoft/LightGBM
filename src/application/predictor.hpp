...
#pragma omp master
  ...
  void Predict(const char* data_filename, const char* result_filename,
               bool header, bool disable_shape_check) {
    ...
    auto parser = std::unique_ptr<Parser>(Parser::CreateParser(
        data_filename, header, boosting_->MaxFeatureIdx() + 1, label_idx));
    ...
    if (header) {
      ...
      for (int i = 0; i < static_cast<int>(fnames.size()); ++i) {
        if (header_mapper.count(fnames[i]) <= 0) {
          Log::Warning(
              "Feature (%s) is missed in data file. If it is "
              "weight/query/group/ignore_column, you can ignore this warning.",
              fnames[i].c_str());
        } else {
    ...
    std::function<void(const char*, std::vector<std::pair<int, double>>*)>
        parser_fun;
    double tmp_label;
    parser_fun = [&](const char* buffer,
                     std::vector<std::pair<int, double>>* feature) {
    ...
    std::function<void(data_size_t, const std::vector<std::string>&)>
        process_fun = [&](data_size_t, const std::vector<std::string>& lines) {
...
#pragma omp parallel for schedule(static) firstprivate(oneline_features)
          ...
          OMP_THROW_EX();
          for (data_size_t i = 0;
               i < static_cast<data_size_t>(result_to_write.size()); ++i) {
            writer->Write(result_to_write[i].c_str(),
                          result_to_write[i].size());
            writer->Write("\n", 1);
          }
        };
 ...
 private:
  void CopyToPredictBuffer(
      double* pred_buf, const std::vector<std::pair<int, double>>& features) {

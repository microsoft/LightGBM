...
PredictionEarlyStopInstance CreateNone(const PredictionEarlyStopConfig&) {
  return PredictionEarlyStopInstance{
      [](const double*, int) { return false; },
      std::numeric_limits<int>::max()  // make sure the lambda is almost never
                                       // called
  };
}

PredictionEarlyStopInstance CreateMulticlass(
  ...
  return PredictionEarlyStopInstance{
      [margin_threshold](const double* pred, int sz) {
        if (sz < 2) {
          Log::Fatal(
              "Multiclass early stopping needs predictions to be of length two "
              "or larger");
        }
        ...
        std::partial_sort(votes.begin(), votes.begin() + 2, votes.end(),
                          std::greater<double>());
...
PredictionEarlyStopInstance CreateBinary(
    const PredictionEarlyStopConfig& config) {
  // margin_threshold will be captured by value
  const double margin_threshold = config.margin_threshold;
  ...
  return PredictionEarlyStopInstance{
      [margin_threshold](const double* pred, int sz) {
        if (sz != 1) {
          Log::Fatal(
              "Binary early stopping needs predictions to be of length one");
        }
...
PredictionEarlyStopInstance CreatePredictionEarlyStopInstance(

 ...
 public:
  ...
  INDEX_T ReadAllAndProcess(const std::function<void(INDEX_T, const char*, size_t)>& process_fun) {
    ...
    PipelineReader::Read(
        ...
        [&process_fun, &bytes_read, &total_cnt, this](const char* buffer_process,
                                                      size_t read_cnt) {
          size_t cnt = 0;
          size_t i = 0;
          size_t last_i = 0;
  ...
  INDEX_T ReadAllLines() {
    return ReadAllAndProcess(
        [=](INDEX_T, const char* buffer, size_t size) { lines_.emplace_back(buffer, size); });
  }
  ...
  INDEX_T ReadAndFilterLines(const std::function<bool(INDEX_T)>& filter_fun,
    ...
    INDEX_T total_cnt = ReadAllAndProcess([&filter_fun, &out_used_data_indices, this](
                                              INDEX_T line_idx, const char* buffer, size_t size) {
  ...
  INDEX_T SampleAndFilterFromFile(const std::function<bool(INDEX_T)>& filter_fun,
    ...
    out_used_data_indices->clear();
    INDEX_T total_cnt =
        ReadAllAndProcess([=, &filter_fun, &out_used_data_indices, &random, &cur_sample_cnt,
          ...
          if (is_used) {
            ...
            if (cur_sample_cnt < sample_cnt) {
              ...
              const size_t idx = static_cast<size_t>(
                  random->NextInt(0, static_cast<int>(out_used_data_indices->size())));
              if (idx < static_cast<size_t>(sample_cnt)) {
                out_sampled_data->operator[](idx) = std::string(buffer, size);
              }
            }

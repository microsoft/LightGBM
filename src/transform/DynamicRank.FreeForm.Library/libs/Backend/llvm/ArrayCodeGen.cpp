...
ArrayCodeGen::ArrayCountType 
FreeForm2::ArrayCodeGen::DecodeDimensions(ArrayBoundsType p_bounds, 
                                          unsigned int p_numDimensions, 
                                          std::vector<unsigned int>& p_dimensions)
{
    ...
    // Decode bounds.
    for (unsigned int i = 0; i < p_numDimensions; ++i, p_bounds >>= c_bitsPerFlatDimension)
    {
...
FreeForm2::ArrayCodeGen::IssueReturn(CompilationState& p_state, 
                                     LlvmCodeGenerator::CompiledValue& p_array, 
                                     ...
                                     LlvmCodeGenerator::CompiledValue& p_arraySpace)
{
    ...
    LlvmCodeGenerator::CompiledValue* elementSize 
        = llvm::ConstantInt::get(p_state.GetContext(), 
                                 llvm::APInt(sizeof(ArrayBoundsType) * 8, bytes));
...
FreeForm2::ArrayCodeGen::CreateArrayResult(const ArrayType& p_arrayType, 
                                           ArrayCodeGen::ArrayBoundsType p_bounds, 
                                           const boost::shared_array<T>& p_space)
{
    // Decode bounds.
    SharedDimensions dimensions(new std::vector<unsigned int>());
    ArrayCodeGen::DecodeDimensions(p_bounds, 
                                   p_arrayType.GetDimensionCount(), 
                                   *dimensions);
    return boost::shared_ptr<Result>(new ArrayResult<T>(p_arrayType, 
                                                        0, 
                                                        dimensions, 
                                                        p_space.get(), 
                                                        p_space));
...
LlvmCodeGenerator::CompiledValue&
FreeForm2::ArrayCodeGen::CreateArray(CompilationState& p_state, 
                                     const ArrayType& p_arrayType, 
                                     ...
                                     LlvmCodeGenerator::CompiledValue& p_pointer)
{
    ...
    structure 
        = p_state.GetBuilder().CreateInsertValue(structure, &p_count, countPosition);
    CHECK_LLVM_RET(structure);

    structure = p_state.GetBuilder().CreateInsertValue(structure, &p_pointer, pointerPosition);
    CHECK_LLVM_RET(structure);
...
LlvmCodeGenerator::CompiledValue&
FreeForm2::ArrayCodeGen::CreateEmptyArray(CompilationState& p_state, 
                                          const ArrayType& p_arrayType)
{
    LlvmCodeGenerator::CompiledValue* boundsZero 
        = llvm::ConstantInt::get(p_state.GetContext(), 
                                 llvm::APInt(sizeof(ArrayBoundsType) * 8, 0));
    CHECK_LLVM_RET(boundsZero);
 
    LlvmCodeGenerator::CompiledValue* countZero 
        = llvm::ConstantInt::get(p_state.GetContext(), 
                                 llvm::APInt(sizeof(ArrayCountType) * 8, 0));
    CHECK_LLVM_RET(countZero);

    llvm::Type& childType = p_state.GetType(p_arrayType.GetChildType().AsConstType());
    llvm::Type* nullType = llvm::PointerType::get(&childType, 0);
    CHECK_LLVM_RET(nullType);
    LlvmCodeGenerator::CompiledValue* pointer = llvm::Constant::getNullValue(nullType);
    CHECK_LLVM_RET(pointer);
...
LlvmCodeGenerator::CompiledValue& 
FreeForm2::ArrayCodeGen::MaskBounds(CompilationState& p_state, 
                                    LlvmCodeGenerator::CompiledValue& p_bounds)
{
    LlvmCodeGenerator::CompiledValue* mask 
        = llvm::ConstantInt::get(p_state.GetContext(), 
                llvm::APInt(sizeof(ArrayBoundsType) * 8, (1 << c_bitsPerFlatDimension) - 1));
    CHECK_LLVM_RET(mask);
...
LlvmCodeGenerator::CompiledValue& 
FreeForm2::ArrayCodeGen::ShiftBounds(CompilationState& p_state, 
                                     LlvmCodeGenerator::CompiledValue& p_bounds, 
                                     unsigned int p_dimensions)
{
    LlvmCodeGenerator::CompiledValue* shift 
        = llvm::ConstantInt::get(p_state.GetContext(), 
                llvm::APInt(sizeof(ArrayBoundsType) * 8, 
...
LlvmCodeGenerator::CompiledValue&
FreeForm2::ArrayCodeGen::UnshiftBound(CompilationState& p_state,
                                      LlvmCodeGenerator::CompiledValue& p_bounds,
                                      LlvmCodeGenerator::CompiledValue& p_newBound)
{
    llvm::Value* bitCount = llvm::ConstantInt::get(p_bounds.getType(), c_bitsPerFlatDimension);
    CHECK_LLVM_RET(bitCount);

    llvm::Value* leftShift = p_state.GetBuilder().CreateShl(&p_bounds, bitCount);
    CHECK_LLVM_RET(leftShift);

    llvm::Value* maxBound = llvm::ConstantInt::get(p_newBound.getType(), (1 << c_bitsPerFlatDimension) - 1);
    CHECK_LLVM_RET(maxBound);

    llvm::Value* checkBound = p_state.GetBuilder().CreateICmpUGT(&p_newBound, maxBound);
    CHECK_LLVM_RET(checkBound);

    llvm::Value* realBound = p_state.GetBuilder().CreateSelect(checkBound, maxBound, &p_newBound);
    CHECK_LLVM_RET(realBound);

    if (realBound->getType()->getPrimitiveSizeInBits() < p_bounds.getType()->getPrimitiveSizeInBits())
    {
        realBound = p_state.GetBuilder().CreateZExt(realBound, p_bounds.getType());
        CHECK_LLVM_RET(realBound);
    }

    llvm::Value* finalBounds = p_state.GetBuilder().CreateOr(leftShift, realBound);

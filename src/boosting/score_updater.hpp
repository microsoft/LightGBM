...
class ScoreUpdater {
 public:
  ...
  ScoreUpdater(const Dataset* data, int num_tree_per_iteration) : data_(data) {
    ...
    int64_t total_size =
        static_cast<int64_t>(num_data_) * num_tree_per_iteration;
...
#pragma omp parallel for schedule(static)
  ...
  /*!
   * \brief Using tree model to get prediction number, then adding to scores for
   * all data Note: this function generally will be used on validation data too.
  ...
  /*!
   * \brief Using tree model to get prediction number, then adding to scores for
   * parts of data Used for prediction of training out-of-bag data \param tree
   * Trained tree model \param data_indices Indices of data that will be
   * processed \param data_cnt Number of data that will be processed \param
  ...
  inline void AddScore(const Tree* tree, const data_size_t* data_indices,
    ...
    Common::FunctionTimer fun_timer("ScoreUpdater::AddScore", global_timer);
    const size_t offset = static_cast<size_t>(num_data_) * cur_tree_id;
    tree->AddPredictionToScore(data_, data_indices, data_cnt,
                               score_.data() + offset);
  }

...
class RegressionL2loss : public ObjectiveFunction {
 public:
  explicit RegressionL2loss(const Config& config) { sqrt_ = config.reg_sqrt; }
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data; ++i) {
        trans_label_[i] =
            Common::Sign(label_[i]) * std::sqrt(std::fabs(label_[i]));
      }
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] =
            static_cast<score_t>((score[i] - label_[i]) * weights_[i]);
        hessians[i] = static_cast<score_t>(weights_[i]);
      }
    }
  }

  const char* GetName() const override { return "regression"; }
...
class RegressionL1loss : public RegressionL2loss {
 public:
  ...
  explicit RegressionL1loss(const std::vector<std::string>& strs)
...
#define weight_reader(i) (weights_[i])
      WeightedPercentileFun(label_t, data_reader, weight_reader, num_data_,
                            alpha);
#undef data_reader
#undef weight_reader
    } else {
...
#undef data_reader
  ...
  double RenewTreeOutput(
      double, std::function<double(const label_t*, int)> residual_getter,
      const data_size_t* index_mapper, const data_size_t* bagging_mapper,
...
#define data_reader(i) \
  (residual_getter(label_, bagging_mapper[index_mapper[i]]))
        PercentileFun(double, data_reader, num_data_in_leaf, alpha);
#undef data_reader
      }
    } else {
...
#define weight_reader(i) (weights_[index_mapper[i]])
        WeightedPercentileFun(double, data_reader, weight_reader,
                              num_data_in_leaf, alpha);
#undef data_reader
#undef weight_reader
      } else {
#define data_reader(i) \
...
#undef weight_reader
  ...
  const char* GetName() const override { return "regression_l1"; }
};
...
class RegressionHuberLoss : public RegressionL2loss {
 public:
  explicit RegressionHuberLoss(const Config& config)
  ...
  explicit RegressionHuberLoss(const std::vector<std::string>& strs)
      : RegressionL2loss(strs) {
    if (sqrt_) {
      Log::Warning(
          "Cannot use sqrt transform in %s Regression, will auto disable it",
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        if (std::abs(diff) <= alpha_) {
          ...
          gradients[i] =
              static_cast<score_t>(Common::Sign(diff) * weights_[i] * alpha_);
        }
        hessians[i] = static_cast<score_t>(weights_[i]);
      }
    }
  ...
  const char* GetName() const override { return "huber"; }

  bool IsConstantHessian() const override { return false; }
...
class RegressionFairLoss : public RegressionL2loss {
 public:
  ...
  explicit RegressionFairLoss(const std::vector<std::string>& strs)
      : RegressionL2loss(strs) {}
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        ...
        hessians[i] = static_cast<score_t>(
            c_ * c_ / ((std::fabs(x) + c_) * (std::fabs(x) + c_)));
      }
    } else {
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        const double x = score[i] - label_[i];
        gradients[i] =
            static_cast<score_t>(c_ * x / (std::fabs(x) + c_) * weights_[i]);
        hessians[i] = static_cast<score_t>(
            c_ * c_ / ((std::fabs(x) + c_) * (std::fabs(x) + c_)) *
            weights_[i]);
      }
  ...
  const char* GetName() const override { return "fair"; }
...
class RegressionPoissonLoss : public RegressionL2loss {
 public:
  explicit RegressionPoissonLoss(const Config& config)
      : RegressionL2loss(config) {
  ...
  explicit RegressionPoissonLoss(const std::vector<std::string>& strs)
      : RegressionL2loss(strs) {}
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    label_t miny;
    double sumy;
    Common::ObtainMinMaxSum(label_, num_data_, &miny,
                            static_cast<label_t*>(nullptr), &sumy);
    if (miny < 0.0f) {
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] = static_cast<score_t>((std::exp(score[i]) - label_[i]) *
                                            weights_[i]);
        hessians[i] = static_cast<score_t>(
            std::exp(score[i] + max_delta_step_) * weights_[i]);
      }
    }
  }
  ...
  const char* GetName() const override { return "poisson"; }
...
class RegressionQuantileloss : public RegressionL2loss {
 public:
  explicit RegressionQuantileloss(const Config& config)
      : RegressionL2loss(config) {
    alpha_ = static_cast<score_t>(config.alpha);
    CHECK(alpha_ > 0 && alpha_ < 1);
  }

  explicit RegressionQuantileloss(const std::vector<std::string>& strs)
      : RegressionL2loss(strs) {}
...
#pragma omp parallel for schedule(static)
  ...
  const char* GetName() const override { return "quantile"; }
...
class RegressionMAPELOSS : public RegressionL1loss {
 public:
  explicit RegressionMAPELOSS(const Config& config)
      : RegressionL1loss(config) {}

  explicit RegressionMAPELOSS(const std::vector<std::string>& strs)
      : RegressionL1loss(strs) {}
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    for (data_size_t i = 0; i < num_data_; ++i) {
      if (std::fabs(label_[i]) < 1) {
        Log::Warning(
            "Met 'abs(label) < 1', will convert them to '1' in MAPE objective "
            "and metric");
        break;
      }
    }
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        label_weight_[i] =
            1.0f / std::max(1.0f, std::fabs(label_[i])) * weights_[i];
      }
    }
  }
...
#undef weight_reader
  ...
  const char* GetName() const override { return "mape"; }
...
class RegressionGammaLoss : public RegressionPoissonLoss {
 public:
  explicit RegressionGammaLoss(const Config& config)
      : RegressionPoissonLoss(config) {}

  explicit RegressionGammaLoss(const std::vector<std::string>& strs)
      : RegressionPoissonLoss(strs) {}
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] =
            static_cast<score_t>(1.0 - label_[i] / std::exp(score[i]));
        hessians[i] = static_cast<score_t>(label_[i] / std::exp(score[i]));
      }
    } else {
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] = static_cast<score_t>(
            1.0 - label_[i] / std::exp(score[i]) * weights_[i]);
        hessians[i] =
            static_cast<score_t>(label_[i] / std::exp(score[i]) * weights_[i]);
      }
    }
  }
...
class RegressionTweedieLoss : public RegressionPoissonLoss {
 public:
  explicit RegressionTweedieLoss(const Config& config)
      : RegressionPoissonLoss(config) {
    rho_ = config.tweedie_variance_power;
  }

  explicit RegressionTweedieLoss(const std::vector<std::string>& strs)
      : RegressionPoissonLoss(strs) {}
...
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        gradients[i] =
            static_cast<score_t>(-label_[i] * std::exp((1 - rho_) * score[i]) +
                                 std::exp((2 - rho_) * score[i]));
        hessians[i] = static_cast<score_t>(
...
#pragma omp parallel for schedule(static)
  ...
  const char* GetName() const override { return "tweedie"; }

...
BinMapper::BinMapper()
    : num_bin_(1), is_trivial_(true), bin_type_(BinType::NumericalBin) {
...
bool NeedFilter(const std::vector<int>& cnt_in_bin, int total_cnt,
                int filter_cnt, BinType bin_type) {
...
std::vector<double> GreedyFindBin(const double* distinct_values,
                                  const int* counts, int num_distinct_values,
                                  int max_bin, size_t total_cnt,
                                  int min_data_in_bin) {
  ...
  if (num_distinct_values <= max_bin) {
    ...
    for (int i = 0; i < num_distinct_values - 1; ++i) {
      ...
      if (cur_cnt_inbin >= min_data_in_bin) {
        auto val = Common::GetDoubleUpperBound(
    ...
    if (min_data_in_bin > 0) {
      max_bin =
          std::min(max_bin, static_cast<int>(total_cnt / min_data_in_bin));
...
std::vector<double> FindBinWithPredefinedBin(
    const double* distinct_values, const int* counts, int num_distinct_values,
  ...
  for (size_t i = 0; i < bin_upper_bound.size(); ++i) {
    ...
    while ((value_ind < num_distinct_values) &&
    ...
    int num_sub_bins = static_cast<int>(std::lround(
        (static_cast<double>(cnt_in_bin) * free_bins / total_sample_cnt)));
    ...
    std::vector<double> new_upper_bounds = GreedyFindBin(
        distinct_values + bin_start, counts + bin_start, distinct_cnt_in_bin,
  ...
  bin_upper_bound.insert(bin_upper_bound.end(), bounds_to_add.begin(),
                         bounds_to_add.end());
  std::stable_sort(bin_upper_bound.begin(), bin_upper_bound.end());
...
std::vector<double> FindBinWithZeroAsOneBin(
    const double* distinct_values, const int* counts, int num_distinct_values,
    int max_bin, size_t total_sample_cnt, int min_data_in_bin) {
  std::vector<double> bin_upper_bound;
  int left_cnt_data = 0;
  ...
  if ((left_cnt > 0) && (max_bin > 1)) {
    int left_max_bin =
        static_cast<int>(static_cast<double>(left_cnt_data) /
                         (total_sample_cnt - cnt_zero) * (max_bin - 1));
    left_max_bin = std::max(1, left_max_bin);
    bin_upper_bound =
        GreedyFindBin(distinct_values, counts, left_cnt, left_max_bin,
  ...
  if (right_start >= 0 && right_max_bin > 0) {
    auto right_bounds =
        GreedyFindBin(distinct_values + right_start, counts + right_start,
                      num_distinct_values - right_start, right_max_bin,
                      right_cnt_data, min_data_in_bin);
    bin_upper_bound.push_back(kZeroThreshold);
    bin_upper_bound.insert(bin_upper_bound.end(), right_bounds.begin(),
                           right_bounds.end());
  } else {
    bin_upper_bound.push_back(std::numeric_limits<double>::infinity());
  }
...
std::vector<double> FindBinWithZeroAsOneBin(
  ...
  if (forced_upper_bounds.empty()) {
    return FindBinWithZeroAsOneBin(distinct_values, counts, num_distinct_values,
                                   max_bin, total_sample_cnt, min_data_in_bin);
  } else {
    return FindBinWithPredefinedBin(
        distinct_values, counts, num_distinct_values, max_bin, total_sample_cnt,
        min_data_in_bin, forced_upper_bounds);
  }
}

void BinMapper::FindBin(double* values, int num_sample_values,
                        size_t total_sample_cnt, int max_bin,
                        int min_data_in_bin, int min_split_data,
                        BinType bin_type, bool use_missing,
  ...
  default_bin_ = 0;
  int zero_cnt =
      static_cast<int>(total_sample_cnt - num_sample_values - na_cnt);
  ...
  if (num_sample_values > 0 && values[num_sample_values - 1] < 0.0f &&
      zero_cnt > 0) {
    distinct_values.push_back(0.0f);
    counts.push_back(zero_cnt);
  }
  ...
  if (bin_type_ == BinType::NumericalBin) {
    if (missing_type_ == MissingType::Zero) {
      bin_upper_bound_ = FindBinWithZeroAsOneBin(
          distinct_values.data(), counts.data(), num_distinct_values, max_bin,
      ...
      bin_upper_bound_ = FindBinWithZeroAsOneBin(
          distinct_values.data(), counts.data(), num_distinct_values,
          max_bin - 1, total_sample_cnt - na_cnt, min_data_in_bin,
          forced_upper_bounds);
    ...
    for (size_t i = 0; i < distinct_values.size(); ++i) {
      ...
      if (val < 0) {
        na_cnt += counts[i];
        Log::Warning(
            "Met negative value in categorical features, will convert it to "
            "NaN");
      } else {
    ...
    if (rest_cnt > 0) {
      ...
      if (distinct_values_int.back() / SPARSE_RATIO >
          static_cast<int>(distinct_values_int.size())) {
        Log::Warning(
  ...
  // check useless bin
  if (!is_trivial_ && NeedFilter(cnt_in_bin, static_cast<int>(total_sample_cnt),
                                 min_split_data, bin_type_)) {
    is_trivial_ = true;
  }
  ...
  if (!is_trivial_) {
    ...
    sparse_rate_ =
        static_cast<double>(cnt_in_bin[default_bin_]) / total_sample_cnt;
...
size_t BinMapper::SizesInByte() const {
  size_t ret = sizeof(num_bin_) + sizeof(missing_type_) + sizeof(is_trivial_) +
               sizeof(sparse_rate_) + sizeof(bin_type_) + sizeof(min_val_) +
               sizeof(max_val_) + sizeof(default_bin_) + sizeof(most_freq_bin_);
  if (bin_type_ == BinType::NumericalBin) {

...
class FeatureHistogram {
 public:
   ...
   */
  template <bool USE_DIST_GRAD = false, typename THIS_HIST_T = hist_t,
            typename OTHER_HIST_T = hist_t, typename RESULT_HIST_T = hist_t,
  ...
  void Subtract(const FeatureHistogram& other, const int32_t* buffer = nullptr) {
    if (USE_DIST_GRAD) {
      const THIS_HIST_T* this_int_data =
          THIS_HIST_BITS == 16
              ? reinterpret_cast<const THIS_HIST_T*>(data_int16_)
              : (RESULT_HIST_BITS == 16 ? reinterpret_cast<const THIS_HIST_T*>(buffer)
  ...
  void FindBestThreshold(double sum_gradient, double sum_hessian, data_size_t num_data,
                         const FeatureConstraint* constraints, double parent_output,
                         SplitInfo* output) {
    output->default_left = true;
    output->gain = kMinScore;
    find_best_threshold_fun_(sum_gradient, sum_hessian + 2 * kEpsilon, num_data, constraints,
                             parent_output, output);
    output->gain *= meta_->penalty;
  }

  void FindBestThresholdInt(int64_t sum_gradient_and_hessian, double grad_scale, double hess_scale,
                            const uint8_t num_bits_bin, const uint8_t num_bits_acc,
                            data_size_t num_data, const FeatureConstraint* constraints,
                            double parent_output, SplitInfo* output) {
    output->default_left = true;
    output->gain = kMinScore;
    int_find_best_threshold_fun_(sum_gradient_and_hessian, grad_scale, hess_scale, num_bits_bin,
                                 num_bits_acc, num_data, constraints, parent_output, output);
    output->gain *= meta_->penalty;
  }

  template <bool USE_RAND, bool USE_L1, bool USE_MAX_OUTPUT, bool USE_SMOOTHING>
  double BeforeNumerical(double sum_gradient, double sum_hessian, double parent_output,
                         data_size_t num_data, SplitInfo* output, int* rand_threshold) {
  ...
  template <bool USE_RAND, bool USE_L1, bool USE_MAX_OUTPUT, bool USE_SMOOTHING>
  double BeforeNumericalInt(int64_t sum_gradient_and_hessian, double grad_scale, double hess_scale,
                            double parent_output, data_size_t num_data, SplitInfo* output,
                            int* rand_threshold) {
    is_splittable_ = false;
    output->monotone_type = meta_->monotone_type;
    const int32_t int_sum_gradient = static_cast<int32_t>(sum_gradient_and_hessian >> 32);
    const uint32_t int_sum_hessian =
        static_cast<uint32_t>(sum_gradient_and_hessian & 0x00000000ffffffff);
...
#define LAMBDA_ARGUMENTS_INT                                                          \
  int64_t sum_gradient_and_hessian, double grad_scale, double hess_scale,             \
...
#define BEFORE_ARGUMENTS_INT                                                         \
  sum_gradient_and_hessian, grad_scale, hess_scale, parent_output, num_data, output, \
      &rand_threshold
      ...
      if (meta_->num_bin > 2 && meta_->missing_type != MissingType::None) {
        if (meta_->missing_type == MissingType::Zero) {
          int_find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS_INT) {
            ...
            if (hist_bits_acc <= 16) {
              ...
              FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, true, false, int32_t,
                                               int32_t, int16_t, int16_t, 16, 16>(
                  FUNC_ARGUMENTS_INT);
              FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, false, true, false, int32_t,
                                               int32_t, int16_t, int16_t, 16, 16>(
                  FUNC_ARGUMENTS_INT);
            } else {
              if (hist_bits_bin == 32) {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, true, false, int64_t,
                                                 int64_t, int32_t, int32_t, 32, 32>(
                    FUNC_ARGUMENTS_INT);
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, false, true, false, int64_t,
                                                 int64_t, int32_t, int32_t, 32, 32>(
                    FUNC_ARGUMENTS_INT);
              } else {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, true, false, int32_t,
                                                 int64_t, int16_t, int32_t, 16, 32>(
                    FUNC_ARGUMENTS_INT);
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, false, true, false, int32_t,
                                                 int64_t, int16_t, int32_t, 16, 32>(
                    FUNC_ARGUMENTS_INT);
          ...
          int_find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS_INT) {
            ...
            if (hist_bits_acc <= 16) {
              CHECK_LE(hist_bits_bin, 16);
              FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, true, int32_t,
                                               int32_t, int16_t, int16_t, 16, 16>(
                  FUNC_ARGUMENTS_INT);
              FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, false, false, true, int32_t,
                                               int32_t, int16_t, int16_t, 16, 16>(
                  FUNC_ARGUMENTS_INT);
            } else {
              if (hist_bits_bin == 32) {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, true, int64_t,
                                                 int64_t, int32_t, int32_t, 32, 32>(
                    FUNC_ARGUMENTS_INT);
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, false, false, true, int64_t,
                                                 int64_t, int32_t, int32_t, 32, 32>(
                    FUNC_ARGUMENTS_INT);
              } else {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, true, int32_t,
        ...
        if (meta_->missing_type != MissingType::NaN) {
          int_find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS_INT) {
            ...
            if (hist_bits_acc <= 16) {
              CHECK_LE(hist_bits_bin, 16);
              FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, false, int32_t,
                                               int32_t, int16_t, int16_t, 16, 16>(
                  FUNC_ARGUMENTS_INT);
            } else {
              if (hist_bits_bin == 32) {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, false, int64_t,
                                                 int64_t, int32_t, int32_t, 32, 32>(
                    FUNC_ARGUMENTS_INT);
              } else {
                FindBestThresholdSequentiallyInt<TEMPLATE_PREFIX_INT, true, false, false, int32_t,
...
#define LAMBDA_ARGUMENTS                                         \
  double sum_gradient, double sum_hessian, data_size_t num_data, \
      const FeatureConstraint *constraints, double parent_output, SplitInfo *output
#define BEFORE_ARGUMENTS \
  sum_gradient, sum_hessian, parent_output, num_data, output, &rand_threshold
#define FUNC_ARGUMENTS                                                                      \
  sum_gradient, sum_hessian, num_data, constraints, min_gain_shift, output, rand_threshold, \
      parent_output
      ...
      if (meta_->num_bin > 2 && meta_->missing_type != MissingType::None) {
        if (meta_->missing_type == MissingType::Zero) {
          find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS) {
            int rand_threshold = 0;
            double min_gain_shift =
                BeforeNumerical<USE_RAND, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(BEFORE_ARGUMENTS);
            FindBestThresholdSequentially<TEMPLATE_PREFIX, true, true, false>(FUNC_ARGUMENTS);
            FindBestThresholdSequentially<TEMPLATE_PREFIX, false, true, false>(FUNC_ARGUMENTS);
          };
        } else {
          find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS) {
            int rand_threshold = 0;
            double min_gain_shift =
                BeforeNumerical<USE_RAND, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(BEFORE_ARGUMENTS);
            FindBestThresholdSequentially<TEMPLATE_PREFIX, true, false, true>(FUNC_ARGUMENTS);
            FindBestThresholdSequentially<TEMPLATE_PREFIX, false, false, true>(FUNC_ARGUMENTS);
          };
        }
      } else {
        if (meta_->missing_type != MissingType::NaN) {
          find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS) {
            ...
            double min_gain_shift =
                BeforeNumerical<USE_RAND, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(BEFORE_ARGUMENTS);
            FindBestThresholdSequentially<TEMPLATE_PREFIX, true, false, false>(FUNC_ARGUMENTS);
          };
        } else {
          find_best_threshold_fun_ = [=](LAMBDA_ARGUMENTS) {
            int rand_threshold = 0;
...
#undef FUNC_ARGURMENTS
  ...
  template <bool USE_RAND, bool USE_MC, bool USE_L1, bool USE_MAX_OUTPUT, bool USE_SMOOTHING>
  void FindBestThresholdCategoricalInner(double sum_gradient, double sum_hessian,
  ...
  void GatherInfoForThreshold(double sum_gradient, double sum_hessian, uint32_t threshold,
                              data_size_t num_data, double parent_output, SplitInfo* output) {
    if (meta_->bin_type == BinType::NumericalBin) {
      GatherInfoForThresholdNumerical(sum_gradient, sum_hessian, threshold, num_data,
                                      parent_output, output);
    } else {
      GatherInfoForThresholdCategorical(sum_gradient, sum_hessian, threshold, num_data,
                                        parent_output, output);
    }
  }

  void GatherInfoForThresholdNumerical(double sum_gradient, double sum_hessian, uint32_t threshold,
                                       data_size_t num_data, double parent_output,
                                       SplitInfo* output) {
    bool use_smoothing = meta_->config->path_smooth > kEpsilon;
    if (use_smoothing) {
      GatherInfoForThresholdNumericalInner<true>(sum_gradient, sum_hessian, threshold, num_data,
                                                 parent_output, output);
    } else {
      GatherInfoForThresholdNumericalInner<false>(sum_gradient, sum_hessian, threshold, num_data,
                                                  parent_output, output);
    }
  }
  ...
  void GatherInfoForThresholdNumericalInner(double sum_gradient, double sum_hessian,
                                            ...
                                            double parent_output, SplitInfo* output) {
    double gain_shift =
        GetLeafGainGivenOutput<true>(sum_gradient, sum_hessian, meta_->config->lambda_l1,
    ...
    for (; t >= t_end; --t) {
      ...
      if (skip_default_bin && (t + offset) == static_cast<int>(meta_->default_bin)) {
        continue;
      }
    ...
    double current_gain = GetLeafGain<true, true, USE_SMOOTHING>(
    ...
    output->left_output = CalculateSplittedLeafOutput<true, true, USE_SMOOTHING>(
        sum_left_gradient, sum_left_hessian, meta_->config->lambda_l1, meta_->config->lambda_l2,
        meta_->config->max_delta_step, meta_->config->path_smooth, left_count, parent_output);
    ...
    output->right_output = CalculateSplittedLeafOutput<true, true, USE_SMOOTHING>(
        sum_gradient - sum_left_gradient, sum_hessian - sum_left_hessian, meta_->config->lambda_l1,
        meta_->config->lambda_l2, meta_->config->max_delta_step, meta_->config->path_smooth,
  ...
  void GatherInfoForThresholdCategorical(double sum_gradient, double sum_hessian,
    ...
    bool use_smoothing = meta_->config->path_smooth > kEpsilon;
    if (use_smoothing) {
      GatherInfoForThresholdCategoricalInner<true>(sum_gradient, sum_hessian, threshold, num_data,
                                                   parent_output, output);
    } else {
      GatherInfoForThresholdCategoricalInner<false>(sum_gradient, sum_hessian, threshold, num_data,
                                                    parent_output, output);
    }
  }

  template <bool USE_SMOOTHING>
  void GatherInfoForThresholdCategoricalInner(double sum_gradient, double sum_hessian,
                                              uint32_t threshold, data_size_t num_data,
                                              double parent_output, SplitInfo* output) {
    ...
    output->right_output = CalculateSplittedLeafOutput<true, true, USE_SMOOTHING>(
        sum_right_gradient, sum_right_hessian, meta_->config->lambda_l1, l2,
        meta_->config->max_delta_step, meta_->config->path_smooth, right_count, parent_output);
   ...
   */
  int SizeOfHistogram() const { return (meta_->num_bin - meta_->offset) * kHistEntrySize; }
  ...
  static double CalculateSplittedLeafOutput(double sum_gradients, double sum_hessians, double l1,
                                            double l2, double max_delta_step, double smoothing,
                                            data_size_t num_data, double parent_output) {
    double ret;
    if (USE_L1) {
  ...
  static double CalculateSplittedLeafOutput(double sum_gradients, double sum_hessians, double l1,
                                            ...
                                            const BasicConstraint& constraints, double smoothing,
                                            data_size_t num_data, double parent_output) {
 ...
 private:
  ...
  static double GetSplitGains(double sum_left_gradients, double sum_left_hessians,
                              double sum_right_gradients, double sum_right_hessians, double l1,
                              double l2, double max_delta_step,
                              const FeatureConstraint* constraints, int8_t monotone_constraint,
                              double smoothing, data_size_t left_count, data_size_t right_count,
                              double parent_output) {
    if (!USE_MC) {
      return GetLeafGain<USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
                 sum_left_gradients, sum_left_hessians, l1, l2, max_delta_step, smoothing,
                 left_count, parent_output) +
             GetLeafGain<USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
                 sum_right_gradients, sum_right_hessians, l1, l2, max_delta_step, smoothing,
                 right_count, parent_output);
    } else {
      double left_output =
      ...
      return GetLeafGainGivenOutput<USE_L1>(sum_left_gradients, sum_left_hessians, l1, l2,
                                            left_output) +
             GetLeafGainGivenOutput<USE_L1>(sum_right_gradients, sum_right_hessians, l1, l2,
                                            right_output);
    }
  }

  template <bool USE_L1, bool USE_MAX_OUTPUT, bool USE_SMOOTHING>
  static double GetLeafGain(double sum_gradients, double sum_hessians, double l1, double l2,
                            double max_delta_step, double smoothing, data_size_t num_data,
                            double parent_output) {
  ...
  static double GetLeafGainGivenOutput(double sum_gradients, double sum_hessians, double l1,
  ...
  void FindBestThresholdSequentially(double sum_gradient, double sum_hessian, data_size_t num_data,
                                     const FeatureConstraint* constraints, double min_gain_shift,
                                     SplitInfo* output, int rand_threshold, double parent_output) {
    const int8_t offset = meta_->offset;
    ...
    if (REVERSE) {
      ...
      for (; t >= t_end; --t) {
        ...
        double current_gain = GetSplitGains<USE_MC, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
            ...
            constraints, meta_->monotone_type, meta_->config->path_smooth, left_count, right_count,
            parent_output);
        // gain with split is worse than without split
      ...
      for (; t <= t_end; ++t) {
        ...
        if (t >= 0) {
          ...
          left_count +=
              static_cast<data_size_t>(Common::RoundInt(GET_HESS(data_, t) * cnt_factor));
        }
        // if data not enough, or sum hessian too small
        if (left_count < meta_->config->min_data_in_leaf ||
    ...
    if (is_splittable_ && best_gain > output->gain + min_gain_shift) {
      ...
      output->left_output =
          CalculateSplittedLeafOutput<USE_MC, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
              best_sum_left_gradient, best_sum_left_hessian, meta_->config->lambda_l1,
              meta_->config->lambda_l2, meta_->config->max_delta_step, best_left_constraints,
  ...
  template <bool USE_RAND, bool USE_MC, bool USE_L1, bool USE_MAX_OUTPUT, bool USE_SMOOTHING,
            bool REVERSE, bool SKIP_DEFAULT_BIN, bool NA_AS_MISSING, typename PACKED_HIST_BIN_T,
            typename PACKED_HIST_ACC_T, typename HIST_BIN_T, typename HIST_ACC_T,
  ...
  void FindBestThresholdSequentiallyInt(int64_t int_sum_gradient_and_hessian,
    ...
    if (REVERSE) {
      ...
      for (; t >= t_end; --t) {
        ...
        const uint32_t int_sum_right_hessian =
            HIST_BITS_ACC == 16
        ...
        const PACKED_HIST_ACC_T sum_left_gradient_and_hessian =
            local_int_sum_gradient_and_hessian - sum_right_gradient_and_hessian;
        const uint32_t int_sum_left_hessian =
            HIST_BITS_ACC == 16
        ...
        double sum_right_gradient =
            HIST_BITS_ACC == 16
                ? static_cast<double>(static_cast<int16_t>(sum_right_gradient_and_hessian >> 16)) *
                      grad_scale
                : static_cast<double>(static_cast<int32_t>(
                      static_cast<int64_t>(sum_right_gradient_and_hessian) >> 32)) *
                      grad_scale;
        double sum_left_gradient =
            HIST_BITS_ACC == 16
                ? static_cast<double>(static_cast<int16_t>(sum_left_gradient_and_hessian >> 16)) *
                      grad_scale
                : static_cast<double>(static_cast<int32_t>(
                      static_cast<int64_t>(sum_left_gradient_and_hessian) >> 32)) *
                      grad_scale;
        if (USE_RAND) {
          if (t - 1 + offset != rand_threshold) {
            continue;
        ...
        double current_gain = GetSplitGains<USE_MC, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
            ...
            sum_right_hessian + kEpsilon, meta_->config->lambda_l1, meta_->config->lambda_l2,
            meta_->config->max_delta_step, constraints, meta_->monotone_type,
      ...
      for (; t <= t_end; ++t) {
        ...
        const data_size_t left_count =
            Common::RoundInt(static_cast<double>(int_sum_left_hessian) * cnt_factor);
        ...
        const PACKED_HIST_ACC_T sum_right_gradient_and_hessian =
            local_int_sum_gradient_and_hessian - sum_left_gradient_and_hessian;
        const uint32_t int_sum_right_hessian =
            HIST_BITS_ACC == 16
                ? static_cast<uint32_t>(sum_right_gradient_and_hessian & 0x0000ffff)
                : static_cast<uint32_t>(sum_right_gradient_and_hessian & 0x00000000ffffffff);
    ...
    if (is_splittable_ && best_gain > output->gain + min_gain_shift) {
      const int32_t int_best_sum_left_gradient =
          HIST_BITS_ACC == 16
              ...
              : static_cast<int32_t>(static_cast<int64_t>(best_sum_left_gradient_and_hessian) >>
                                     32);
      ...
      const double best_sum_left_gradient =
          static_cast<double>(int_best_sum_left_gradient) * grad_scale;
      const double best_sum_left_hessian =
          static_cast<double>(int_best_sum_left_hessian) * hess_scale;
      ...
      const int64_t best_sum_right_gradient_and_hessian =
      ...
      const double best_sum_right_gradient =
          static_cast<double>(int_best_sum_right_gradient) * grad_scale;
      const double best_sum_right_hessian =
          static_cast<double>(int_best_sum_right_hessian) * hess_scale;
      ...
      output->right_output =
          CalculateSplittedLeafOutput<USE_MC, USE_L1, USE_MAX_OUTPUT, USE_SMOOTHING>(
              best_sum_right_gradient, best_sum_right_hessian, meta_->config->lambda_l1,
              meta_->config->lambda_l2, meta_->config->max_delta_step, best_right_constraints,
  ...
  std::function<void(double, double, data_size_t, const FeatureConstraint*, double, SplitInfo*)>
      find_best_threshold_fun_;

  std::function<void(int64_t, double, double, const uint8_t, const uint8_t, data_size_t,
                     const FeatureConstraint*, double, SplitInfo*)>
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (int i = old_cache_size; i < cache_size; ++i) {
        ...
        for (int j = 0; j < train_data->num_features(); ++j) {
          ...
          pool_[i][j].Init(data_[i].data() + offsets[j], data_ptr + 2 * offsets[j],
                           &feature_metas_[j]);
        }
        OMP_LOOP_EX_END();
      }

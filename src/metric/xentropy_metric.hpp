...
inline static double XentLambdaLoss(label_t label, label_t weight,
                                    double hhat) {
  return XentLoss(label, 1.0f - std::exp(-weight * hhat));
}

// Computes the (negative) entropy for label p; p should be in interval [0, 1];
// This is used to presum the KL-divergence offset term (to be _added_ to the
// cross-entropy loss). NOTE: x*log(x) = 0 for x=0,1; so only add when in (0,
// 1); avoid log(0)*0
 ...
 public:
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    Common::CheckElementsIntervalClosed<label_t>(label_, 0.0f, 1.0f, num_data_,
                                                 GetName()[0].c_str());
    Log::Info("[%s:%s]: (metric) labels passed interval [0, 1] check",
              GetName()[0].c_str(), __func__);
    ...
    if (weights_ == nullptr) {
      ...
      Common::ObtainMinMaxSum(weights_, num_data_, &minw,
                              static_cast<label_t*>(nullptr), &sum_weights_);
      if (minw < 0.0f) {
        Log::Fatal("[%s:%s]: (metric) weights not allowed to be negative",
                   GetName()[0].c_str(), __func__);
      }
    }
    ...
    if (sum_weights_ <= 0.0f) {
      Log::Fatal("[%s:%s]: sum-of-weights = %f is non-positive", __func__,
                 GetName()[0].c_str(), sum_weights_);
    }
    Log::Info("[%s:%s]: sum-of-weights = %f", GetName()[0].c_str(), __func__,
              sum_weights_);
  }
...
#pragma omp parallel for schedule(static) reduction(+ : sum_loss)
        for (data_size_t i = 0; i < num_data_; ++i) {
          sum_loss += XentLoss(
              label_[i],
              score[i]);  // NOTE: does not work unless score is a probability
        }
      } else {
#pragma omp parallel for schedule(static) reduction(+ : sum_loss)
        for (data_size_t i = 0; i < num_data_; ++i) {
          sum_loss +=
              XentLoss(label_[i], score[i]) *
...
#pragma omp parallel for schedule(static) reduction(+ : sum_loss)
  ...
  double factor_to_bigger_better() const override {
    return -1.0f;  // negative means smaller loss is better, positive means
                   // larger loss is better
  }
...
//
// CrossEntropyLambdaMetric : "xentlambda" : (optional) weights have a different
// meaning than for "xentropy" ATTENTION: Supposed to be used when the objective
// also is "xentlambda"
//
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    if (weights_ != nullptr) {
      ...
      Common::ObtainMinMaxSum(weights_, num_data_, &minw,
                              ...
                              static_cast<label_t*>(nullptr));
      if (minw <= 0.0f) {
        Log::Fatal("[%s:%s]: (metric) all weights must be positive",
                   GetName()[0].c_str(), __func__);
      }
    }
  }
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    if (weights_ == nullptr) {
      ...
      Common::ObtainMinMaxSum(weights_, num_data_, &minw,
                              static_cast<label_t*>(nullptr), &sum_weights_);
      if (minw < 0.0f) {
        Log::Fatal("[%s:%s]: (metric) at least one weight is negative",
                   GetName()[0].c_str(), __func__);
      }
    }
    ...
    // check weight sum
    if (sum_weights_ <= 0.0f) {
      Log::Fatal("[%s:%s]: sum-of-weights = %f is non-positive",
                 GetName()[0].c_str(), __func__, sum_weights_);
    }
    ...
    Log::Info("%s offset term = %f", GetName()[0].c_str(),

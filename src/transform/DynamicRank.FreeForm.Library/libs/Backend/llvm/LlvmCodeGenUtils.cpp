...
void LLVMRetFailed(const char *p_file, unsigned int p_line,
                   const char *p_description) {
  std::ostringstream err;
  err << "Received NULL " << p_description << " from LLVM at " << p_file << ":"
      << p_line;
  throw std::runtime_error(err.str());
}

std::string NameBlock(const char *p_description, const char *p_block) {
...
llvm::Value *ConvertToBool(llvm::Value *p_value, const TypeImpl &p_sourceType,
...
llvm::Value *ConvertToInt(llvm::Value *p_value, const TypeImpl &p_sourceType,
  ...
  if (p_sourceType.IsIntegerType()) {
    ...
    // conversion.
    if (destType.getPrimitiveSizeInBits() ==
        sourceType.getPrimitiveSizeInBits()) {
      return p_value;
...
// conversion.
llvm::Value *ConvertToFloat(llvm::Value *p_value, const TypeImpl &p_sourceType,
                            const TypeImpl &p_destType,
                            CompilationState &p_state) {
  llvm::Type &destType = p_state.GetType(p_destType);
  const llvm::Type &sourceType = p_state.GetType(p_sourceType);
...
FreeForm2::GenerateConditional::GenerateConditional(CompilationState &p_state,
    ...
    : m_state(&p_state),
      ...
      m_then(llvm::BasicBlock::Create(
          p_state.GetContext(), llvm::Twine(NameBlock(p_description, "then")),
          p_state.GetBuilder().GetInsertBlock()->getParent())),
      m_else(llvm::BasicBlock::Create(
          p_state.GetContext(), llvm::Twine(NameBlock(p_description, "else")))),
      m_finalBlock(llvm::BasicBlock::Create(
          p_state.GetContext(),
...
void FreeForm2::GenerateConditional::FinishThen(llvm::Value *p_value) {
...
void FreeForm2::GenerateConditional::FinishElse(llvm::Value *p_value) {
  CHECK_LLVM_RET(p_value);
...
llvm::Value &FreeForm2::GenerateConditional::Finish(llvm::Type &p_type) {
...
llvm::Value &FreeForm2::ValueConversion::Do(llvm::Value &p_value,
...
FreeForm2::GenerateConditional::Case::Case(llvm::BasicBlock *p_block)
    : m_block(p_block), m_value(NULL) {
  CHECK_LLVM_RET(p_block);
}

void FreeForm2::CheckLLVMRet(const llvm::Value *p_value, const char *p_file,
                             unsigned int p_line) {
  if (p_value == NULL) {
...
void FreeForm2::CheckLLVMRet(const llvm::Type *p_type, const char *p_file,
                             unsigned int p_line) {
  if (p_type == NULL) {
...
void FreeForm2::CheckLLVMRet(const llvm::BasicBlock *p_block,
                             const char *p_file, unsigned int p_line) {
  if (p_block == NULL) {
    LLVMRetFailed(p_file, p_line, "basic block");
  }
}

void FreeForm2::CheckLLVMRet(const llvm::Instruction *p_ins, const char *p_file,
                             unsigned int p_line) {
  if (p_ins == NULL) {

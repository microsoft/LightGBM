...
VotingParallelTreeLearner<TREELEARNER_T>::VotingParallelTreeLearner(
    const Config* config)
...
void VotingParallelTreeLearner<TREELEARNER_T>::Init(const Dataset* train_data,
  ...
  size_t buffer_size = 2 * top_k_ *
                       std::max(max_bin * KHistEntrySize,
                                sizeof(LightSplitInfo) * num_machines_);
  ...
  smaller_leaf_splits_global_.reset(
      new LeafSplits(this->train_data_->num_data()));
  larger_leaf_splits_global_.reset(
      new LeafSplits(this->train_data_->num_data()));
  ...
  smaller_leaf_histogram_array_global_.reset(
      new FeatureHistogram[this->num_features_]);
  larger_leaf_histogram_array_global_.reset(
      new FeatureHistogram[this->num_features_]);
...
#pragma omp parallel for schedule(static)
  for (int i = 0; i < train_data->num_features(); ++i) {
    ...
    feature_metas_[i].default_bin =
        train_data->FeatureBinMapper(i)->GetDefaultBin();
    feature_metas_[i].missing_type =
  ...
  for (int j = 0; j < train_data->num_features(); ++j) {
    ...
    smaller_leaf_histogram_array_global_[j].Init(
        smaller_leaf_histogram_data_.data() + offset, &feature_metas_[j]);
    larger_leaf_histogram_array_global_[j].Init(
        larger_leaf_histogram_data_.data() + offset, &feature_metas_[j]);
...
template <typename TREELEARNER_T>
void VotingParallelTreeLearner<TREELEARNER_T>::ResetConfig(
    const Config* config) {
...
void VotingParallelTreeLearner<TREELEARNER_T>::BeforeTrain() {
  ...
  std::tuple<data_size_t, double, double> data(
  ...
  Network::Allreduce(
      ...
      sizeof(std::tuple<data_size_t, double, double>), output_buffer_.data(),
      [](const char* src, char* dst, int type_size, comm_size_t len) {
        comm_size_t used_size = 0;
        const std::tuple<data_size_t, double, double>* p1;
        std::tuple<data_size_t, double, double>* p2;
        while (used_size < len) {
          p1 = reinterpret_cast<const std::tuple<data_size_t, double, double>*>(
              src);
...
template <typename TREELEARNER_T>
bool VotingParallelTreeLearner<TREELEARNER_T>::BeforeFindBestSplit(
    const Tree* tree, int left_leaf, int right_leaf) {
...
template <typename TREELEARNER_T>
void VotingParallelTreeLearner<TREELEARNER_T>::GlobalVoting(
    int leaf_idx, const std::vector<LightSplitInfo>& splits,
    std::vector<int>* out) {
  out->clear();
  if (leaf_idx < 0) {
    return;
  ...
  score_t mean_num_data =
      GetGlobalDataCountInLeaf(leaf_idx) / static_cast<score_t>(num_machines_);
  std::vector<LightSplitInfo> feature_best_split(
      this->train_data_->num_total_features(), LightSplitInfo());
  for (auto& split : splits) {
    ...
    // weighted gain
    double gain =
        split.gain * (split.left_count + split.right_count) / mean_num_data;
  ...
  std::stable_sort(top_k_splits.begin(), top_k_splits.end(),
                   std::greater<LightSplitInfo>());
...
template <typename TREELEARNER_T>
void VotingParallelTreeLearner<TREELEARNER_T>::CopyLocalHistogram(
    const std::vector<int>& smaller_top_features,
    const std::vector<int>& larger_top_features) {
  for (int i = 0; i < this->num_features_; ++i) {
    smaller_is_feature_aggregated_[i] = false;
    larger_is_feature_aggregated_[i] = false;
  }
  size_t total_num_features =
      smaller_top_features.size() + larger_top_features.size();
  size_t average_feature =
      (total_num_features + num_machines_ - 1) / num_machines_;
  ...
  for (int i = 0; i < num_machines_; ++i) {
    ...
    size_t cur_total_feature =
        std::min(average_feature, total_num_features - used_num_features);
    // copy histograms.
    while (cur_used_features < cur_total_feature) {
      // copy smaller leaf histograms first
      if (smaller_idx < smaller_top_features.size()) {
        int inner_feature_index = this->train_data_->InnerFeatureIndex(
            smaller_top_features[smaller_idx]);
        ++cur_used_features;
        // mark local aggregated feature
        if (i == rank_) {
          smaller_is_feature_aggregated_[inner_feature_index] = true;
          smaller_buffer_read_start_pos_[inner_feature_index] =
              static_cast<int>(cur_size);
        }
        // copy
        std::memcpy(
            input_buffer_.data() + reduce_scatter_size_,
            this->smaller_leaf_histogram_array_[inner_feature_index].RawData(),
            this->smaller_leaf_histogram_array_[inner_feature_index]
                .SizeOfHistgram());
        cur_size += this->smaller_leaf_histogram_array_[inner_feature_index]
                        .SizeOfHistgram();
      ...
      if (larger_idx < larger_top_features.size()) {
        int inner_feature_index = this->train_data_->InnerFeatureIndex(
            larger_top_features[larger_idx]);
        ++cur_used_features;
        // mark local aggregated feature
        if (i == rank_) {
          larger_is_feature_aggregated_[inner_feature_index] = true;
          larger_buffer_read_start_pos_[inner_feature_index] =
              static_cast<int>(cur_size);
        }
        // copy
        std::memcpy(
            ...
            this->larger_leaf_histogram_array_[inner_feature_index]
                .SizeOfHistgram());
        cur_size += this->larger_leaf_histogram_array_[inner_feature_index]
                        .SizeOfHistgram();
        reduce_scatter_size_ +=
            this->larger_leaf_histogram_array_[inner_feature_index]
                .SizeOfHistgram();
...
#pragma omp parallel for schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_;
       ++feature_index) {
    ...
    if (this->parent_leaf_histogram_array_ != nullptr &&
      ...
      this->smaller_leaf_histogram_array_[feature_index].set_is_splittable(
          false);
      continue;
    }
...
#pragma omp parallel for schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_;
    ...
    const int real_feature_index =
        this->train_data_->RealFeatureIndex(feature_index);
    ...
    if (this->larger_leaf_splits_ == nullptr ||
        this->larger_leaf_splits_->LeafIndex() < 0) {
      continue;
    }
    ...
    if (use_subtract) {
      ...
      this->train_data_->FixHistogram(
          feature_index, this->larger_leaf_splits_->sum_gradients(),
  ...
  // local voting
  ArrayArgs<SplitInfo>::MaxK(smaller_bestsplit_per_features, top_k_,
                             &smaller_top_k_splits);
  ArrayArgs<SplitInfo>::MaxK(larger_bestsplit_per_features, top_k_,
                             &larger_top_k_splits);
  ...
  int offset = 0;
  for (int i = 0; i < top_k_; ++i) {
    std::memcpy(input_buffer_.data() + offset, &smaller_top_k_light_splits[i],
                sizeof(LightSplitInfo));
    offset += sizeof(LightSplitInfo);
    std::memcpy(input_buffer_.data() + offset, &larger_top_k_light_splits[i],
                sizeof(LightSplitInfo));
    offset += sizeof(LightSplitInfo);
  }
  ...
  GlobalVoting(this->smaller_leaf_splits_->LeafIndex(),
               smaller_top_k_splits_global, &smaller_top_features);
  GlobalVoting(this->larger_leaf_splits_->LeafIndex(),
               larger_top_k_splits_global, &larger_top_features);
  // copy local histgrams to buffer
  CopyLocalHistogram(smaller_top_features, larger_top_features);

  // Reduce scatter for histogram
  Network::ReduceScatter(
      input_buffer_.data(), reduce_scatter_size_, sizeof(hist_t),
...
#pragma omp parallel for schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_;
    ...
    if (smaller_is_feature_aggregated_[feature_index]) {
      ...
      smaller_leaf_histogram_array_global_[feature_index].FromMemory(
          output_buffer_.data() +
          smaller_buffer_read_start_pos_[feature_index]);

      this->train_data_->FixHistogram(
          feature_index, smaller_leaf_splits_global_->sum_gradients(),
      ...
      smaller_leaf_histogram_array_global_[feature_index].FindBestThreshold(
          ...
          smaller_leaf_splits_global_->max_constraint(), &smaller_split);
    ...
    if (larger_is_feature_aggregated_[feature_index]) {
      ...
      this->train_data_->FixHistogram(
          feature_index, larger_leaf_splits_global_->sum_gradients(),
      ...
      larger_leaf_histogram_array_global_[feature_index].FindBestThreshold(
          ...
          larger_leaf_splits_global_->max_constraint(), &larger_split);
  ...
  auto smaller_best_idx =
      ArrayArgs<SplitInfo>::ArgMax(smaller_bests_per_thread);
  ...
  smaller_best_split =
      this->best_split_per_leaf_[this->smaller_leaf_splits_->LeafIndex()];
  // find local best split for larger leaf
  if (this->larger_leaf_splits_->LeafIndex() >= 0) {
    larger_best_split =
        this->best_split_per_leaf_[this->larger_leaf_splits_->LeafIndex()];
  }
  // sync global best info
  SyncUpGlobalBestSplit(input_buffer_.data(), input_buffer_.data(),
                        &smaller_best_split, &larger_best_split,
                        this->config_->max_cat_threshold);

  // copy back
  this->best_split_per_leaf_[smaller_leaf_splits_global_->LeafIndex()] =
      smaller_best_split;
  if (larger_best_split.feature >= 0 &&
      larger_leaf_splits_global_->LeafIndex() >= 0) {
    this->best_split_per_leaf_[larger_leaf_splits_global_->LeafIndex()] =
        larger_best_split;
  }
}

template <typename TREELEARNER_T>
void VotingParallelTreeLearner<TREELEARNER_T>::Split(Tree* tree, int best_Leaf,
                                                     int* left_leaf,
                                                     int* right_leaf) {
  ...
  const int inner_feature_index =
      this->train_data_->InnerFeatureIndex(best_split_info.feature);
  bool is_numerical_split =
      this->train_data_->FeatureBinMapper(inner_feature_index)->bin_type() ==
      BinType::NumericalBin;
  p_left->SetValueConstraint(best_split_info.min_constraint,
                             best_split_info.max_constraint);
  p_right->SetValueConstraint(best_split_info.min_constraint,
                              best_split_info.max_constraint);
  if (is_numerical_split) {
    double mid =
        (best_split_info.left_output + best_split_info.right_output) / 2.0f;

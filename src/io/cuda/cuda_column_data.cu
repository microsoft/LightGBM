...
__global__ void CopySubrowKernel_ColumnData(
  ...
  if (bit_type == 8) {
    ...
    uint8_t* true_out_column_data = reinterpret_cast<const uint8_t*>(out_column_data);
    ...
    uint16_t* true_out_column_data = reinterpret_cast<const uint16_t*>(out_column_data);
    for (data_size_t local_data_index = local_data_index_start; local_data_index < num_used_indices; local_data_index += static_cast<data_size_t>(blockDim.x)) {
      const data_size_t global_data_index = cuda_used_indices[local_data_index];
      true_out_column_data[local_data_index] = true_in_column_data[global_data_index];
    }
    ...
    uint32_t* true_out_column_data = reinterpret_cast<const uint32_t*>(out_column_data);
    for (data_size_t local_data_index = local_data_index_start; local_data_index < num_used_indices; local_data_index += static_cast<data_size_t>(blockDim.x)) {
      const data_size_t global_data_index = cuda_used_indices[local_data_index];
      true_out_column_data[local_data_index] = true_in_column_data[global_data_index];
    }
...
void CUDAColumnData::LanchCopySubrowKernel(void* const* in_cuda_data_by_column, const data_size_t num_used_indices) {

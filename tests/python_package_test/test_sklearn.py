...
class TestSklearn(unittest.TestCase):
    ...
    def test_multiclass(self):
        ...
        self.assertAlmostEqual(
            ret,
            gbm.evals_result_["valid_0"]["multi_logloss"][gbm.best_iteration_ - 1],
            places=5,
        )
    ...
    def test_xendcg(self):
        ...
        q_test = np.loadtxt(
            os.path.join(dir_path, "../../examples/xendcg/rank.test.query")
        )
        gbm = lgb.LGBMRanker(
    ...
    def test_grid_search(self):
        ...
        self.assertLess(
            grid.best_estimator_.best_score_["valid_0"]["multi_logloss"], 0.25
        )
    ...
    def test_random_state_object(self):
        ...
        self.assertIs(clf2.random_state, state2)
        self.assertRaises(
            AssertionError, np.testing.assert_allclose, y_pred1, y_pred1_refit
        )
    ...
    def test_sklearn_integration(self):
        ...
        for name, estimator in (
            (lgb.sklearn.LGBMClassifier.__name__, lgb.sklearn.LGBMClassifier),
            ...
            for check in _yield_all_checks(name, estimator):
                check_name = (
                    check.func.__name__ if hasattr(check, "func") else check.__name__
                )
    ...
    def test_pandas_categorical(self):
        ...
        gbm6 = lgb.sklearn.LGBMClassifier(n_estimators=10).fit(
            X, y, categorical_feature=[]
        )
        ...
        self.assertRaises(
            AssertionError, np.testing.assert_allclose, pred0, pred5
    ...
    def test_predict(self):
        ...
        self.assertRaises(
            AssertionError, np.testing.assert_allclose, res_engine, res_sklearn_params
        )
    ...
    def test_metrics(self):
        ...
        gbm = lgb.LGBMRegressor(
            objective="regression_l1", metric="gamma", **params
        ...
        gbm = lgb.LGBMRegressor(
            objective=custom_dummy_obj, metric="mape", **params
        ).fit(**params_fit)
        self.assertEqual(len(gbm.evals_result_["training"]), 1)
        self.assertIn("mape", gbm.evals_result_["training"])
        ...
        # no metric
        gbm = lgb.LGBMRegressor(
            objective=custom_dummy_obj, metric="None", **params
        ).fit(**params_fit)
        self.assertIs(gbm.evals_result_, None)
    ...
    def test_first_metric_only(self):
        def fit_and_check(
            ...
            for eval_set_name in eval_set_names:
                ...
                self.assertEqual(
                    len(gbm.evals_result_[eval_set_name]), len(metric_names)
                )
    ...
    def test_class_weight(self):
        ...
        gbm_str = lgb.LGBMClassifier(
            n_estimators=10, class_weight="balanced", silent=True
        )
        gbm_str.fit(
            X_train,
            y_train_str,
        ...
        for eval_set1, eval_set2 in itertools.combinations(
            gbm_str.evals_result_.keys(), 2
        ):
    ...
    def test_continue_training_with_model(self):
        ...
        self.assertEqual(
            len(init_gbm.evals_result_["valid_0"]["multi_logloss"]),

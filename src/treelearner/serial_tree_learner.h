...
class SerialTreeLearner : public TreeLearner {
 public:
  ...
  virtual void ResetTrainingDataInner(const Dataset* train_data, bool is_constant_hessian,
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static, 1)
 ...
 protected:
  void ComputeBestSplitForFeature(FeatureHistogram* histogram_array_, int feature_index,
                                  int real_fidx, int8_t is_feature_used, int num_data,
                                  const LeafSplits* leaf_splits, SplitInfo* best_split,
  ...
  virtual void FindBestSplitsFromHistograms(const std::vector<int8_t>& is_feature_used,
                                            bool use_subtract, const Tree*);
   ...
   */
  inline virtual void Split(Tree* tree, int best_leaf, int* left_leaf, int* right_leaf) {
  ...
  int32_t ForceSplits(Tree* tree, int* left_leaf, int* right_leaf, int* cur_depth);

  std::set<int> FindAllForceFeatures(Json force_split_leaf_setting);

#ifdef DEBUG
  void CheckSplit(const SplitInfo& best_split_info, const int left_leaf_index,

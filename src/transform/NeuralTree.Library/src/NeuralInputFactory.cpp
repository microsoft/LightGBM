...
namespace
{
    ...
    NeuralInput* 
    LoadTanh(NeuralInputFactory::IConfiguration& p_config,
    ...
    NeuralInput* 
    LoadLinear(NeuralInputFactory::IConfiguration& p_config,
               const char* p_section,
               IFeatureMap& p_featureMap)
    {
    ...
    NeuralInput* 
    LoadLogLinear(NeuralInputFactory::IConfiguration& p_config,
                  const char* p_section,
                  IFeatureMap& p_featureMap)
    {
    ...
    NeuralInput* 
    LoadRational(NeuralInputFactory::IConfiguration& p_config,
                 const char* p_section,
                 IFeatureMap& p_featureMap)
    {
    ...
    // Sahred base loader for the inputs defined in neuraltree.library, not including freeform2.
    // (Not sure about aggregatedfreeform now)
    // 
    ...
    public:
        virtual NeuralInput* FromBond(const UnionBondInput& p_data) const
        {
            ...
            else if ("tanh" == p_data.m_inputType)
            {
                ...
                {
                    Log::Fatal(" p_data.m_tanh.value().m_neuralInputTanhUnaryBondData doesn't has value");
                    UnaryPtr unary(new NeuralInputTanhUnary(p_data.m_tanh.value().m_neuralInputTanhUnaryBondData.value()));
                    if (unary.get())
                    {
                        return NeuralInputCached::Load(1024, unary.release());
                    }
                ...
                {
                    Log::Fatal(" p_data.m_tanh.value().m_neuralInputTanhBondData doesn't has value");
                    return new NeuralInputTanh(p_data.m_tanh.value().m_neuralInputTanhBondData.value());
                }    
            }
    ...
    {
    public:
        FunctionLoader(NeuralInputFactory::LoadFunction p_fun)
            : m_fun(p_fun)
        {}

        virtual NeuralInput* 
        operator()(NeuralInputFactory::IConfiguration& p_config, 
                   const char* p_section,
    ...
    public:
        ...
        typedef NeuralInputSubtype* (*MetaStreamFunction)(const IConfiguration& p_config,
                                                          const char* p_section,
                                                          IFeatureMap* p_featureMap);

        explicit MetaStreamFunctionLoader(MetaStreamFunction p_fun)
            : m_fun(p_fun)
        {}

        virtual NeuralInput* 
        operator()(const IConfiguration& p_config, 
                   const char* p_section,
                   IFeatureMap& p_featureMap) const
        {
            return m_fun(p_config, p_section, &p_featureMap);
...
void 
NeuralInputFactory::AddTransform(const char* p_transform, LoadFunction p_loader, bool p_replace)
{
    if (p_loader == NULL)
    {
...
void 
NeuralInputFactory::AddTransform(const char* p_transform, Loader::Ptr p_loader, bool p_replace)
{
...
void 
NeuralInputFactory::ClearTransforms()
{
    m_transform.clear();
}
...
NeuralInput* 
NeuralInputFactory::Load(const char* p_transform, 
                         NeuralInputFactory::IConfiguration& p_config,
                         const char* p_section,
                         IFeatureMap& p_featureMap) const
{
    std::string transform(p_transform);
...
NeuralInput* 
NeuralInputFactory::Load(NeuralInputFactory::IConfiguration& p_config,
                         int p_ID,
                         IFeatureMap& p_featureMap) const
{
    char szSection[20];
    if (_snprintf_s(szSection, sizeof(szSection), _TRUNCATE, "Input:%d", p_ID) == -1)
    {
    ...
    if (!p_config.GetStringParameter(szSection, 
                                     "Transform", 
                                     szTransform, 
                                     sizeof(szTransform)))
    {
...
NeuralInput* 
NeuralInputFactory::FromBond(const UnionBondInput& p_data) const
{

...
FreeForm2::FeatureSpecExpression::FeatureName::FeatureName(const std::string& p_name)
    : m_name(p_name),
      m_isParameterized(false)
{
}
...
FreeForm2::FeatureSpecExpression::FeatureName::Parse(const std::string& p_name,
    ...
    else
    {
        ...
        if (p_parameterization.size() > 0)
        {
            try
            {
                ...
                auto paramRange = boost::make_iterator_range(params.cbegin() + 1, params.cend() - 1);
            
                std::vector<std::string> values;
                for (auto iter = boost::make_split_iterator(paramRange, boost::first_finder(",")); 
                     !iter.eof(); 
                     ++iter)
                {
                    ...
                    boost::split(values, range, boost::bind(&boost::is_equal::operator()<char, char>,
                                                          boost::is_equal(),
                                                          _1,
                                                          '='));
            ...
            catch (...)
            {
                ...
                err << "Unable to parse parameterization " << p_parameterization
                    << "; expected format is \"<parameter_name>=<parameter_value>,...\"";
...
const std::string&
FreeForm2::FeatureSpecExpression::FeatureName::GetName() const
{
    return m_name;
}
...
bool
FreeForm2::FeatureSpecExpression::FeatureName::IsParameterized() const
{
...
bool
FreeForm2::FeatureSpecExpression::FeatureName::operator==(const FeatureName& p_other) const
{
...
std::ostream&
operator<<(std::ostream& p_out, const FreeForm2::FeatureSpecExpression::FeatureName& p_name)
{
...
bool
FreeForm2::FeatureSpecExpression::IgnoreParameterLess::operator()(
    const FeatureName& p_left,
...
void
FreeForm2::FeatureSpecExpression::Accept(Visitor& p_visitor) const
{
...
FreeForm2::FeatureSpecExpression::GetType() const
{
    if (m_returnsValue)
    {
...
size_t
FreeForm2::FeatureSpecExpression::GetNumChildren() const
{
    return 1;
}
...
FreeForm2::FeatureSpecExpression::GetBody() const
{
    return m_body;
}
...
bool
FreeForm2::FeatureSpecExpression::IsDerived() const
{
    return m_featureSpecType == DerivedFeature || m_featureSpecType == AggregatedDerivedFeature;
}
...
FreeForm2::ImportFeatureExpression::ImportFeatureExpression(
    ...
    : Expression(p_annotations),
      ...
      m_type(p_typeManager.GetArrayType(
          ...
          true,
          static_cast<unsigned int>(p_dimensions.size()), 
          &p_dimensions[0], 
          std::accumulate(p_dimensions.begin(), p_dimensions.end(), 1u, std::multiplies<UInt32>()))),
      m_id(p_id)
{
}
...
FreeForm2::ImportFeatureExpression::ImportFeatureExpression(
    ...
    const FreeForm2::FeatureSpecExpression::FeatureName& p_featureName,
    VariableID p_id)
    : Expression(Annotations(p_annotations.m_sourceLocation, ValueBounds(TypeImpl::GetUInt32Instance(true)))),
      m_featureName(p_featureName),
...
void
FreeForm2::ImportFeatureExpression::Accept(Visitor& p_visitor) const
{
...
const FreeForm2::TypeImpl&
FreeForm2::ImportFeatureExpression::GetType() const
{
    return m_type;
}
...
size_t
FreeForm2::ImportFeatureExpression::GetNumChildren() const
{
    return 0;
}
...
FreeForm2::ImportFeatureExpression::GetId() const
{
    return m_id;
}


FreeForm2::FeatureGroupSpecExpression::FeatureGroupSpecExpression(const Annotations& p_annotations,
      ...
      m_metaStreamName(p_metaStreamName)
{
    ...
    BOOST_FOREACH(const FeatureSpecExpression* featureSpec, p_featureSpecs)
    {
        ...
        {
            std::ostringstream err;
            err << "All feature specifications within feature group '" << p_name << "' must be of the same type.";
            throw ParseError(err.str(), GetSourceLocation());
        }
    }
}
...
void
FreeForm2::FeatureGroupSpecExpression::Accept(Visitor& p_visitor) const
{
    if (!p_visitor.AlternativeVisit(*this))
    {
        for(std::vector<const FeatureSpecExpression*>::const_iterator specExpression = m_featureSpecs.begin();
            specExpression != m_featureSpecs.end();
            ++specExpression)
        {
...
const FreeForm2::TypeImpl& 
FreeForm2::FeatureGroupSpecExpression::GetType() const
{
...
size_t
FreeForm2::FeatureGroupSpecExpression::GetNumChildren() const
{
...
const std::string&
FreeForm2::FeatureGroupSpecExpression::GetName() const
{
    return m_name;
}
...
bool
FreeForm2::FeatureGroupSpecExpression::IsExtendedExperimental() const
{
...
bool
FreeForm2::FeatureGroupSpecExpression::IsSmallExperimental() const
{
...
bool
FreeForm2::FeatureGroupSpecExpression::IsBlockLevelFeature() const
{
...
bool
FreeForm2::FeatureGroupSpecExpression::IsBodyBlockFeature() const
{
...
bool
FreeForm2::FeatureGroupSpecExpression::IsForwardIndexFeature() const
{
...
const std::string&
FreeForm2::FeatureGroupSpecExpression::GetMetaStreamName() const
{
...
bool
FreeForm2::FeatureGroupSpecExpression::IsPerStream() const
{
    return !(m_featureSpecType == FeatureSpecExpression::AggregatedDerivedFeature 
             || m_featureSpecType == FeatureSpecExpression::AbInitioFeature
             || !m_metaStreamName.empty()
             || m_isBodyBlockFeature);
}


FreeForm2::AggregateContextExpression::AggregateContextExpression(const Annotations& p_annotations,
                                                                  const Expression& p_body)
...
void
FreeForm2::AggregateContextExpression::Accept(Visitor& p_visitor) const
{
    const size_t stackSize = p_visitor.StackSize();
...
const FreeForm2::TypeImpl&
FreeForm2::AggregateContextExpression::GetType() const
{
    return TypeImpl::GetVoidInstance();
}
...
size_t
FreeForm2::AggregateContextExpression::GetNumChildren() const
{
    return 1;
}
...
const FreeForm2::Expression&
FreeForm2::AggregateContextExpression::GetBody() const
{
    return m_body;
}

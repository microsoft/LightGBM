...
class Network {
 public:
   ...
   * \brief Initialize
   */
  static void Init(int num_machines, int rank, ReduceScatterFunction reduce_scatter_ext_fun,
  ...
  */
  static void Allreduce(char* input, comm_size_t input_size, int type_size, char* output,
                        const ReduceFunction& reducer);

  /*!
   * \brief Perform all_reduce by using all_gather. it can be use to reduce communication time when
   * data is small
   * \param input Input data
   * \param input_size The size of input data
   * \param type_size The size of one object in the reduce function
   ...
   * \param reducer Reduce function
   */
  static void AllreduceByAllGather(char* input, comm_size_t input_size, int type_size,
                                   char* output, const ReduceFunction& reducer);

  /*!
  * \brief Performing all_gather by using Bruck algorithm.
           Communication times is O(log(n)), and communication cost is O(send_size *
  number_machine)
  ...
  */
  static void Allgather(char* input, const comm_size_t* block_start, const comm_size_t* block_len,
  ...
  */
  ...
  static std::vector<T> GlobalArray(T local) {
    ...
    Allgather(reinterpret_cast<char*>(&local), block_start.data(), block_len.data(),
              reinterpret_cast<char*>(global.data()), type_size * num_machines_);
    return global;
  }
 ...
 private:
  ...
  static void AllgatherRecursiveDoubling(char* input, const comm_size_t* block_start,

...
FreeForm2::ProgramParseState::ExpressionParseState::ExpressionParseState(
    const ExpressionFactory& p_factory,
    SIZED_STRING p_atom,
...
void 
FreeForm2::ProgramParseState::ExpressionParseState::Add(const Expression& p_expr)
{
...
FreeForm2::ProgramParseState::ExpressionParseState::Finish(SimpleExpressionOwner& p_owner,
                                                           TypeManager& p_typeManager) const
{
    const FreeForm2::Expression& ret = m_factory->Create(*this, p_owner, p_typeManager);

    return ret;
}


FreeForm2::ProgramParseState::ProgramParseState(SIZED_STRING p_input, 
      ...
      m_parsingAggregatedExpression(p_parsingAggregatedExpression)
{
   ...
   // to handle that.
   const ExpressionFactory* rootFactory = &GetFeatureSpecInstance(p_mustProduceFloat);
   ExpressionParseState initialState(*rootFactory, CStackSizedString("<initial>"), 0);
...
FreeForm2::ProgramParseState::Finish()
{
    ...
    // Use the FunctionInlineVisitor to replace FunctionCallExpressions with LetExpressions.
    // This needs to be done now since the parsed expression is then
    // wrapped with a FeatureSpecExpression that uses the type of the parsed expression.
    boost::shared_ptr<SimpleExpressionOwner> functionVisitorOwner(new SimpleExpressionOwner());
    boost::shared_ptr<TypeManager> functionVisitorTypeManager(TypeManager::CreateTypeManager().release());
    FunctionInlineVisitor functionInlineVisitor(functionVisitorOwner, 
    ...
    // The FunctionInlineVisitor may assign variable ids to ensure unique variable ids for each
    // function call. Update the id of the ProgramParseState to ensure unique variable ids.
    ...
    return boost::tuples::make_tuple(&root, 
...
FreeForm2::ProgramParseState::GetNextVariableId()
{
...
const FreeForm2::Expression& 
FreeForm2::ProgramParseState::GetLastParsed() const
{
...
void
FreeForm2::ProgramParseState::CloseExpression()
{
    const Expression& finished = m_parseStack.back().Finish(*m_owner, *m_typeManager);
    m_parseStack.pop_back();

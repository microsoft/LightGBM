 ...
 * track of number of inserted elements, but is not thread-safe (this is ok as usually input is a
 * streaming iterator). For parallel input sources the low-level API must be used.
 *
 ...
 *  2. data() or void_data()     # retrieves a T** or void** pointer (useful for
 ...
 *
 *  With those you can generate int32_t sizes[]. Last chunk can be smaller than chunk_size, so, for
 * any i:
...
class ChunkedArray {
 public:
   ...
   */
  size_t get_add_count() const { return _last_chunk_idx * _chunk_size + _last_idx_in_last_chunk; }

  /**
   * @return Number of allocated chunks.
   */
  size_t get_chunks_count() const { return _chunks.size(); }

  /**
   * @return Number of elemends add()'ed in the last chunk.
   */
  size_t get_last_chunk_add_count() const { return _last_idx_in_last_chunk; }
   ...
   */
  T** data() noexcept { return _chunks.data(); }
   ...
   */
  void** data_as_void() noexcept { return reinterpret_cast<void**>(_chunks.data()); }
  ...
  void coalesce_to(T* other, bool all_valid_addresses = false) const {
    ...
    const size_t last_chunk_elems_to_copy =
        all_valid_addresses ? _chunk_size : this->get_last_chunk_add_count();

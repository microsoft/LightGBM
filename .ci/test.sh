...
if [[ $TASK == "lint" ]]; then
    ...
    pycodestyle --ignore=E501,W503 --exclude=./eigen,./.nuget,./external_libs . || exit -1
    pydocstyle --convention=numpy --add-ignore=D105 --match-dir="^(?!^eigen|external_libs|test|example).*" --match="(?!^test_|setup).*\.py" . || exit -1
    echo "Linting R code"

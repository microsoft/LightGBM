...
int 
FreeForm2::Result::Compare(const Result& p_other) const
{
    if (!GetType().GetImplementation().IsSameAs(p_other.GetType().GetImplementation(), true))
    {
        std::ostringstream err;
        err << "Mismatched compare between " 
            << GetType() << " and " << p_other.GetType();
...
void 
FreeForm2::Result::Print(std::ostream& p_out) const
{
    switch (GetType().Primitive())
    {
        case Type::Bool:
...
std::ostream& 
FreeForm2::operator<<(std::ostream& p_out, const Result& p_result)
{
...
int 
FreeForm2::Result::CompareFloat(FloatType p_left, FloatType p_right)
{
...
FreeForm2::ResultIterator::ResultIterator(std::auto_ptr<ResultIteratorImpl> p_impl)
    : m_impl(p_impl)
{
}
...
void 
FreeForm2::ResultIterator::increment()
{
...
void 
FreeForm2::ResultIterator::decrement()
{
...
bool 
FreeForm2::ResultIterator::equal(const ResultIterator& p_other) const
{
...
const Result&
FreeForm2::ResultIterator::dereference() const
{
...
void 
FreeForm2::ResultIterator::advance(std::ptrdiff_t p_distance)
{
...
std::ptrdiff_t 
FreeForm2::ResultIterator::distance_to(const ResultIterator& p_other) const
{

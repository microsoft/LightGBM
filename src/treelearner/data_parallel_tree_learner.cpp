...
void DataParallelTreeLearner<TREELEARNER_T>::BeforeTrain() {
  ...
  for (int i = 0; i < this->train_data_->num_total_features(); ++i) {
    ...
    if (this->is_feature_used_[inner_feature_index]) {
      int cur_min_machine =
          static_cast<int>(ArrayArgs<int>::ArgMin(num_bins_distributed));
      feature_distribution[cur_min_machine].push_back(inner_feature_index);
      auto num_bin = this->train_data_->FeatureNumBin(inner_feature_index);
      if (this->train_data_->FeatureBinMapper(inner_feature_index)
              ->GetMostFreqBin() == 0) {
        num_bin -= 1;
      }
...
#pragma omp parallel for schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_;
       ++feature_index) {
    if ((!this->is_feature_used_.empty() &&
         this->is_feature_used_[feature_index] == false))
      continue;
    // copy to buffer
    std::memcpy(
...
void DataParallelTreeLearner<TREELEARNER_T>::FindBestSplitsFromHistograms(
  ...
  std::vector<SplitInfo> smaller_bests_per_thread(this->num_threads_,
                                                  SplitInfo());
  std::vector<SplitInfo> larger_bests_per_thread(this->num_threads_,
                                                 SplitInfo());
...
#pragma omp parallel for schedule(static)
  for (int feature_index = 0; feature_index < this->num_features_;
    ...
    this->smaller_leaf_histogram_array_[feature_index].FindBestThreshold(
        ...
        this->smaller_leaf_splits_->max_constraint(), &smaller_split);
    ...
    // construct histgroms for large leaf, we init larger leaf as the parent, so
    // we can just subtract the smaller leaf's histograms
    ...
    this->larger_leaf_histogram_array_[feature_index].FindBestThreshold(
        ...
        this->larger_leaf_splits_->max_constraint(), &larger_split);
  ...
  if (this->larger_leaf_splits_ != nullptr &&
    ...
    auto larger_best_idx =
        ArrayArgs<SplitInfo>::ArgMax(larger_bests_per_thread);
...
template <typename TREELEARNER_T>
void DataParallelTreeLearner<TREELEARNER_T>::Split(Tree* tree, int best_Leaf,
                                                   int* left_leaf,
                                                   int* right_leaf) {

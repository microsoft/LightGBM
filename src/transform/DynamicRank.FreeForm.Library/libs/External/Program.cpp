...
boost::tuples::tuple<const Expression *, boost::shared_ptr<ExpressionOwner>,
                     boost::shared_ptr<TypeManager> >
ParseInternal(SIZED_STRING p_input, DynamicRank::IFeatureMap &p_map,
...
boost::tuples::tuple<const Expression *, boost::shared_ptr<ExpressionOwner>,
                     boost::shared_ptr<TypeManager> >
ParseInternal<FreeForm2::Program::sexpression>(
...
boost::tuples::tuple<const Expression *, boost::shared_ptr<ExpressionOwner>,
                     boost::shared_ptr<TypeManager> >
ParseInternal<FreeForm2::Program::aggregatedSExpression>(
    SIZED_STRING p_input, DynamicRank::IFeatureMap &p_map,
    bool p_mustProduceFloat, const ExternalDataManager *p_externalData,
    std::ostream *p_debugOutput) {
...
std::string ConstructParseErrorMessage(const char *p_message,
                                       unsigned int p_line,
                                       unsigned int p_lineChar) {
  std::ostringstream err;
  err << "Parse error: " << p_message << " at line " << p_line << ", char "
      << p_lineChar;
  return err.str();
}
}  // namespace

FreeForm2::SourceLocation::SourceLocation() : m_lineNo(0), m_lineOffset(0) {}

FreeForm2::SourceLocation::SourceLocation(unsigned int p_lineNo,
                                          unsigned int p_lineOffset)
    : m_lineNo(p_lineNo), m_lineOffset(p_lineOffset) {}

FreeForm2::ParseError::ParseError(const std::string &p_message,
                                  const SourceLocation &p_location)
    : runtime_error(ConstructParseErrorMessage(
          p_message.c_str(), p_location.m_lineNo, p_location.m_lineOffset)),
      m_message(p_message),
      m_sourceLocation(p_location) {}

FreeForm2::ParseError::ParseError(const std::exception &p_inner,
                                  const SourceLocation &p_location)
    : runtime_error(ConstructParseErrorMessage(
          p_inner.what(), p_location.m_lineNo, p_location.m_lineOffset)),
      m_message(p_inner.what()),
      m_sourceLocation(p_location) {}

const std::string &FreeForm2::ParseError::GetMessage() const {
  return m_message;
}

const FreeForm2::SourceLocation &FreeForm2::ParseError::GetSourceLocation()
    const {
  return m_sourceLocation;
}

FreeForm2::ProgramImpl::ProgramImpl(
    const Expression &p_exp, boost::shared_ptr<ExpressionOwner> p_owner,
...
const Type &FreeForm2::ProgramImpl::GetType() const { return m_type; }

void FreeForm2::ProgramImpl::ProcessFeaturesUsed(
    DynamicRank::INeuralNetFeatures &p_features) const {
  ProcessFeaturesUsedVisitor visitor(p_features);
  m_exp->Accept(visitor);
}

const Expression &FreeForm2::ProgramImpl::GetExpression() const {
  return *m_exp;
}

DynamicRank::IFeatureMap &FreeForm2::ProgramImpl::GetFeatureMap() const {
  return m_map;
}
...
template <FreeForm2::Program::Syntax p_syntax>
boost::shared_ptr<FreeForm2::Program> FreeForm2::Program::Parse(
    SIZED_STRING p_input, DynamicRank::IFeatureMap &p_map,
    bool p_mustProduceFloat, const ExternalDataManager *p_externalData,
    std::ostream *p_debugOutput) {
  ...
  boost::tuples::tuple<const Expression *, boost::shared_ptr<ExpressionOwner>,
                       boost::shared_ptr<TypeManager> >
      ret = ::ParseInternal<p_syntax>(p_input, p_map, p_mustProduceFloat,
                                      p_externalData, p_debugOutput);
  ...
  std::auto_ptr<ProgramImpl> ptr(
      new ProgramImpl(*syntaxTree, owner, typeManager, p_map));
...
const FreeForm2::Type &FreeForm2::Program::GetType() const {
  return m_impl->GetType();
}

const FreeForm2::Expression &FreeForm2::Program::GetExpression() const {
  return m_impl->GetExpression();
}

void FreeForm2::Program::ProcessFeaturesUsed(
    DynamicRank::INeuralNetFeatures &p_features) const {
...
FreeForm2::ProgramImpl &FreeForm2::Program::GetImplementation() {
  return *m_impl;
}

const FreeForm2::ProgramImpl &FreeForm2::Program::GetImplementation() const {
  return *m_impl;
}

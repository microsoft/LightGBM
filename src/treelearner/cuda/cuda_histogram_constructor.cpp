...
CUDAHistogramConstructor::CUDAHistogramConstructor(
    const Dataset* train_data, const int num_leaves, const int num_threads,
    const std::vector<uint32_t>& feature_hist_offsets, const int min_data_in_leaf,
    const double min_sum_hessian_in_leaf, const int gpu_device_id, const bool gpu_use_dp,
    const bool use_quantized_grad, const int num_grad_quant_bins)
...
void CUDAHistogramConstructor::InitFeatureMetaInfo(
    const Dataset* train_data, const std::vector<uint32_t>& feature_hist_offsets) {
...
void CUDAHistogramConstructor::Init(const Dataset* train_data, TrainingShareStates* share_state) {
  ...
  cuda_need_fix_histogram_features_num_bin_aligned_.InitFromHostVector(
      need_fix_histogram_features_num_bin_aligend_);
  ...
  if (cuda_row_data_->NumLargeBinPartition() > 0) {
    ...
    const size_t buffer_size =
        static_cast<size_t>(grid_dim_y) * static_cast<size_t>(num_total_bin_);
    if (!use_quantized_grad_) {
      if (gpu_use_dp_) {
        // need to double the size of histogram buffer in global memory when using double precision
      ...
      // use only half the size of histogram buffer in global memory when quantized training since
      // each gradient and hessian takes only 2 bytes
...
void CUDAHistogramConstructor::SubtractHistogramForLeaf(
    const CUDALeafSplitsStruct* cuda_smaller_leaf_splits,
    const CUDALeafSplitsStruct* cuda_larger_leaf_splits, const bool use_quantized_grad,
    const uint8_t parent_num_bits_in_histogram_bins,
    const uint8_t smaller_num_bits_in_histogram_bins,
    const uint8_t larger_num_bits_in_histogram_bins) {
  global_timer.Start(
      "CUDAHistogramConstructor::ConstructHistogramForLeaf::LaunchSubtractHistogramKernel");
  LaunchSubtractHistogramKernel(cuda_smaller_leaf_splits, cuda_larger_leaf_splits,
                                use_quantized_grad, parent_num_bits_in_histogram_bins,
...
void CUDAHistogramConstructor::CalcConstructHistogramKernelDim(
  ...
  *grid_dim_y = std::max(min_grid_dim_y_, ((num_data_in_smaller_leaf + NUM_DATA_PER_THREAD - 1) /
                                               NUM_DATA_PER_THREAD +
                                           (*block_dim_y) - 1) /
                                              (*block_dim_y));
}

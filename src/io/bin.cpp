...
namespace LightGBM {
  ...
  MultiValBin* MultiValBin::CreateMultiValDenseBin(data_size_t num_data,
    ...
    if (max_bin <= 256) {
      ...
      return new MultiValDenseBin<uint8_t>(num_data, num_bin, num_feature, offsets, 8);
    } else if (max_bin <= 65536) {
      Log::Warning("using uint16_t");
      return new MultiValDenseBin<uint16_t>(num_data, num_bin, num_feature, offsets, 16);
    } else {
      Log::Warning("using uint32_t");
      return new MultiValDenseBin<uint32_t>(num_data, num_bin, num_feature, offsets, 32);
    }
  }

...
class AtofPreciseTest : public testing::Test {
 public:
  ...
  static double TestAtofPrecise(const char* data, double expected, bool test_eq = true) {
    double got = 0;
...
TEST_F(AtofPreciseTest, CornerCases) {
  AtofTestCase test_cases[] = {
      ...
      // FLT_MIN
      {"1."
       "1754943508222875079687365372222456778186655567720875215087517062784172594547271728515625e-"
       "38",
...
TEST_F(AtofPreciseTest, Inf) {
  ...
  for (auto const& test : test_cases) {
    ...
    EXPECT_EQ(LightGBM::Common::Sign(test.expected), LightGBM::Common::Sign(got))
        << "sign differs parsing: " << test.data;

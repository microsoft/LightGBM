...
class TreeLearner {
 public:
  ...
  /*! Initialise some temporary storage, only needed for the linear tree; needs to be a method of
   * TreeLearner since we call it in GBDT::RefitTree */
   ...
   * \param is_first_tree If linear tree learning is enabled, first tree needs to be handled
   * differently
   ...
   */
  virtual Tree* FitByExistingTree(const Tree* old_tree, const score_t* gradients,
                                  const score_t* hessians) const = 0;
   ...
   */
  virtual void SetBaggingData(const Dataset* subset, const data_size_t* used_indices,
  ...
  virtual void RenewTreeOutput(Tree* tree, const ObjectiveFunction* obj,
                               std::function<double(const label_t*, int)> residual_getter,
                               data_size_t total_num_data, const data_size_t* bag_indices,
                               data_size_t bag_cnt, const double* train_score) const = 0;

...
inline static void HistogramSumReducer(const char* src, char* dst,
...
class BinMapper {
  ...
  * \param total_sample_cnt number of total sample count,
           equal with values.size() + num_zeros
  ...
  * \param forced_upper_bounds Vector of split points that must be used
           (if this has size less than max_bin, remaining splits are
            found by the algorithm)
  */
  void FindBin(double* values, int num_values, size_t total_sample_cnt,
...
/*!
 * \brief Interface for bin data. This class will store bin data for one
 * feature. unlike OrderedBin, this class will store data by original order.
 *        Note that it may cause cache misses when construct histogram,
 *        but it doesn't need to re-order operation, So it will be faster than
 * OrderedBin for dense feature
 */
class Bin {
 public:
  ...
  virtual void CopySubset(const Bin* full_bin, const data_size_t* used_indices,
  ...
  *        Note: We use ordered_gradients and ordered_hessians
           to improve cache hit chance
  ...
  * \param ordered_gradients Pointer to gradients,
           the data_indices[i]-th data's gradient is ordered_gradients[i]
  * \param ordered_hessians Pointer to hessians,
           the data_indices[i]-th data's hessian is ordered_hessians[i]
  ...
  */
  ...
  /*!
  * \brief Split data according to threshold, if bin <= threshold, will put into
  left(lte_indices), else put into right(gt_indices)
  * \param min_bin min_bin of current used feature
  * \param max_bin max_bin of current used feature
  ...
  * \param data_indices Used data indices. After called this function.
           The less than or equal data indices will store on this object.
  * \param num_data Number of used data
  * \param lte_indices After called this function.
           The less or equal data indices will store on this object.
  * \param gt_indices After called this function.
           The greater data indices will store on this object.
  ...
  */
  virtual data_size_t Split(uint32_t min_bin, uint32_t max_bin,
                            ...
                            uint32_t threshold, data_size_t* data_indices,
  ...
  virtual data_size_t SplitCategorical(
      ...
      const uint32_t* threshold, int num_threshold, data_size_t* data_indices,
      data_size_t num_data, data_size_t* lte_indices,
      data_size_t* gt_indices) const = 0;

  /*!
   * \brief After pushed all feature data, call this could have better refactor
   * for bin data
   */
...
class MultiValBin {
 public:
  ...
  virtual void PushOneRow(int tid, data_size_t idx,

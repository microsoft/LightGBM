...
FreeForm2::PublishExpression::PublishExpression(
...
void FreeForm2::PublishExpression::Accept(Visitor &p_visitor) const {
...
size_t FreeForm2::PublishExpression::GetNumChildren() const { return 1; }

const FreeForm2::TypeImpl &FreeForm2::PublishExpression::GetType() const {
  return FreeForm2::TypeImpl::GetVoidInstance();
}

const FreeForm2::Expression &FreeForm2::PublishExpression::GetValue() const {
  return m_value;
}

const std::string &FreeForm2::PublishExpression::GetFeatureName() const {
  return m_featureName;
}

FreeForm2::DirectPublishExpression::DirectPublishExpression(
    const Annotations &p_annotations, const std::string &p_featureName,
...
FreeForm2::DirectPublishExpression::Alloc(const Annotations &p_annotations,
  ...
  // allocated in a char[].
  boost::shared_ptr<DirectPublishExpression> exp(
      new (new char[bytes]) DirectPublishExpression(
          p_annotations, p_featureName, p_indices, p_numIndices, p_value),
      DeleteAlloc);
  return exp;
}

void FreeForm2::DirectPublishExpression::DeleteAlloc(
    DirectPublishExpression *p_allocated) {
...
void FreeForm2::DirectPublishExpression::Accept(Visitor &p_visitor) const {
  if (!p_visitor.AlternativeVisit(*this)) {
...
size_t FreeForm2::DirectPublishExpression::GetNumChildren() const {
  return 1 + m_numIndices;
}

const FreeForm2::TypeImpl &FreeForm2::DirectPublishExpression::GetType() const {
  for (unsigned int i = 0; i < m_numIndices; ++i) {
...
const FreeForm2::Expression &FreeForm2::DirectPublishExpression::GetValue()
    const {
  return m_value;
}

const std::string &FreeForm2::DirectPublishExpression::GetFeatureName() const {
  return m_featureName;
}

unsigned int FreeForm2::DirectPublishExpression::GetNumIndices() const {
  return m_numIndices;
}

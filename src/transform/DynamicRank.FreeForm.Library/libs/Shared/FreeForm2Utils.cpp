...
void 
FreeForm2::VectorFromNeuralNetFeatures::ProcessFeature(UInt32 p_featureIndex)
{
...
void 
FreeForm2::VectorFromNeuralNetFeatures::ProcessFeature(UInt32 p_featureIndex, 
                                                       const std::vector<std::string>& p_segments)
{
...
void 
FreeForm2::SetFromNeuralNetFeatures::ProcessFeature(UInt32 p_featureIndex)
{
...
std::ostream& 
FreeForm2::operator<<(std::ostream& p_out, SIZED_STRING p_str)
{
...
void 
FreeForm2::LogHardwareException(DWORD p_exceptionCode, 
...
bool 
FreeForm2::IsSimpleName(SIZED_STRING p_name)
{
    for (unsigned int i = 0; i < p_name.cbData; i++)
    {
        if (!isalnum(p_name.pbData[i]) 
            && p_name.pcData[i] != '-'
            && p_name.pcData[i] != '_')
        {
            return false;
...
void 
FreeForm2::WriteCompressedVectorRLE(const UInt32* p_data, size_t p_numElements, std::ostream& p_out)
{
    ...
    for (size_t i = 0; i < p_numElements; i++)
    {
        switch (state)
        {
            case MatchingNulls:
            {
                ...
                {
                    state = MatchingNonNulls;
                    p_out.write(reinterpret_cast<const char*>(&numNulls), sizeof(UInt32));
                    p_out.write(reinterpret_cast<const char*>(&p_data[i]), sizeof(UInt32));
                }
                break;
            }
...
void 
FreeForm2::ReadCompressedVectorRLE(UInt32* p_data, size_t p_numElements, std::istream& p_in)
{
    FF2_ASSERT(p_data != NULL);
    size_t i = 0;

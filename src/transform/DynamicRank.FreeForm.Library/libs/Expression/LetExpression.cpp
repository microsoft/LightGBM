...
FreeForm2::LetExpression::LetExpression(const Annotations& p_annotations,
...
FreeForm2::LetExpression::GetType() const
{
...
size_t
FreeForm2::LetExpression::GetNumChildren() const
{
    return m_numBound + 1;
}
...
FreeForm2::LetExpression::GetValue() const
{
    return *m_value;
}
...
void
FreeForm2::LetExpression::Accept(Visitor& p_visitor) const
{
...
boost::shared_ptr<FreeForm2::LetExpression> 
FreeForm2::LetExpression::Alloc(const Annotations& p_annotations,
                                const std::vector<IdExpressionPair>& p_children,
                                const Expression* p_value)
{
    size_t bytes = sizeof(LetExpression) + (p_children.size() - 1) * sizeof(IdExpressionPair);
    ...
    // allocated in a char[].
    boost::shared_ptr<LetExpression> exp(new (new char[bytes]) LetExpression(p_annotations, p_children, p_value), 
                                         DeleteAlloc);
    return exp;
}
...
void 
FreeForm2::LetExpression::DeleteAlloc(LetExpression* p_allocated)
{

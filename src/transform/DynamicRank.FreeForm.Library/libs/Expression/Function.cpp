...
FreeForm2::FunctionExpression::FunctionExpression(const Annotations& p_annotations,
...
FreeForm2::FunctionExpression::GetFunctionType() const
{
    return m_type;
}
...
FreeForm2::FunctionExpression::GetType() const
{
    return m_type;
}
...
const std::string&
FreeForm2::FunctionExpression::GetName() const
{
    return m_name;
}
...
size_t
FreeForm2::FunctionExpression::GetNumChildren() const
{
...
size_t
FreeForm2::FunctionExpression::GetNumParameters() const
{
...
FreeForm2::FunctionExpression::GetBody() const
{
    return m_body;
}
...
void
FreeForm2::FunctionExpression::Accept(Visitor& p_visitor) const
{
...
FreeForm2::FunctionCallExpression::FunctionCallExpression(const Annotations& p_annotations,
...
void
FreeForm2::FunctionCallExpression::Accept(Visitor& p_visitor) const
{
    size_t stackSize = p_visitor.StackSize();
...
const FreeForm2::Expression&
FreeForm2::FunctionCallExpression::GetFunction() const
{
    return m_function;
}
...
const FreeForm2::TypeImpl&
FreeForm2::FunctionCallExpression::GetType() const
{
...
size_t
FreeForm2::FunctionCallExpression::GetNumChildren() const
{
    return GetNumParameters() + 1;
}
...
size_t
FreeForm2::FunctionCallExpression::GetNumParameters() const
{
...
FreeForm2::FunctionCallExpression::Alloc(const Annotations& p_annotations,
                                         const Expression& p_function,
                                         const std::vector<const Expression*>& p_parameters)
{
    FF2_ASSERT(p_function.GetType().Primitive() == Type::Function);

    size_t bytes = sizeof(FunctionCallExpression) 
        + (std::max((size_t) 1ULL, p_parameters.size()) - 1) * sizeof(Expression*);

    // Allocate a shared_ptr that deletes an FunctionCallExpression 
    // allocated in a char[].
    boost::shared_ptr<FunctionCallExpression> exp(new (new char[bytes]) 
        FunctionCallExpression(p_annotations, p_function, p_parameters), DeleteAlloc);
    return exp;
}


void 
FreeForm2::FunctionCallExpression::DeleteAlloc(FunctionCallExpression* p_allocated)
{
...
void
FreeForm2::ReturnExpression::Accept(Visitor& p_visitor) const 
{
    size_t stackSize = p_visitor.StackSize();
...
size_t
FreeForm2::ReturnExpression::GetNumChildren() const
{
    return 1;
}
...
const FreeForm2::TypeImpl&
FreeForm2::ReturnExpression::GetType() const
{
...
const FreeForm2::Expression&
FreeForm2::ReturnExpression::GetValue() const
{
    return m_value;
}

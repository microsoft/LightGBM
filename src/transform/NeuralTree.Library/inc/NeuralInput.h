...
class NeuralInput : boost::noncopyable {
 ...
 public:
  ...
  virtual void BatchSerialize(
      const std::vector<const NeuralInput *> & /*p_inputs*/,
      FreeForm2CodeBondData & /*p_blob*/) const {};

  virtual void BatchUnSerialize(
      const std::vector<NeuralInput *> & /*p_inputs*/,
      const FreeForm2CodeBondData & /*p_blob*/) const {};
  ...
  virtual void GetAllAssociatedFeatures(
      std::vector<UInt32> &associatedFeaturesList) const = 0;

  virtual double GetSlope() const;
  virtual double GetIntercept() const;

  virtual bool Save(FILE *fpOutput, size_t nInputId,
  ...
  // (memory accessed through pointers or objects contain pointers e.g.
  // std::string, std::vector, etc.).
  virtual size_t GetSize() const = 0;

  // Get the list of segments this input belongs to. Empty means it belongs to
  // base ranker.
  const std::vector<std::string> &GetSegments() const;

 private:
  // Segments to which this input applies; a size zero vector means the input is
  // part of the main L2 ranker
  boost::scoped_ptr<std::vector<std::string> > m_segments;

  friend class NeuralInputTest;
};

// An exception that is thrown when trying to bulk-optimize a list of
// NeuralInputs if at least one of them is not compatible with bulk
// optimization.
...
// Base class that provides a replacement function to evaluate a set of neural
// inputs.
class BulkNeuralInput : boost::noncopyable {
 ...
 public:
  ...
  /* virtual void AddNeuralNetFeatures(INeuralNetFeatures& p_neuralNetFeatures)
   * const = 0; */

  // Evaluates the neural inputs and places the result of their evaluation in
  // the corresponding indices in the p_output array.
  ...
  virtual void Evaluate(UInt32 **p_featureVectorArray, UInt32 p_currentDocument,
                        UInt32 p_documentCount, float p_output[]) const;

  // Populates p_associatedFeaturesList with all the feature indices that this
  // BulkNeuralInput object requires.
  virtual void GetAllAssociatedFeatures(
      std::vector<UInt32> &p_associatedFeaturesList) const = 0;
...
class NeuralInputUnary : public NeuralInput {
 public:
  ...
  bool Save(FILE *fpOutput, size_t nInputId,
            const IFeatureMap &p_featureMap) const;
...
class NeuralInputLinear : public NeuralInputUnary {
 ...
 public:
  ...
  static NeuralInputLinear *Load(DynamicRank::Config &p_config,
                                 const char *szSection,
                                 IFeatureMap &p_featureMap);
...
class NeuralInputRational : public NeuralInputUnary {
 ...
 public:
  ...
  static NeuralInputRational *Load(DynamicRank::Config &p_config,
                                   const char *szSection,
                                   IFeatureMap &p_featureMap);
...
class NeuralInputLogLinear : public NeuralInputLinear {
 ...
 public:
  ...
  static NeuralInputLogLinear *Load(DynamicRank::Config &p_config,
                                    const char *szSection,
                                    IFeatureMap &p_featureMap);
...
class NeuralInputBucket : public NeuralInputUnary {
 ...
 public:
  NeuralInputBucket(int p_id, double p_min, bool p_mininclusive, double p_max,
                    bool p_maxinclusive);
  ...
  static NeuralInputBucket *Load(DynamicRank::Config &p_config,
                                 const char *szSection,
                                 IFeatureMap &p_featureMap);
...
class NeuralInputCached : public NeuralInputUnary {
 ...
 public:
  ...
  bool Train(double dblLearningRate, double outputHigh, double outputLow,
             double dblOutputDelta, UInt32 inputHigh[], UInt32 inputLow[]);

  static NeuralInputUnary *Load(size_t nCacheSize, NeuralInputUnary *pChild);
...
class NeuralInputTanh : public NeuralInput {
 public:
  ...
  static NeuralInputTanh *Load(DynamicRank::Config &p_config,
                               const char *szSection,
                               IFeatureMap &p_featureMap);
...
class NeuralInputTanhUnary : public NeuralInputUnary {
 public:
  ...
  bool Train(double dblLearningRate, double outputHigher, double outputLower,
             double dblOutputDelta, UInt32 inputHigh[], UInt32 inputLow[]);

  static NeuralInputTanhUnary *Load(DynamicRank::Config &p_config,
                                    const char *szSection,
                                    IFeatureMap &p_featureMap);
 ...
 protected:
  NeuralInputTanhUnary(UInt32 iFeature, double dblWeights, double dblThreshold,
                       bool fLocked);
...
class NeuralInputUseAsFloat : public NeuralInputUnary {
 ...
 public:
  ...
  static NeuralInputUseAsFloat *Load(DynamicRank::Config &p_config,
                                     const char *szSection,
                                     IFeatureMap &p_featureMap);

...
The position file corresponds with training data file line by line, and has one position per line. The position file path can be specified using the parameter ``position_filename``, or through the ``Dataset`` constructor when using Python API.

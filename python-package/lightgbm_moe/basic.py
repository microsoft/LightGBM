...
class Booster:
    ...
    def predict_adaptive(
        ...
        **kwargs: Any,
        ...
        """Make predictions with adaptive gate weights based on recent prediction errors.

        This method dynamically adjusts gate weights based on how well each expert
        has performed on recent samples. Experts with lower recent errors get higher weights.

        Parameters
        ----------
        data : numpy array, pandas DataFrame, or scipy.sparse
            Feature data for prediction. Must be 2D.
        window_size : int, optional (default=10)
            Number of recent samples to consider for error calculation.
        adaptation_rate : float, optional (default=0.1)
            How much to adjust weights based on recent errors.
            0 = no adaptation, 1 = fully adaptive.
        **kwargs
            Other parameters passed to predict methods.

        Returns
        -------
        result : numpy array
            Predictions with adaptive gate weights.

        Note
        ----
        This method requires knowing the true labels for adaptation.
        For true online learning, use predict_adaptive_online() with streaming data.
        """
        if not self.is_mixture():
            raise LightGBMError("predict_adaptive can only be used with MoE models")

        # Get base predictions and expert predictions
        expert_preds = self.predict_expert_pred(data)  # (nrow, n_experts)
        base_proba = self.predict_regime_proba(data)  # (nrow, n_experts)

        nrow, num_experts = expert_preds.shape
        result = np.empty(nrow, dtype=np.float64)

        # Initialize adaptive weights
        adaptive_weights = np.ones(num_experts) / num_experts

        for i in range(nrow):
            # Blend base proba with adaptive weights
            blended_proba = (1.0 - adaptation_rate) * base_proba[i] + adaptation_rate * adaptive_weights
            blended_proba /= blended_proba.sum()

            # Make prediction
            result[i] = (blended_proba * expert_preds[i]).sum()

            # Update adaptive weights based on prediction "confidence"
            # (In real use, you'd update based on actual errors once labels are known)
            # Here we use a heuristic: boost weights for experts with high gate proba
            if i >= window_size:
                # Simple heuristic: use recent gate probabilities as proxy for performance
                recent_proba = base_proba[max(0, i - window_size):i].mean(axis=0)
                adaptive_weights = 0.9 * adaptive_weights + 0.1 * recent_proba
                adaptive_weights /= adaptive_weights.sum()

        return result

    def predict_with_transition(
        self,
        data: _LGBM_PredictDataType,
        ...
        **kwargs: Any,
        ...
        """Make predictions using a custom transition matrix for regime switching.

        Parameters
        ----------
        data : numpy array, pandas DataFrame, or scipy.sparse
            Feature data for prediction.
        transition_matrix : numpy array or None, optional
            K×K transition matrix where transition_matrix[i,j] = P(regime[t]=j | regime[t-1]=i).
            If None, uses uniform transitions.
        smoothing_lambda : float, optional (default=0.5)
            Blending coefficient between gate output and transition-based prior.
            0 = pure gate, 1 = pure transition.

        Returns
        -------
        result : numpy array
            Predictions with transition-based regime switching.
        """
        if not self.is_mixture():
            raise LightGBMError("predict_with_transition can only be used with MoE models")

        expert_preds = self.predict_expert_pred(data)
        base_proba = self.predict_regime_proba(data)
        nrow, num_experts = expert_preds.shape

        # Default to uniform transition matrix
        if transition_matrix is None:
            transition_matrix = np.ones((num_experts, num_experts)) / num_experts

        result = np.empty(nrow, dtype=np.float64)
        current_proba = base_proba[0].copy()

        for i in range(nrow):
            if i == 0:
                final_proba = base_proba[i]
            else:
                # Apply transition: prior[j] = sum_i( P(j|i) * proba[i] )
                trans_prior = transition_matrix.T @ current_proba
                trans_prior /= trans_prior.sum() + 1e-12

                # Blend with current gate output
                final_proba = (1.0 - smoothing_lambda) * base_proba[i] + smoothing_lambda * trans_prior
                final_proba /= final_proba.sum()

            result[i] = (final_proba * expert_preds[i]).sum()
            current_proba = final_proba.copy()
...
# =============================================================================
# Helper functions for regime-switching / time-series MoE
# =============================================================================
...
def create_lagged_features(
    ...
    include_y_lags: bool = True,
    include_diff: bool = False,
    drop_na: bool = True,
) -> Tuple[np.ndarray, Optional[np.ndarray], np.ndarray]:
    """Create lagged features for time-series regime switching.

    This helper function adds lagged versions of features and/or target values,
    which can help the model learn temporal dependencies without modifying the
    core MoE algorithm.
    ...
    y : numpy array of shape (n_samples,) or None
        Target values. If provided and include_y_lags=True, lagged y values
        will be added as features.
    lags : list of int or None, optional (default=None)
        List of lag values to use. If None, defaults to [1, 2, 3].
    include_y_lags : bool, optional (default=True)
        Whether to include lagged target values as features.
    include_diff : bool, optional (default=False)
        Whether to include differenced values (y[t] - y[t-1]) as features.
    drop_na : bool, optional (default=True)
        Whether to drop rows with NaN values (from lagging).

    Returns
    -------
    X_extended : numpy array
        Feature matrix with lagged features added.
    y_trimmed : numpy array or None
        Target values aligned with X_extended (if y was provided).
    valid_indices : numpy array
        Indices of valid (non-NaN) rows in the original data.
    ...
    >>> X_ext, y_ext, idx = create_lagged_features(X, y, lags=[1, 2])
    >>> print(f"Original: {X.shape}, Extended: {X_ext.shape}")
    Original: (100, 5), Extended: (98, 17)
    """
    if lags is None:
        lags = [1, 2, 3]
    ...
    # Collect all lagged features
    features_list = [X]
    feature_names_suffix = []

    # Add lagged X features
    for lag in lags:
        lagged_X = np.empty_like(X)
        lagged_X[:lag] = np.nan
        lagged_X[lag:] = X[:-lag]
        features_list.append(lagged_X)

    # Add lagged y features
    if y is not None and include_y_lags:
        for lag in lags:
            lagged_y = np.empty(n_samples)
            lagged_y[:lag] = np.nan
            lagged_y[lag:] = y[:-lag]
            features_list.append(lagged_y.reshape(-1, 1))

        # Add differenced y
        if include_diff:
            diff_y = np.empty(n_samples)
            diff_y[0] = np.nan
            diff_y[1:] = np.diff(y)
            features_list.append(diff_y.reshape(-1, 1))

    # Combine all features
    X_extended = np.hstack(features_list)

    # Handle NaN values
    if drop_na:
        valid_mask = ~np.any(np.isnan(X_extended), axis=1)
        valid_indices = np.where(valid_mask)[0]
        X_extended = X_extended[valid_mask]
        y_trimmed = y[valid_mask] if y is not None else None
    else:
        valid_indices = np.arange(n_samples)
        y_trimmed = y

    return X_extended, y_trimmed, valid_indices
...
def estimate_transition_matrix(
    ...
    """Estimate transition matrix from regime probabilities.

    Parameters
    ----------
    regime_proba : numpy array of shape (n_samples, n_regimes)
        Regime probabilities for each sample (assumed to be in time order).
    prior : float, optional (default=0.1)
        Dirichlet prior for smoothing (higher = more uniform).

    Returns
    -------
    transition_matrix : numpy array of shape (n_regimes, n_regimes)
        Estimated transition matrix where transition_matrix[i,j] = P(regime[t]=j | regime[t-1]=i).

    Examples
    --------
    >>> proba = np.array([[0.8, 0.2], [0.7, 0.3], [0.3, 0.7], [0.2, 0.8]])
    >>> trans = estimate_transition_matrix(proba)
    >>> print(trans)  # Shows probability of regime transitions
    """
    n_samples, n_regimes = regime_proba.shape

    # Initialize counts with prior
    counts = np.full((n_regimes, n_regimes), prior)

    # Count soft transitions
    for t in range(1, n_samples):
        for i in range(n_regimes):
            for j in range(n_regimes):
                counts[i, j] += regime_proba[t - 1, i] * regime_proba[t, j]

    # Normalize rows to get probabilities
    row_sums = counts.sum(axis=1, keepdims=True)
    transition_matrix = counts / (row_sums + 1e-12)

    return transition_matrix


def compute_regime_duration_stats(
    regime_proba: np.ndarray,
    threshold: float = 0.5,
) -> Dict[str, Any]:
    """Compute statistics about regime durations.
    ...
    threshold : float, optional (default=0.5)
        Probability threshold for determining dominant regime.

    Returns
    -------
    stats : dict
        Dictionary containing:
        - 'regime_sequence': array of dominant regimes
        - 'durations': list of (regime, duration) tuples
        - 'mean_duration': mean duration across all regimes
        - 'regime_counts': count of each regime
        - 'regime_mean_durations': mean duration per regime
    """
    n_samples, n_regimes = regime_proba.shape

    # Get dominant regime for each sample
    dominant_regime = regime_proba.argmax(axis=1)
    ...
    for t in range(1, n_samples):
        if dominant_regime[t] == current_regime:
            current_duration += 1
        else:
            durations.append((current_regime, current_duration))
            current_regime = dominant_regime[t]
            current_duration = 1
    ...
    regime_durations = {k: [] for k in range(n_regimes)}
    for regime, dur in durations:
        regime_durations[regime].append(dur)

    regime_counts = {k: len(v) for k, v in regime_durations.items()}
    regime_mean_durations = {k: np.mean(v) if v else 0 for k, v in regime_durations.items()}

    all_durations = [d for _, d in durations]

    return {
        'regime_sequence': dominant_regime,
        'durations': durations,
        'mean_duration': np.mean(all_durations) if all_durations else 0,
        'regime_counts': regime_counts,
        'regime_mean_durations': regime_mean_durations,
    }

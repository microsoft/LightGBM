 ...
 */
LIGHTGBM_C_EXPORT LGBM_SE LGBM_GetLastError_R(LGBM_SE buf_len,
                                              LGBM_SE actual_len,
                                              LGBM_SE err_msg);
...
with other dataset, nullptr means not used
 ...
 * \return 0 when succeed, -1 when failure happens
 */
LIGHTGBM_C_EXPORT LGBM_SE LGBM_DatasetGetNumData_R(LGBM_SE handle, LGBM_SE out,
                                                   LGBM_SE call_state);
 ...
 * gradient, this can be used to support customized loss function \param handle
 * handle \param grad gradient statistics \param hess second order gradient
 * statistics \param len length of grad/hess \return 0 when succeed, -1 when
 * failure happens
 */
 ...
 * num_data for leaf index, its length is equal to num_class * num_data *
 * num_iteration \param handle handle \param indptr pointer to row headers
 * \param indices findex
 * \param data fvalue
 * \param nindptr number of cols in the matrix + 1
 ...
 *               for normal and raw score: its length is equal to num_class *
 * num_data for leaf index, its length is equal to num_class * num_data *
 * num_iteration \param handle handle \param data pointer to the data space
 * \param nrow number of rows
 * \param ncol number columns
 * \param is_rawscore
 * \param is_leafidx

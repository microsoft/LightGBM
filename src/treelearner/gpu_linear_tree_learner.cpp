...
void GPULinearTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  GPUTreeLearner::Init(train_data, is_constant_hessian);
  GPULinearTreeLearner::InitLinear(train_data, config_->num_leaves);
}

void GPULinearTreeLearner::InitLinear(const Dataset* train_data, const int max_leaves) {
...
Tree* GPULinearTreeLearner::Train(const score_t* gradients, const score_t *hessians, bool is_first_tree) {
...
Tree* GPULinearTreeLearner::FitByExistingTree(const Tree* old_tree, const score_t* gradients, const score_t *hessians) const {
...
Tree* GPULinearTreeLearner::FitByExistingTree(const Tree* old_tree, const std::vector<int>& leaf_pred,
                                           const score_t* gradients, const score_t *hessians) const {
  data_partition_->ResetByLeafPred(leaf_pred, old_tree->num_leaves());
  return GPULinearTreeLearner::FitByExistingTree(old_tree, gradients, hessians);
}

void GPULinearTreeLearner::GetLeafMap(Tree* tree) const {
...
void GPULinearTreeLearner::CalculateLinear(Tree* tree, bool is_refit, const score_t* gradients, const score_t* hessians, bool is_first_tree) const {

...
__global__ void ReduceLeafStatKernel_SharedMemory(const score_t* gradients,
                                                  const score_t* hessians, const int num_leaves,
  ...
  for (int leaf_index = static_cast<int>(threadIdx.x); leaf_index < num_leaves;
...
__global__ void ReduceLeafStatKernel_GlobalMemory(const score_t* gradients,
                                                  const score_t* hessians, const int num_leaves,
                                                  const data_size_t num_data,
                                                  const int* data_index_to_leaf_index,
                                                  double* leaf_grad_stat_buffer,
...
__global__ void CalcRefitLeafOutputKernel(
    const int num_leaves, const double* leaf_grad_stat_buffer, const double* leaf_hess_stat_buffer,
    const data_size_t* num_data_in_leaf, const int* leaf_parent, const int* left_child,
    const int* right_child, const double lambda_l1, const double lambda_l2,
    const double path_smooth, const double shrinkage_rate, const double refit_decay_rate,
    double* leaf_value) {
  ...
  if (leaf_index < num_leaves) {
    ...
    if (!USE_SMOOTHING) {
      new_leaf_value = CUDALeafSplits::CalculateSplittedLeafOutput<false, false>(
          sum_gradients, sum_hessians, lambda_l1, lambda_l2, 0.0f, 0, 0.0f);
    } else {
      const int parent = leaf_parent[leaf_index];
      if (parent >= 0) {
        const int sibliing =
            left_child[parent] == leaf_index ? right_child[parent] : left_child[parent];
        ...
        const double parent_output = CUDALeafSplits::CalculateSplittedLeafOutput<false, true>(
        ...
        new_leaf_value = CUDALeafSplits::CalculateSplittedLeafOutput<false, true>(
            sum_gradients, sum_hessians, lambda_l1, lambda_l2, path_smooth, num_data_in_parent,
            parent_output);
      } else {
        new_leaf_value = CUDALeafSplits::CalculateSplittedLeafOutput<false, false>(
            sum_gradients, sum_hessians, lambda_l1, lambda_l2, 0.0f, 0, 0.0f);
    ...
    leaf_value[leaf_index] =
        refit_decay_rate * old_leaf_value + (1.0f - refit_decay_rate) * new_leaf_value;
  }
}
...
void CUDASingleGPUTreeLearner::LaunchReduceLeafStatKernel(
  ...
  int num_block = (num_data + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
                  CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;
  if (num_leaves <= 2048) {
    ReduceLeafStatKernel_SharedMemory<<<num_block, CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE,
                                        2 * num_leaves * sizeof(double)>>>(
        gradients, hessians, num_leaves, num_data,
  ...
  num_block = (num_leaves + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
              CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;

#define CalcRefitLeafOutputKernel_ARGS                                                           \
  num_leaves, cuda_leaf_gradient_stat_buffer_.RawData(),                                         \
      cuda_leaf_hessian_stat_buffer_.RawData(), num_data_in_leaf, leaf_parent, left_child,       \
      right_child, config_->lambda_l1, config_->lambda_l2, config_->path_smooth, shrinkage_rate, \
      config_->refit_decay_rate, cuda_leaf_value
...
__global__ void SetRealThresholdKernel(const CUDASplitInfo* best_split_info,
  ...
  const int* categorical_bin_to_value_ptr =
      categorical_bin_to_value + categorical_bin_offsets[best_split_info->inner_feature_index];
...
template <typename T, bool IS_INNER>
void CUDAConstructBitset(const CUDASplitInfo* best_split_info, const int num_cat_threshold,
                         uint32_t* out, size_t bitset_len) {
  const int num_blocks = (num_cat_threshold + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
                         CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;
  // clear the bitset vector first
  SetCUDAMemory<uint32_t>(out, 0, bitset_len, __FILE__, __LINE__);
  CUDAConstructBitsetKernel<T, IS_INNER>
      <<<num_blocks, CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE>>>(best_split_info, out, bitset_len);
}

template <typename T, bool IS_INNER>
size_t CUDABitsetLen(const CUDASplitInfo* best_split_info, const int num_cat_threshold,
                     size_t* out_len_buffer) {
  const int num_blocks = (num_cat_threshold + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
                         CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;
  CalcBitsetLenKernel<T, IS_INNER>
      <<<num_blocks, CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE>>>(best_split_info, out_len_buffer);
...
void CUDASingleGPUTreeLearner::LaunchConstructBitsetForCategoricalSplitKernel(
  ...
  const int num_blocks = (num_cat_threshold_ + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
                         CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;
  ...
  cuda_bitset_inner_len_ = CUDABitsetLen<uint32_t, true>(best_split_info, num_cat_threshold_,
                                                         cuda_block_bitset_len_buffer_);
  CUDAConstructBitset<uint32_t, true>(best_split_info, num_cat_threshold_, cuda_bitset_inner_,
                                      cuda_bitset_inner_len_);
  cuda_bitset_len_ = CUDABitsetLen<int, false>(best_split_info, num_cat_threshold_,
                                               cuda_block_bitset_len_buffer_);
...
#define CalcRefitLeafOutputKernel_ARGS                                                           \
  cuda_tree->num_leaves(), cuda_leaf_gradient_stat_buffer_.RawData(),                            \
      cuda_leaf_hessian_stat_buffer_.RawData(), num_data_in_leaf, cuda_tree->cuda_leaf_parent(), \
      cuda_tree->cuda_left_child(), cuda_tree->cuda_right_child(), config_->lambda_l1,           \
      config_->lambda_l2, config_->path_smooth, 1.0f, config_->refit_decay_rate,                 \
      cuda_tree->cuda_leaf_value_ref()
  const bool use_l1 = config_->lambda_l1 > 0.0f;
  const bool use_smoothing = config_->path_smooth > 0.0f;
  const int num_block = (cuda_tree->num_leaves() + CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE - 1) /
                        CUDA_SINGLE_GPU_TREE_LEARNER_BLOCK_SIZE;
  if (!use_l1) {
    if (!use_smoothing) {
      CalcRefitLeafOutputKernel<false, false>

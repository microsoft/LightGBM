...
void CUDARegressionL1loss::Init(const Metadata& metadata, data_size_t num_data) {
  ...
  if (cuda_weights_ != nullptr) {
    const int num_blocks = (num_data + GET_GRADIENTS_BLOCK_SIZE_REGRESSION - 1) /
...
double CUDARegressionPoissonLoss::LaunchCalcInitScoreKernel(const int class_id) const {
  return Common::SafeLog(
      CUDARegressionObjectiveInterface<RegressionPoissonLoss>::LaunchCalcInitScoreKernel(
          class_id));
}

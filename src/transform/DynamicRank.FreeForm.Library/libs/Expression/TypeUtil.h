...
class TypeUtil {
 public:
  ...
  // value.
  static boost::shared_ptr<Expression> ConvertConstant(
  ...
  static const TypeImpl &Unify(const TypeImpl &p_type1, const TypeImpl &p_type2,
                               TypeManager &p_typeManager, bool p_allowArray,
                               bool p_allowPromotion);

  // Determine if a source type is assignable to a destination type.
  static bool IsAssignable(const TypeImpl &p_dest, const TypeImpl &p_source);

...
void CUDAAUCMetric::LaunchEvalKernel(const double* score) const {
  ...
  if (cuda_weights_ == nullptr) {
    GlobalCalcAUC<false>(cuda_sum_pos_buffer_, nullptr, cuda_threshold_mark_, num_data_, cuda_block_sum_pos_buffer_);
  } else {
    GlobalCalcAUC<true>(cuda_sum_pos_buffer_, cuda_sum_neg_buffer_, cuda_threshold_mark_, num_data_, cuda_block_sum_pos_buffer_);
  }
}

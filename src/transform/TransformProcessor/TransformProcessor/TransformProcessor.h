...
class TransformedData {
 ...
 public:
  ...
  static vector<string> ToString(vector<TransformedData> dataset,
                                 int precision = 5);
};
...
class TransformProcessor {
 ...
 public:
  TransformProcessor(const string &transform_str, const std::string &header_str,
                     int label_id);
  void Parse(const char *str, vector<string> *out_feature_strs,
             double *out_label, string delimiter = "\t");
  void Apply(vector<string> *input_row,

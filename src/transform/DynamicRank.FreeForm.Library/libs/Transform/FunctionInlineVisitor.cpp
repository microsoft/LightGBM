...
FreeForm2::FunctionInlineVisitor::FunctionInlineVisitor(const boost::shared_ptr<SimpleExpressionOwner>& p_owner,
                                                        const boost::shared_ptr<TypeManager>& p_typeManager,
                                                        VariableID p_variableId)
    : CopyingVisitor(p_owner, p_typeManager),
...
FreeForm2::FunctionInlineVisitor::GetVariableId()
{
    return m_variableId;
}
...
bool
FreeForm2::FunctionInlineVisitor::AlternativeVisit(const FunctionCallExpression& p_expr)
{
    ...
    const std::vector<FunctionExpression::Parameter>& functionParams = functionExpression.GetParameters();
    ...
    for (size_t i = 0; i < funType.GetParameterCount(); i++)
    {
        ...
        // Try to assign the type of the parameter to the type of the Function input.
        ...
        m_parameterTypeTranslation.insert(std::make_pair(functionParams[i].m_parameter->GetId(), &paramType));
    ...
    for (size_t i = 0; i < funType.GetParameterCount(); i++)
    {
        ...
        m_newVariableIdMapping.insert(std::make_pair(functionParams[i].m_parameter->GetId(), newVariableID));
        letValues.push_back(std::make_pair(newVariableID, parameterExpressions[i]));
    }

    // Then visit the Function Body. This should replace unknown types with known types.
    // And old variable ids with the new ids.
    ...
    // Add a Let expression containing the parameter values in the Invoke statement
    // and the function's body.
    AddExpression(LetExpression::Alloc(p_expr.GetAnnotations(),
                                       letValues,
                                       newFunctionBody));
...
void 
FreeForm2::FunctionInlineVisitor::Visit(const VariableRefExpression& p_expr)
{
...
void 
FreeForm2::FunctionInlineVisitor::Visit(const ReturnExpression& p_expr)
{

...
namespace FreeForm2
{
    ...
    public:
        enum DestinationFunctionType
        {
            ...
            // typedef float (*AggregatedEvalFun)(StreamFeatureInput*, const FeatureType[][], UInt32, UInt32, OutputType[]).
        ...
        CreateExecutableCompiler(unsigned int p_optimizationLevel,
                                 DestinationFunctionType p_destinationFunctionType = SingleDocumentEvaluation);
        ...
        static
        std::unique_ptr<Compiler> 
        CreateCppIFMCompiler(const Cpp::ExternResolver& p_resolver);
        ...
        static
        std::unique_ptr<Compiler> 
        CreateCppBarramundiCompiler(const Cpp::ExternResolver& p_resolver,
                                    const std::string& p_metadataPath);
        ...
        static
        std::unique_ptr<Compiler>
        CreateDebuggingCppBarramundiCompiler(const Cpp::ExternResolver& p_resolver,
                                             const std::string& p_metadataPath,
        ...
        static
        std::unique_ptr<Compiler>
        CreateCppFpgaCompiler(const Cpp::ExternResolver& p_resolver,
                              const std::string& p_outputMappingPath,
                              const std::string& p_msdlPath);
    };
}

...
vector<uint32_t> FeatureEvaluator::GetRequiredRawFeatureIndices() {
  ...
  this->parser_interface_->GetInputFeatures(this->inputptr_, reinterpret_cast<uint32_t*>(features.data()), (uint32_t)features.size(), &feature_count);
  std::vector<uint32_t> subvector(features.begin(), features.begin() + feature_count);

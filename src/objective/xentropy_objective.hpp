...
class CrossEntropy : public ObjectiveFunction {
 public:
  explicit CrossEntropy(const Config& config) : deterministic_(config.deterministic) {}

  explicit CrossEntropy(const std::vector<std::string>&) : deterministic_(false) {}
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        if (score[i] > -37.0) {
          ...
          gradients[i] =
              static_cast<score_t>(((1.0f - label_[i]) - label_[i] * exp_tmp) / (1.0f + exp_tmp));
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
        if (score[i] > -37.0) {
          const double exp_tmp = std::exp(-score[i]);
          gradients[i] = static_cast<score_t>(((1.0f - label_[i]) - label_[i] * exp_tmp) /
                                              (1.0f + exp_tmp) * weights_[i]);
          hessians[i] =
              static_cast<score_t>(exp_tmp / ((1 + exp_tmp) * (1 + exp_tmp)) * weights_[i]);
        } else {
          const double exp_tmp = std::exp(score[i]);
...
/*!
 * \brief Objective function for alternative parameterization of cross-entropy (see top of file for
 * explanation)
 */
class CrossEntropyLambda : public ObjectiveFunction {
 public:
  explicit CrossEntropyLambda(const Config& config) : deterministic_(config.deterministic) {
    min_weight_ = max_weight_ = 0.0f;
  }

  explicit CrossEntropyLambda(const std::vector<std::string>&) : deterministic_(false) {}
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    ...
    if (weights_ != nullptr) {
      ...
      Log::Info("[%s:%s]: min, max weights = %f, %f; ratio = %f", GetName(), __func__, min_weight_,
                max_weight_, weight_ratio);
    } else {
  ...
  void GetGradients(const double* score, score_t* gradients, score_t* hessians) const override {
    if (weights_ == nullptr) {
// compute pointwise gradients and Hessians with implied unit weights; exactly equivalent to
// CrossEntropy with unit weights
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
      for (data_size_t i = 0; i < num_data_; ++i) {
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static)
  ...
  //
  // ATTENTION: the function output is the "normalized exponential parameter" lambda > 0, not the
  // probability
  //
  ...
  //
  // Let z = 1 / (1 + exp(-f)), then prob(z) = 1-(1-z)^w, where w is the weight for the specific
  // point.
  //

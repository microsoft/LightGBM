...
_ctypes_int_ptr = Union["ctypes._Pointer[ctypes.c_int32]", "ctypes._Pointer[ctypes.c_int64]"]
_ctypes_int_array = Union[
    "ctypes.Array[ctypes._Pointer[ctypes.c_int32]]", "ctypes.Array[ctypes._Pointer[ctypes.c_int64]]"
]
_ctypes_float_ptr = Union["ctypes._Pointer[ctypes.c_float]", "ctypes._Pointer[ctypes.c_double]"]
_ctypes_float_array = Union[
    "ctypes.Array[ctypes._Pointer[ctypes.c_float]]", "ctypes.Array[ctypes._Pointer[ctypes.c_double]]"
]
...
class _ConfigAliases:
    ...
    def _get_all_param_aliases() -> Dict[str, List[str]]:
        ...
        if actual_len > buffer_len:
            ...
            ptr_string_buffer = ctypes.c_char_p(ctypes.addressof(string_buffer))
            _safe_call(
                _LIB.LGBM_DumpParamAliases(ctypes.c_int64(actual_len), ctypes.byref(tmp_out_len), ptr_string_buffer)
            )
        return json.loads(
...
def _dump_pandas_categorical(
    pandas_categorical: Optional[List[List]], file_name: Optional[Union[str, Path]] = None
) -> str:
...
class _InnerPredictor:
    ...
    def from_model_file(cls, model_file: Union[str, Path], pred_parameter: Dict[str, Any]) -> "_InnerPredictor":
        ...
        _safe_call(
            _LIB.LGBM_BoosterCreateFromModelfile(
                _c_str(str(model_file)), ctypes.byref(out_num_iterations), ctypes.byref(booster_handle)
            )
        )
        return cls(
    ...
    def predict(
        ...
        if isinstance(data, pd_DataFrame):
            data = _data_from_pandas(
                data=data, feature_name="auto", categorical_feature="auto", pandas_categorical=self.pandas_categorical
            )[0]
        ...
        if isinstance(data, (str, Path)):
            ...
            preds, nrow = self.__pred_for_csr(
                csr=data, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        elif isinstance(data, scipy.sparse.csc_matrix):
            preds, nrow = self.__pred_for_csc(
                csc=data, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        elif isinstance(data, np.ndarray):
            preds, nrow = self.__pred_for_np2d(
                mat=data, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        elif _is_pyarrow_table(data):
            preds, nrow = self.__pred_for_pyarrow_table(
                table=data, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
            ...
            preds, nrow = self.__pred_for_csr(
                csr=csr, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        if pred_leaf:
    ...
    def __inner_predict_np2d(
        self, mat: np.ndarray, start_iteration: int, num_iteration: int, predict_type: int, preds: Optional[np.ndarray]
        ...
        n_preds = self.__get_num_preds(
            start_iteration=start_iteration, num_iteration=num_iteration, nrow=mat.shape[0], predict_type=predict_type
        )
        if preds is None:
    ...
    def __pred_for_np2d(
        self, mat: np.ndarray, start_iteration: int, num_iteration: int, predict_type: int
    ) -> Tuple[np.ndarray, int]:
    ...
    def __create_sparse_native(
        ...
        _safe_call(
            _LIB.LGBM_BoosterFreePredictSparse(
                out_ptr_indptr, out_ptr_indices, out_ptr_data, ctypes.c_int(indptr_type), ctypes.c_int(data_type)
            )
        )
    ...
    def __inner_predict_csr(
        ...
        n_preds = self.__get_num_preds(
            start_iteration=start_iteration, num_iteration=num_iteration, nrow=nrow, predict_type=predict_type
        )
        if preds is None:
            preds = np.empty(n_preds, dtype=np.float64)
        elif len(preds.shape) != 1 or len(preds) != n_preds:
    ...
    def __inner_predict_sparse_csc(
        self, csc: scipy.sparse.csc_matrix, start_iteration: int, num_iteration: int, predict_type: int
    ):
    ...
    def __pred_for_csc(
        ...
        if nrow > _MAX_INT32:
            return self.__pred_for_csr(
                csr=csc.tocsr(), start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        if predict_type == _C_API_PREDICT_CONTRIB:
            return self.__inner_predict_sparse_csc(
                csc=csc, start_iteration=start_iteration, num_iteration=num_iteration, predict_type=predict_type
            )
        n_preds = self.__get_num_preds(
            start_iteration=start_iteration, num_iteration=num_iteration, nrow=nrow, predict_type=predict_type
        )
    ...
    def __pred_for_pyarrow_table(
        self, table: pa_Table, start_iteration: int, num_iteration: int, predict_type: int
    ) -> Tuple[np.ndarray, int]:
        ...
        n_preds = self.__get_num_preds(
            start_iteration=start_iteration, num_iteration=num_iteration, nrow=table.num_rows, predict_type=predict_type
        )
        preds = np.empty(n_preds, dtype=np.float64)
        out_num_preds = ctypes.c_int64(0)
...
class Dataset:
        ...
        if isinstance(data, (str, Path)):
            ...
            _safe_call(
                _LIB.LGBM_DatasetCreateFromFile(
                    _c_str(str(data)), _c_str(params_str), ref_dataset, ctypes.byref(self._handle)
                )
            )
        elif isinstance(data, scipy.sparse.csr_matrix):
    ...
    def __init_from_list_np2d(
        self, mats: List[np.ndarray], params_str: str, ref_dataset: Optional[_DatasetHandle]
    ) -> "Dataset":
    ...
    def __init_from_csr(
        self, csr: scipy.sparse.csr_matrix, params_str: str, ref_dataset: Optional[_DatasetHandle]
    ) -> "Dataset":
    ...
    def __init_from_pyarrow_table(
        self, table: pa_Table, params_str: str, ref_dataset: Optional[_DatasetHandle]
    ) -> "Dataset":
    ...
    def _compare_params_for_warning(
        params: Dict[str, Any], other_params: Dict[str, Any], ignore_keys: Set[str]
    ) -> bool:
    ...
    def save_binary(self, filename: Union[str, Path]) -> "Dataset":
        ...
        self : Dataset
            Returns self.
        """
        _safe_call(_LIB.LGBM_DatasetSaveBinary(self.construct()._handle, _c_str(str(filename))))
        return self
    ...
    def _update_params(self, params: Optional[Dict[str, Any]]) -> "Dataset":
        ...
        if self._handle is None:
            ...
            ret = _LIB.LGBM_DatasetUpdateParamChecking(
                _c_str(_param_dict_to_str(self.params)), _c_str(_param_dict_to_str(params))
            )
            if ret != 0:
    ...
    def set_field(
        ...
        _safe_call(
            _LIB.LGBM_DatasetSetField(
                self._handle, _c_str(field_name), ptr_data, ctypes.c_int(len(data)), ctypes.c_int(type_data)
            )
        )
        self.version += 1
        return self
    ...
    def get_field(self, field_name: str) -> Optional[np.ndarray]:
        ...
        _safe_call(
            _LIB.LGBM_DatasetGetField(
                self._handle, _c_str(field_name), ctypes.byref(tmp_out_len), ctypes.byref(ret), ctypes.byref(out_type)
            )
        )
        ...
        if out_type.value == _C_API_DTYPE_INT32:
            arr = _cint32_array_to_numpy(
                cptr=ctypes.cast(ret, ctypes.POINTER(ctypes.c_int32)), length=tmp_out_len.value
            )
        elif out_type.value == _C_API_DTYPE_FLOAT32:
            arr = _cfloat32_array_to_numpy(
                cptr=ctypes.cast(ret, ctypes.POINTER(ctypes.c_float)), length=tmp_out_len.value
            )
        elif out_type.value == _C_API_DTYPE_FLOAT64:
            arr = _cfloat64_array_to_numpy(
                cptr=ctypes.cast(ret, ctypes.POINTER(ctypes.c_double)), length=tmp_out_len.value
            )
        else:
        ...
        if self._handle is not None and feature_name is not None and feature_name != "auto":
            ...
            _safe_call(
                _LIB.LGBM_DatasetSetFeatureNames(
                    self._handle, _c_array(ctypes.c_char_p, c_feature_name), ctypes.c_int(len(feature_name))
                )
            )
        return self
    ...
    def _dump_text(self, filename: Union[str, Path]) -> "Dataset":
        ...
        self : Dataset
            Returns self.
        """
        _safe_call(_LIB.LGBM_DatasetDumpText(self.construct()._handle, _c_str(str(filename))))
        return self


_LGBM_CustomObjectiveFunction = Callable[[np.ndarray, Dataset], Tuple[np.ndarray, np.ndarray]]
...
class Booster:
        ...
        if train_set is not None:
            ...
            if params["machines"] is None:
                ...
                params = _choose_param_value(
                    main_param_name="num_machines", params=params, default_value=num_machines_from_machine_list
                )
            ...
            params_str = _param_dict_to_str(params)
            _safe_call(_LIB.LGBM_BoosterCreate(train_set._handle, _c_str(params_str), ctypes.byref(self._handle)))
            ...
            if self.__init_predictor is not None:
                _safe_call(_LIB.LGBM_BoosterMerge(self._handle, self.__init_predictor._handle))
            out_num_class = ctypes.c_int(0)
            _safe_call(_LIB.LGBM_BoosterGetNumClasses(self._handle, ctypes.byref(out_num_class)))
            ...
            _safe_call(
                _LIB.LGBM_BoosterCreateFromModelfile(
                    _c_str(str(model_file)), ctypes.byref(out_num_iterations), ctypes.byref(self._handle)
                )
            )
            out_num_class = ctypes.c_int(0)
            _safe_call(_LIB.LGBM_BoosterGetNumClasses(self._handle, ctypes.byref(out_num_class)))
        ...
        if model_str is not None:
            ...
            _safe_call(
                _LIB.LGBM_BoosterLoadModelFromString(
                    _c_str(model_str), ctypes.byref(out_num_iterations), ctypes.byref(handle)
                )
            )
    ...
    def _get_loaded_param(self) -> Dict[str, Any]:
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetLoadedParam(
                self._handle, ctypes.c_int64(buffer_len), ctypes.byref(tmp_out_len), ptr_string_buffer
            )
        )
        actual_len = tmp_out_len.value
        # if buffer length is not long enough, re-allocate a buffer
        if actual_len > buffer_len:
            ...
            _safe_call(
                _LIB.LGBM_BoosterGetLoadedParam(
                    self._handle, ctypes.c_int64(actual_len), ctypes.byref(tmp_out_len), ptr_string_buffer
                )
            )
    ...
    def update(self, train_set: Optional[Dataset] = None, fobj: Optional[_LGBM_CustomObjectiveFunction] = None) -> bool:
        ...
        if train_set is not None and not is_the_same_train_set:
            ...
            self.train_set = train_set
            _safe_call(_LIB.LGBM_BoosterResetTrainingData(self._handle, self.train_set.construct()._handle))
    ...
    def model_from_string(self, model_str: str) -> "Booster":
        ...
        _safe_call(
            _LIB.LGBM_BoosterLoadModelFromString(
                _c_str(model_str), ctypes.byref(out_num_iterations), ctypes.byref(self._handle)
            )
        )
        out_num_class = ctypes.c_int(0)
        _safe_call(_LIB.LGBM_BoosterGetNumClasses(self._handle, ctypes.byref(out_num_class)))
    ...
    def refit(
        ...
        **kwargs
        ...
        new_params = _choose_param_value(main_param_name="linear_tree", params=self.params, default_value=None)

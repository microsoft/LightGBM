                                    ...
                                    const size_t* p_incomingVersions)
{
    ...
    size_t bytes = sizeof(PhiNodeExpression) + (p_incomingVersionsCount - 1) * sizeof(unsigned long long);

    // Allocate a shared_ptr that deletes an BlockExpression 
    // allocated in a char[].
    boost::shared_ptr<PhiNodeExpression>
        exp(new (new char[bytes]) PhiNodeExpression(p_annotations,
                                                    p_version,
...
FreeForm2::PhiNodeExpression::GetType() const
{
...
size_t 
FreeForm2::PhiNodeExpression::GetNumChildren() const
{
    return 0;
}
...
void 
FreeForm2::PhiNodeExpression::Accept(Visitor& p_visitor) const
{
...
size_t
FreeForm2::PhiNodeExpression::GetVersion() const
{
    return m_version;
}
...
size_t
FreeForm2::PhiNodeExpression::GetIncomingVersionsCount() const
{
...
const size_t*
FreeForm2::PhiNodeExpression::GetIncomingVersions() const
{
    return m_incomingVersions;
}


FreeForm2::PhiNodeExpression::PhiNodeExpression(const Annotations& p_annotations,
                                                size_t p_version,
                                                size_t p_incomingVersionsCount,
                                                const size_t* p_incomingVersions)
...
void 
FreeForm2::PhiNodeExpression::DeleteAlloc(PhiNodeExpression* p_allocated)
{

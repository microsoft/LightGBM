...
# TODO(jameslamb): make this not require string comparisons, but also not have cyclic imports
def _env_model_is_cvbooster(env: CallbackEnv) -> bool:
    return env.model.__class__.__name__ == "CVBooster"
...
class _RecordEvaluationCallback:
    ...
    def __call__(self, env: CallbackEnv) -> None:
        ...
        for item in env.evaluation_result_list:
            if model_is_cvbooster:
                # cv()
                dataset_name, metric_name, metric_mean, _, metric_std_dev = item  # type: ignore[misc]
                self.eval_result[dataset_name][f"{metric_name}-mean"].append(metric_mean)
...
class _EarlyStoppingCallback:
    ...
    def _is_train_set(self, dataset_name: str, env: CallbackEnv) -> bool:
        ...
        # TODO(jameslamb): make this not require string comparisons, but also not have cyclic imports
        if _env_model_is_cvbooster(env) and dataset_name == "train":
            return True
    ...
    def _init(self, env: CallbackEnv) -> None:
        ...
        if isinstance(env.model, Booster):
            only_train_set = len(env.evaluation_result_list) == 1 and self._is_train_set(
                dataset_name=env.evaluation_result_list[0][0],
                env=env,
            )
            if only_train_set:
                self.enabled = False
        ...
        for eval_ret, delta in zip(env.evaluation_result_list, deltas):
            ...
            _, _, _, is_higher_better, *_ = eval_ret
            if is_higher_better:
    ...
    def __call__(self, env: CallbackEnv) -> None:
        ...
        for i in range(len(env.evaluation_result_list)):
            # NOTE: for cv(), 'metric_value' is a mean over all CV folds

...
BinMapper::BinMapper(const void* memory) { CopyFrom(reinterpret_cast<const char*>(memory)); }
...
std::vector<double> GreedyFindBin(const double* distinct_values, const int* counts,
                                  int num_distinct_values, int max_bin, size_t total_cnt,
                                  int min_data_in_bin) {
  ...
  if (num_distinct_values <= max_bin) {
    ...
    for (int i = 0; i < num_distinct_values - 1; ++i) {
      ...
      if (cur_cnt_inbin >= min_data_in_bin) {
        auto val =
            Common::GetDoubleUpperBound((distinct_values[i] + distinct_values[i + 1]) / 2.0);
...
std::vector<double> FindBinWithPredefinedBin(const double* distinct_values, const int* counts,
  ...
  for (size_t i = 0; i < bin_upper_bound.size(); ++i) {
    ...
    std::vector<double> new_upper_bounds =
        GreedyFindBin(distinct_values + bin_start, counts + bin_start, distinct_cnt_in_bin,
...
std::vector<double> FindBinWithZeroAsOneBin(const double* distinct_values, const int* counts,
  ...
  if ((left_cnt > 0) && (max_bin > 1)) {
    ...
    bin_upper_bound = GreedyFindBin(distinct_values, counts, left_cnt, left_max_bin, left_cnt_data,
                                    min_data_in_bin);
...
void BinMapper::FindBin(double* values, int num_sample_values, size_t total_sample_cnt,
  ...
  if (bin_type_ == BinType::NumericalBin) {
    if (missing_type_ == MissingType::Zero) {
      bin_upper_bound_ =
          FindBinWithZeroAsOneBin(distinct_values.data(), counts.data(), num_distinct_values,
...
size_t BinMapper::SizesInByte() const {
  size_t ret = VirtualFileWriter::AlignedSize(sizeof(num_bin_)) +
               ...
               VirtualFileWriter::AlignedSize(sizeof(bin_type_)) + sizeof(min_val_) +
               sizeof(max_val_) + VirtualFileWriter::AlignedSize(sizeof(default_bin_)) +
...
MultiValBin* MultiValBin::CreateMultiValDenseBin(data_size_t num_data, int num_bin,
                                                 int num_feature,
                                                 const std::vector<uint32_t>& offsets) {
...
MultiValBin* MultiValBin::CreateMultiValSparseBin(data_size_t num_data, int num_bin,
  ...
  if (estimate_total_entries <= std::numeric_limits<uint16_t>::max()) {
    if (num_bin <= 256) {
      ...
      return new MultiValSparseBin<uint16_t, uint16_t>(num_data, num_bin,
                                                       estimate_element_per_row);
    } else {
      return new MultiValSparseBin<uint16_t, uint32_t>(num_data, num_bin,
                                                       estimate_element_per_row);
    }
    ...
    if (num_bin <= 256) {
      ...
      return new MultiValSparseBin<uint32_t, uint16_t>(num_data, num_bin,
                                                       estimate_element_per_row);
    } else {
      return new MultiValSparseBin<uint32_t, uint32_t>(num_data, num_bin,
                                                       estimate_element_per_row);
    }
  } else {
    if (num_bin <= 256) {
...
template <>
const void* DenseBin<uint8_t, false>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
...
template <>
const void* DenseBin<uint16_t, false>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                                      std::vector<BinIterator*>* bin_iterator,
                                                      const int /*num_threads*/) const {
  *is_sparse = false;
  *bit_type = 16;
...
template <>
const void* DenseBin<uint32_t, false>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                                      std::vector<BinIterator*>* bin_iterator,
                                                      const int /*num_threads*/) const {
  *is_sparse = false;
  *bit_type = 32;
...
template <>
const void* DenseBin<uint8_t, true>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                                    std::vector<BinIterator*>* bin_iterator,
                                                    const int /*num_threads*/) const {
  *is_sparse = false;
  *bit_type = 4;
...
template <>
const void* SparseBin<uint8_t>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                               std::vector<BinIterator*>* bin_iterator,
                                               const int num_threads) const {
  *is_sparse = true;
  *bit_type = 8;
...
template <>
const void* SparseBin<uint16_t>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                                std::vector<BinIterator*>* bin_iterator,
                                                const int num_threads) const {
  *is_sparse = true;
  *bit_type = 16;
...
template <>
const void* SparseBin<uint32_t>::GetColWiseData(uint8_t* bit_type, bool* is_sparse,
                                                std::vector<BinIterator*>* bin_iterator,
                                                const int num_threads) const {
  *is_sparse = true;
  *bit_type = 32;
...
#ifdef USE_CUDA
template <>
const void* MultiValDenseBin<uint8_t>::GetRowWiseData(uint8_t* bit_type, size_t* total_size,
                                                      bool* is_sparse, const void** out_data_ptr,
...
template <>
const void* MultiValDenseBin<uint16_t>::GetRowWiseData(uint8_t* bit_type, size_t* total_size,
                                                       bool* is_sparse, const void** out_data_ptr,
                                                       uint8_t* data_ptr_bit_type) const {
...
template <>
const void* MultiValDenseBin<uint32_t>::GetRowWiseData(uint8_t* bit_type, size_t* total_size,
                                                       bool* is_sparse, const void** out_data_ptr,
                                                       uint8_t* data_ptr_bit_type) const {
...
const void* MultiValSparseBin<uint16_t, uint8_t>::GetRowWiseData(
    uint8_t* bit_type, size_t* total_size, bool* is_sparse, const void** out_data_ptr,
    uint8_t* data_ptr_bit_type) const {
  const uint8_t* to_return = data_.data();
  *bit_type = 8;

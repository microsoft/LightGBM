...
def early_stopping(stopping_rounds, min_acceptable_gain=0.02, verbose=True):
    ...
    def _init(env):
        ...
        if verbose:
            msg = "Training until validation scores don't improve by {} for {} rounds."
            print(msg.format(min_acceptable_gain, stopping_rounds))
    ...
    def _callback(env):
        ...
        for i in range_(len(env.evaluation_result_list)):
            ...
            if best_score_list[i] is None or (cmp_op[i] == lt and cmp_op[i](score, best_score[i]-min_acceptable_gain)) or (cmp_op[i] == gt and cmp_op[i](score, best_score[i]+min_acceptable_gain)):
                best_score[i] = score
                ...
                if verbose:
                    ...
                    print('Early stopping from non-greater-than', min_acceptable_gain, 'increase, best iteration is:\n[%d]\t%s' % (best_iter[i] + 1, '\t'.join([_format_eval_result(x) for x in best_score_list[i]])))

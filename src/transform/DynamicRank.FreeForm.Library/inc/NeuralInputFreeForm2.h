...
namespace FreeForm2
{
    ...
    // a NeuralInput that compiles several FreeFormv2 expressions together into fewer functions that
    // can be bulk evaluated. This is done to increase performance.
    ...
    public:
        ...
        virtual void Evaluate(const UInt32 p_input[], float p_output[]) const override;
        
        virtual void 
        GetAllAssociatedFeatures(std::vector<UInt32>& p_associatedFeaturesList) const override;
        ...
        // Bond Serialization.        
        virtual void FillBond(DynamicRank::UnionBondInput& p_data) const override;

        virtual bool Equal(const DynamicRank::BulkNeuralInput* p_other) const override;

        BulkCompiledNeuralInput(const DynamicRank::UnionBondInput& p_data);

        // BulkCompiledNeuralInput never load from config.ini since it's constructed by ff2 compiler
        // or reconstructed from UnionBondInput. 
        static BulkCompiledNeuralInput*
        Load(DynamicRank::Config& p_config,
    ...
    private:
        ...
        // Functions to do low-level serialization and deserialization of executable code.
        static std::pair<boost::shared_array<unsigned char>, size_t> SerializeBlob(const std::vector<boost::shared_ptr<Executable>>& p_execs);
        static std::pair<boost::shared_array<unsigned char>, size_t> SerializePhoenixBlob(const std::vector<boost::shared_ptr<Executable>>& p_execs);
        static std::pair<boost::shared_array<unsigned char>, size_t> SerializeLlvmBlob(const std::vector<boost::shared_ptr<Executable>>& p_execs);

        static void DeserializeBlob(const unsigned char* p_buffer,
                                    size_t p_length,
    ...
    public:
        ...
        // the program.
        NeuralInputFreeForm2(const std::string& p_input,
                             const char* p_transform, 
        ...
        // Evaluate.
        virtual double Evaluate(UInt32 p_input[]) const override;
        
        // Train.
        virtual bool Train(double p_learningRate, 
                           double p_outputHigh, 
                           double p_outputLow, 
                           double p_outputDelta,
                           UInt32 p_inputHigh[], 
                           UInt32 p_inputLow[]) override;
 
        // Get all associated features.
        virtual void GetAllAssociatedFeatures(std::vector<UInt32>& p_associatedFeaturesList) const override;
        ...
        virtual bool Equal(const DynamicRank::NeuralInput* p_other) const override;

        NeuralInputFreeForm2(const DynamicRank::UnionBondInput& p_data);

        // Interface to support batch serialization.
        // Just for special batch serializaton of freeform2.
        virtual bool IsFreeForm2() const override; 

        virtual double GetMin() const override;

        virtual double GetMax() const override;

        virtual bool Save(FILE *p_out, size_t p_input, const DynamicRank::IFeatureMap& p_map) const override;
        ...
        Load(//const apsdk::configuration::IConfiguration& p_config, 
    ...
    protected:
        ...
        // Gets a string representation of the input. This is used to log if an error occurs
        // while compiling.
        ...
        static std::string
        LoadProgram(DynamicRank::Config& p_config,
                    const char* p_section,
    ...
    // compile them.
    template<class T>
    class CompiledNeuralInputLoader : public DynamicRank::NeuralInputFactory::Loader
    {
    public:
        // Constructor, taking the name of the transform which this loader will
        // be responsible for.
        CompiledNeuralInputLoader(const char* p_transform)
            : m_transform(p_transform)
        {
        }        

        // Functor to create a NeuralInput given appropriate inputs.
        virtual DynamicRank::NeuralInput* 
        operator()(DynamicRank::Config& p_config,
                   const char* p_section,
        ...
        virtual DynamicRank::NeuralInput* FromBond(const DynamicRank::UnionBondInput& p_data) const
        {
        ...
        virtual DynamicRank::BulkNeuralInput* FromBulkBond(const DynamicRank::UnionBondInput& p_data) const
        {
            // To handle all the cases we let all the cases' constructor from bond
            // Take the DynamicRank::UnionBondInput as parameter.           
            T* t = new T(p_data);
            DynamicRank::BulkNeuralInput* t1 = dynamic_cast<DynamicRank::BulkNeuralInput*>(t);
            if (!t1)
            {
                delete t;
                t = nullptr;
    ...
    // A base class that groups a large amount of NeuralInputs and combines them into
    // a few executable functions. They should have the same effect as running all the
    // individual functions.
    class CompiledBulkNeuralInputLoaderBase : public DynamicRank::BulkNeuralInputFactory
    {
    public:
        ...
        CompiledBulkNeuralInputLoaderBase(UInt32 p_reductionFactor);

        virtual std::unique_ptr<DynamicRank::BulkNeuralInput>
        ConvertToBulkInput(const std::vector<DynamicRank::BulkNeuralInputFactory::InputAndIndex>& p_inputs,
                           DynamicRank::IFeatureMap& p_featureMap) const = 0;
    
    protected:

        // Load all neuralinput to a vector, extracted from ConvertToBulkInput for BulkAggregatedNeuralInput.
        size_t
        LoadProgramsForBulkInput(std::vector<boost::shared_ptr<Program>>& p_programs,
                                 std::vector<UInt32>& p_features,
                                 const std::vector<DynamicRank::BulkNeuralInputFactory::InputAndIndex>& p_inputs,
                                 DynamicRank::IFeatureMap& p_featureMap) const;
    ...
    public:
        // p_reductionFactor must be greater or equal to 1.
        CompiledBulkNeuralInputLoader(UInt32 p_reductionFactor)
                           ...
                           DynamicRank::IFeatureMap& p_featureMap) const
        {
            ...
            size_t convertedInputsSize = LoadProgramsForBulkInput(programs, features, p_inputs, p_featureMap);

            return std::unique_ptr<T>(new T(programs,
                                            convertedInputsSize,
                                            features));
        }
    ...
    public:
        ...
        const CompiledNeuralInputLoader<NeuralInputFreeForm2>& GetFreeForm2Loader() const;

    private: 
        boost::shared_ptr<CompiledNeuralInputLoader<NeuralInputFreeForm2>> m_ff2Loader;

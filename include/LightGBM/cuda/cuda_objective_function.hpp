...
class CUDAObjectiveInterface : public HOST_OBJECTIVE {
 public:
  ...
  virtual const double* ConvertOutputCUDA(const data_size_t num_data, const double* input,
                                          double* output) const {
  ...
  void GetGradients(const double* scores, const data_size_t /*num_sampled_queries*/,
  ...
  void RenewTreeOutputCUDA(const double* score, const data_size_t* data_indices_in_leaf,
    ...
    LaunchRenewTreeOutputCUDAKernel(score, data_indices_in_leaf, num_data_in_leaf,
                                    data_start_in_leaf, num_leaves, leaf_value);
    SynchronizeCUDADevice(__FILE__, __LINE__);
    global_timer.Stop("CUDAObjectiveInterface::LaunchRenewTreeOutputCUDAKernel");
  }

 protected:
  virtual void LaunchGetGradientsKernel(const double* scores, score_t* gradients,
  ...
  virtual const double* LaunchConvertOutputCUDAKernel(const data_size_t /*num_data*/,
                                                      const double* input,
                                                      double* /*output*/) const {
    return input;
  }

  virtual void LaunchRenewTreeOutputCUDAKernel(const double* /*score*/,

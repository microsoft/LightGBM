...
boost::shared_ptr<FreeForm2::Result> FreeForm2::Executable::Evaluate(
    StreamFeatureInput *p_input, const FeatureType p_features[]) const {
...
boost::shared_ptr<FreeForm2::Result> FreeForm2::Executable::Evaluate(
  ...
  return m_impl->Evaluate(p_features, p_currentDocument, p_documentCount,
                          p_cache);
}

FreeForm2::Executable::DirectEvalFun FreeForm2::Executable::EvaluationFunction()
    const {
...
const FreeForm2::Type &FreeForm2::Executable::GetType() const {
  return m_impl->GetType();
}

size_t FreeForm2::Executable::GetExternalSize() const {
  return sizeof(FreeForm2::ExecutableImpl) + m_impl->GetExternalSize();
}

const FreeForm2::ExecutableImpl &FreeForm2::Executable::GetImplementation()
    const {
  return *m_impl;
}
...
FreeForm2::CompilerFactory::CreateExecutableCompiler(
    ...
    FreeForm2::CompilerFactory::DestinationFunctionType
        p_destinationFunctionType) {
  std::auto_ptr<CompilerImpl> impl;
  impl.reset(
      new LlvmCompilerImpl(p_optimizationLevel, p_destinationFunctionType));

...
void GradientDiscretizer::Init(const data_size_t num_data, const int num_leaves,
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
  for (int feature_index = 0; feature_index < num_features; ++feature_index) {
    ...
    change_hist_bits_buffer_[feature_index].resize(
        (bin_mapper->num_bin() - static_cast<int>(bin_mapper->GetMostFreqBin() == 0)) * 2);
  }

  ordered_int_gradients_and_hessians_.resize(2 * num_data);
}

void GradientDiscretizer::DiscretizeGradients(const data_size_t num_data,
  ...
  Threading::For<data_size_t>(0, num_data, 1024,
                              [input_gradients, input_hessians, &thread_max_gradient,
                               &thread_max_hessian](int, data_size_t start, data_size_t end) {
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
      for (data_size_t i = 0; i < num_data; ++i) {
        ...
        discretized_int8[2 * i + 1] =
            gradient >= 0.0f ? static_cast<int8_t>(gradient * inverse_gradient_scale_ +
                                                   gradient_random_values_[random_value_pos])
                             : static_cast<int8_t>(gradient * inverse_gradient_scale_ -
                                                   gradient_random_values_[random_value_pos]);
      ...
      for (data_size_t i = 0; i < num_data; ++i) {
        ...
        discretized_int8[2 * i + 1] =
            gradient >= 0.0f ? static_cast<int8_t>(gradient * inverse_gradient_scale_ +
                             ...
                             : static_cast<int8_t>(gradient * inverse_gradient_scale_ -
                                                   gradient_random_values_[random_value_pos]);
        discretized_int8[2 * i] = static_cast<int8_t>(input_hessians[i] * inverse_hessian_scale_ +
                                                      hessian_random_values_[random_value_pos]);
      }
    }
  } else {
    if (is_constant_hessian_) {
#pragma omp parallel for schedule(static) num_threads(num_threads)
      for (data_size_t i = 0; i < num_data; ++i) {
        const double gradient = input_gradients[i];
        discretized_int8[2 * i + 1] =
            gradient >= 0.0f ? static_cast<int8_t>(gradient * inverse_gradient_scale_ + 0.5)
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
      for (data_size_t i = 0; i < num_data; ++i) {
        ...
        discretized_int8[2 * i + 1] =
            gradient >= 0.0f ? static_cast<int8_t>(gradient * inverse_gradient_scale_ + 0.5)
                             : static_cast<int8_t>(gradient * inverse_gradient_scale_ - 0.5);
        discretized_int8[2 * i] =
            static_cast<int8_t>(input_hessians[i] * inverse_hessian_scale_ + 0.5);
      }
    }
  }
}
...
void GradientDiscretizer::SetNumBitsInHistogramBin(const int left_leaf_index,
                                                   const int right_leaf_index,
                                                   const data_size_t num_data_in_left_leaf,
                                                   const data_size_t num_data_in_right_leaf) {
  std::vector<int8_t>& leaf_num_bits_in_histogram_bin =
      IS_GLOBAL ? global_leaf_num_bits_in_histogram_bin_ : leaf_num_bits_in_histogram_bin_;
  std::vector<int8_t>& node_num_bits_in_histogram_bin =
      IS_GLOBAL ? global_node_num_bits_in_histogram_bin_ : node_num_bits_in_histogram_bin_;
  if (right_leaf_index == -1) {
    const uint64_t max_stat_per_bin =
        static_cast<uint64_t>(num_data_in_left_leaf) * static_cast<uint64_t>(num_grad_quant_bins_);
    ...
    const uint64_t max_stat_left_per_bin =
        static_cast<uint64_t>(num_data_in_left_leaf) * static_cast<uint64_t>(num_grad_quant_bins_);
    const uint64_t max_stat_right_per_bin = static_cast<uint64_t>(num_data_in_right_leaf) *
                                            static_cast<uint64_t>(num_grad_quant_bins_);
    node_num_bits_in_histogram_bin[left_leaf_index] =
        leaf_num_bits_in_histogram_bin[left_leaf_index];
...
template void GradientDiscretizer::SetNumBitsInHistogramBin<false>(
    const int left_leaf_index, const int right_leaf_index, const data_size_t num_data_in_left_leaf,
    const data_size_t num_data_in_right_leaf);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static) \
    reduction(+ : sum_gradient, sum_hessian)
    ...
    std::vector<double> global_leaf_grad_hess_stats =
        Network::GlobalSum<double>(&leaf_grad_hess_stats_);
    for (int leaf_id = 0; leaf_id < tree->num_leaves(); ++leaf_id) {
      ...
      const double leaf_output = FeatureHistogram::CalculateSplittedLeafOutput<true, true, false>(
          sum_gradient, sum_hessian, config->lambda_l1, config->lambda_l2, config->max_delta_step,
          config->path_smooth, leaf_index_to_global_num_data(leaf_id), 0.0f);
...
#pragma omp parallel for num_threads(OMP_NUM_THREADS()) schedule(static) \
    reduction(+ : sum_gradient, sum_hessian)
      ...
      const double leaf_output = FeatureHistogram::CalculateSplittedLeafOutput<true, true, false>(
          sum_gradient, sum_hessian, config->lambda_l1, config->lambda_l2, config->max_delta_step,
          config->path_smooth, leaf_cnt, 0.0f);
      tree->SetLeafOutput(leaf_id, leaf_output);
    }
  }

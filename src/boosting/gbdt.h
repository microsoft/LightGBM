...
/*!
 * \brief GBDT algorithm implementation. including Training, prediction,
 * bagging.
 */
class GBDT : public GBDTBase {
 public:
  ...
  /*!
   * \brief Merge model from other boosting object. Will insert to the front of
   * current boosting object \param other
   */
  ...
  void ShuffleModels(int start_iter, int end_iter) override {
    ...
    for (int i = 0; i < total_iter; ++i) {
      for (int j = 0; j < num_tree_per_iteration_; ++j) {
        ...
        auto new_tree =
            std::unique_ptr<Tree>(new Tree(*(original_models[tree_idx].get())));
        models_.push_back(std::move(new_tree));
      }
    }
  }
  ...
  void RefitTree(
      const std::vector<std::vector<int>>& tree_leaf_prediction) override;

  /*!
   * \brief Training logic
   * \param gradients nullptr for using default objective, otherwise use
   * self-defined boosting \param hessians nullptr for using default objective,
   * otherwise use self-defined boosting \return True if cannot train any more
   */
   ...
   * \param data_idx 0: training data, 1: 1st validation data
   * \param result used to store prediction result, should allocate memory
   * before call this function \param out_len length of returned score
   */
  void GetPredictAt(int data_idx, double* out_result,
                    int64_t* out_len) override;
   ...
   */
  inline int NumPredictOneRow(int num_iteration, bool is_pred_leaf,
  ...
  void PredictContrib(
      const double* features, double* output,
      const PredictionEarlyStopInstance* earlyStop) const override;

  /*!
   * \brief Dump model to json format string
   * \param start_iteration The model will be saved start from
   * \param num_iteration Number of iterations that want to dump, -1 means dump
   * all \return Json format string of model
   */
  std::string DumpModel(int start_iteration, int num_iteration) const override;

  /*!
   * \brief Translate model to if-else statement
   * \param num_iteration Number of iterations that want to translate, -1 means
   * translate all \return if-else format codes of model
   */
   ...
   * \brief Translate model to if-else statement
   * \param num_iteration Number of iterations that want to translate, -1 means
   * translate all \param filename Filename that want to save to \return
   * is_finish Is training finished or not
   */
  bool SaveModelToIfElse(int num_iteration,
   ...
   */
  std::string SaveModelToString(int start_iteration,
   ...
   * \param num_iteration Number of model that want to use for feature
   * importance, -1 means use all \param importance_type: 0 for split, 1 for
   * gain \return vector of feature_importance
   */
  std::vector<double> FeatureImportance(int num_iteration,
                                        int importance_type) const override;
  ...
  inline void InitPredict(int num_iteration, bool is_pred_contrib) override {
    ...
    if (num_iteration > 0) {
      num_iteration_for_pred_ =
          std::min(num_iteration, num_iteration_for_pred_);
    }
    if (is_pred_contrib) {
#pragma omp parallel for schedule(static)
 ...
 protected:
   ...
   */
  data_size_t BaggingHelper(Random* cur_rand, data_size_t start,
                            data_size_t cnt, data_size_t* buffer);

  /*!
   * \brief Helper function for bagging, used for multi-threading optimization,
   * balanced sampling \param start start indice of bagging \param cnt count
   * \param buffer output buffer
   * \return count of left size
   */
  data_size_t BalancedBaggingHelper(Random* cur_rand, data_size_t start,
                                    data_size_t cnt, data_size_t* buffer);
  ...
  */
  virtual std::vector<double> EvalOneMetric(const Metric* metric,
  ...
  std::vector<score_t, Common::AlignmentAllocator<score_t, kAlignedSize>>
      gradients_;
  ...
  std::vector<data_size_t,
              Common::AlignmentAllocator<data_size_t, kAlignedSize>>
      bag_data_indices_;

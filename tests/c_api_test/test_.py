...
def find_lib_path():
    ...
    if system() in ("Windows", "Microsoft"):
        dll_path.append(
            os.path.join(curr_path, "../../python-package/compile/Release/")
        )
        dll_path.append(
            os.path.join(curr_path, "../../python-package/compile/windows/x64/DLL/")
        )
...
def load_from_file(filename, reference):
    ...
    LIB.LGBM_DatasetCreateFromFile(
        c_str(filename), c_str("max_bin=15"), ref, ctypes.byref(handle)
    )
...
def test_booster():
    ...
    LIB.LGBM_BoosterCreateFromModelfile(
        c_str("model.txt"), ctypes.byref(num_total_model), ctypes.byref(booster2)
    )
    data = []
    with open(
        os.path.join(
